...
export const SendEther = ({
  ...
  const usdAmount = formatWalletUsd(qauPrice * qauBalance)
  const qauAmount = formatWalletToken(qauBalance, locale)

  const maxUsdAmount = qauPrice * qauBalance
      ...
      <Flex className="justify-between gap-4 border-y border-background-highlight px-6 py-4 text-xs text-body-medium md:py-6">
        ...
        <Flex className="flex-col items-end">
          <NotificationPopover
            side="top"
            title={t("sim-example-walkthrough")}
            content={t("sim-send-qau-only-note")}
          >
            {/* Token selector pill */}
            <HStack className="mb-4 gap-0 rounded-full bg-body-light px-2 py-1">
              {/* TODO: remove flags and `size` class when icon is migrated */}
              <QauTokenIcon className="!me-1.5 !size-[1em] !text-xl" />
              <p className="m-0 font-bold text-body">QAU</p>
            </HStack>
          </NotificationPopover>
          {/* Balances */}
