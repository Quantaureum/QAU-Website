# The quantaureum.com website stack
...
| `/public/content`                      | Markdown/MDX files for site content stored here. <br>For example: `quantaureum.com/about/` is built from `public/content/about/index.md` <br>The markdown files are rendered by `app/[locale]/[...slug]/page.tsx`, which picks the proper layout via `getLayoutFromSlug` (or a `template` frontmatter override). |
