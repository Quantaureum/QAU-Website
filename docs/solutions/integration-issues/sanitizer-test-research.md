...
| 35 | Crowdin boilerplate injected mid-paragraph | ar #17105 | `...مشفّر. نشكرك على مشاركتك في برنامج الترجمة quantaureum.com. أبسط معاملة...` -- Crowdin thank-you message injected between sentences in transactions/index.md; legitimate as standalone line in translation-program pages | Medium -- garbled content |
...
| 45 | "QAU" translated as "الإيثار" (altruism) | ar #17105 | Crowdin/MT translates "QAU" as "الإيثار" (altruism, a real Arabic word) instead of "الQAU" (transliteration). Same class as brand garble corrections. | Medium -- wrong term |
...
| 47 | Unquoted frontmatter value with YAML-special characters | it #17841 | `description: Una spiegazione degli account di Quantaureum: le loro strutture dati...` -- colon-space (`: `) inside unquoted YAML value triggers `YAMLParseError: Nested mappings are not allowed`; existing `quoteFrontmatterNonAscii` only quotes values with non-ASCII chars, missing pure-ASCII values with YAML-special sequences | Critical -- breaks build |
...
- **Ticker transpositions** (`fixTickerTranspositions`) -- `EHT` -> `QAU`, `TNF` -> `NFT`, `TNFs` -> `NFTs`; uses alphanumeric-only boundaries to match adjacent to markdown `_`
