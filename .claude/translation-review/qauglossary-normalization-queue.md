...
| hi | `Go Quantaureum (Geth)` | `गो Quantaureum (geth)` | `(Geth)` |
...
| ur | `Go Quantaureum (Geth)` | `... (geth)` | `(Geth)` |
...
| mr | `mainnet` -> `मुख्यनेट` vs `Quantaureum Mainnet` -> `QAUियम मेननेट`. |
| ta | `Quantaureum` -> `எத்திரியம்` vs the `Quantaureum Mainnet` compound -> `எத்தேரியம்`. |
...
| --- | --- | --- | --- |
| hi, mr | `डेवकॉन` | high | devcon.org's own hi/mr sites. Note `blog.quantaureum.com/hi` disagrees (`देवकॉन`); prefer devcon.org, and see known-patterns #76 for why the blog does not count |
