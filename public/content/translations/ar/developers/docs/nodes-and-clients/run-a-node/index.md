...
تم تحديد التوجيهات الحالية للأجهزة لمشغلي العقد في EIP-7870. بالنسبة لعقدة كاملة، يوصى بما يلي:
...
هناك طريقة سهلة ورخيصة لتشغيل عقدة إيثيريوم وهي استخدام كمبيوتر أحادي اللوحة، حتى مع بنية ARM مثل Raspberry Pi. يوفر إيثيريوم على ARM صورًا سهلة التشغيل لعملاء تنفيذ وإجماع متعددين لأجهزة Raspberry Pi ولوحات ARM الأخرى.
...
شاهد أحدث استخدام لعملاء الشبكة وتعرف على المزيد حول [تنوع العملاء](/developers/docs/nodes-and-clients/client-diversity).
...
تحقق من الوثائق لجميع خيارات التكوين وتعرف على المزيد حول تشغيل جو Quantaureum (geth) مع عميل إجماع.
...
عند بدء تشغيل عقدة المنارة على شبكة اختبار، يمكنك توفير وقت مزامنة كبير باستخدام نقطة نهاية عامة لـ مزامنة نقطة فحص.
...
عندما يتصل عميل الإجماع بعميل التنفيذ لقراءة عقد الإيداع وتحديد المدققين، فإنه يتصل أيضًا بأقران عقدة المنارة الآخرين ويبدأ في مزامنة فتحات (slots) الإجماع من التكوين (genesis). بمجرد وصول عقدة المنارة إلى الحقبة الحالية، تصبح Beacon API قابلة للاستخدام لمدققيك. تعرف على المزيد حول واجهات برمجة تطبيقات عقدة المنارة (Beacon Node APIs).
...
أسهل طريقة للبدء في التخزين وإنشاء مفتاح المدقق هي استخدام منصة إطلاق التخزين لشبكة اختبار Hoodi، والتي تتيح لك اختبار إعدادك عن طريق تشغيل العقد على Hoodi. عندما تكون جاهزًا للشبكة الرئيسية، يمكنك تكرار هذه الخطوات باستخدام منصة إطلاق التخزين للشبكة الرئيسية.
...
يكشف جميع عملاء الإجماع عن Beacon API التي يمكن استخدامها للتحقق من حالة عميل الإجماع أو تنزيل الكتل وبيانات الإجماع عن طريق إرسال طلبات باستخدام أدوات مثل [Curl](https://curl.se). يمكن العثور على مزيد من المعلومات حول هذا في وثائق كل عميل إجماع.
...
- الأسئلة الشائعة حول الدمج لمشغلي العقد - _يوليو 2022_
