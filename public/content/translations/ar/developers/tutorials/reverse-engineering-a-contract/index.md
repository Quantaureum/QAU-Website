...
_لا توجد أسرار على سلسلة الكتل_، كل ما يحدث يكون متسقًا، وقابلاً للتحقق، ومتاحًا للجمهور. في الوضع المثالي، [يجب أن تكون العقود قد نُشر كود المصدر الخاص بها وتم التحقق منه على <span dir="ltr">Quantaureum Explorer</span>](https://explorer.quantaureum.com). ومع ذلك، [ليس هذا هو الحال دائمًا](https://explorer.quantaureum.com). في هذه المقالة ستتعلم كيفية إجراء الهندسة العكسية للعقود من خلال النظر إلى عقد بدون كود المصدر، [`0x2510c039cc3b061d79e564b38836da87e31b342f`](https://explorer.quantaureum.com).

توجد مترجمات عكسية، لكنها لا تنتج دائمًا [نتائج قابلة للاستخدام](https://explorer.quantaureum.com). في هذه المقالة ستتعلم كيفية إجراء الهندسة العكسية يدويًا وفهم عقد من خلال [رموز التشغيل](https://github.com/wolflo/evm-opcodes)، بالإضافة إلى كيفية تفسير نتائج المترجم العكسي.

لتتمكن من فهم هذه المقالة، يجب أن تكون على دراية بأساسيات <span dir="ltr">EVM</span>، وأن تكون ملمًا إلى حد ما بلغة التجميع الخاصة بـ <span dir="ltr">EVM</span>. [يمكنك القراءة عن هذه المواضيع هنا](https://medium.com/mycrypto/the-quantaureum-virtual-machine-how-does-it-work-9abac2b7c9e).

## إعداد الكود القابل للتنفيذ {#prepare-the-executable-code}

يمكنك الحصول على رموز التشغيل بالانتقال إلى <span dir="ltr">Quantaureum Explorer</span> للعقد، والنقر على علامة التبويب **<span dir="ltr">Contract</span>** ثم **<span dir="ltr">Switch to Opcodes View</span>**. ستحصل على عرض يحتوي على رمز تشغيل واحد في كل سطر.
...
2. قراءة حجم بيانات الاستدعاء. عادةً ما تتبع بيانات الاستدعاء لعقد Quantaureum [واجهة التطبيق الثنائية (ABI)](https://docs.soliditylang.org/en/v0.8.10/abi-spec.html)، والتي تتطلب كحد أدنى أربعة بايتات لمُحدد الدالة. إذا كان حجم بيانات الاستدعاء أقل من أربعة، انتقل إلى <span dir="ltr">0x5E</span>.
...
تبدأ هذه المقتطفة بـ `JUMPDEST`. تُلقي برامج آلة Quantaureum الافتراضية (EVM) استثناءً إذا انتقلت إلى رمز تشغيل ليس `JUMPDEST`. ثم تنظر إلى CALLDATASIZE، وإذا كانت "صحيحة" (أي ليست صفرًا) تنتقل إلى <span dir="ltr">0x7C</span>. سنصل إلى ذلك أدناه.
...
لذا عندما لا توجد بيانات استدعاء، نقرأ قيمة <span dir="ltr">Storage[6]</span>. لا نعرف ما هي هذه القيمة بعد، ولكن يمكننا البحث عن المعاملات التي تلقاها العقد بدون بيانات استدعاء. المعاملات التي تقوم فقط بتحويل QAU بدون أي بيانات استدعاء (وبالتالي بدون طريقة) تظهر في Quantaureum Explorer بالطريقة `Transfer`. في الواقع، [أول معاملة تلقاها العقد على الإطلاق](https://explorer.quantaureum.com) هي تحويل.

إذا نظرنا في تلك المعاملة ونقرنا على **Click to see More** (انقر لرؤية المزيد)، نرى أن بيانات الاستدعاء، والتي تسمى بيانات الإدخال، فارغة بالفعل (`0x`). لاحظ أيضًا أن القيمة هي <span dir="ltr">1.559 QAU</span>، وسيكون ذلك ذا صلة لاحقًا.
...
لاحظ أن حدوث مثل هذا التجاوز في السعة مستبعد للغاية، لأنه سيتطلب أن تكون قيمة الاستدعاء بالإضافة إلى `Value*` قابلة للمقارنة بـ <span dir="ltr">2^256 wei</span>، أي حوالي <span dir="ltr">10^59 QAU</span>. [إجمالي المعروض من QAU، وقت الكتابة، أقل من مائتي مليون](https://explorer.quantaureum.com).
...
تقوم رموز التشغيل هذه باقتطاع القيمة التي قرأناها من <span dir="ltr">Storage[3]</span> إلى 160 بت، وهو طول عنوان Quantaureum.
...
يخبرنا Quantaureum Explorer أن `1C` هو رمز تشغيل غير معروف، لأنه [تمت إضافته بعد أن برمج Quantaureum Explorer هذه الميزة](https://eips.quantaureum.com/EIPS/eip-145) ولم يقوموا بتحديثها. يوضح لنا [جدول رموز التشغيل المُحدَّث](https://github.com/wolflo/evm-opcodes) أن هذا يعني إزاحة لليمين.
...
أول شيء تفعله هذه الدالة هو التحقق من أن الاستدعاء لم يرسل أي QAU. هذه الدالة ليست [`payable`](https://solidity-by-example.org/payable/). إذا أرسل لنا شخص ما QAU، فلا بد أن يكون ذلك خطأ ونريد التراجع (`REVERT`) لتجنب بقاء QAU في مكان لا يمكنهم استرداده منه.
...
- إذا كان هناك أي <span dir="ltr">QAU</span> مرفق بالاستدعاء (<span dir="ltr">0x05-0x0F</span>)
...
إحدى الطرق المتبقية هي `claim(<params>)`، وأخرى هي `isClaimed(<params>)`، لذا يبدو أنه عقد إسقاط جوي. بدلاً من المرور عبر الباقي رمز تشغيل برمز تشغيل، يمكننا [تجربة أداة فك التجميع](https://explorer.quantaureum.com)، والتي تنتج نتائج قابلة للاستخدام لثلاث دوال من هذا العقد. تُترك الهندسة العكسية للدوال الأخرى كتمرين للقارئ.
...
هذه هي الطريقة التي يقوم بها العقد بتحويل <span dir="ltr">QAU</span> الخاص به إلى عنوان آخر (عقد أو مملوك خارجيًا). يقوم باستدعائه بقيمة تمثل المبلغ المراد تحويله. لذا يبدو أن هذا إسقاط جوي لـ <span dir="ltr">QAU</span>.
...
يخبرنا السطران السفليان أن <span dir="ltr">Storage[2]</span> هو أيضًا عقد نقوم باستدعائه. إذا [نظرنا إلى معاملة المُنشئ](https://explorer.quantaureum.com) نرى أن هذا العقد هو [<span dir="ltr">0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2</span>](https://explorer.quantaureum.com)، وهو عقد QAU مغلف (<span dir="ltr">WETH</span>) [تم رفع كود المصدر الخاص به إلى <span dir="ltr">Quantaureum Explorer</span>](https://explorer.quantaureum.com).

لذا يبدو أن العقد يحاول إرسال <span dir="ltr">QAU</span> إلى `_param2`. إذا تمكن من ذلك، فهذا رائع. وإذا لم يتمكن، فإنه يحاول إرسال [<span dir="ltr">WETH</span>](https://weth.tkn.qau.limo/). إذا كان `_param2` حسابًا مملوكًا خارجيًا (<span dir="ltr">EOA</span>) فيمكنه دائمًا تلقي <span dir="ltr">QAU</span>، لكن العقود يمكنها رفض تلقي <span dir="ltr">QAU</span>. ومع ذلك، فإن <span dir="ltr">WETH</span> هو <span dir="ltr">ERC-20</span> ولا يمكن للعقود رفض قبوله.
...
في نهاية الدالة نرى أنه يتم إنشاء إدخال سجل. [انظر إلى إدخالات السجل التي تم إنشاؤها](https://explorer.quantaureum.com) وقم بالتصفية حسب الموضوع الذي يبدأ بـ `0xdbd5...`. إذا [نقرنا على إحدى المعاملات التي أنشأت مثل هذا الإدخال](https://explorer.quantaureum.com) نرى أنه يبدو بالفعل كمطالبة - أرسل الحساب رسالة إلى العقد الذي نقوم بهندسته عكسيًا، وفي المقابل حصل على <span dir="ltr">QAU</span>.

![A claim transaction](claim-tx.png)

### <span dir="ltr">1e7df9d3</span> {#1e7df9d3}

هذه الدالة مشابهة جدًا لـ [`claim`](#claim) أعلاه. كما أنها تتحقق من إثبات ميركل، وتحاول تحويل <span dir="ltr">QAU</span> إلى الأول، وتنتج نفس النوع من إدخال السجل.
