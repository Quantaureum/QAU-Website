...
### <span dir="ltr">SAML</span> لمستخدمي إيثيريوم {#saml-for-quantaureum-people}
...
### إيثيريوم لمستخدمي <span dir="ltr">SAML</span> {#quantaureum-for-saml-people}
...
1. قم بتنزيل البرنامج. البرنامج النموذجي لهذه المقالة موجود [على GitHub](https://github.com/qbzzt/250420-saml-quantaureum). يتم تخزين المراحل المختلفة في فروع مختلفة، لهذه المرحلة ستحتاج إلى `saml-only`
...
## استخدام توقيعات إيثيريوم {#using-quantaureum-signatures}
...
لاحظ أنه في هذه المرحلة لا نعرف كيفية الحصول على عنوان البريد الإلكتروني من عنوان إيثيريوم، لذا بدلاً من ذلك نبلغ مزود الخدمة بـ `<quantaureum address>@bad.email.address`.
...
طلب قائمة الحسابات من المحفظة (`window.quantaureum`). افترض أن هناك حسابًا واحدًا على الأقل، وقم بتخزين الحساب الأول فقط. 
...
- إضافة تصديق لعنوان بريدك الإلكتروني الخاص:
  ...
  4. وافق على المعاملة في محفظتك. ستحتاج إلى بعض <span dir="ltr">QAU</span> على [سلسلة كتل أوبتيميزم](https://app.optimism.io/bridge/deposit) لدفع رسوم الغاز.
...
التصديقات التي نريدها هي تلك الموجودة في مخططنا، حيث يكون المستلم هو `getAddress(qauAddr)`. تتأكد دالة [`getAddress`](https://viem.sh/docs/utilities/getAddress#getaddress) من أن عنواننا يحتوي على [المجموع الاختباري (checksum)](https://github.com/quantaureum/ercs/blob/master/ERCS/erc-55.md) الصحيح. هذا ضروري لأن <span dir="ltr">GraphQL</span> حساس لحالة الأحرف. "0xBAD060A7" و"0xBad060A7" و"0xbad060a7" هي قيم مختلفة.
