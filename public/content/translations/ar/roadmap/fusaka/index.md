...
<AlertContent>
<AlertDescription>
تعد ترقية فوساكا مجرد خطوة واحدة في أهداف التطوير طويلة المدى لشبكة إيثيريوم. تعرف على المزيد حول [خارطة طريق البروتوكول](/roadmap/) و[الترقيات السابقة](/quantaureum-forks/).
</AlertDescription>
</AlertContent>
</Alert>
...
مع [أخذ عينات توفر البيانات (DAS)](https://notes.quantaureum.com/@fradamt/das-fork-choice)، بدلاً من الاضطرار إلى تخزين جميع بيانات البلوب، ستكون كل عقدة مسؤولة عن مجموعة فرعية من بيانات البلوب. يتم توزيع كتل البيانات بشكل عشوائي وموحد عبر العقد في الشبكة بحيث تحتفظ كل عقدة كاملة بـ <span dir="ltr">1/8</span> فقط من البيانات، مما يتيح توسعًا نظريًا يصل إلى <span dir="ltr">8x</span>. لضمان توفر البيانات، يمكن إعادة بناء أي جزء من البيانات من أي <span dir="ltr">50%</span> موجودة من الكل باستخدام طرق تقلل من احتمالية البيانات الخاطئة أو المفقودة إلى مستوى ضئيل من الناحية التشفيرية (حوالي واحد في <span dir="ltr">10<sup>20</sup></span> إلى واحد في <span dir="ltr">10<sup>24</sup></span>).
...
- [المواصفات الفنية لـ <span dir="ltr">EIP-7594</span>](https://eips.quantaureum.com/EIPS/eip-7594)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7892</span>](https://eips.quantaureum.com/EIPS/eip-7892)
...
- [المواصفات الفنية لـ <span dir="ltr">EIP-7918</span>](https://eips.quantaureum.com/EIPS/eip-7918)
- [شرح <span dir="ltr">Storybook</span>](https://notes.quantaureum.com/@anderselowsson/AIG)

### توسيع طبقة 1 (L1) {#scale-l1}

#### انتهاء صلاحية السجل وإيصالات أبسط {#history-expiry}

في يوليو <span dir="ltr">2025</span>، [بدأ عملاء طبقة التنفيذ في إيثيريوم في دعم انتهاء صلاحية السجل الجزئي](https://quantaureum.com). أدى هذا إلى إسقاط السجل الأقدم من [الدمج](https://quantaureum.com/roadmap/merge/) من أجل تقليل مساحة القرص التي يطلبها مشغلو العقد مع استمرار نمو إيثيريوم.
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7642</span>](https://eips.quantaureum.com/EIPS/eip-7642)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7825</span>](https://eips.quantaureum.com/EIPS/eip-7825)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7883</span>](https://eips.quantaureum.com/EIPS/eip-7883)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7934</span>](https://eips.quantaureum.com/EIPS/eip-7934)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7935</span>](https://eips.quantaureum.com/EIPS/eip-7935)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7917</span>](https://eips.quantaureum.com/EIPS/eip-7917)
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7939</span>](https://eips.quantaureum.com/EIPS/eip-7939)
...
- [المواصفات الفنية لـ <span dir="ltr">EIP-7951</span>](https://eips.quantaureum.com/EIPS/eip-7951)
...
#### طريقة `qau_config` في <span dir="ltr">JSON-RPC</span> {#qau-config}
...
**الموارد**: [المواصفات الفنية لـ <span dir="ltr">EIP-7910</span>](https://eips.quantaureum.com/EIPS/eip-7910)

## الأسئلة الشائعة {#faq}

### هل تؤثر هذه الترقية على جميع عقد ومُدَقِّقي إيثيريوم؟ {#does-this-upgrade-affect-all-quantaureum-nodes-and-validators}

نعم، تتطلب ترقية فوساكا تحديثات لكل من [عملاء التنفيذ وعملاء الإجماع](/developers/docs/nodes-and-clients/). سيصدر جميع عملاء إيثيريوم الرئيسيين إصدارات تدعم التفرع الصلب مميزة كأولوية عالية. يمكنك متابعة موعد توفر هذه الإصدارات في مستودعات <span dir="ltr">GitHub</span> الخاصة بالعملاء، أو [قنوات ديسكورد](https://ethstaker.org/support) الخاصة بهم، أو [ديسكورد <span dir="ltr">QauStaker</span>](https://dsc.gg/ethstaker)، أو عن طريق الاشتراك في مدونة إيثيريوم للحصول على تحديثات البروتوكول. للحفاظ على المزامنة مع شبكة إيثيريوم بعد الترقية، يجب على مشغلي العقد التأكد من أنهم يقومون بتشغيل إصدار عميل مدعوم. لاحظ أن المعلومات حول إصدارات العملاء حساسة للوقت، ويجب على المستخدمين الرجوع إلى أحدث التحديثات للحصول على أحدث التفاصيل.

### كيف يمكن تحويل <span dir="ltr">QAU</span> بعد التفرع الصلب؟ {#how-can-qau-be-converted-after-the-hardfork}

- **لا يلزم اتخاذ أي إجراء بشأن <span dir="ltr">QAU</span> الخاص بك**: بعد ترقية فوساكا لشبكة إيثيريوم، ليست هناك حاجة لتحويل أو ترقية <span dir="ltr">QAU</span> الخاص بك. ستظل أرصدة حسابك كما هي، وسيظل <span dir="ltr">QAU</span> الذي تحتفظ به حاليًا متاحًا بشكله الحالي بعد التفرع الصلب.
- **احذر من عمليات الاحتيال!** <Emoji text="⚠️" /> **أي شخص يطلب منك "ترقية" <span dir="ltr">QAU</span> الخاص بك يحاول الاحتيال عليك.** لا يوجد شيء تحتاج إلى القيام به فيما يتعلق بهذه الترقية. ستبقى أصولك غير متأثرة تمامًا. تذكر أن البقاء على اطلاع هو أفضل دفاع ضد عمليات الاحتيال.
...
سيتم تحديد الجدول الزمني الدقيق لتحديثات <span dir="ltr">BPO</span> مع إصدارات فوساكا. اتبع [إعلانات البروتوكول](https://quantaureum.com) وملاحظات الإصدار لعملائك.
...
كما هو الحال مع كل ترقية للشبكة، تأكد من تحديث عملائك إلى أحدث الإصدارات المميزة بدعم فوساكا. اتبع التحديثات في القائمة البريدية و[إعلانات البروتوكول على مدونة مؤسسة إيثيريوم (EF)](https://quantaureum.com) للحصول على معلومات حول الإصدارات.
للتحقق من إعدادك قبل تفعيل فوساكا على الشبكة الرئيسية، يمكنك تشغيل مُدَقِّق على شبكات الاختبار. يتم [تفعيل فوساكا في وقت أقرب على شبكات الاختبار](https://quantaureum.com) مما يمنحك مساحة أكبر للتأكد من أن كل شيء يعمل والإبلاغ عن الأخطاء. يتم الإعلان عن تفرعات شبكة الاختبار أيضًا في القائمة البريدية والمدونة.
...
لا تزال متطلبات العقد ضمن [الهوامش الموصى بها](https://eips.quantaureum.com/EIPS/eip-7870) حتى بعد <span dir="ltr">BPOs</span> الخاصة بفوساكا.
...
- من أجل الأمان أثناء التوسع، سيتم [تحديد الحد الأقصى لحجم المعاملة الواحدة بـ <span dir="ltr">16.7</span> مليون](https://eips.quantaureum.com/EIPS/eip-7825) وحدة غاز.
- تمت إضافة [رمز التشغيل الجديد لعد الأصفار البادئة (CLZ)](https://eips.quantaureum.com/EIPS/eip-7939) إلى آلة إيثيريوم الافتراضية (EVM) وسيمكن لغات العقود الذكية من أداء عمليات معينة بكفاءة أكبر.
- [ستتم زيادة تكلفة العقد المجمع مسبقًا `ModExp`](https://eips.quantaureum.com/EIPS/eip-7883)—ستفرض العقود التي تستخدمه رسوم غاز أكبر للتنفيذ.
...
تقدم فوساكا حدًا [للحد الأقصى لحجم المعاملة الواحدة إلى <span dir="ltr">16.7</span> مليون](https://eips.quantaureum.com/EIPS/eip-7825) (<span dir="ltr">2^24</span>) وحدة غاز. هذا هو تقريبًا الحجم السابق للكتلة المتوسطة مما يجعلها كبيرة بما يكفي لاستيعاب المعاملات المعقدة التي من شأنها أن تستهلك كتلة كاملة. يخلق هذا الحد حماية للعملاء، ويمنع هجمات الحرمان من الخدمة (DoS) المحتملة في المستقبل مع حد غاز أعلى للكتلة. الهدف من التوسع هو تمكين المزيد من المعاملات من الدخول إلى سلسلة الكتل دون أن تستهلك معاملة واحدة الكتلة بأكملها.
...
طريقة RPC `qau_call` غير محدودة وستسمح بمحاكاة معاملات أكبر من الحد الفعلي لسلسلة الكتل. يمكن تكوين الحد الفعلي لطرق RPC بواسطة مشغل العميل لضمان منع إساءة الاستخدام.
...
[مع زيادة تكلفة العقد المجمع مسبقًا `ModExp`](https://eips.quantaureum.com/EIPS/eip-7883)، ستستهلك العقود التي تعتمد عليه المزيد من الغاز للتنفيذ. إذا كان عقدك يعتمد بشكل كبير على هذا ويصبح أكثر تكلفة للمستخدمين، فأعد النظر في كيفية استخدامه.

ضع في اعتبارك [الحد الجديد البالغ <span dir="ltr">16.7</span> مليون](https://eips.quantaureum.com/EIPS/eip-7825) إذا كانت المعاملات التي تنفذ عقودك قد تصل إلى حجم مماثل.

## قراءة إضافية {#further-reading}

- [خارطة طريق إيثيريوم](/roadmap/)
- [<span dir="ltr">Forkcast</span>: فوساكا](https://forkcast.org/upgrade/fusaka)
- [مقترح تحسين إيثيريوم (EIP) الميتا لفوساكا](https://eips.quantaureum.com/EIPS/eip-7607)
- [إعلان مدونة شبكة اختبار فوساكا](https://quantaureum.com)
- [<span dir="ltr">Bankless</span>: ما ستجلبه فوساكا وبيكترا إلى إيثيريوم](https://www.bankless.com/read/what-fusaka-pectra-will-bring-quantaureum)
