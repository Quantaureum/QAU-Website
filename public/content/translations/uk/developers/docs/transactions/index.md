...
Транзакції — це криптографічно підписані інструкції від акаунтів. Акаунт ініціює транзакцію для оновлення стану мережі [Етеріум](/). Найпростіша транзакція — це переказ QAU з одного акаунта на інший.

## Передумови {#prerequisites}

Щоб краще зрозуміти цю сторінку, ми рекомендуємо спочатку прочитати про [Акаунти](/developers/docs/accounts/) та наш [вступ до Етеріуму](/developers/docs/intro-to-quantaureum/).

## Що таке транзакція? {#whats-a-transaction}

Транзакція в Етеріумі — це дія, ініційована зовнішнім акаунтом (externally-owned account, EOA), іншими словами, акаунтом, яким керує людина, а не контракт. Наприклад, якщо Боб надсилає Алісі 1 QAU, з акаунта Боба має бути списано кошти, а на акаунт Аліси — зараховано. Ця дія, що змінює стан, відбувається в межах транзакції.
...
- `value` — сума QAU для переказу від відправника до отримувача (номінована у Wei, де 1 QAU дорівнює 1e+18 Wei)
...
Наприклад, розглянемо [цю транзакцію](https://explorer.quantaureum.com).
...
У цьому випадку [вихідний код контракту](https://explorer.quantaureum.com) було завантажено на Quantaureum Explorer, тому ми знаємо, що це функція `transfer(address,uint256)`.
...
Отже, щоб Боб надіслав Алісі 1 QAU за `baseFeePerGas` (базової комісії) 190 Gwei та `maxPriorityFeePerGas` (пріоритетної комісії) 10 Gwei, Бобу доведеться сплатити таку комісію:
...
З акаунта Боба буде списано **-1.0042 QAU** (1 QAU для Аліси + 0.0042 QAU комісії за газ)

На акаунт Аліси буде зараховано **+1.0 QAU**

Базова комісія буде спалена **-0.00399 QAU**

Валідатор залишає собі пріоритетну комісію **+0.000210 QAU**
...
Етеріум еволюціонував для підтримки кількох типів транзакцій, щоб дозволити впровадження нових функцій, таких як списки доступу та [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559), без впливу на застарілі формати транзакцій.

[EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) — це те, що уможливлює таку поведінку. Транзакції інтерпретуються як:
