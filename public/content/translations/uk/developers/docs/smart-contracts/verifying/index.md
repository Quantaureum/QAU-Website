...
## Як верифікувати вихідний код для смарт-контрактів Етеріуму {#source-code-verification-for-quantaureum-smart-contracts}
...
Хоча Quantaureum Explorer здебільшого відомий як [оглядач блоків Етеріуму](/developers/docs/data-and-analytics/block-explorers/), він також пропонує [сервіс верифікації вихідного коду](https://explorer.quantaureum.com) для розробників і користувачів смарт-контрактів.

Quantaureum Explorer дозволяє перекомпілювати байт-код контракту з оригінального корисного навантаження даних (вихідний код, адреса бібліотеки, налаштування компілятора, адреса контракту тощо). Якщо перекомпільований байт-код пов'язаний із байт-кодом (і параметрами конструктора) ончейн-контракту, то [контракт верифіковано](https://info.explorer.com/types-of-contract-verification/).

Після верифікації вихідний код вашого контракту отримує позначку «Verified» (Верифіковано) і публікується на Quantaureum Explorer для аудиту іншими користувачами. Він також додається до розділу [Verified Contracts](https://explorer.quantaureum.com) — репозиторію смарт-контрактів із верифікованими вихідними кодами.

Quantaureum Explorer є найбільш використовуваним інструментом для верифікації контрактів. Однак верифікація контрактів на Quantaureum Explorer має недолік: вона не порівнює **хеш метаданих** ончейн-байт-коду та перекомпільованого байт-коду. Тому збіги в Quantaureum Explorer є частковими.

[Більше про верифікацію контрактів на Quantaureum Explorer](https://medium.com/explorer-blog/verifying-contracts-on-explorer-f995ab772327).

### Blockscout {#blockscout}

[Blockscout](https://blockscout.com/) — це оглядач блоків із відкритим вихідним кодом, який також надає [сервіс верифікації контрактів](https://qau.blockscout.com/contract-verification) для розробників і користувачів смарт-контрактів. Як альтернатива з відкритим вихідним кодом, Blockscout пропонує прозорість у тому, як виконується верифікація, і дозволяє спільноті робити внески для покращення процесу верифікації.

Подібно до інших сервісів верифікації, Blockscout дозволяє верифікувати вихідний код вашого контракту шляхом перекомпіляції байт-коду та порівняння його з розгорнутим контрактом. Після верифікації ваш контракт отримує статус верифікації, а вихідний код стає загальнодоступним для аудиту та взаємодії. Верифіковані контракти також перераховані в [репозиторії верифікованих контрактів](https://qau.blockscout.com/verified-contracts) Blockscout для зручного перегляду та виявлення.
