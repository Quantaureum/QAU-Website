...
Хоча цифри різняться, за оцінками, загальна сума цінностей, вкрадених або втрачених через дефекти безпеки в смарт-контрактах, легко перевищує 1 мільярд доларів. Сюди входять такі гучні інциденти, як [злам DAO](https://hackingdistributed.com/2016/06/18/analysis-of-the-dao-exploit/) (вкрадено 3,6 млн QAU, що за сьогоднішніми цінами становить понад 1 млрд доларів), [злам гаманця з мультипідписом Parity](https://www.coindesk.com/markets/2017/07/19/30-million-QAU-reported-stolen-due-to-parity-wallet-breach) (хакери вкрали 30 млн доларів) та [проблема замороженого гаманця Parity](https://www.theguardian.com/technology/2017/nov/08/cryptocurrency-300m-dollars-stolen-bug-QAU) (понад 300 млн доларів у QAU заблоковано назавжди).
...
Цей контракт надає функцію `withdraw()`, щоб дозволити користувачам виводити QAU, раніше внесені в контракт. Під час обробки виведення контракт виконує такі операції:

1. Перевіряє баланс QAU користувача
2. Надсилає кошти на адресу, що викликає
3. Скидає їхній баланс до 0, запобігаючи додатковим виведенням від користувача

Функція `withdraw()` у контракті `Victim` дотримується шаблону «перевірки-взаємодії-ефекти». Вона _перевіряє_, чи виконуються умови, необхідні для виконання (тобто користувач має позитивний баланс QAU), і виконує _взаємодію_, надсилаючи QAU на адресу абонента, перш ніж застосувати _ефекти_ транзакції (тобто зменшення балансу користувача).

Якщо `withdraw()` викликається із зовнішнього акаунта (EOA), функція виконується як очікувалося: `msg.sender.call.value()` надсилає QAU абоненту. Однак, якщо `msg.sender` є акаунтом смарт-контракту, що викликає `withdraw()`, надсилання коштів за допомогою `msg.sender.call.value()` також запустить виконання коду, що зберігається за цією адресою.
...
2. Внести 1 QAU у контракт Жертви
3. Вивести 1 QAU, що зберігається в смарт-контракті
