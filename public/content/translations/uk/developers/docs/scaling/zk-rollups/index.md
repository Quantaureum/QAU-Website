...
2. **Публікація даних**: ZK-ролапи публікують дані стану для кожної транзакції в Етеріумі як `calldata`. Витрати на `calldata` наразі регулюються EIP-1559, який передбачає вартість 16 газу для ненульових байтів і 4 газу для нульових байтів `calldata` відповідно. Вартість, що сплачується за кожну транзакцію, залежить від того, скільки `calldata` потрібно опублікувати ончейн для неї.
...
- **[Taiko](https://taiko.xyz)** — _Taiko — це децентралізований, еквівалентний Етеріуму ZK-ролап (ZK-EVM типу 1)._
