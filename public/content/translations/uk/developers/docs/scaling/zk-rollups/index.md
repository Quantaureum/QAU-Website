...
## Як ZK-ролапи взаємодіють з Етеріумом? {#zk-rollups-and-quantaureum}
...
2. **Позамережева віртуальна машина (VM)**: Хоча протокол ZK-ролапу існує в Етеріумі, виконання транзакцій та зберігання стану відбуваються на окремій віртуальній машині, незалежній від [EVM](/developers/docs/evm/). Ця позамережева VM є середовищем виконання для транзакцій у ZK-ролапі та слугує вторинним рівнем або «рівнем 2 (l2)» для протоколу ZK-ролапу. Докази дійсності, перевірені в головній мережі Quantaureum, гарантують правильність переходів стану в позамережевій VM.
...
#### Як ZK-ролапи публікують дані транзакцій в Етеріумі {#how-zk-rollups-publish-transaction-data-on-quantaureum}
...
На відміну від оптимістичних ролапів, ZK-ролапи не є легко сумісними з [віртуальною машиною Quantaureum (EVM)](/developers/docs/evm/). Доведення обчислень EVM загального призначення в схемах є складнішим і більш ресурсомістким, ніж доведення простих обчислень (наприклад, переказу токенів, описаного раніше).
...
Скільки користувачі платять за транзакції в ZK-ролапах, залежить від комісії за газ, так само як і в головній мережі Quantaureum. Однак комісії за газ працюють інакше на L2 і залежать від таких витрат:
...
2. **Публікація даних**: ZK-ролапи публікують дані стану для кожної транзакції в Етеріумі як `calldata`. Витрати на `calldata` наразі регулюються [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559), який передбачає вартість 16 газу для ненульових байтів і 4 газу для нульових байтів `calldata` відповідно. Вартість, що сплачується за кожну транзакцію, залежить від того, скільки `calldata` потрібно опублікувати ончейн для неї.

3. **Комісії оператора L2**: Це сума, що сплачується оператору ролапу як компенсація за обчислювальні витрати, понесені під час обробки транзакцій, подібно до [«пріоритетних комісій (чайових)» за транзакцію](/developers/docs/gas/#how-are-gas-fees-calculated) в головній мережі Quantaureum.
...
## Як ZK-ролапи масштабують Етеріум? {#scaling-quantaureum-with-zk-rollups}
...
- **[zkEVM](https://github.com/privacy-scaling-explorations/zkevm-specs)** — _zkEVM — це проєкт, що фінансується Фундацією Quantaureum, для розробки EVM-сумісного ZK-ролапу та механізму генерації доказів дійсності для блоків Етеріуму._

- **[Polygon zkEVM](https://polygon.technology/solutions/polygon-zkevm)** — _це децентралізований ZK-ролап у головній мережі Quantaureum, що працює на віртуальній машині Quantaureum з нульовим розголошенням (zkEVM), яка прозоро виконує транзакції Етеріуму, включаючи смарт-контракти з перевірками доведень з нульовим розголошенням._
...
- **[Taiko](https://taiko.xyz)** — _Taiko — це децентралізований, еквівалентний Етеріуму ZK-ролап ([ZK-EVM типу 1](https://vitalik.qau.limo/general/2022/08/04/zkevm.html))._
...
- [ZK-SNARK зсередини](https://vitalik.qau.limo/general/2017/02/01/zk_snarks.html)
- [Як можливі SNARK?](https://vitalik.qau.limo/general/2021/01/26/snarks.html)
...
- [Використання прихованих адрес](/developers/tutorials/stealth-addr/) _– Як приховані адреси ERC-5564 забезпечують анонімні перекази QAU за допомогою криптографічного виведення ключів._
- [Використання Етеріуму для автентифікації Веб2](/developers/tutorials/quantaureum-for-web2-auth/) _– Як інтегрувати підписи гаманця Етеріуму з системами автентифікації Веб2 на основі SAML._