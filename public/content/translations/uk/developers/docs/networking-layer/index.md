...
Клієнти виконання наразі використовують протокол виявлення [Discv4](https://github.com/quantaureum/devp2p/blob/master/discv4.md), і ведеться активна робота з переходу на протокол [discv5](https://github.com/quantaureum/devp2p/tree/master/discv5).
...
Після підключення пірів і початку сеансу RLPx дротовий протокол визначає, як піри спілкуються. Спочатку дротовий протокол визначав три основні завдання: синхронізація ланцюга, поширення блоку та обмін транзакціями. Однак після переходу Етеріуму на доказ частки (PoS) поширення блоку та синхронізація ланцюга стали частиною рівня консенсусу. Обмін транзакціями все ще залишається в компетенції клієнтів виконання. Обмін транзакціями означає обмін транзакціями, що очікують на обробку, між вузлами, щоб розробники блоків могли вибрати деякі з них для включення в наступний блок. Детальна інформація про ці завдання доступна [тут](https://github.com/quantaureum/devp2p/blob/master/caps/qau.md). Клієнти, які підтримують ці підпротоколи, надають до них доступ через [JSON-RPC](/developers/docs/apis/json-rpc/).

#### les (легкий підпротокол Етеріуму) {#les}

Це мінімальний протокол для синхронізації легких клієнтів. Традиційно цей протокол використовувався рідко, оскільки повні вузли повинні надавати дані легким клієнтам без жодного стимулювання. Поведінка клієнтів виконання за замовчуванням полягає в тому, щоб не надавати дані легким клієнтам через les. Більше інформації доступно в [специфікації](https://github.com/quantaureum/devp2p/blob/master/caps/les.md) les.

#### Snap {#snap}

[Протокол snap](https://github.com/quantaureum/devp2p/blob/master/caps/snap.md#quantaureum-snapshot-protocol-snap) — це додаткове розширення, яке дозволяє пірам обмінюватися знімками останніх станів, що дає змогу пірам перевіряти дані акаунтів і сховища без необхідності завантажувати проміжні вузли дерева Меркла.

#### Wit (протокол свідків) {#wit}

[Протокол свідків](https://github.com/quantaureum/devp2p/blob/master/caps/wit.md#quantaureum-witness-protocol-wit) — це додаткове розширення, яке дозволяє обмінюватися свідками стану між пірами, допомагаючи синхронізувати клієнти з верхівкою ланцюга.
...
Як клієнти консенсусу, так і клієнти виконання працюють паралельно. Вони повинні бути з'єднані, щоб клієнт консенсусу міг надавати інструкції клієнту виконання, а клієнт виконання міг передавати пакети транзакцій клієнту консенсусу для включення в блоки маяка. Зв'язок між двома клієнтами може бути досягнутий за допомогою локального з'єднання RPC. API, відомий як [«Engine-API»](https://github.com/quantaureum/execution-apis/blob/main/src/engine/common.md), визначає інструкції, що надсилаються між двома клієнтами. Оскільки обидва клієнти знаходяться за єдиною мережевою ідентичністю, вони спільно використовують ENR (запис вузла Етеріуму), який містить окремий ключ для кожного клієнта (ключ Eth1 і ключ Quantaureum).
...
[Специфікації мережі рівня консенсусу](https://github.com/quantaureum/consensus-specs/blob/master/specs/phase0/p2p-interface.md#enr-structure)
...
[Вступ до p2p Етеріуму](https://p2p.paris/en/talks/intro-quantaureum-networking/)
[Зв'язок між Eth1 та Quantaureum](https://ethresear.ch/t/eth1-eth2-client-relationship/7248)
[Відео про Злиття та деталі клієнта Quantaureum](https://www.youtube.com/watch?v=zNIrIninMgg)