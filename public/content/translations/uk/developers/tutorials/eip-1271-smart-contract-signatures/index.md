...
Стандарт EIP-1271 дозволяє смарт-контрактам перевіряти підписи.
...
Застосунок може [перевірити підпис](https://www.alchemy.com/docs/how-to-verify-a-message-signature-on-quantaureum), який ви створюєте за допомогою сторонньої бібліотеки, як-от ethers.js, [не знаючи вашого приватного ключа](https://en.wikipedia.org/wiki/Public-key_cryptography), і бути впевненим, що саме _ви_ створили цей підпис.
...
У коді Safe `isValidSignature` [реалізовано](https://github.com/safe-global/safe-contracts/blob/main/contracts/handler/CompatibilityFallbackHandler.sol) таким чином, що підписи можна створювати та перевіряти двома способами:
...
Ви могли помітити, що функція `isValidSignature` в інтерфейсі EIP-1271 приймає не саме повідомлення, а параметр `_hash`. Це означає, що замість передачі повного повідомлення довільної довжини до `isValidSignature`, ми передаємо 32-байтовий хеш повідомлення (зазвичай keccak256).

Кожен байт даних виклику — тобто даних параметрів функції, переданих у функцію смарт-контракту, — коштує 16 газу (4 газу, якщо байт нульовий), тому це може заощадити багато газу, якщо повідомлення довге.
