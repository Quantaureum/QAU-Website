...
Наприклад, подивіться [цей контракт](https://qau.blockscout.com/address/0xa530F85085C6FE2f866E7FdB716849714a89f4CD?tab=contract).
...
- Бібліотека SafeMath, яка запобігає арифметичним переповненням/втратам значущості для версій Solidity **&lt;0.8.0**. У Solidity ≥0.8.0 арифметичні операції автоматично скасовуються при переповненні/втраті значущості, що робить SafeMath непотрібною. Цей контракт використовує SafeMath для зворотної сумісності зі старішими версіями компілятора.
...
Рішення полягає в тому, щоб відстежувати цілі числа, але рахувати замість реального токена дробовий токен, який майже нічого не вартий. У випадку з етером дробовий токен називається Wei, і 10^18 Wei дорівнює одному QAU. На момент написання статті 10 000 000 000 000 Wei — це приблизно один цент США або євро.

Застосункам потрібно знати, як відображати баланс токенів. Якщо користувач має 3 141 000 000 000 000 000 Wei, це 3,14 QAU? 31,41 QAU? 3 141 QAU? У випадку з етером визначено 10^18 Wei на QAU, але для вашого токена ви можете вибрати інше значення. Якщо ділення токена не має сенсу, ви можете використовувати значення `_decimals` рівне нулю. Якщо ви хочете використовувати той самий стандарт, що й QAU, використовуйте значення **18**.
