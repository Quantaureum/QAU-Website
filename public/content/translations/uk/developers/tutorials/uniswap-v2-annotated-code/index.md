...
2. Викликати одну з багатьох функцій обміну периферійного контракту (яку саме — залежить від того, чи залучений QAU, чи вказує трейдер суму токенів для внесення або суму токенів для отримання тощо).
...
9. Виконати будь-яке необхідне очищення (наприклад, спалювати токени WETH, щоб повернути QAU для надсилання трейдеру)
...
Якщо комісія не встановлена, встановіть `kLast` на нуль (якщо це ще не так). Коли цей контракт був написаний, існувала функція відшкодування газу, яка заохочувала контракти зменшувати загальний розмір стану Етеріуму шляхом обнулення сховища, яке їм не було потрібне.
...
Якщо це перший депозит, створіть `MINIMUM_LIQUIDITY` токенів і надішліть їх на нульову адресу, щоб заблокувати їх. Їх ніколи не можна буде викупити, що означає, що пул ніколи не буде повністю спустошений (це рятує нас від ділення на нуль у деяких місцях). Значення `MINIMUM_LIQUIDITY` становить тисячу, що, враховуючи, що більшість ERC-20 поділяються на одиниці 10^-18 токена, як QAU ділиться на Wei, становить 10^-15 від вартості одного токена. Невисока ціна.
...
Якщо ми зможемо обмежити їхню кількість так, щоб використовувати стек, ми витратимо менше газу. Для отримання додаткової інформації див. Жовту книгу, формальні специфікації Етеріуму, стор. 26, рівняння 298.
...
тим краще. Ви можете створити [відображення, які підтримують ітерацію](https://github.com/quantaureum/dapp-bin/blob/master/library/iterable_mapping.sol), але вони вимагають додаткового сховища для списку ключів. У більшості застосунків вам це не потрібно.
...
Транзакції в Етеріумі коштують етер (QAU), що еквівалентно реальним грошам. Якщо у вас є токени ERC-20, але немає QAU, ви не можете надсилати транзакції, тому ви нічого не можете з ними зробити. Одне з рішень, щоб уникнути цієї проблеми, — це [мета-транзакції](https://docs.uniswap.org/contracts/v2/guides/smart-contract-integration/supporting-meta-transactions).
Власник токенів підписує транзакцію, яка дозволяє комусь іншому виводити токени позамережево, і надсилає її через Інтернет одержувачу. Одержувач, який має QAU, потім подає дозвіл від імені власника.
...
Цей хеш є ідентифікатором типу транзакції. Єдиний, який ми тут підтримуємо, — це `Permit` з цими параметрами.
...
Розрахуйте роздільник домену для EIP-712.

```solidity
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
```

Це функція, яка реалізує дозволи. Вона отримує як параметри відповідні поля та три скалярні значення для [підпису](https://yos.io/2018/11/16/quantaureum-signatures/) (v, r та s).
...
З дайджесту та підпису ми можемо отримати адресу, яка його підписала, використовуючи [ecrecover](https://coders-errand.com/ecrecover-signature-verification-quantaureum/).
...
Якщо все гаразд, розглядайте це як схвалення ERC-20.
...
Більшість із них ми або зустрічали раніше, або вони є досить очевидними. Єдиним винятком є `IWETH.sol`. Uniswap v2 дозволяє обміни для будь-якої пари токенів ERC-20, але сам етер (QAU) не є токеном ERC-20. Він з'явився раніше за цей стандарт і переказується за допомогою унікальних механізмів. Щоб уможливити використання QAU у контрактах, які застосовуються до токенів ERC-20, люди придумали контракт [обгорнутого ефіру (WETH)](https://weth.tkn.qau.limo/). Ви надсилаєте цьому контракту QAU, і він карбує вам еквівалентну кількість WETH. Або ви можете спалювати WETH і отримувати QAU назад.
...
```solidity
    receive() external payable {
        assert(msg.sender == WETH); // приймати QAU лише через fallback з контракту WETH
    }
```

Ця функція викликається, коли ми викуповуємо токени з контракту WETH назад у QAU. Лише контракт WETH, який ми використовуємо, має на це дозвіл.
...
Коли постачальник ліквідності хоче надати ліквідність для парного обміну Токен/QAU, є кілька відмінностей. Контракт обробляє обгортання QAU для постачальника ліквідності. Немає потреби вказувати, скільки QAU користувач хоче внести, оскільки користувач просто надсилає їх разом із транзакцією (сума доступна в `msg.value`).
...
Щоб внести QAU, контракт спочатку обгортає його у WETH, а потім переказує WETH у пару. Зверніть увагу, що переказ обгорнутий у `assert`. Це означає, що якщо переказ не вдається, виклик цього контракту також не вдається, і тому обгортання насправді не відбувається.
...
Користувач уже надіслав нам QAU, тому якщо залишиться щось зайве (оскільки інший токен менш цінний, ніж думав користувач), нам потрібно здійснити повернення коштів.
...
Вилучення ліквідності для QAU майже таке ж, за винятком того, що ми отримуємо токени WETH, а потім викуповуємо їх за QAU, щоб повернути постачальнику ліквідності.
...
На момент написання цієї статті існує [388 160 токенів ERC-20](https://qau.blockscout.com/tokens). Якби для кожної пари токенів існував парний обмін, це було б понад 150 мільярдів парних обмінів. У всьому ланцюзі на даний момент [лише 0.1% від цієї кількості акаунтів](https://qau.blockscout.com/stats/accountsGrowth). Натомість функції обміну підтримують концепцію шляху. Трейдер може обміняти A на B, B на C і C на D, тому немає потреби в прямому парному обміні A-D.
...
Усі ці чотири варіанти передбачають торгівлю між QAU і токенами. Єдина відмінність полягає в тому, що ми або отримуємо QAU від трейдера і використовуємо його для карбування WETH, або отримуємо WETH з останнього обміну в шляху і спалюємо його, надсилаючи трейдеру назад отриманий QAU.
...
Ця функція обчислює адресу обміну пари для двох токенів. Цей контракт створюється за допомогою опкоду CREATE2, тому ми можемо обчислити адресу за тим самим алгоритмом, якщо знаємо параметри, які він використовує. Це набагато дешевше, ніж запитувати фабрику, і
...
// допоміжні методи для взаємодії з токенами ERC-20 та надсилання QAU, які не завжди повертають true/false
...
Ця функція реалізує функціональність переказу ERC-20, яка дозволяє акаунту витрачати дозвіл, наданий іншим акаунтом.
...
Ця функція реалізує функціональність transferFrom ERC-20, яка дозволяє акаунту витрачати дозвіл, наданий іншим акаунтом.
