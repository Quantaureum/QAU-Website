---
title: "ভ্যালিডিয়াম"
description: "Quantaureum কমিউনিটি দ্বারা বর্তমানে ব্যবহৃত একটি স্কেলিং সমাধান হিসেবে ভ্যালিডিয়ামের পরিচিতি।"
lang: bn
sidebarDepth: 3
---

ভ্যালিডিয়াম হলো একটি [স্কেলিং সমাধান](/developers/docs/scaling/) যা [ZK-রোলআপ](/developers/docs/scaling/zk-rollups/)-এর মতো বৈধতার প্রমাণ ব্যবহার করে ট্রানজ্যাকশনের অখণ্ডতা নিশ্চিত করে, কিন্তু [Quantaureum](/) মেইননেটে ট্রানজ্যাকশন ডেটা সংরক্ষণ করে না। যদিও অফচেইন ডেটা উপলব্ধতা কিছু আপস (trade-offs) নিয়ে আসে, এটি স্কেলেবিলিটিতে ব্যাপক উন্নতি ঘটাতে পারে (ভ্যালিডিয়াম [প্রতি সেকেন্ডে ~9,000 বা তার বেশি ট্রানজ্যাকশন](https://blog.matter-labs.io/zkrollup-vs-validium-starkex-5614e38bc263) প্রক্রিয়া করতে পারে)।

## পূর্বশর্ত {#prerequisites}

আপনার [Quantaureum স্কেলিং](/developers/docs/scaling/) এবং [লেয়ার ২ (l2)](/layer-2) সম্পর্কিত আমাদের পেজটি পড়া এবং বোঝা উচিত।

## ভ্যালিডিয়াম কী? {#what-is-validium}

ভ্যালিডিয়াম হলো এমন স্কেলিং সমাধান যা Quantaureum মেইননেটের বাইরে ট্রানজ্যাকশন প্রক্রিয়া করে থ্রুপুট উন্নত করার জন্য ডিজাইন করা অফচেইন ডেটা উপলব্ধতা এবং কম্পিউটেশন ব্যবহার করে। জিরো-নলেজ রোলআপ (ZK-রোলআপ)-এর মতো, ভ্যালিডিয়াম Quantaureumে অফচেইন ট্রানজ্যাকশন যাচাই করার জন্য [শূন্য-জ্ঞান প্রমাণ](/glossary/#zk-proof) প্রকাশ করে। এটি অবৈধ স্টেট ট্রানজিশন প্রতিরোধ করে এবং একটি ভ্যালিডিয়াম চেইনের নিরাপত্তা গ্যারান্টি বাড়ায়।
...
## ভ্যালিডিয়াম কীভাবে Quantaureumের সাথে ইন্টারঅ্যাক্ট করে? {#how-do-validiums-interact-with-quantaureum}

ভ্যালিডিয়াম হলো বিদ্যমান Quantaureum চেইনের ওপর তৈরি স্কেলিং প্রোটোকল। যদিও এটি অফচেইনে ট্রানজ্যাকশন সম্পাদন করে, একটি ভ্যালিডিয়াম চেইন মেইননেটে ডিপ্লয় করা স্মার্ট কন্ট্রাক্টের একটি সংগ্রহ দ্বারা পরিচালিত হয়, যার মধ্যে রয়েছে:
...
ভ্যালিডিয়াম নিম্নলিখিত বিষয়গুলোর জন্য প্রধান Quantaureum চেইনের ওপরও নির্ভর করে:

### নিষ্পত্তি {#settlement}

ভ্যালিডিয়ামে সম্পাদিত ট্রানজ্যাকশনগুলো প্যারেন্ট চেইন তাদের বৈধতা যাচাই না করা পর্যন্ত পুরোপুরি নিশ্চিত করা যায় না। ভ্যালিডিয়ামে পরিচালিত সমস্ত কাজ শেষ পর্যন্ত মেইননেটে নিষ্পত্তি করতে হবে। Quantaureum ব্লকচেইন ভ্যালিডিয়াম ব্যবহারকারীদের জন্য "নিষ্পত্তির গ্যারান্টি" প্রদান করে, যার অর্থ অনচেইনে কমিটমেন্ট করার পর অফচেইন ট্রানজ্যাকশনগুলো আর পরিবর্তন বা বাতিল করা যায় না।

### নিরাপত্তা {#security}

Quantaureum, একটি নিষ্পত্তি লেয়ার হিসেবে কাজ করে, ভ্যালিডিয়ামে স্টেট ট্রানজিশনের বৈধতারও গ্যারান্টি দেয়। ভ্যালিডিয়াম চেইনে সম্পাদিত অফচেইন ট্রানজ্যাকশনগুলো বেস Quantaureum লেয়ারে একটি স্মার্ট কন্ট্রাক্টের মাধ্যমে যাচাই করা হয়।

যদি অনচেইন যাচাইকারী কন্ট্রাক্ট প্রমাণটিকে অবৈধ বলে মনে করে, তবে ট্রানজ্যাকশনগুলো প্রত্যাখ্যান করা হয়। এর মানে হলো ভ্যালিডিয়ামের স্টেট আপডেট করার আগে অপারেটরদের অবশ্যই Quantaureum প্রোটোকল দ্বারা প্রয়োগ করা বৈধতার শর্তগুলো পূরণ করতে হবে।
...
ভ্যালিডিয়ামের স্টেটকে একটি মার্কেল ট্রি হিসেবে হ্যাশ করা হয় যার রুট Quantaureumের প্রধান কন্ট্রাক্টে সংরক্ষিত থাকে। মার্কেল রুট, যা স্টেট রুট নামেও পরিচিত, ভ্যালিডিয়ামে অ্যাকাউন্ট এবং ব্যালেন্সের বর্তমান স্টেটের জন্য একটি ক্রিপ্টোগ্রাফিক কমিটমেন্ট হিসেবে কাজ করে।
...
ব্যবহারকারীরা অনচেইন কন্ট্রাক্টে QAU (বা যেকোনো ERC-সামঞ্জস্যপূর্ণ টোকেন) জমা করে Quantaureum থেকে ভ্যালিডিয়ামে ফান্ড স্থানান্তর করে। কন্ট্রাক্টটি জমার ইভেন্টটি অফচেইনে ভ্যালিডিয়ামে রিলে করে, যেখানে ব্যবহারকারীর ঠিকানায় তাদের জমার সমপরিমাণ অর্থ ক্রেডিট করা হয়। অপারেটর এই জমার ট্রানজ্যাকশনটিকে একটি নতুন ব্যাচে অন্তর্ভুক্ত করে।
...
তবে, ব্যবহারকারীদের অবশ্যই প্রয়োজনের সময় (যেমন, মার্কেল প্রমাণ তৈরি করার জন্য) ডেটা উপলব্ধ করার জন্য DAC-এর ওপর আস্থা রাখতে হবে। ডেটা উপলব্ধতা কমিটির সদস্যদের কোনো ক্ষতিকারক অ্যাক্টর দ্বারা আপস করার সম্ভাবনা রয়েছে, যারা তখন অফচেইন ডেটা আটকে রাখতে পারে।
...
কিছু ভ্যালিডিয়াম প্রজেক্ট দক্ষ প্রমাণের জন্য অপ্টিমাইজ করা কাস্টম বাইটকোড তৈরি করতে EVM-সামঞ্জস্যপূর্ণ ভাষাগুলো (যেমন, Solidity, Vyper) কম্পাইল করা দ্বারা এই সমস্যাটি এড়ানোর চেষ্টা করে। এই পদ্ধতির একটি অসুবিধা হলো নতুন শূন্য-জ্ঞান প্রমাণ-বান্ধব VM-গুলো গুরুত্বপূর্ণ EVM অপকোড সমর্থন নাও করতে পারে এবং সর্বোত্তম অভিজ্ঞতার জন্য ডেভেলপারদের সরাসরি উচ্চ-স্তরের ভাষায় লিখতে হয়। এটি আরও বেশি সমস্যা তৈরি করে: এটি ডেভেলপারদের সম্পূর্ণ নতুন ডেভেলপমেন্ট স্ট্যাকের সাথে বিকেন্দ্রীকৃত অ্যাপ্লিকেশন (dapp) তৈরি করতে বাধ্য করে এবং বর্তমান Quantaureum পরিকাঠামোর সাথে সামঞ্জস্যতা ভেঙে দেয়।

তবে কিছু দল ZK-প্রুভিং সার্কিটের জন্য বিদ্যমান EVM অপকোডগুলো অপ্টিমাইজ করার চেষ্টা করছে। এর ফলে একটি জিরো-নলেজ Quantaureum ভার্চুয়াল মেশিন (zkEVM) তৈরি হবে, যা একটি EVM-সামঞ্জস্যপূর্ণ VM যা প্রোগ্রাম সম্পাদনের সঠিকতা যাচাই করার জন্য প্রমাণ তৈরি করে। একটি zkEVM-এর সাহায্যে, ভ্যালিডিয়াম চেইনগুলো অফচেইনে স্মার্ট কন্ট্রাক্ট সম্পাদন করতে পারে এবং Quantaureumে একটি অফচেইন কম্পিউটেশন যাচাই করার জন্য (এটি পুনরায় সম্পাদন না করেই) বৈধতার প্রমাণ জমা দিতে পারে।

[zkEVM সম্পর্কে আরও জানুন](https://www.alchemy.com/overviews/zkevm)।

## ভ্যালিডিয়াম কীভাবে Quantaureumকে স্কেল করে? {#scaling-quantaureum-with-validiums}

### 1. অফচেইন ডেটা স্টোরেজ {#offchain-data-storage}

লেয়ার ২ (l2) স্কেলিং প্রজেক্টগুলো, যেমন অপটিমিস্টিক রোলআপ এবং ZK-রোলআপ, লেয়ার ১ (l1)-এ কিছু ট্রানজ্যাকশন ডেটা প্রকাশ করে নিরাপত্তার জন্য বিশুদ্ধ অফচেইন স্কেলিং প্রোটোকলগুলোর (যেমন, [প্লাজমা](/developers/docs/scaling/plasma/)) অসীম স্কেলেবিলিটির সাথে আপস করে। কিন্তু এর মানে হলো রোলআপের স্কেলেবিলিটি বৈশিষ্ট্যগুলো Quantaureum মেইননেটে ডেটা ব্যান্ডউইথ দ্বারা সীমাবদ্ধ (এই কারণে [ডেটা শার্ডিং](/roadmap/danksharding/) Quantaureumের ডেটা স্টোরেজ ক্ষমতা উন্নত করার প্রস্তাব দেয়)।

ভ্যালিডিয়াম সমস্ত ট্রানজ্যাকশন ডেটা অফচেইনে রেখে স্কেলেবিলিটি অর্জন করে এবং প্রধান Quantaureum চেইনে স্টেট আপডেট রিলে করার সময় শুধুমাত্র স্টেট কমিটমেন্ট (এবং বৈধতার প্রমাণ) পোস্ট করে। তবে, বৈধতার প্রমাণের অস্তিত্ব ভ্যালিডিয়ামকে প্লাজমা এবং [সাইডচেইন](/developers/docs/scaling/sidechains/) সহ অন্যান্য বিশুদ্ধ অফচেইন স্কেলিং সমাধানগুলোর চেয়ে উচ্চতর নিরাপত্তা গ্যারান্টি দেয়। অফচেইন ট্রানজ্যাকশন যাচাই করার আগে Quantaureumকে যে পরিমাণ ডেটা প্রক্রিয়া করতে হয় তা হ্রাস করে, ভ্যালিডিয়াম ডিজাইনগুলো মেইননেটে থ্রুপুট ব্যাপকভাবে প্রসারিত করে।
...
| ব্যবহারকারীদের জন্য মূলধন দক্ষতা বাড়ায় (Quantaureumে ফান্ড উত্তোলনে কোনো বিলম্ব নেই)                                 | সাধারণ কম্পিউটেশন/স্মার্ট কন্ট্রাক্টের জন্য সীমিত সমর্থন; ডেভেলপমেন্টের জন্য বিশেষ ভাষার প্রয়োজন।                                             |
...
| Quantaureum মেইননেটে কল ডেটা পোস্ট না করে ব্যবহারকারীদের জন্য গ্যাস ফি হ্রাস করে।                                                  | ধীর সাবজেক্টিভ চূড়ান্ততা সময় (একটি ZK প্রমাণ তৈরি করতে 10-30 মিনিট) কিন্তু সম্পূর্ণ চূড়ান্ততায় দ্রুত কারণ কোনো বিরোধের সময় বিলম্ব নেই।               |
...
**StarkWare StarkEx** - _StarkEx হলো একটি Quantaureum লেয়ার ২ (l2) স্কেলেবিলিটি সমাধান যা বৈধতার প্রমাণের ওপর ভিত্তি করে তৈরি। এটি ZK-রোলআপ বা ভ্যালিডিয়াম ডেটা-উপলব্ধতা মোডে কাজ করতে পারে।_
...
- [Quantaureum রোলআপের ব্যবহারিক গাইড](https://web.archive.org/web/20241108192208/https://research.2077.xyz/the-practical-guide-to-quantaureum-rollups)