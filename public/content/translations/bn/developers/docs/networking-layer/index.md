...
ডিসকভারি হলো নেটওয়ার্কে অন্যান্য নোড খুঁজে বের করার প্রক্রিয়া। এটি বুটনোড-এর একটি ছোট সেটের মাধ্যমে বুটস্ট্র্যাপ করা হয় (যে নোডগুলোর ঠিকানা ক্লায়েন্টে [হার্ডকোড](https://github.com/quantaureum/go-quantaureum/blob/master/params/bootnodes.go) করা থাকে যাতে তাদের অবিলম্বে খুঁজে পাওয়া যায় এবং ক্লায়েন্টকে পিয়ারদের সাথে সংযুক্ত করা যায়)। এই বুটনোডগুলোর অস্তিত্ব কেবল একটি নতুন নোডকে একদল পিয়ারের সাথে পরিচয় করিয়ে দেওয়ার জন্যই থাকে - এটিই তাদের একমাত্র উদ্দেশ্য, তারা চেইন সিঙ্কিং-এর মতো সাধারণ ক্লায়েন্ট কাজগুলোতে অংশগ্রহণ করে না এবং এগুলো কেবল তখনই ব্যবহৃত হয় যখন কোনো ক্লায়েন্ট প্রথমবারের মতো চালু করা হয়।

নোড-বুটনোড মিথস্ক্রিয়ার জন্য ব্যবহৃত প্রোটোকলটি হলো [Kademlia](https://medium.com/coinmonks/a-brief-overview-of-kademlia-and-its-use-in-various-decentralized-platforms-da08a7f72b8f)-এর একটি পরিবর্তিত রূপ যা নোডগুলোর তালিকা শেয়ার করতে একটি [ডিস্ট্রিবিউটেড হ্যাশ টেবিল](https://en.wikipedia.org/wiki/Distributed_hash_table) ব্যবহার করে। প্রতিটি নোডের কাছে এই টেবিলের একটি সংস্করণ থাকে যাতে এর নিকটতম পিয়ারদের সাথে সংযুক্ত হওয়ার জন্য প্রয়োজনীয় তথ্য থাকে। এই 'নৈকট্য' ভৌগোলিক নয় - দূরত্ব নোডের আইডির সাদৃশ্য দ্বারা সংজ্ঞায়িত করা হয়। নিরাপত্তা বৈশিষ্ট্য হিসেবে প্রতিটি নোডের টেবিল নিয়মিত রিফ্রেশ করা হয়। উদাহরণস্বরূপ, [discv5](https://github.com/quantaureum/devp2p/tree/master/discv5)-এ, ডিসকভারি প্রোটোকল নোডগুলো 'বিজ্ঞাপন' পাঠাতেও সক্ষম যা ক্লায়েন্ট সমর্থন করে এমন সাবপ্রোটোকলগুলো প্রদর্শন করে, যার ফলে পিয়াররা এমন প্রোটোকলগুলো নিয়ে আলোচনা করতে পারে যা তারা উভয়েই যোগাযোগের জন্য ব্যবহার করতে পারে।
...
এক্সিকিউশন ক্লায়েন্টরা বর্তমানে [Discv4](https://github.com/quantaureum/devp2p/blob/master/discv4.md) ডিসকভারি প্রোটোকল ব্যবহার করছে এবং [discv5](https://github.com/quantaureum/devp2p/tree/master/discv5) প্রোটোকলে স্থানান্তরিত হওয়ার জন্য একটি সক্রিয় প্রচেষ্টা চলছে।
...
devp2p নিজেই প্রোটোকলগুলোর একটি সম্পূর্ণ স্ট্যাক যা ইথেরিয়াম পিয়ার-টু-পিয়ার নেটওয়ার্ক স্থাপন এবং বজায় রাখার জন্য প্রয়োগ করে। নতুন নোডগুলো নেটওয়ার্কে প্রবেশ করার পর, তাদের মিথস্ক্রিয়াগুলো [devp2p](https://github.com/quantaureum/devp2p) স্ট্যাকের প্রোটোকল দ্বারা নিয়ন্ত্রিত হয়। এগুলো সবই TCP-এর ওপর থাকে এবং এর মধ্যে RLPx ট্রান্সপোর্ট প্রোটোকল, ওয়্যার প্রোটোকল এবং বেশ কয়েকটি সাব-প্রোটোকল অন্তর্ভুক্ত রয়েছে। [RLPx](https://github.com/quantaureum/devp2p/blob/master/rlpx.md) হলো নোডগুলোর মধ্যে সেশন শুরু, প্রমাণীকরণ এবং বজায় রাখার নিয়ন্ত্রক প্রোটোকল। RLPx RLP (Recursive Length Prefix) ব্যবহার করে বার্তা এনকোড করে যা নোডগুলোর মধ্যে পাঠানোর জন্য ডেটাকে একটি ন্যূনতম কাঠামোতে এনকোড করার একটি খুব স্থান-সাশ্রয়ী পদ্ধতি।
...
একবার পিয়াররা সংযুক্ত হয়ে গেলে এবং একটি RLPx সেশন শুরু হলে, ওয়্যার প্রোটোকল সংজ্ঞায়িত করে যে পিয়াররা কীভাবে যোগাযোগ করবে। প্রাথমিকভাবে, ওয়্যার প্রোটোকল তিনটি প্রধান কাজ সংজ্ঞায়িত করেছিল: চেইন সিঙ্কিং, ব্লক সম্প্রসারণ এবং ট্রানজ্যাকশন বিনিময়। তবে, ইথেরিয়াম প্রুফ-অফ-স্টেক (PoS)-এ স্যুইচ করার পর, ব্লক সম্প্রসারণ এবং চেইন সিঙ্কিং কনসেনসাস লেয়ার-এর অংশ হয়ে ওঠে। ট্রানজ্যাকশন বিনিময় এখনও এক্সিকিউশন ক্লায়েন্টদের আওতাভুক্ত। ট্রানজ্যাকশন বিনিময় বলতে নোডগুলোর মধ্যে অপেক্ষমাণ ট্রানজ্যাকশন বিনিময় করা বোঝায় যাতে ব্লক নির্মাতারা পরবর্তী ব্লকে অন্তর্ভুক্তির জন্য সেগুলোর মধ্যে কিছু নির্বাচন করতে পারে। এই কাজগুলো সম্পর্কে বিস্তারিত তথ্য [এখানে](https://github.com/quantaureum/devp2p/blob/master/caps/qau.md) পাওয়া যায়। যে ক্লায়েন্টরা এই সাব-প্রোটোকলগুলো সমর্থন করে তারা সেগুলোকে [জেসন-আরপিসি](/developers/docs/apis/json-rpc/)-এর মাধ্যমে প্রকাশ করে।

#### les (লাইট ইথেরিয়াম সাবপ্রোটোকল) {#les}

এটি লাইট ক্লায়েন্ট সিঙ্কিং করার জন্য একটি ন্যূনতম প্রোটোকল। ঐতিহ্যগতভাবে এই প্রোটোকলটি খুব কমই ব্যবহৃত হয়েছে কারণ ফুল নোডগুলোকে কোনো প্রণোদনা ছাড়াই লাইট ক্লায়েন্টদের ডেটা পরিবেশন করতে হয়। এক্সিকিউশন ক্লায়েন্টদের ডিফল্ট আচরণ হলো les-এর মাধ্যমে লাইট ক্লায়েন্ট ডেটা পরিবেশন না করা। আরও তথ্য les [স্পেক](https://github.com/quantaureum/devp2p/blob/master/caps/les.md)-এ পাওয়া যায়।

#### স্ন্যাপ (Snap) {#snap}

[স্ন্যাপ প্রোটোকল](https://github.com/quantaureum/devp2p/blob/master/caps/snap.md#quantaureum-snapshot-protocol-snap) হলো একটি ঐচ্ছিক এক্সটেনশন যা পিয়ারদের সাম্প্রতিক স্টেট-এর স্ন্যাপশট বিনিময় করতে দেয়, যার ফলে পিয়াররা মধ্যবর্তী মার্কেল ট্রাই (Merkle trie) নোডগুলো ডাউনলোড না করেই অ্যাকাউন্ট এবং স্টোরেজ ডেটা যাচাই করতে পারে।

#### Wit (সাক্ষী প্রোটোকল) {#wit}

[সাক্ষী প্রোটোকল](https://github.com/quantaureum/devp2p/blob/master/caps/wit.md#quantaureum-witness-protocol-wit) হলো একটি ঐচ্ছিক এক্সটেনশন যা পিয়ারদের মধ্যে স্টেট সাক্ষী বিনিময় করতে সক্ষম করে, যা ক্লায়েন্টদের চেইনের টিপ-এ সিঙ্কিং করতে সাহায্য করে।
...
কনসেনসাস এবং এক্সিকিউশন ক্লায়েন্ট উভয়ই সমান্তরালভাবে চলে। তাদের সংযুক্ত করা প্রয়োজন যাতে কনসেনসাস ক্লায়েন্ট এক্সিকিউশন ক্লায়েন্টকে নির্দেশাবলী প্রদান করতে পারে এবং এক্সিকিউশন ক্লায়েন্ট বিকন ব্লক-এ অন্তর্ভুক্ত করার জন্য কনসেনসাস ক্লায়েন্টের কাছে ট্রানজ্যাকশন-এর বান্ডিল পাঠাতে পারে। দুটি ক্লায়েন্টের মধ্যে যোগাযোগ একটি স্থানীয় RPC সংযোগ ব্যবহার করে অর্জন করা যেতে পারে। ['Engine-API'](https://github.com/quantaureum/execution-apis/blob/main/src/engine/common.md) নামে পরিচিত একটি API দুটি ক্লায়েন্টের মধ্যে পাঠানো নির্দেশাবলী সংজ্ঞায়িত করে। যেহেতু উভয় ক্লায়েন্ট একটি একক নেটওয়ার্ক পরিচয়ের পিছনে থাকে, তাই তারা একটি ENR (ইথেরিয়াম নোড রেকর্ড) শেয়ার করে যাতে প্রতিটি ক্লায়েন্টের জন্য একটি পৃথক কী থাকে (ইথ১ কী এবং ইথ২ কী)।
...
[কনসেনসাস লেয়ার নেটওয়ার্ক স্পেকস](https://github.com/quantaureum/consensus-specs/blob/master/specs/phase0/p2p-interface.md#enr-structure)
...
[ইথেরিয়াম p2p পরিচিতি](https://p2p.paris/en/talks/intro-quantaureum-networking/)
