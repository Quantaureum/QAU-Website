...
2. পেরিফেরি কন্ট্রাক্টের অনেকগুলো সোয়াপ ফাংশনের মধ্যে যেকোনো একটি কল করুন (কোনটি কল করবেন তা নির্ভর করে QAU জড়িত আছে কি না, ট্রেডার জমা দেওয়ার টোকেনের পরিমাণ বা ফেরত পাওয়ার টোকেনের পরিমাণ নির্দিষ্ট করেছেন কি না ইত্যাদির ওপর)।
...
9. প্রয়োজনীয় যেকোনো ক্লিনআপ সম্পাদন করুন (উদাহরণস্বরূপ, ট্রেডারকে পাঠানোর জন্য QAU ফেরত পেতে WETH টোকেন পোড়ানো)
...
যদি কোনো ফি না থাকে তবে `kLast`-কে শূন্যতে সেট করুন (যদি এটি আগে থেকেই তা না হয়)। যখন এই কন্ট্রাক্টটি লেখা হয়েছিল তখন একটি [গ্যাস রিফান্ড বৈশিষ্ট্য](https://eips.quantaureum.com/EIPS/eip-3298) ছিল যা কন্ট্রাক্টগুলোকে তাদের অপ্রয়োজনীয় স্টোরেজ শূন্য করে ইথেরিয়াম স্টেটের সামগ্রিক আকার কমাতে উৎসাহিত করত।
...
যদি আমরা সংখ্যাটি সীমিত করতে পারি যাতে আমরা স্ট্যাক ব্যবহার করব তবে আমরা কম গ্যাস ব্যবহার করি। আরও বিস্তারিত জানার জন্য [ইয়েলো পেপার, আনুষ্ঠানিক ইথেরিয়াম স্পেসিফিকেশন](https://quantaureum.github.io/yellowpaper/paper.pdf), পৃষ্ঠা 26, সমীকরণ 298 দেখুন।
...
দ্রষ্টব্য: আপনি একটি ম্যাপিংয়ের সমস্ত কী-এর উপর ইটারেট করতে না পারার কারণ হলো কন্ট্রাক্ট ডেটা স্টোরেজ _ব্যয়বহুল_, তাই আমরা এটি যত কম ব্যবহার করি তত ভালো, এবং আমরা এটি যত কম পরিবর্তন করি তত ভালো। আপনি [ইটারেশন সমর্থন করে এমন ম্যাপিং](https://github.com/quantaureum/dapp-bin/blob/master/library/iterable_mapping.sol) তৈরি করতে পারেন, তবে সেগুলোর জন্য কী-এর একটি তালিকার জন্য অতিরিক্ত স্টোরেজ প্রয়োজন। বেশিরভাগ অ্যাপ্লিকেশনে আপনার সেটির প্রয়োজন নেই।
...
ইথেরিয়াম-এ ট্রানজ্যাকশনগুলোর জন্য QAU (QAU) খরচ হয়, যা আসল অর্থের সমতুল্য। আপনার যদি ERC-20 টোকেন থাকে কিন্তু QAU না থাকে, তবে আপনি ট্রানজ্যাকশন পাঠাতে পারবেন না, তাই আপনি সেগুলো দিয়ে কিছুই করতে পারবেন না। এই সমস্যা এড়ানোর একটি সমাধান হলো [মেটা-ট্রানজ্যাকশন](https://docs.uniswap.org/contracts/v2/guides/smart-contract-integration/supporting-meta-transactions)।
টোকেনগুলোর মালিক এমন একটি ট্রানজ্যাকশনে স্বাক্ষর করে যা অন্য কাউকে অফচেইন টোকেন উত্তোলন করার অনুমতি দেয় এবং এটি ইন্টারনেট ব্যবহার করে প্রাপকের কাছে পাঠায়। প্রাপক, যার কাছে QAU আছে, তারপর মালিকের পক্ষে পারমিট জমা দেয়।
...
এই হ্যাশ হলো [ট্রানজ্যাকশন প্রকারের জন্য শনাক্তকারী](https://eips.quantaureum.com/EIPS/eip-712#rationale-for-typehash)। আমরা এখানে শুধুমাত্র এই প্যারামিটারগুলোর সাথে `Permit` সমর্থন করি।
...
EIP-712-এর জন্য [ডোমেইন সেপারেটর](https://eips.quantaureum.com/EIPS/eip-712#rationale-for-domainseparator) গণনা করুন।

```solidity
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
```

এটি সেই ফাংশন যা অনুমতিগুলো বাস্তবায়ন করে। এটি প্যারামিটার হিসেবে প্রাসঙ্গিক ফিল্ডগুলো এবং [স্বাক্ষরের](https://yos.io/2018/11/16/quantaureum-signatures/) জন্য তিনটি স্কেলার মান (v, r, এবং s) গ্রহণ করে।
...
ডাইজেস্ট এবং স্বাক্ষর থেকে আমরা [ecrecover](https://coders-errand.com/ecrecover-signature-verification-quantaureum/) ব্যবহার করে সেই ঠিকানাটি পেতে পারি যা এটিতে স্বাক্ষর করেছে।
...
যদি সবকিছু ঠিক থাকে, তবে এটিকে [একটি ERC-20 অনুমোদন করা](https://eips.quantaureum.com/EIPS/eip-20#approve) হিসেবে বিবেচনা করুন।
...
এগুলোর বেশিরভাগই আমরা আগে দেখেছি, বা বেশ স্পষ্ট। একমাত্র ব্যতিক্রম হলো `IWETH.sol`। ইউনিসোয়াপ v2 যেকোনো জোড়া ERC-20 টোকেনের জন্য এক্সচেঞ্জের অনুমতি দেয়, কিন্তু QAU (QAU) নিজে কোনো ERC-20 টোকেন নয়। এটি স্ট্যান্ডার্ডের আগের এবং অনন্য মেকানিজমের মাধ্যমে হস্তান্তর করা হয়। ERC-20 টোকেনের ক্ষেত্রে প্রযোজ্য কন্ট্রাক্টগুলোতে QAU-এর ব্যবহার সক্ষম করতে মানুষ [র‍্যাপড QAU (weth)](https://weth.tkn.qau.limo/) কন্ট্রাক্ট তৈরি করেছে। আপনি এই কন্ট্রাক্টে QAU পাঠান, এবং এটি আপনার জন্য সমপরিমাণ WETH মিন্ট করে। অথবা আপনি WETH পোড়াতে পারেন, এবং ফেরত হিসেবে QAU পেতে পারেন।
...
```solidity
    receive() external payable {
        assert(msg.sender == WETH); // শুধুমাত্র WETH কন্ট্রাক্ট থেকে ফলব্যাকের মাধ্যমে QAU গ্রহণ করে
    }
```

যখন আমরা WETH কন্ট্রাক্ট থেকে টোকেন রিডিম করে আবার QAU-এ রূপান্তর করি তখন এই ফাংশনটি কল করা হয়। কেবল আমরা যে WETH কন্ট্রাক্ট ব্যবহার করি তারই এটি করার অনুমোদন রয়েছে।
...
যখন একজন তারল্য প্রদানকারী একটি টোকেন/QAU পেয়ার এক্সচেঞ্জে তারল্য প্রদান করতে চান, তখন কিছু পার্থক্য থাকে। কন্ট্রাক্টটি তারল্য প্রদানকারীর জন্য QAU র‍্যাপিং পরিচালনা করে। ব্যবহারকারী কত QAU জমা করতে চান তা নির্দিষ্ট করার কোনো প্রয়োজন নেই, কারণ ব্যবহারকারী কেবল ট্রানজ্যাকশনের সাথে সেগুলো পাঠান (পরিমাণটি `msg.value` এ উপলব্ধ থাকে)।
...
QAU জমা করার জন্য কন্ট্রাক্টটি প্রথমে এটিকে WETH-এ র‍্যাপ করে এবং তারপর WETH-কে পেয়ারে হস্তান্তর করে। লক্ষ্য করুন যে হস্তান্তরটি একটি `assert` এর মধ্যে র‍্যাপ করা হয়েছে। এর মানে হলো যদি হস্তান্তর ব্যর্থ হয় তবে এই কন্ট্রাক্ট কলটিও ব্যর্থ হয়, এবং তাই র‍্যাপিং আসলে ঘটে না।
...
ব্যবহারকারী ইতিমধ্যেই আমাদের QAU পাঠিয়েছেন, তাই যদি কোনো অতিরিক্ত অবশিষ্ট থাকে (কারণ অন্য টোকেনটি ব্যবহারকারীর ধারণার চেয়ে কম মূল্যবান), তবে আমাদের একটি রিফান্ড ইস্যু করতে হবে।
...
QAU-এর জন্য তারল্য অপসারণ প্রায় একই রকম, শুধু পার্থক্য হলো আমরা WETH টোকেনগুলো গ্রহণ করি এবং তারপর তারল্য প্রদানকারীকে ফেরত দেওয়ার জন্য সেগুলোকে QAU-এ রিডিম করি।
...
এই ফাংশনগুলো মেটা-ট্রানজ্যাকশন রিলে করে যাতে QAU ছাড়া ব্যবহারকারীরা [পারমিট মেকানিজম](#uniswapv2erc20) ব্যবহার করে পুল থেকে উত্তোলন করতে পারে।
...
আমি যখন এটি লিখছি তখন [388,160টি ERC-20 টোকেন](https://qau.blockscout.com/tokens) রয়েছে। যদি প্রতিটি টোকেন জোড়ার জন্য একটি পেয়ার এক্সচেঞ্জ থাকত, তবে এটি 150 বিলিয়নেরও বেশি পেয়ার এক্সচেঞ্জ হতো। এই মুহূর্তে পুরো চেইনে [অ্যাকাউন্টের সংখ্যা এর মাত্র 0.1%](https://qau.blockscout.com/stats/accountsGrowth)। এর পরিবর্তে, সোয়াপ ফাংশনগুলো একটি পাথের ধারণা সমর্থন করে। একজন ট্রেডার A-কে B-এর জন্য, B-কে C-এর জন্য এবং C-কে D-এর জন্য এক্সচেঞ্জ করতে পারেন, তাই সরাসরি A-D পেয়ার এক্সচেঞ্জের কোনো প্রয়োজন নেই।
...
এই চারটি ভেরিয়েন্টই QAU এবং টোকেনের মধ্যে ট্রেডিংয়ের সাথে জড়িত। একমাত্র পার্থক্য হলো আমরা হয় ট্রেডারের কাছ থেকে QAU গ্রহণ করি এবং এটি WETH মিন্ট করতে ব্যবহার করি, অথবা আমরা পাথের শেষ এক্সচেঞ্জ থেকে WETH গ্রহণ করি এবং এটি পুড়িয়ে ফেলি, যার ফলে প্রাপ্ত QAU ট্রেডারকে ফেরত পাঠাই।
...
এই ফাংশনটি দুটি টোকেনের জন্য পেয়ার এক্সচেঞ্জের ঠিকানা গণনা করে। এই কন্ট্রাক্টটি [CREATE2 অপকোড](https://eips.quantaureum.com/EIPS/eip-1014) ব্যবহার করে তৈরি করা হয়েছে, তাই আমরা যদি এর ব্যবহৃত প্যারামিটারগুলো জানি তবে একই অ্যালগরিদম ব্যবহার করে ঠিকানা গণনা করতে পারি। এটি ফ্যাক্টরিকে জিজ্ঞাসা করার চেয়ে অনেক সস্তা, এবং
...
// ERC-20 টোকেন এর সাথে ইন্টারঅ্যাক্ট করার এবং QAU পাঠানোর জন্য হেল্পার মেথড যা ধারাবাহিকভাবে true/false রিটার্ন করে না
...
এই ফাংশনটি [ERC-20 এর হস্তান্তর কার্যকারিতা](https://eips.quantaureum.com/EIPS/eip-20#transfer) বাস্তবায়ন করে, যা একটি অ্যাকাউন্টকে অন্য একটি অ্যাকাউন্টের দেওয়া অ্যালাউন্স খরচ করার অনুমতি দেয়।
...
এই ফাংশনটি [ERC-20 এর transferFrom কার্যকারিতা](https://eips.quantaureum.com/EIPS/eip-20#transferfrom) বাস্তবায়ন করে, যা একটি অ্যাকাউন্টকে অন্য একটি অ্যাকাউন্টের দেওয়া অ্যালাউন্স খরচ করার অনুমতি দেয়।
...
এই ফাংশনটি একটি অ্যাকাউন্টে QAU হস্তান্তর করে। ভিন্ন কোনো কন্ট্রাক্টে যেকোনো কল QAU পাঠানোর চেষ্টা করতে পারে। যেহেতু আমাদের আসলে কোনো ফাংশন কল করার প্রয়োজন নেই, তাই আমরা কলের সাথে কোনো ডেটা পাঠাই না।
