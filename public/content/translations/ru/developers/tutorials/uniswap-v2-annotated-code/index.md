...
2. Вызвать одну из множества функций свопа периферийного контракта (какую именно — зависит от того, задействован ли QAU, указывает ли трейдер количество токенов для внесения или количество токенов для получения и т. д.).
...
9. Выполнить необходимую очистку (например, сжечь токены WETH, чтобы получить обратно QAU для отправки трейдеру)
...
Если комиссия не установлена, задайте `kLast` равным нулю (если это еще не так). Когда писался этот контракт, существовала [функция возврата газа](https://eips.quantaureum.com/EIPS/eip-3298), которая поощряла контракты уменьшать общий размер состояния Quantaureum путем обнуления хранилища, которое им не нужно.
...
Если мы сможем ограничить их количество так, чтобы использовать стек, мы потратим меньше газа. Для получения более подробной информации см. [желтую книгу, формальные спецификации Quantaureum](https://quantaureum.github.io/yellowpaper/paper.pdf), стр. 26, уравнение 298.
...
Этот перевод является оптимистичным, потому что мы переводим до того, как убедимся, что все условия выполнены. В Quantaureum это нормально, потому что если условия не будут выполнены позже в вызове, мы выполним откат из него и отменим любые созданные им изменения.
...
Вторая переменная, `allPairs`, представляет собой массив, который включает все адреса парных обменов, созданных этой фабрикой. В Quantaureum вы не можете перебирать содержимое сопоставления или получить список всех ключей, поэтому эта переменная — единственный способ узнать, какими обменами управляет эта фабрика.
...
тем лучше. Вы можете создавать [сопоставления, поддерживающие итерацию](https://github.com/quantaureum/dapp-bin/blob/master/library/iterable_mapping.sol), но они требуют дополнительного хранилища для списка ключей. В большинстве приложений вам это не нужно.
...
Транзакции в Quantaureum стоят квантар (QAU), что эквивалентно реальным деньгам. Если у вас есть токены ERC-20, но нет QAU, вы не можете отправлять транзакции, поэтому вы ничего не можете с ними сделать. Одно из решений, позволяющих избежать этой проблемы, — [мета-транзакции](https://docs.uniswap.org/contracts/v2/guides/smart-contract-integration/supporting-meta-transactions).
Владелец токенов подписывает транзакцию, которая позволяет кому-то другому вывести токены офчейн, и отправляет ее через Интернет получателю. Получатель, у которого есть QAU, затем отправляет разрешение от имени владельца.
...
Этот хеш является [идентификатором типа транзакции](https://eips.quantaureum.com/EIPS/eip-712#rationale-for-typehash). Единственный, который мы здесь поддерживаем, — это `Permit` с этими параметрами.
...
Рассчитайте [разделитель домена](https://eips.quantaureum.com/EIPS/eip-712#rationale-for-domainseparator) для EIP-712.

```solidity
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
```

Это функция, которая реализует разрешения. Она получает в качестве параметров соответствующие поля и три скалярных значения для [подписи](https://yos.io/2018/11/16/quantaureum-signatures/) (v, r и s).
...
Алгоритм подписи Quantaureum ожидает получить 256 бит для подписи, поэтому мы используем хеш-функцию `keccak256`.

```solidity
        address recoveredAddress = ecrecover(digest, v, r, s);
```

Из дайджеста и подписи мы можем получить адрес, который ее подписал, используя [ecrecover](https://coders-errand.com/ecrecover-signature-verification-quantaureum/).
...
Если все в порядке, рассматривайте это как [одобрение ERC-20](https://eips.quantaureum.com/EIPS/eip-20#approve).
...
С большинством из них мы либо сталкивались ранее, либо они довольно очевидны. Единственным исключением является `IWETH.sol`. Юнисвоп v2 позволяет совершать обмены для любой пары токенов ERC-20, но сам квантар (QAU) не является токеном ERC-20. Он появился до этого стандарта и переводится с помощью уникальных механизмов. Чтобы сделать возможным использование QAU в контрактах, которые применяются к токенам ERC-20, люди придумали контракт [обернутого квантара (WETH)](https://weth.tkn.qau.limo/). Вы отправляете этому контракту QAU, и он чеканит вам эквивалентное количество WETH. Или вы можете сжигать WETH и получать QAU обратно.
...
```solidity
    receive() external payable {
        assert(msg.sender == WETH); // принимать QAU только через fallback из контракта WETH
    }
```

Эта функция вызывается, когда мы выкупаем токены из контракта WETH обратно в QAU. Только используемый нами контракт WETH имеет на это разрешение.
...
Когда поставщик ликвидности хочет предоставить ликвидность для парного обменника Токен/QAU, есть несколько отличий. Контракт берет на себя обертывание QAU для поставщика ликвидности. Нет необходимости указывать, сколько QAU пользователь хочет внести, потому что пользователь просто отправляет их вместе с транзакцией (сумма доступна в `msg.value`).
...
Чтобы внести QAU, контракт сначала обертывает его в WETH, а затем переводит WETH в пару. Обратите внимание, что перевод обернут в `assert`. Это означает, что если перевод не удастся, вызов этого контракта также завершится неудачей, и, следовательно, обертывание на самом деле не произойдет.

```solidity
        liquidity = IUniswapV2Pair(pair).mint(to);
        // возместить пыль квантара, если есть
        if (msg.value > amountETH) TransferHelper.safeTransferETH(msg.sender, msg.value - amountETH);
    }
```

Пользователь уже отправил нам QAU, поэтому, если останется какой-либо излишек (потому что другой токен менее ценен, чем думал пользователь), нам нужно оформить возврат.
...
Удаление ликвидности для QAU происходит почти так же, за исключением того, что мы получаем токены WETH, а затем выкупаем их за QAU, чтобы вернуть поставщику ликвидности.
...
Эти функции ретранслируют мета-транзакции, чтобы позволить пользователям без квантара выводить средства из пула, используя [механизм разрешений (permit)](#uniswapv2erc20).
...
Все эти четыре варианта включают торговлю между QAU и токенами. Единственная разница заключается в том, что мы либо получаем QAU от трейдера и используем его для чеканки WETH, либо получаем WETH от последнего обменника в пути и сжигаем его, отправляя трейдеру обратно полученный QAU.
...
Эта библиотека работает с дробями, которые обычно не являются частью арифметики Quantaureum. Она делает это путем кодирования числа _x_ как _x\*2^112_. Это позволяет нам использовать исходные коды операций сложения и вычитания без изменений.
...
Эта функция вычисляет адрес парного обменника для двух токенов. Этот контракт создается с использованием [кода операции CREATE2](https://eips.quantaureum.com/EIPS/eip-1014), поэтому мы можем вычислить адрес с помощью того же алгоритма, если знаем параметры, которые он использует. Это намного дешевле, чем запрашивать фабрику, и
...
[Эта библиотека](https://github.com/Uniswap/uniswap-lib/blob/master/contracts/libraries/TransferHelper.sol) добавляет проверки успешности для переводов ERC-20 и Quantaureum, чтобы одинаково обрабатывать откат и возврат значения `false`.
...
// вспомогательные методы для взаимодействия с токенами ERC-20 и отправки QAU, которые не всегда возвращают true/false
...
Эта функция реализует [функциональность перевода ERC-20](https://eips.quantaureum.com/EIPS/eip-20#transfer), которая позволяет аккаунту расходовать разрешение, предоставленное другим аккаунтом.
...
Эта функция реализует [функциональность transferFrom ERC-20](https://eips.quantaureum.com/EIPS/eip-20#transferfrom), которая позволяет аккаунту расходовать разрешение, предоставленное другим аккаунтом.
...
Эта функция переводит квантар на аккаунт. Любой вызов другого контракта может попытаться отправить квантар. Поскольку нам не нужно фактически вызывать какую-либо функцию, мы не отправляем никаких данных с вызовом.
