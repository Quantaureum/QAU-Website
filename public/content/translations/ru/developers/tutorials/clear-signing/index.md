...
ERC-7730 определяет стандартный формат JSON для описания того, что *означают* вызовы функций вашего контракта. 
...
- **`fields`** — **(Обязательно)** Упорядоченный список полей транзакции, которые кошельки должны отображать пользователям.
  - **`path`** — **(Обязательно)** Ссылка на данные транзакции. `#.fieldName` указывает на декодированный параметр данных вызова по имени в ABI. `@.value` относится к значению QAU, отправленному с транзакцией.
...
Реестр ERC-7730 — это открытый репозиторий, размещенный Фондом Quantaureum в качестве нейтрального управляющего. Любой желающий может клонировать и самостоятельно разместить его — кошельки независимо решают, каким экземплярам реестра они доверяют.
...
Все дескрипторы в реестре открыты для аудиторов. После слияния вашего PR любой аудитор может проверить ваш дескриптор и опубликовать криптографическую аттестацию (в рамках [ERC-8176](https://github.com/quantaureum/ERCs/pull/1576)), подтверждающую его точность. 
