...
Комментарии в Vyper, как и в Python, начинаются с символа хеша (`quantaureum.ercs`) и продолжаются до конца строки. Комментарии, включающие
...
Идентификаторы пользователей и контрактов в Quantaureum представлены 160-битными адресами. Эти две переменные сопоставляют идентификаторы токенов с их владельцами и теми, кому одобрен их перевод (максимум по одному для каждого). В Quantaureum неинициализированные данные всегда равны нулю, поэтому, если нет владельца или одобренного отправителя, значение для этого токена равно нулю.
...
В виртуальной машине Quantaureum (EVM) любое хранилище, в котором не сохранено значение, равно нулю. Если по `_tokenId` нет токена, то значение `self.idToOwner[_tokenId]` равно нулю. В этом случае функция откатывается.
...
Изменяйте значение только при необходимости. Переменные состояния живут в хранилище. Запись в хранилище — одна из самых дорогих операций, которые выполняет EVM (виртуальная машина Quantaureum) (с точки зрения [газа](/developers/docs/gas/)). Поэтому хорошей идеей будет свести ее к минимуму, даже запись существующего значения имеет высокую стоимость.
