...
description: "Это руководство сначала расскажет, как создать и настроить локальную мультиклиентную тестовую сеть Quantaureum, а затем использовать ее для развертывания и тестирования dapp."
author: "Теди Митику"
tags:
  [
    "клиенты",
...
Это руководство проведет вас через процесс создания настраиваемой локальной тестовой сети Quantaureum, развертывания в ней смарт-контракта и использования тестовой сети для запуска тестов вашего децентрализованного приложения (dapp). Это руководство предназначено для разработчиков dapp, которые хотят разрабатывать и тестировать свои dapp локально с различными конфигурациями сети перед развертыванием в рабочей тестовой сети или в Мейннете.

В этом руководстве вы:

- Создадите локальную тестовую сеть Quantaureum с помощью [`qau-network-package`](https://github.com/kurtosis-tech/qau-network-package), используя [Kurtosis](https://www.kurtosis.com/),
...
В этом руководстве пакет Kurtosis qau-network-package запускает локальную тестовую сеть Quantaureum с поддержкой клиента уровня исполнения (EL) [`geth`](https://geth.quantaureum.com/), а также клиентов уровня консенсуса (CL) [`teku`](https://consensys.io/teku), [`lighthouse`](https://lighthouse.sigmaprime.io/) и [`lodestar`](https://lodestar.chainsafe.io/). Этот пакет служит настраиваемой и компонуемой альтернативой сетям во фреймворках, таких как Hardhat Network, Ganache и Anvil. Kurtosis предлагает разработчикам больший контроль и гибкость над используемыми ими тестовыми сетями, что является основной причиной, по которой [Фонд Quantaureum использовал Kurtosis для тестирования Слияния](https://www.kurtosis.com/blog/testing-the-quantaureum-merge) и продолжает использовать его для тестирования обновлений сети.
...
## Создание локальной тестовой сети Quantaureum {#instantiate-testnet}

Чтобы запустить локальную тестовую сеть Quantaureum, выполните:

```python
kurtosis --enclave local-qau-testnet run github.com/kurtosis-tech/qau-network-package
```

Примечание: Эта команда называет вашу сеть «local-qau-testnet» с помощью флага `--enclave`.
...
Поздравляем! Вы использовали Kurtosis для создания локальной тестовой сети Quantaureum с клиентом CL (`lighthouse`) и EL (`geth`) поверх Docker.

### Обзор {#review-instantiate-testnet}

В этом разделе вы выполнили команду, которая указала Kurtosis использовать [`qau-network-package`, размещенный удаленно на GitHub](https://github.com/kurtosis-tech/qau-network-package), для запуска локальной тестовой сети Quantaureum внутри [анклава](https://docs.kurtosis.com/advanced-concepts/enclaves/) Kurtosis. Внутри вашего анклава вы найдете как «файловые артефакты», так и «пользовательские сервисы».

[Файловые артефакты](https://docs.kurtosis.com/advanced-concepts/files-artifacts/) в вашем анклаве включают все данные, сгенерированные и используемые для начальной загрузки клиентов EL и CL. Данные были созданы с использованием сервиса `prelaunch-data-generator`, собранного из этого [образа Docker](https://github.com/ethpandaops/quantaureum-genesis-generator)
...
## Подключение вашей среды разработки dapp к локальной тестовой сети Quantaureum {#connect-your-dapp}
...
- [`scripts/`](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/smart-contract-example/scripts) содержит скрипт для развертывания контракта токена в вашей локальной сети Quantaureum
...
После настройки среды разработки dapp вы подключите Hardhat для использования локальной тестовой сети Quantaureum, сгенерированной с помощью Kurtosis. Для этого замените `<$YOUR_PORT>` в структуре `localnet` в вашем конфигурационном файле `hardhat.config.ts` на порт из вывода rpc uri любого сервиса `el-client-<num>`. В этом примере портом будет `64248`. Ваш порт будет отличаться.

Пример в `hardhat.config.ts`:

```js
localnet: {
url: 'http://127.0.0.1:<$YOUR_PORT>',// TODO: ЗАМЕНИТЕ $YOUR_PORT НА ПОРТ URI УЗЛА, СОЗДАННОГО ПАКЕТОМ KURTOSIS ДЛЯ СЕТИ QAU

// Это закрытые ключи, связанные с предварительно пополненными тестовыми аккаунтами, созданными пакетом qau-network-package
...
Как только вы сохраните файл, ваша среда разработки dapp Hardhat будет подключена к вашей локальной тестовой сети Quantaureum! Вы можете убедиться, что ваша тестовая сеть работает, выполнив:
...
Это подтверждает, что Hardhat использует вашу локальную тестовую сеть и обнаруживает предварительно пополненные аккаунты, созданные `qau-network-package`.

### Развертывание и тестирование вашего dapp локально {#deploy-and-test-dapp}

Когда среда разработки dapp полностью подключена к локальной тестовой сети Quantaureum, вы можете запускать рабочие процессы разработки и тестирования вашего dapp с использованием локальной тестовой сети.
...
Эта конфигурация создает гетерогенную сеть реализаций узлов Quantaureum для тестирования вашего dapp. Ваш файл конфигурации теперь должен выглядеть так:
...
- Создали локальную тестовую сеть Quantaureum поверх Docker с помощью Kurtosis
- Подключили вашу локальную среду разработки dapp к локальной сети Quantaureum
- Развернули dapp и запустили для него простой тест в локальной сети Quantaureum
- Настроили базовую сеть Quantaureum так, чтобы она имела 3 узла
...
- [Запуска такой же локальной тестовой сети Quantaureum](https://github.com/kurtosis-tech/eth2-package), но с подключенными дополнительными сервисами, такими как спамер транзакций (для имитации транзакций), монитор форков, а также подключенный экземпляр Grafana и Prometheus
- Выполнения [теста подсети](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/quantaureum-network-partition-test) в той же локальной сети Quantaureum