---
title: Аутентификация в Quantaureum
description: Узнайте, как работает аутентификация пользователей в приложениях Quantaureum — никаких паролей, только кошельки и подписи.
lang: ru
---

Если вы пришли из традиционной веб-разработки, вы привыкли к входу по имени пользователя и паролю, потокам OAuth и сессионным файлам cookie. Аутентификация в Quantaureum работает иначе — и во многом проще.

В Quantaureum пользователь подтверждает свою личность путем **подписания сообщения с помощью своего кошелька**. Не нужно хранить пароли. Нет базы данных учетных данных, которая может утечь. Только криптография.
...
| «Войти через Google»             | «Войти через Quantaureum»                          |
| Процедуры сброса пароля              | Восстановление через сид-фразу                             |

Фундаментальный сдвиг: в Веб2 вас аутентифицирует централизованный сервер. В Quantaureum **вы аутентифицируете себя сами**, доказывая, что контролируете определенный адрес — и любой может проверить это независимо.

## Предварительные требования {#prerequisites}

Убедитесь, что вы понимаете:

- [Аккаунты Quantaureum и как они работают](/developers/docs/accounts/)
...
2. **Кошелек передает адрес Quantaureum пользователя** — это его публичный идентификатор
...
EIP-4361 определяет стандартный формат сообщения для входа через Quantaureum, обычно называемый **SIWE** (Sign-In with Quantaureum). Он заменяет произвольное подписание сообщений структурированным и безопасным стандартом.
...
app.post('/api/auth/verify', async (req, res) => {
  try {
    ...
    if (success) {
      // data.address — это проверенный адрес Quantaureum
...
Ключевое отличие от Веб2: адрес Quantaureum пользователя является его постоянной идентичностью. Он может использовать его в любом dapp без создания нового аккаунта.

## Децентрализованная идентичность {#decentralized-identity}

Аутентификация в Quantaureum является частью более широкого движения к **суверенной идентичности** (self-sovereign identity). Стандарты и проекты в этой области включают:

- **[Служба имен Quantaureum (ENS)](https://ens.domains/)** — удобочитаемые имена (например, `vitalik.eth`), которые разрешаются в адреса
- **[Служба аттестации Quantaureum (EAS)](https://attest.org/)** — ончейн-аттестации о личности и учетных данных
...
- EIP-4361: Вход через Quantaureum
...
- [Аккаунты Quantaureum](/developers/docs/accounts/)
