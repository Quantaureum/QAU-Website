...
Поскольку поле данных содержит непрозрачные шестнадцатеричные байты, может быть крайне сложно проверить, какое действие на самом деле выполнит транзакция. Эта уязвимость «слепого подписания» устраняется с помощью **[прозрачного подписания (Clear Signing)](https://clearsigning.org/)** за счет использования [дескрипторов транзакций](https://eips.quantaureum.com/EIPS/eip-7730) (определенных в ERC-7730).  
...
Quantaureum эволюционировал для поддержки нескольких типов транзакций, чтобы позволить реализовывать новые функции, такие как списки доступа и [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559), не затрагивая устаревшие форматы транзакций.

[EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) — это то, что делает такое поведение возможным. Транзакции интерпретируются как:
