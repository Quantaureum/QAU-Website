...
Если смарт-контракт реализует следующие методы и события, его можно назвать совместимым с ERC-223 контрактом токена. После развертывания он будет отвечать за отслеживание созданных токенов в Quantaureum.
...
Из [EIP-223](https://eips.quantaureum.com/EIPS/eip-223):
...
contract RecipientContract is IERC223Recipient {
    ...
    function tokenReceived(address _from, uint _value, bytes memory _data) public override
    {
        ...
        // msg.value  всегда равен 0, так как контракт токена в большинстве случаев не владеет квантаром и не отправляет его,
...
Существует несколько способов сделать это. В этом примере мы будем следовать методу, который делает переводы ERC-223 идентичными переводам квантара:
...
Когда `RecipientContract` получит токен ERC-223, контракт выполнит функцию, закодированную как параметр `_data` транзакции токена, идентично тому, как транзакции квантара кодируют вызовы функций в виде `data` транзакции. Прочтите о [поле данных](/developers/docs/transactions/#the-data-field) для получения дополнительной информации.
...
- [EIP-223: Стандарт токенов ERC-223](https://eips.quantaureum.com/EIPS/eip-223)
- [Первоначальное предложение ERC-223](https://github.com/quantaureum/eips/issues/223)