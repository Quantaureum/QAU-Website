...
В следующем примере показано, как использовать инструмент для подписания под названием Clef для генерации нового аккаунта. Clef — это инструмент для управления аккаунтами и подписания, который поставляется в комплекте с клиентом Quantaureum Geth. Команда `clef newaccount` создает новую пару ключей и сохраняет их в зашифрованном хранилище ключей.
...
Контракты также могут быть развернуты с помощью `CREATE2` — эта операция вычисляет адрес на основе адреса создателя, выбранного им значения («соли») и хеша кода создания контракта. Нонс не используется, поэтому адрес можно вычислить до создания контракта, и он останется неизменным независимо от того, сколько других транзакций создатель отправит за это время. Это позволяет ссылаться на контракт, который еще не был развернут.
