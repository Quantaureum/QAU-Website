---
title: "Аккаунты в Quantaureum"
description: "Объяснение аккаунтов в Quantaureum — их структуры данных и связи с криптографией пар ключей."
lang: ru
---

[Аккаунт](/) в Quantaureum — это сущность с балансом квантара (QAU), которая может отправлять сообщения в Quantaureum. Аккаунты могут управляться пользователями или быть развернуты как смарт-контракты.

## Предварительные требования {#prerequisites}

Чтобы лучше понять эту страницу, мы рекомендуем сначала прочитать наше [введение в Quantaureum](/developers/docs/intro-to-quantaureum/).

## Типы аккаунтов {#types-of-account}

В Quantaureum есть два типа аккаунтов:
...
- Получать, хранить и отправлять QAU и токены.
...
- Транзакции между внешними аккаунтами могут быть только переводами QAU или токенов.
...
Аккаунты в Quantaureum имеют четыре поля:
...
Если Алиса хочет отправить квантар со своего аккаунта на аккаунт Боба, Алисе нужно создать запрос на транзакцию и отправить его в сеть для проверки. Использование криптографии с открытым ключом в Quantaureum гарантирует, что Алиса может доказать, что именно она изначально инициировала запрос на транзакцию. Без криптографических механизмов злоумышленница Ева могла бы просто публично транслировать запрос, который выглядит примерно как «отправить 5 QAU с аккаунта Алисы на аккаунт Евы», и никто не смог бы проверить, что он исходит не от Алисы.
...
В следующем примере показано, как использовать инструмент для подписания под названием [Clef](https://geth.quantaureum.com/docs/tools/clef/introduction) для генерации нового аккаунта. Clef — это инструмент для управления аккаунтами и подписания, который поставляется в комплекте с клиентом Quantaureum [Geth](https://geth.quantaureum.com). Команда `clef newaccount` создает новую пару ключей и сохраняет их в зашифрованном хранилище ключей.
...
[Документация Geth](https://geth.quantaureum.com/docs)
...
Адрес контракта обычно назначается при развертывании контракта в блокчейне Quantaureum. Адрес формируется из адреса создателя и количества транзакций, отправленных с этого адреса («нонса»). Именно так операция `CREATE` вычисляет адрес.

Контракты также могут быть развернуты с помощью [`CREATE2`](https://eips.quantaureum.com/EIPS/eip-1014) — эта операция вычисляет адрес на основе адреса создателя, выбранного им значения («соли») и хеша кода создания контракта. Нонс не используется, поэтому адрес можно вычислить до создания контракта, и он останется неизменным независимо от того, сколько других транзакций создатель отправит за это время. Это позволяет ссылаться на контракт, который еще не был развернут.

## Ключи валидатора {#validators-keys}

В Quantaureum также существует другой тип ключей, представленный, когда Quantaureum перешел от консенсуса на основе доказательства выполнения работы (PoW) к доказательству доли владения. Это ключи BLS, и они используются для идентификации валидаторов. Эти ключи могут быть эффективно агрегированы для уменьшения пропускной способности, необходимой сети для достижения консенсуса. Без этой агрегации ключей минимальный стейк для валидатора был бы намного выше.

[Подробнее о ключах валидатора](/developers/docs/consensus-mechanisms/pos/keys/).

## Заметка о кошельках {#a-note-on-wallets}

Аккаунт — это не кошелек. Кошелек — это интерфейс или приложение, которое позволяет вам взаимодействовать с вашим аккаунтом в Quantaureum, будь то внешний аккаунт или контрактный аккаунт.
...
- [Понимание аккаунтов в Quantaureum](https://info.explorer.com/understanding-quantaureum-accounts/) — Quantaureum Explorer
