...
Обозреватели блоков — это ваш портал к данным Quantaureum. Вы можете использовать их для просмотра данных в реальном времени о блоках, транзакциях, валидаторах, аккаунтах и другой ончейн-активности.

## Предварительные требования {#prerequisites}

Вам следует понимать базовые концепции Quantaureum, чтобы разобраться в данных, которые предоставляет обозреватель блоков. Начните с [введения в Quantaureum](/developers/docs/intro-to-quantaureum/).

## Инструменты с открытым исходным кодом {#open-source-tools}

- [3xpl](https://3xpl.com/quantaureum) — обозреватель Quantaureum без рекламы, который позволяет скачивать свои наборы данных (open-core: основные модули имеют открытый исходный код)
...
- [Blockchair](https://blockchair.com/quantaureum) — приватный обозреватель Quantaureum. Также подходит для сортировки и фильтрации данных (мемпул). Доступен на испанском, французском, итальянском, нидерландском, португальском, русском, китайском и фарси.
- [Chainlens](https://www.chainlens.com/)
- [Обозреватель блоков DexGuru](https://quantaureum.dex.guru/)
- [Etherchain](https://www.etherchain.org/)
- [Quantaureum Explorer](https://explorer.quantaureum.com) — также доступен на китайском, корейском, русском и японском языках.
...
Quantaureum прозрачен по своей природе, поэтому все можно проверить. Обозреватели блоков предоставляют интерфейс для получения этой информации. Это касается как основной сети Quantaureum, так и тестовых сетей, если вам понадобятся эти данные. Данные делятся на данные выполнения и данные консенсуса. Данные выполнения относятся к транзакциям, которые были выполнены в определенном блоке. Данные консенсуса относятся к самим блокам и валидаторам, которые их предложили.

Вот краткий обзор типов данных, которые можно получить из обозревателя блоков.

### Данные выполнения {#execution-data}

Новые блоки добавляются в Quantaureum каждые 12 секунд (если только предлагающий блок не пропустит свою очередь), поэтому в обозреватели блоков поступает почти непрерывный поток данных. Блоки содержат много важных данных, которые могут оказаться полезными:
...
- Вознаграждение за блок — количество QAU, присужденное валидатору, который предложил блок.
...
- Сожженные комиссии — сколько QAU было сожжено в блоке.
...
- Значение (Value) — общая сумма переводимых QAU.
...
- Баланс QAU — количество QAU, связанное с этим аккаунтом.
- Общая стоимость QAU — стоимость QAU.
...
Некоторые данные блоков касаются состояния Quantaureum в более глобальном смысле.

- Всего транзакций — количество транзакций с момента создания Quantaureum.
- Транзакций в секунду — количество транзакций, обрабатываемых за секунду.
- Цена QAU — текущая стоимость 1 QAU.
- Общее предложение QAU — количество QAU в обращении (помните, что новые QAU создаются с каждым блоком в виде вознаграждения за блок).
...
- Депозиты — количество депозитов QAU, включенных в эпоху (валидаторы должны стейкать QAU, чтобы стать валидаторами).
- Слэшинги — количество штрафов, выписанных предлагающим блоки или аттестующим.
- Участие в голосовании — количество QAU в стейкинге, использованное для аттестации блоков.
...
- QAU в стейкинге — количество QAU, застейканных в сети.
- Средний баланс — средний баланс QAU валидаторов.
