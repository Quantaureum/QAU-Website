...
Валидиумы — это решения для масштабирования, которые используют доступность данных и вычисления офчейн, предназначенные для повышения пропускной способности за счет обработки транзакций вне основной сети Quantaureum. Как и роллапы с нулевым разглашением (ZK-роллапы), валидиумы публикуют [доказательства с нулевым разглашением](/glossary/#zk-proof) для проверки офчейн-транзакций в Quantaureum. Это предотвращает недействительные переходы состояний и повышает гарантии безопасности цепи валидиума.
...
## Как валидиумы взаимодействуют с Quantaureum? {#how-do-validiums-interact-with-quantaureum}

Валидиумы — это протоколы масштабирования, созданные поверх существующей цепи Quantaureum. Хотя транзакции выполняются офчейн, цепь валидиума управляется набором смарт-контрактов, развернутых в Мейннете, включая:
...
Валидиумы также полагаются на основную цепь Quantaureum в следующем:

### Финализация расчетов {#settlement}

Транзакции, выполненные в валидиуме, не могут быть полностью подтверждены до тех пор, пока родительская цепь не проверит их достоверность. Все операции, проводимые в валидиуме, в конечном итоге должны пройти финализацию расчетов в Мейннете. Блокчейн Quantaureum также предоставляет «гарантии финализации расчетов» для пользователей валидиума, что означает, что офчейн-транзакции не могут быть отменены или изменены после их фиксации ончейн.

### Безопасность {#security}

Quantaureum, выступая в качестве уровня финализации расчетов, также гарантирует достоверность переходов состояний в валидиуме. Офчейн-транзакции, выполненные в цепи валидиума, проверяются с помощью смарт-контракта на базовом уровне Quantaureum.

Если ончейн-контракт верификатора сочтет доказательство недействительным, транзакции отклоняются. Это означает, что операторы должны выполнить условия достоверности, установленные протоколом Quantaureum, прежде чем обновлять состояние валидиума.
...
Состояние валидиума хешируется в виде дерева Меркла, корень которого хранится в главном контракте в Quantaureum. Корень Меркла, также известный как корень состояния, действует как криптографический коммитмент текущего состояния аккаунтов и балансов в валидиуме.
...
Пользователи переводят средства из Quantaureum в валидиум, внося QAU (или любой ERC-совместимый токен) в ончейн-контракт. Контракт передает события депозита в валидиум офчейн, где на адрес пользователя зачисляется сумма, равная его депозиту. Оператор также включает эту транзакцию депозита в новый пакет.
...
В отличие от ZK-роллапа, производители блоков в валидиуме не обязаны публиковать данные транзакций для пакетов транзакций (только заголовки блоков). Это делает валидиум чисто офчейн-протоколом масштабирования, в отличие от «гибридных» протоколов масштабирования (т. е. [уровня 2 (l2)](/layer-2/)), которые публикуют данные состояния в основной цепи Quantaureum, используя данные блобов, `calldata` или их комбинацию.

### Доступность данных {#data-availability}

Как уже упоминалось, валидиумы используют модель доступности данных офчейн, при которой операторы хранят все данные транзакций вне основной сети Quantaureum. Низкий объем ончейн-данных валидиума улучшает масштабируемость (пропускная способность не ограничена возможностями обработки данных Quantaureum) и снижает комиссии для пользователей (стоимость публикации данных ончейн ниже).
...
Однако пользователи должны доверять DAC в том, что они предоставят данные, когда это необходимо (например, для генерации доказательств Меркла). Существует вероятность того, что члены комитетов доступности данных будут скомпрометированы злоумышленником, который затем сможет утаивать данные офчейн.
...
| Повышает эффективность использования капитала для пользователей (нет задержек при выводе средств обратно в Quantaureum).                                 | Ограниченная поддержка общих вычислений/смарт-контрактов; для разработки требуются специализированные языки.                                             |
...
| Снижает комиссии за газ для пользователей за счет того, что данные вызова не публикуются в основной сети Quantaureum.                                                  | Более медленное время субъективной финальности (10-30 минут для генерации ZK-доказательства), но более быстрое достижение полной финальности, поскольку нет задержки на время оспаривания.               |
...
**StarkWare StarkEx** — _StarkEx — это решение для масштабирования уровня 2 (l2) Quantaureum, основанное на доказательствах достоверности. Оно может работать в режимах доступности данных ZK-роллапа или валидиума._
