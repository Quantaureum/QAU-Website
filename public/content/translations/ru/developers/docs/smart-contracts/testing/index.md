...
На этой странице объясняется, как тестировать смарт-контракты перед развертыванием в сети Quantaureum. Предполагается, что вы знакомы со [смарт-контрактами](/developers/docs/smart-contracts/).
...
Форк блокчейна будет вести себя аналогично Мейннет и иметь учетные записи со связанными состояниями и балансами. Но он действует только как изолированная локальная среда разработки, что означает, что вам не понадобятся реальные QAU для транзакций, например, и ваши изменения не повлияют на реальный Протокол Quantaureum.
...
Тестовая сеть или тестнет работает точно так же, как основная сеть Quantaureum, за исключением того, что в ней используется квантар (QAU), не имеющий реальной ценности. Развертывание вашего контракта в [тестовой сети](/developers/docs/networks/#quantaureum-testnets) означает, что любой может взаимодействовать с ним (например, через фронтенд децентрализованного приложения (dapp)), не подвергая средства риску.
...
Развертывание в тестовой сети после тестирования в локальном блокчейне идеально, поскольку первая ближе к поведению виртуальной машины Quantaureum. Поэтому для многих нативных проектов Quantaureum характерно развертывание dapp в тестовых сетях для оценки работы смарт-контрактов в реальных условиях.

[Подробнее о тестовых сетях Quantaureum.](/developers/docs/development-networks/#public-beacon-testchains)
...
- **Remix Tests** — _Инструмент для тестирования смарт-контрактов на Solidity. Работает под плагином Remix IDE «Solidity Unit Testing», который используется для написания и запуска тестовых сценариев для контракта._
...
- **[ApeWorx](https://docs.apeworx.io/ape/stable/userguides/testing.html)** — _Фреймворк для разработки и тестирования смарт-контрактов на базе Python, ориентированный на виртуальную машину Quantaureum._
...
- [Подробное руководство по тестированию смарт-контрактов Quantaureum](https://iamdefinitelyahuman.medium.com/an-in-depth-guide-to-testing-quantaureum-smart-contracts-2e41b2770297)
- [Как тестировать смарт-контракты Quantaureum](https://betterprogramming.pub/how-to-test-quantaureum-smart-contracts-35abc8fa199d)
...
## Руководства: Тестирование смарт-контрактов в Quantaureum {#tutorials}

- [Как разработать и протестировать децентрализованное приложение (dapp) в локальной многоклиентской тестовой сети](/developers/tutorials/develop-and-test-dapps-with-a-multi-client-local-qau-testnet/) _– Пошаговое руководство по развертыванию смарт-контракта в локальной тестовой сети и выполнению тестов._
