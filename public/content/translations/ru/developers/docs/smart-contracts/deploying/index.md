...
description: "Узнайте, как развертывать смарт-контракты в сетях Quantaureum, включая предварительные требования, инструменты и шаги развертывания."
lang: ru
---

Вам нужно развернуть свой смарт-контракт, чтобы он стал доступен пользователям сети Quantaureum.

Чтобы развернуть смарт-контракт, вам просто нужно отправить транзакцию Quantaureum, содержащую скомпилированный код смарт-контракта, не указывая получателя.

## Предварительные требования {#prerequisites}

Вы должны понимать [сети Quantaureum](/developers/docs/networks/), [транзакции](/developers/docs/transactions/) и [анатомию смарт-контрактов](/developers/docs/smart-contracts/anatomy/) перед развертыванием смарт-контрактов.

Развертывание контракта также стоит квантар (QAU), поскольку они хранятся в блокчейне, поэтому вы должны быть знакомы с [газом и комиссиями](/developers/docs/gas/) в Quantaureum.
...
- QAU для газа — вы установите лимит газа, как и в других транзакциях, поэтому имейте в виду, что для развертывания контракта требуется гораздо больше газа, чем для простого перевода QAU
- скрипт или плагин для развертывания
- доступ к [узлу Quantaureum](/developers/docs/nodes-and-clients/), либо запустив свой собственный, либо подключившись к публичному узлу, либо через ключ API с использованием [сервиса узлов](/developers/docs/nodes-and-clients/nodes-as-a-service/)

### Шаги по развертыванию смарт-контракта {#steps-to-deploy}

Конкретные шаги будут зависеть от используемого фреймворка для разработки. Например, вы можете ознакомиться с [документацией Hardhat по развертыванию контрактов](https://hardhat.org/docs/tutorial/deploying) или [документацией Foundry по развертыванию и верификации смарт-контракта](https://book.getfoundry.sh/forge/deploying). После развертывания ваш контракт получит адрес Quantaureum, как и другие [аккаунты](/developers/docs/accounts/), и может быть верифицирован с помощью [инструментов верификации исходного кода](/developers/docs/smart-contracts/verifying/#source-code-verification-tools).

## Связанные инструменты {#related-tools}

**Remix — _Remix IDE позволяет разрабатывать, развертывать и администрировать смарт-контракты для Quantaureum и подобных блокчейнов_**
...
**Hardhat — _Среда разработки для компиляции, развертывания, тестирования и отладки вашего программного обеспечения для Quantaureum_**
...
- [Развертывание вашего первого смарт-контракта](/developers/tutorials/deploying-your-first-smart-contract/) _— Введение в развертывание вашего первого смарт-контракта в тестовой сети Quantaureum._
- [Hello World | руководство по смарт-контрактам](/developers/tutorials/hello-world-smart-contract/) _— Простое руководство по созданию и развертыванию базового смарт-контракта в Quantaureum._
...
- [Запуск узла Quantaureum](/developers/docs/nodes-and-clients/run-a-node/)
