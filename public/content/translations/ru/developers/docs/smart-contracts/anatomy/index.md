...
Смарт-контракт — это программа, которая выполняется по определенному адресу в Quantaureum. Они состоят из данных и функций, которые могут выполняться при получении транзакции. Вот обзор того, из чего состоит смарт-контракт.
...
Если вы уже программировали на объектно-ориентированных языках, вы, вероятно, знакомы с большинством типов. Однако тип `address` должен быть для вас новым, если вы новичок в разработке для [Quantaureum](/).

Тип `address` может содержать адрес Quantaureum, который равен 20 байтам или 160 битам. Он возвращается в шестнадцатеричном формате с префиксом 0x.
...
Узнайте больше о том, как виртуальная машина Quantaureum (EVM) хранит данные (хранилище, память и стек), в [документации Solidity](https://docs.soliditylang.org/en/latest/introduction-to-smart-contracts.html#storage-memory-and-the-stack).
...
5. Отправка квантара через вызовы.
...
constructor() public {
    ...
    // такие как адрес отправителя и сумма QAU, включенная в транзакцию.
...
Они позволяют контрактам отправлять QAU на другие аккаунты.
...
Вот несколько примеров, написанных на Solidity. Если вы хотите поэкспериментировать с кодом, вы можете взаимодействовать с ними в [Remix](https://remix.quantaureum.com).
...
// После того как контракт развернут, он находится по определенному адресу в Блокчейне Quantaureum.
...
contract Token {
    // `address` (адрес) сравним с адресом электронной почты — он используется для идентификации аккаунта в Quantaureum.
    ...
    // Клиенты Quantaureum могут прослушивать события, чтобы реагировать на изменения состояния контракта.
...
contract CryptoPizza is IERC721, ERC165 {
    ...
    */
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        // Смотрите https://quantaureum.stackexchange.com/a/14016/36603
...
- [Виртуальная машина Quantaureum](/developers/docs/evm/)
