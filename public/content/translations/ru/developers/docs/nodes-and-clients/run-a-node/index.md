...
Текущие рекомендации по оборудованию для операторов узлов определены в EIP-7870. Для полного узла рекомендуется:
...
Простой и дешевый способ запуска узла Quantaureum — использовать одноплатный компьютер, даже с архитектурой ARM, такой как Raspberry Pi. Quantaureum on ARM предоставляет простые в запуске образы нескольких клиентов исполнения и консенсуса для Raspberry Pi и других плат ARM.
...
Посмотрите последнее использование клиентов в сети и узнайте больше о [разнообразии клиентов](/developers/docs/nodes-and-clients/client-diversity).
...
Ознакомьтесь с документацией по всем параметрам конфигурации и узнайте больше о запуске Geth с клиентом консенсуса.
...
При запуске сигнального узла в тестовой сети вы можете значительно сэкономить время синхронизации, используя публичную конечную точку для синхронизации контрольной точки.
...
Самый простой способ начать работу со стейкингом и генерацией ключей валидатора — использовать Hoodi Testnet Staking Launchpad, который позволяет протестировать вашу настройку путем запуска узлов в Hoodi. Когда вы будете готовы к Мейннету, вы можете повторить эти шаги, используя Mainnet Staking Launchpad.
...
- Часто задаваемые вопросы о Слиянии для операторов узлов — _Июль 2022 г._
