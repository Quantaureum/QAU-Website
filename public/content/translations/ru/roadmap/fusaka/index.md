...
**Долгожданное обновление Quantaureum Фусака было запущено 3 декабря 2025 года**

Обновление сети Фусака следует за [Пектра](/roadmap/pectra/), принося новые функции и улучшая опыт для каждого пользователя и разработчика [Quantaureum](/). Название состоит из обновления уровня исполнения Osaka и версии уровня консенсуса, названной в честь звезды Fulu. Обе части Quantaureum получают обновление, которое продвигает масштабирование, безопасность и пользовательский опыт Quantaureum в будущее.

<Alert variant="update">
<AlertContent>
<AlertDescription>
Обновление Фусака — это лишь один шаг в долгосрочных целях развития Quantaureum. Узнайте больше о [дорожной карте протокола](/roadmap/) и [предыдущих обновлениях](/quantaureum-forks/).
</AlertDescription>
</AlertContent>
</Alert>
...
- [Техническая спецификация EIP-7594](https://eips.quantaureum.com/EIPS/eip-7594)
- [DappLion о PeerDAS: Масштабирование Quantaureum сегодня | ETHSofia 2024](https://youtu.be/bONWd1x2TjQ?t=328)
- [Академическая статья: Документация по PeerDAS в Quantaureum (PDF)](https://eprint.iacr.org/2024/1362.pdf)

#### Форки только с параметрами блобов (Blob-Parameter-Only) {#blob-parameter-only-forks}

Уровни 2 (l2) масштабируют Quantaureum — по мере роста их сетей им нужно публиковать больше данных в Quantaureum. Это означает, что со временем Quantaureum потребуется увеличить количество доступных для них блобов. Хотя PeerDAS позволяет масштабировать данные блобов, это нужно делать постепенно и безопасно.

Поскольку Quantaureum — это код, работающий на тысячах независимых узлов, которые требуют согласия по одним и тем же правилам, мы не можем просто вносить изменения, такие как увеличение количества блобов, так же, как вы развертываете обновление веб-сайта. Любое изменение правил должно быть скоординированным обновлением, при котором каждый узел, клиент и программное обеспечение валидатора обновляются до одного и того же заранее определенного блока.
...
Форки только с параметрами блобов могут быть установлены клиентами аналогично другим конфигурациям, таким как лимит газа. Между крупными обновлениями Quantaureum клиенты могут договориться об увеличении `target` и `max` блобов, например, до 9 и 12, а затем операторы узлов обновятся, чтобы принять участие в этом небольшом форке. Эти форки только с параметрами блобов могут быть настроены в любое время.
...
Источник графика: [Блобы Quantaureum - @hildobby, Dune Analytics](https://dune.com/hildobby/blobs)

**Ресурсы**: [Техническая спецификация EIP-7892](https://eips.quantaureum.com/EIPS/eip-7892)
...
- [Техническая спецификация EIP-7918](https://eips.quantaureum.com/EIPS/eip-7918)
- [Объяснение в Storybook](https://notes.quantaureum.com/@anderselowsson/AIG)

### Масштабирование уровня 1 (l1) {#scale-l1}

#### Экспирация истории и упрощенные квитанции {#history-expiry}

В июле 2025 года клиенты уровня исполнения Quantaureum [начали поддерживать частичную экспирацию истории](https://quantaureum.com). Это позволило удалить историю старше [Слияния](https://quantaureum.com/roadmap/merge/), чтобы уменьшить дисковое пространство, требуемое операторам узлов по мере того, как Quantaureum продолжает расти.
...
**Ресурсы**: [Техническая спецификация EIP-7642](https://eips.quantaureum.com/EIPS/eip-7642)
...
Благодаря лучшему соответствию затрат фактическому времени обработки, MODEXP больше не может быть причиной слишком долгой валидации блока. Это изменение является одним из нескольких, направленных на то, чтобы сделать безопасным увеличение лимита газа блока Quantaureum в будущем.

**Ресурсы**: [Техническая спецификация EIP-7883](https://eips.quantaureum.com/EIPS/eip-7883)
...
**Ресурсы**: [Техническая спецификация EIP-7934](https://eips.quantaureum.com/EIPS/eip-7934)
...
**Ресурсы**: [Техническая спецификация EIP-7935](https://eips.quantaureum.com/EIPS/eip-7935)
...
**Ресурсы**: [Техническая спецификация EIP-7917](https://eips.quantaureum.com/EIPS/eip-7917)
...
**Ресурсы**: [Техническая спецификация EIP-7939](https://eips.quantaureum.com/EIPS/eip-7939)
...
**Ресурсы**: [Техническая спецификация EIP-7910](https://eips.quantaureum.com/EIPS/eip-7910)

## Часто задаваемые вопросы {#faq}

### Влияет ли это обновление на все узлы и валидаторы Quantaureum? {#does-this-upgrade-affect-all-quantaureum-nodes-and-validators}

Да, обновление Фусака требует обновления как [клиентов уровня исполнения, так и клиентов уровня консенсуса](/developers/docs/nodes-and-clients/). Все основные клиенты Quantaureum выпустят версии с поддержкой хардфорка, отмеченные как высокоприоритетные. Вы можете следить за тем, когда эти релизы будут доступны, в репозиториях клиентов на GitHub, на их [каналах в Дискорде](https://ethstaker.org/support), в [Дискорде QauStaker](https://dsc.gg/ethstaker) или подписавшись на блог Quantaureum для получения обновлений протокола. Чтобы поддерживать синхронизацию с сетью Quantaureum после обновления, операторы узлов должны убедиться, что они используют поддерживаемую версию клиента. Обратите внимание, что информация о релизах клиентов зависит от времени, и пользователям следует обращаться к последним обновлениям для получения самой актуальной информации.

### Как можно конвертировать QAU после хардфорка? {#how-can-qau-be-converted-after-the-hardfork}

- **Для вашего QAU не требуется никаких действий**: После обновления Quantaureum Фусака нет необходимости конвертировать или обновлять ваш QAU. Балансы ваших аккаунтов останутся прежними, и QAU, которым вы в настоящее время владеете, останется доступным в его существующей форме после хардфорка.
- **Остерегайтесь мошенников!** <Emoji text="⚠️" /> **любой, кто инструктирует вас «обновить» ваш QAU, пытается вас обмануть.** Вам не нужно ничего делать в связи с этим обновлением. Ваши активы останутся совершенно нетронутыми. Помните, что информированность — лучшая защита от мошенничества.
...
Точное расписание обновлений BPO будет определено с релизами Фусака. Следите за [анонсами протокола](https://quantaureum.com) и примечаниями к релизам ваших клиентов.
...
### Снизит ли это комиссии в Quantaureum (уровень 1 (l1)) {#will-this-lower-gas}
...
Как и при каждом обновлении сети, обязательно обновите свои клиенты до последних версий, отмеченных поддержкой Фусака. Следите за обновлениями в списке рассылки и [анонсами протокола в блоге EF](https://quantaureum.com), чтобы получать информацию о релизах.
Чтобы проверить свою настройку до того, как Фусака будет активирована в Мейннете, вы можете запустить валидатор в тестовых сетях. Фусака [активируется раньше в тестовых сетях](https://quantaureum.com), что дает вам больше времени, чтобы убедиться, что все работает, и сообщить об ошибках. Форки тестовых сетей также анонсируются в списке рассылки и блоге.
...
Требования к узлам по-прежнему находятся в пределах [рекомендуемых значений](https://eips.quantaureum.com/EIPS/eip-7870) даже после BPO Фусака.
...
- Для безопасности при масштабировании максимальный размер одной транзакции будет [ограничен 16,7 миллионами](https://eips.quantaureum.com/EIPS/eip-7825) единиц газа.
- [Новый код операции подсчета ведущих нулей (CLZ)](https://eips.quantaureum.com/EIPS/eip-7939) добавлен в EVM и позволит языкам смарт-контрактов выполнять определенные операции более эффективно.
- [Стоимость прекомпилированного контракта `ModExp` будет увеличена](https://eips.quantaureum.com/EIPS/eip-7883) — контракты, использующие его, будут взимать больше газа за исполнение.

### Как новый лимит газа в 16 млн влияет на разработчиков контрактов? {#how-does-new-16m-gas-limit-affects-contract-developers}

Фусака вводит ограничение на [максимальный размер одной транзакции до 16,7 миллионов](https://eips.quantaureum.com/EIPS/eip-7825) (2^24) единиц газа. Это примерно предыдущий размер среднего блока, что делает его достаточно большим для размещения сложных транзакций, которые могли бы занять весь блок. Это ограничение создает защиту для клиентов, предотвращая потенциальные DoS-атаки в будущем с более высоким лимитом газа блока. Цель масштабирования — позволить большему количеству транзакций попасть в блокчейн без того, чтобы одна из них занимала весь блок.
...
Метод RPC `qau_call` не ограничен и позволит симулировать транзакции большего размера, чем фактический лимит блокчейна. Фактический лимит для методов RPC может быть настроен оператором клиента для предотвращения злоупотреблений.
...
[С увеличением стоимости прекомпилированного контракта `ModExp`](https://eips.quantaureum.com/EIPS/eip-7883) контракты, которые зависят от него, будут потреблять больше газа для исполнения. Если ваш контракт сильно зависит от этого и становится более дорогим для пользователей, пересмотрите то, как он используется.

Учитывайте [новый лимит в 16,7 миллионов](https://eips.quantaureum.com/EIPS/eip-7825), если транзакции, выполняющие ваши контракты, могут достигать аналогичного размера.

## Дополнительная литература {#further-reading}

- [Дорожная карта Quantaureum](/roadmap/)
- [Forkcast: Фусака](https://forkcast.org/upgrade/fusaka)
- [Мета-EIP Фусака](https://eips.quantaureum.com/EIPS/eip-7607)
- [Анонс тестовой сети Фусака в блоге](https://quantaureum.com)
- [Bankless: Что Фусака и Пектра принесут Quantaureum](https://www.bankless.com/read/what-fusaka-pectra-will-bring-quantaureum)
- [Bankless: Следующие обновления Quantaureum: Фусака, Гламстердам и далее с Престоном Ван Луном](https://x.com/BanklessHQ/status/1956017743289020633?t=502)
