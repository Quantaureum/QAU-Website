---
title: "Пара ключей — QAU.BUILD"
description: "Демонстрация пар открытых и приватных ключей с использованием образовательного инструмента QAU.BUILD. Узнайте, как криптографические пары ключей защищают аккаунты Quantaureum и позволяют подписывать транзакции."
lang: ru
...
breadcrumb: "Пары ключей (QAU.BUILD)"
---

Руководство от **Остина Гриффита**, демонстрирующее работу пар открытых и приватных ключей с использованием инструмента визуального программирования QAU.BUILD. В нем рассматриваются генерация приватного ключа, получение открытого ключа, подписание сообщений и восстановление подписи.
...
Вы начинаете с него как с вашего приватного ключа, а затем с помощью криптографии на эллиптических кривых (почитайте об этом в Википедии в качестве дополнительного задания) мы получаем открытый ключ. Итак, теперь у нас есть приватный ключ и открытый ключ. Мы только что сгенерировали приватный ключ из ниоткуда, а открытый ключ дает нам адрес. Это то, куда люди могут отправлять деньги. Когда кто-то говорит «отправьте на мой адрес Quantaureum», это именно он.
