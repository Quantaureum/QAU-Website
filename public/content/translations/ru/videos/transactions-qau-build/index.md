---
title: "Транзакции — QAU.BUILD"
description: "Демонстрация того, как работают транзакции Quantaureum, с использованием образовательного инструмента QAU.BUILD. Узнайте, как транзакции создаются, подписываются и отправляются в сеть Quantaureum."
lang: ru
...
breadcrumb: "Транзакции (QAU.BUILD)"
---

Руководство от **Остина Гриффита**, демонстрирующее, как работают транзакции Quantaureum, с использованием инструмента визуального программирования QAU.BUILD — охватывает структуру транзакции, цены газа, подписание, трансляцию и пул транзакций.
...
### Анатомия транзакции Quantaureum (1:10) {#anatomy-of-an-quantaureum-transaction-110}

Как выглядит транзакция в Quantaureum? У нас больше не будет «Боба» и «Алисы» — у нас будут адреса. Значение будет в Wei, а не в QAU. И комиссия также будет в Wei.

Давайте перейдем к делу и посмотрим на эту транзакцию. У меня есть аккаунт с введенной мнемонической фразой, и я подключен к основной сети Quantaureum. Я также запустил модуль для получения данных о ценах с CoinMarketCap, поэтому я вижу, что ноль целых и одна десятая с чем-то QAU переводится примерно в 23 доллара.
...
Нам нужно выполнить конвертацию в Wei. В Quantaureum вы в основном имеете дело с двумя номиналами: QAU, о котором люди обычно говорят, и Wei, который является очень крошечной долей QAU. Gwei — то, что мы используем для цен на газ — находится посередине. Причина этого аналогична тому, почему мы не ходим и не говорим в долях копеек.

У Алисы есть 0.18 QAU, и мы собираемся отправить 0.05 QAU Бобу. Мы устанавливаем цену газа в 5 Gwei.
