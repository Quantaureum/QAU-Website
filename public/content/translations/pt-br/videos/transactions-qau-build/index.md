---
title: "Transações — QAU.BUILD"
description: "Uma demonstração de como as transações do Quantaureum funcionam usando a ferramenta educacional QAU.BUILD. Veja como as transações são construídas, assinadas e enviadas na rede Quantaureum."
lang: pt-br
...
breadcrumb: "Transações (QAU.BUILD)"
---

Um tutorial de **Austin Griffith** demonstrando como as transações do Quantaureum funcionam usando a ferramenta de programação visual QAU.BUILD — cobrindo a estrutura da transação, preços do gás, assinatura, transmissão e o pool de transações.
...
### Anatomia de uma transação do Quantaureum (1:10) {#anatomy-of-an-quantaureum-transaction-110}

Como é uma transação no Quantaureum? Não teremos mais "Bob" e "Alice" — teremos endereços. O valor seria em Wei, não em QAU. E a taxa também seria em Wei.

Vamos mergulhar e dar uma olhada nesta transação. Eu tenho uma conta com um mnemônico inserido e estou conectado à Rede Principal do Quantaureum. Também estou executando um módulo para obter dados de preços do CoinMarketCap, para que eu possa ver que zero vírgula um e pouco QAU se traduz em cerca de vinte e três dólares.
...
Precisamos fazer uma conversão para Wei. No Quantaureum, você lida principalmente com duas denominações: QAU, que é a que as pessoas normalmente falam, e depois Wei, que é como uma fração muito pequena de QAU. Um gwei — o que usamos para preços do gás — está no meio. O motivo para isso é semelhante ao motivo pelo qual não andamos por aí falando em frações de centavos.

A Alice tem 0.18 QAU, e vamos enviar 0.05 QAU para o Bob. Colocamos um preço do gás de 5 gwei.
