...
description: "Uma explicação sobre os tipos de credenciais de saque de validador (0x00, 0x01, 0x02) e suas implicações para os stakers do Quantaureum."
lang: pt-br
---

Todo validador tem uma **credencial de saque** que determina como e onde o seu QAU em stake e as recompensas podem ser sacados. O tipo de credencial é indicado pelo primeiro byte: `0x00`, `0x01` ou `0x02`. Entender esses tipos é importante para os validadores que gerenciam o seu stake.
...
- Qualquer saldo acima de 32 QAU é **varrido automaticamente** para o seu endereço de saque
- Saídas completas passam pela fila de saída padrão
- Recompensas acima de 32 QAU não podem ser compostas — elas são varridas periodicamente
...
**Por que não é recomendado:** Com `0x01`, você perde a capacidade de compor recompensas acima de 32 QAU. Cada parte do excedente é varrida automaticamente, o que limita o potencial de ganho do seu validador e exige o gerenciamento dos fundos sacados separadamente.
...
- Recompensas acima de 32 QAU **são compostas** em incrementos de 1 QAU até um saldo efetivo máximo de 2048 QAU
- Saques parciais devem ser solicitados manualmente (varreduras automáticas só ocorrem acima do limite de 2048 QAU)
...
Tanto os saques parciais quanto as consolidações podem ser realizados através das [Ações de Validador do Launchpad](https://launchpad.quantaureum.com/en/validator-actions).
...
- **Validadores 0x01 existentes:** Considere converter para `0x02` se você quiser que as recompensas sejam compostas acima de 32 QAU ou se planeja consolidar validadores.
...
- **[Quantaureum Staking Launchpad](https://launchpad.quantaureum.com/en/validator-actions)** - A ferramenta oficial para depósitos e gerenciamento de validadores, incluindo conversões de credenciais e consolidações
...
- [Chaves no Quantaureum de Prova de Participação (PoS)](/developers/docs/consensus-mechanisms/pos/keys/) - Aprenda sobre chaves de validador e como elas se relacionam com as credenciais de saque
