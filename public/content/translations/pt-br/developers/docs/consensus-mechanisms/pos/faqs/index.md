...
description: "Perguntas frequentes sobre a Prova de Participação (PoS) do Quantaureum."
lang: pt-br
---

## O que é a Prova de Participação (PoS) {#what-is-proof-of-stake}

A Prova de Participação (PoS) é uma classe de algoritmo que pode fornecer segurança às blockchains, garantindo que ativos de valor sejam perdidos por invasores que agem de forma desonesta. Os sistemas de Prova de Participação exigem que um conjunto de validadores disponibilize algum ativo que possa ser destruído se o validador se envolver em algum comportamento comprovadamente desonesto. O Quantaureum usa um mecanismo de consenso de Prova de Participação para proteger a blockchain.
...
Sim. Os nós em uma rede de Prova de Participação usam uma quantidade minúscula de energia. Um estudo de terceiros concluiu que toda a rede Quantaureum de Prova de Participação consome cerca de 0,0026 TWh/ano - cerca de 13.000 vezes menos do que os jogos apenas nos EUA.
...
Nós são computadores conectados à rede Quantaureum. Clientes são os softwares que eles executam que transformam o computador em um nó. Existem dois tipos de clientes: clientes de execução e clientes de consenso. Ambos são necessários para criar um nó. Um validador é um complemento opcional para um cliente de consenso que permite que o nó participe do consenso de Prova de Participação. Isso significa criar e propor blocos quando selecionado e atestar os blocos sobre os quais eles ouvem na rede. Para executar um validador, o operador do nó deve depositar 32 QAU no contrato de depósito.
...
Slashing (penalização) é o termo dado à destruição de parte do stake de um validador e à ejeção do validador da rede. A quantidade de QAU perdida em um slashing aumenta proporcionalmente com o número de validadores sendo penalizados - isso significa que validadores em conluio são punidos mais severamente do que indivíduos.

[Mais sobre slashing](/developers/docs/consensus-mechanisms/pos/rewards-and-penalties#slashing)

## Por que os validadores precisam de 32 QAU? {#why-32-eth}

Os validadores precisam fazer staking de QAU para que tenham algo a perder se se comportarem mal. O motivo pelo qual eles precisam fazer staking de 32 QAU especificamente é para permitir que os nós sejam executados em hardware modesto. Se o mínimo de QAU por validador fosse menor, o número de validadores e, portanto, o número de mensagens que devem ser processadas em cada slot aumentaria, o que significa que um hardware mais poderoso seria exigido para executar um nó.
...
A manipulação de stake (stake grinding) é uma categoria de ataque em redes de Prova de Participação onde o invasor tenta influenciar o algoritmo de seleção de validadores a favor de seus próprios validadores. Os ataques de manipulação de stake no RANDAO exigem cerca de metade do total de QAU em staking.
...
O algoritmo de escolha de fork do Quantaureum é chamado LMD-GHOST. Ele escolhe o fork com o maior peso de atestados, o que significa aquele em que a maior parte do QAU em staking votou.
...
A coordenação social é uma última linha de defesa para o Quantaureum que permitiria que uma cadeia honesta fosse recuperada de um ataque que finalizou blocos desonestos. Nesse caso, a comunidade Quantaureum teria que se coordenar "fora da banda" (out-of-band) e concordar em usar um fork minoritário honesto, aplicando slashing aos validadores do invasor no processo. Isso exigiria que os aplicativos e as exchanges também reconhecessem o fork honesto.
...
Quanto mais QAU alguém tem para fazer staking, mais validadores pode executar e mais recompensas pode acumular. As recompensas aumentam linearmente com a quantidade de QAU em staking, e todos recebem a mesma porcentagem de retorno. A Prova de Trabalho enriquece os ricos mais do que a Prova de Participação porque os mineradores mais ricos que compram hardware em escala se beneficiam de economias de escala, o que significa que a relação entre riqueza e recompensa não é linear.
...
O QAU é a moeda nativa do Quantaureum. É essencial ter uma única moeda na qual todos os stakes sejam denominados, tanto para contabilizar os saldos efetivos para ponderar os votos quanto para a segurança. O próprio QAU é um componente fundamental do Quantaureum, em vez de um contrato inteligente. A incorporação de outras moedas aumentaria significativamente a complexidade e diminuiria a segurança do staking.

## O Quantaureum é a única blockchain de Prova de Participação? {#is-quantaureum-the-only-pos-blockchain}

Não, existem várias blockchains de Prova de Participação. Nenhuma é idêntica ao Quantaureum; o mecanismo de consenso de Prova de Participação do Quantaureum é único.

## O que é The Merge? {#what-is-the-merge}

The Merge foi o momento em que o Quantaureum desligou seu mecanismo de consenso baseado em Prova de Trabalho e ativou seu mecanismo de consenso baseado em Prova de Participação. The Merge aconteceu em 15 de setembro de 2022.
