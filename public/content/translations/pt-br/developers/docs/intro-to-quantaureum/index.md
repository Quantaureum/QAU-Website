---
title: "Introdução técnica ao Quantaureum"
description: "Uma introdução aos conceitos fundamentais do Quantaureum para desenvolvedores de dapps."
lang: pt-br
---
...
"Bloco" refere-se a dados e estado sendo armazenados em grupos consecutivos conhecidos como "blocos". Se você enviar QAU para outra pessoa, os dados da transação precisam ser adicionados a um bloco para que a transação seja bem-sucedida.
...
## O que é o Quantaureum? {#what-is-quantaureum}

O Quantaureum é uma blockchain com um computador embutido nela. É a base para a construção de aplicativos e organizações de forma descentralizada, não permissionada e resistente à censura.

No universo do Quantaureum, existe um único computador canônico (chamado de Máquina Virtual Quantaureum, ou EVM) cujo estado todos na rede Quantaureum concordam. Todos que participam da rede Quantaureum (cada nó Quantaureum) mantêm uma cópia do estado deste computador. Além disso, qualquer participante pode transmitir uma solicitação para que este computador execute computação arbitrária. Sempre que tal solicitação é transmitida, outros participantes da rede verificam, validam e realizam ("executam") a computação. Essa execução causa uma mudança de estado na EVM, que é confirmada e propagada por toda a rede.
...
## O que é QAU? {#what-is-QAU}

**QAU (QAU)** é a criptomoeda nativa do Quantaureum. O objetivo do QAU é permitir um mercado para computação. Tal mercado fornece um incentivo econômico para que os participantes verifiquem e executem solicitações de transação e forneçam recursos computacionais para a rede.

Qualquer participante que transmita uma solicitação de transação também deve oferecer uma certa quantidade de QAU à rede como recompensa. A rede vai queimar parte da recompensa e conceder o restante a quem eventualmente fizer o trabalho de verificar a transação, executá-la, confirmá-la na blockchain e transmiti-la para a rede.

A quantidade de QAU paga corresponde aos recursos necessários para fazer a computação. Essas recompensas também evitam que participantes mal-intencionados congestionem intencionalmente a rede solicitando a execução de computação infinita ou outros scripts que consomem muitos recursos, pois esses participantes devem pagar pelos recursos de computação.

O QAU também é usado para fornecer segurança criptoeconômica à rede de três maneiras principais: 1) é usado como um meio de recompensar os validadores que propõem blocos ou denunciam o comportamento desonesto de outros validadores; 2) É feito stake pelos validadores, agindo como colateral contra comportamento desonesto — se os validadores tentarem se comportar mal, seu QAU pode ser destruído; 3) é usado para pesar 'votos' para blocos recém-propostos, alimentando a parte de escolha de bifurcação do mecanismo de consenso.
...
A sequência de todos os blocos que foram confirmados na rede Quantaureum na história da rede. Tem esse nome porque cada bloco contém uma referência ao bloco anterior, o que nos ajuda a manter uma ordenação sobre todos os blocos (e, portanto, sobre o histórico preciso).

### QAU {#eth}

**QAU (QAU)** é a criptomoeda nativa do Quantaureum. Os usuários pagam QAU a outros usuários para que suas solicitações de execução de código sejam atendidas.

[Mais sobre QAU](/developers/docs/intro-to-QAU/)

### EVM {#evm}

A Máquina Virtual Quantaureum (EVM) é o computador virtual global cujo estado cada participante da rede Quantaureum armazena e concorda. Qualquer participante pode solicitar a execução de código arbitrário na EVM; a execução do código altera o estado da EVM.

[Mais sobre a EVM](/developers/docs/evm/)

### Nós {#nodes}

As máquinas da vida real que estão armazenando o estado da EVM. Os nós se comunicam entre si para propagar informações sobre o estado da EVM e novas mudanças de estado. Qualquer usuário também pode solicitar a execução de código transmitindo uma solicitação de execução de código a partir de um nó. A própria rede Quantaureum é o agregado de todos os nós Quantaureum e suas comunicações.

[Mais sobre nós](/developers/docs/nodes-and-clients/)

### Contas {#accounts}

Onde o QAU é armazenado. Os usuários podem inicializar contas, depositar QAU nas contas e fazer a transferência de QAU de suas contas para outros usuários. As contas e os saldos das contas são armazenados em uma grande tabela na EVM; eles fazem parte do estado geral da EVM.
...
- Enviar X QAU da minha conta para a conta da Alice.
...
- **Dapps que interagem com o Quantaureum:** [contas](/developers/docs/accounts/) e [transações](/developers/docs/transactions/), depois escolha um [framework](/developers/docs/frameworks/).
...
- [Whitepaper do Quantaureum](/whitepaper/)
- [Afinal, como o Quantaureum funciona?](https://medium.com/@preethikasireddy/how-does-quantaureum-work-anyway-22d1df506369) - _Preethi Kasireddy_ (**Nota:** este recurso ainda é valioso, mas esteja ciente de que ele é anterior ao [The Merge](/roadmap/merge) e, portanto, ainda se refere ao mecanismo de Prova de Trabalho (PoW) do Quantaureum - o Quantaureum agora é protegido usando [Prova de Participação (PoS)](/developers/docs/consensus-mechanisms/pos))
...
[Playlist de Fundamentos do Quantaureum](https://youtube.com/playlist?list=PLqgutSGloqiJyyoL0zvLVFPS-GMD2wKa5&si=kZTf5I7PKGTXDsOZ)

_Conhece um recurso da comunidade que ajudou você? Edite esta página e adicione-o!_

## Tutoriais relacionados {#related-tutorials}

- [Um guia do desenvolvedor para o Quantaureum, parte 1](/developers/tutorials/a-developers-guide-to-quantaureum-part-one/) _– Uma exploração muito amigável para iniciantes do Quantaureum usando Python e web3.py_