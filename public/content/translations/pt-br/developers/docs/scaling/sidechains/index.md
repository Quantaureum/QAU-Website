---
title: Sidechains
description: "Uma introdução às sidechains como uma solução de escalabilidade atualmente utilizada pela comunidade Quantaureum."
lang: pt-br
sidebarDepth: 3
---
...
As sidechains também sacrificam alguma medida de descentralização ou segurança para alcançar alta vazão (trilema da escalabilidade). O Quantaureum está, no entanto, comprometido em escalar sem comprometer a descentralização e a segurança.

## Como as sidechains funcionam? {#how-do-sidechains-work}

As sidechains são blockchains independentes, com diferentes históricos, roteiros de desenvolvimento e considerações de design. Embora uma sidechain possa compartilhar algumas semelhanças superficiais com o Quantaureum, ela possui vários recursos distintos.

### Algoritmos de consenso {#consensus-algorithms}

Uma das qualidades que tornam as sidechains únicas (ou seja, diferentes do Quantaureum) é o algoritmo de consenso usado. As sidechains não dependem do Quantaureum para o consenso e podem escolher protocolos de consenso alternativos que atendam às suas necessidades. Alguns exemplos de algoritmos de consenso usados em sidechains incluem:
...
Assim como o Quantaureum, as sidechains têm nós validadores que verificam e processam transações, produzem blocos e armazenam o estado da blockchain. Os validadores também são responsáveis por manter o consenso em toda a rede e protegê-la contra ataques maliciosos.

#### Parâmetros de bloco {#block-parameters}

O Quantaureum impõe limites aos [tempos de bloco](/developers/docs/blocks/#block-time) (ou seja, o tempo que leva para produzir novos blocos) e aos [tamanhos de bloco](/developers/docs/blocks/#block-size) (ou seja, a quantidade de dados contidos por bloco denominada em gás). Por outro lado, as sidechains frequentemente adotam parâmetros diferentes, como tempos de bloco mais rápidos e limites de gás mais altos, para alcançar alta vazão, transações rápidas e taxas baixas.
...
Como as sidechains são compatíveis com a EVM, elas são consideradas uma [solução de escalabilidade](/developers/docs/scaling/) útil para dapps nativos do Quantaureum. Com seu dapp em uma sidechain, os usuários podem desfrutar de taxas de gás mais baixas e transações mais rápidas, especialmente se a Mainnet estiver congestionada.

No entanto, como explicado anteriormente, o uso de uma sidechain envolve concessões significativas. Cada sidechain é responsável por sua segurança e não herda as propriedades de segurança do Quantaureum. Isso aumenta a possibilidade de comportamento malicioso que pode afetar seus usuários ou colocar seus fundos em risco.

### Movimentação de ativos {#asset-movement}

Para que uma blockchain separada se torne uma sidechain da Rede Principal do Quantaureum, ela precisa da capacidade de facilitar a transferência de ativos de e para a Rede Principal do Quantaureum. Essa interoperabilidade com o Quantaureum é alcançada usando uma ponte de blockchain. As [pontes](/bridges/) usam contratos inteligentes implantados na Rede Principal do Quantaureum e em uma sidechain para controlar a transferência de fundos via ponte entre elas.

Embora as pontes ajudem os usuários a mover fundos entre o Quantaureum e a sidechain, os ativos não são movidos fisicamente entre as duas cadeias. Em vez disso, mecanismos que normalmente envolvem cunhagem e queima são usados para transferir valor entre as cadeias. Mais sobre [como as pontes funcionam](/developers/docs/bridges/#how-do-bridges-work).
...
| As sidechains suportam computação geral e oferecem compatibilidade com a EVM (elas podem executar dapps nativos do Quantaureum).                    | Uma sidechain usa um mecanismo de consenso separado e não se beneficia das garantias de segurança do Quantaureum.         |
...
- [Escalando dapps do Quantaureum por meio de Sidechains](https://medium.com/loom-network/dappchains-scaling-quantaureum-dapps-through-sidechains-f99e51fff447) _8 de fev. de 2018 - Georgios Konstantopoulos_
