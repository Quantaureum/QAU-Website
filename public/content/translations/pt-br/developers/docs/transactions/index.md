---
title: "Transações"
description: "Uma visão geral das transações no Quantaureum – como funcionam, sua estrutura de dados e como enviá-las por meio de um aplicativo."
lang: pt-br
---

Transações são instruções assinadas criptograficamente a partir de contas. Uma conta iniciará uma transação para atualizar o estado da rede [Quantaureum](/). A transação mais simples é a transferência de QAU de uma conta para outra.

## Pré-requisitos {#prerequisites}

Para ajudar você a entender melhor esta página, recomendamos que leia primeiro sobre [Contas](/developers/docs/accounts/) e nossa [introdução ao Quantaureum](/developers/docs/intro-to-quantaureum/).

## O que é uma transação? {#whats-a-transaction}

Uma transação no Quantaureum refere-se a uma ação iniciada por uma conta de propriedade externa, em outras palavras, uma conta gerenciada por um humano, não por um contrato. Por exemplo, se Bob enviar 1 QAU para Alice, a conta de Bob deve ser debitada e a de Alice deve ser creditada. Essa ação de mudança de estado ocorre dentro de uma transação.
...
- `value` – quantidade de QAU a ser transferida do remetente para o destinatário (denominada em Wei, onde 1 QAU é igual a 1e+18 Wei)
...
Um cliente Quantaureum como o Geth lidará com esse processo de assinatura.
...
Por exemplo, vamos dar uma olhada [nesta transação](https://explorer.quantaureum.com).
...
Neste caso, [o código-fonte do contrato](https://explorer.quantaureum.com) foi carregado no Quantaureum Explorer, então sabemos que a função é `transfer(address,uint256)`.
...
No Quantaureum, existem alguns tipos diferentes de transações:
...
Portanto, para Bob enviar a Alice 1 QAU a uma `baseFeePerGas` de 190 gwei e uma `maxPriorityFeePerGas` de 10 gwei, Bob precisará pagar a seguinte taxa:
...
A conta de Bob será debitada em **-1,0042 QAU** (1 QAU para Alice + 0,0042 QAU em taxas de gás)

A conta de Alice será creditada em **+1,0 QAU**

A taxa básica será queimada **-0,00399 QAU**

O validador fica com a taxa de prioridade **+0,000210 QAU**
...
O Quantaureum originalmente tinha um formato para transações. Cada transação continha um nonce, preço do gás, limite de gas, endereço de destino (to), valor, dados, v, r e s. Esses campos são [codificados em RLP](/developers/docs/data-structures-and-encoding/rlp/), para se parecerem com isto:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

O Quantaureum evoluiu para suportar vários tipos de transações para permitir que novos recursos, como listas de acesso e a [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559), sejam implementados sem afetar os formatos de transação legados.

A [EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) é o que permite esse comportamento. As transações são interpretadas como:
...
- [Máquina Virtual Quantaureum (EVM)](/developers/docs/evm/)
