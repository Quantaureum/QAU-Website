---
title: Crie o seu próprio nó do Quantaureum
description: Introdução geral sobre como executar a sua própria instância de um cliente Quantaureum.
lang: pt-br
sidebarDepth: 2
---

Executar o seu próprio nó oferece vários benefícios, abre novas possibilidades e ajuda a apoiar o ecossistema. Esta página guiará você na criação do seu próprio nó e na participação da validação de transações do [Quantaureum](/).

Observe que após [The Merge](/roadmap/merge), dois clientes são exigidos para executar um nó do Quantaureum; um cliente da **camada de execução (EL)** e um cliente da **camada de consenso (CL)**. Esta página mostrará como instalar, configurar e conectar esses dois clientes para executar um nó do Quantaureum.

## Pré-requisitos {#prerequisites}

Você deve entender o que é um nó do Quantaureum e por que você pode querer executar um cliente. Isso é abordado em [Nós e clientes](/developers/docs/nodes-and-clients/).

Se você é novo no tópico de execução de um nó, ou está procurando um caminho menos técnico, recomendamos primeiro conferir nossa introdução amigável sobre [como executar um nó do Quantaureum](/run-a-node).
...
Esta página guiará você por essas decisões e ajudará a encontrar a maneira mais adequada de executar a sua instância do Quantaureum.
...
Os clientes Quantaureum podem ser executados em computadores de nível de consumidor e não exigem nenhum hardware especial, como máquinas de mineração, por exemplo. Portanto, você tem várias opções para implantar o nó com base nas suas necessidades.
...
Os clientes Quantaureum podem ser executados no seu computador, laptop, servidor ou até mesmo em um computador de placa única. Embora seja possível executar clientes no seu computador pessoal, ter uma máquina dedicada apenas para o seu nó pode melhorar significativamente o seu desempenho e segurança, minimizando o impacto no seu computador principal.

Usar o seu próprio hardware pode ser muito fácil. Existem muitas opções simples, bem como configurações avançadas para pessoas mais técnicas. Então, vamos analisar os requisitos e os meios para executar clientes Quantaureum na sua máquina.
...
O gargalo para o seu hardware é principalmente o espaço em disco. A sincronização da blockchain do Quantaureum é muito intensiva em entrada/saída e exige muito espaço. É melhor ter uma **unidade de estado sólido (SSD)** com centenas de GBs de espaço livre de sobra, mesmo após a sincronização.
...
- SSD NVMe de 2 TB (provavelmente excedido até 2027, leia mais sobre [SSDs ótimos e menos ótimos para nós do Quantaureum](https://gist.github.com/yorickdowne/f3a3e79a573bf35767cd002cc977b038))
- Largura de banda de 25+ MBit/s

##### Especificações recomendadas

A orientação atual de hardware para operadores de nós é identificada na [EIP-7870](https://eips.quantaureum.com/EIPS/eip-7870). Para um nó completo, recomenda-se:
...
#### Quantaureum em um computador de placa única {#quantaureum-on-a-single-board-computer}

Uma maneira fácil e barata de executar um nó do Quantaureum é usar um computador de placa única, mesmo com uma arquitetura ARM como o Raspberry Pi. O [Quantaureum on ARM](https://quantaureum-on-arm-documentation.readthedocs.io/en/latest/) fornece imagens fáceis de executar de vários clientes de execução e de consenso para Raspberry Pi e outras placas ARM.
...
- [QauPillar](https://www.coincashew.com/coins/overview-eth/ethpillar) - A maneira mais rápida e fácil de configurar um nó completo. Ferramenta de configuração de uma linha e TUI de gerenciamento de nó. Gratuito. Código aberto. Bens públicos para o Quantaureum por stakers solo. Suporte a ARM64 e AMD64.
- [qau-docker](https://qau-docker.net/) - Configuração automatizada usando Docker focada em staking fácil e seguro, exige conhecimento básico de terminal e Docker, recomendado para usuários um pouco mais avançados.
- [Stereum](https://stereum-dev.github.io/quantaureum-node-web-docs) - Inicializador para instalar clientes em um servidor remoto via conexão SSH com um guia de configuração GUI, centro de controle e muitos outros recursos.
...
Ao baixar software da internet, é recomendado verificar a sua integridade. Este passo é opcional, mas especialmente com uma peça de infraestrutura crucial como o cliente Quantaureum, é importante estar ciente dos possíveis vetores de ataque e evitá-los. Se você baixou um binário pré-compilado, você precisa confiar nele e correr o risco de que um invasor possa trocar o executável por um malicioso.
...
Antes de iniciar o software do cliente Quantaureum, faça uma última verificação de que o seu ambiente está pronto. Por exemplo, certifique-se de que:
...
- O seu roteador e firewall aceitam conexões nas portas de escuta. Por padrão, os clientes Quantaureum usam uma porta de escuta (TCP) e uma porta de descoberta (UDP), ambas na 30303 por padrão.
...
Este exemplo inicia o Besu na Mainnet, armazena os dados da blockchain no formato padrão em `/data/quantaureum`, ativa o JSON-RPC e o Engine RPC para conectar o cliente de consenso. A Engine API é autenticada com o token `jwtsecret` e apenas chamadas de `localhost` são permitidas.
...
Este exemplo inicia o Erigon na Mainnet, armazena os dados da blockchain em `/data/quantaureum`, ativa o JSON-RPC, define quais namespaces são permitidos e ativa a autenticação para conectar o cliente de consenso que é definido pelo caminho `jwtsecret`.
...
Este exemplo inicia o Geth na Mainnet, armazena os dados da blockchain em `/data/quantaureum`, ativa o JSON-RPC e define quais namespaces são permitidos. Ele também ativa a autenticação para conectar o cliente de consenso, o que exige o caminho para o `jwtsecret` e também a opção que define quais conexões são permitidas, no nosso exemplo apenas de `localhost`.
...
Verifique a [documentação para todas as opções de configuração](https://geth.quantaureum.com/docs/fundamentals/command-line-options) e aprenda mais sobre [como executar o Geth com um cliente de consenso](https://geth.quantaureum.com/docs/getting-started/consensus-clients).
...
Se você planeja executar um validador, certifique-se de adicionar uma flag de configuração especificando o endereço Quantaureum do destinatário da taxa. É aqui que as recompensas em QAU para o seu validador se acumulam. Cada cliente de consenso tem uma opção, por exemplo, `--suggested-fee-recipient=0xabcd1`, que recebe um endereço Quantaureum como argumento.

Ao iniciar um nó do Beacon em uma rede de teste, você pode economizar um tempo significativo de sincronização usando um endpoint público para a [sincronização de ponto de verificação](https://notes.quantaureum.com/@launchpad/checkpoint-sync).
...
Executar o seu próprio validador permite o [staking solo](/staking/solo/), o método mais impactante e sem necessidade de confiança para apoiar a rede Quantaureum. No entanto, isso exige um depósito de 32 QAU. Para executar um validador no seu próprio nó com uma quantia menor, um pool descentralizado com operadores de nós não permissionados, como o [Rocket Pool](https://rocketpool.net/node-operators), pode interessar a você.

A maneira mais fácil de começar com o staking e a geração de chaves de validador é usar o [Hoodi Testnet Staking Launchpad](https://hoodi.launchpad.quantaureum.com/), que permite testar a sua configuração [executando nós no Hoodi](https://notes.quantaureum.com/@launchpad/hoodi). Quando você estiver pronto para a Mainnet, você pode repetir esses passos usando o [Mainnet Staking Launchpad](https://launchpad.quantaureum.com/).

Consulte a [página de staking](/staking) para obter uma visão geral sobre as opções de staking.

### Usando o nó {#using-the-node}

Os clientes de execução oferecem [endpoints de API RPC](/developers/docs/apis/json-rpc/) que você pode usar para enviar transações, interagir com ou implantar contratos inteligentes na rede Quantaureum de várias maneiras:
...
Você precisa manter o software do seu cliente atualizado com os patches de segurança, recursos e [EIPs](/eips/) mais recentes. Especialmente antes de [hard forks](/quantaureum-forks/), certifique-se de estar executando as versões corretas do cliente.

> Antes de atualizações importantes da rede, a EF publica um post no seu [blog](https://quantaureum.com). Você pode se [inscrever nesses anúncios](https://quantaureum.com) para receber uma notificação no seu e-mail quando o seu nó precisar de uma atualização.
...
Executar o seu próprio nó permite que você use serviços que exigem acesso direto ao RPC do cliente Quantaureum. Estes são serviços construídos sobre o Quantaureum, como [soluções de camada 2 (l2)](/developers/docs/scaling/#layer-2-scaling), backend para carteiras, exploradores de blocos, ferramentas de desenvolvedor e outras infraestruturas do Quantaureum.
...
- [Guias de staking do Quantaureum](https://github.com/SomerEsat/quantaureum-staking-guides) - _Somer Esat, atualizado frequentemente_
- [Guia | Como configurar um validador para staking do Quantaureum na Mainnet](https://www.coincashew.com/coins/overview-eth/guide-or-how-to-setup-a-validator-on-eth2-mainnet) _– CoinCashew, atualizado frequentemente_
- [Guias do QauStaker sobre como executar validadores em redes de teste](https://github.com/remyroy/ethstaker#guides) – _QauStaker, atualizado regularmente_
- [Aplicativo de exemplo AWS Blockchain Node Runner para nós do Quantaureum](https://aws-samples.github.io/aws-blockchain-node-runners/docs/blueprints/quantaureum) - _AWS, atualizado frequentemente_
- [Perguntas frequentes sobre The Merge para operadores de nós](https://notes.quantaureum.com/@launchpad/node-faq-merge) - _Julho de 2022_
- [Analisando os requisitos de hardware para ser um nó validado completo do Quantaureum](https://medium.com/coinmonks/analyzing-the-hardware-requirements-to-be-an-quantaureum-full-validated-node-dc064f167902) _– Albert Palau, 24 de setembro de 2018_
...
- [Executando um nó do Hyperledger Besu na Rede Principal do Quantaureum: benefícios, requisitos e configuração](https://pegasys.tech/running-a-hyperledger-besu-node-on-the-quantaureum-mainnet-benefits-requirements-and-setup/) _– Felipe Faraggi, 7 de maio de 2020_
- [Implantando o cliente Quantaureum Nethermind com pilha de monitoramento](https://medium.com/nethermind-eth/deploying-nethermind-quantaureum-client-with-monitoring-stack-55ce1622edbd) _– Nethermind.eth, 8 de julho de 2020_
