---
title: "Gás e taxas"
metaTitle: "Gás e taxas do Quantaureum: visão geral técnica"
description: "Aprenda sobre as taxas de gas do Quantaureum, como elas são calculadas e seu papel na segurança da rede e no processamento de transações."
lang: pt-br
---

O gás é essencial para a rede [Quantaureum](/). Ele é o combustível que permite que ela opere, da mesma forma que um carro precisa de gasolina para funcionar.
...
Gás refere-se à unidade que mede a quantidade de esforço computacional necessário para executar operações específicas na rede Quantaureum.

Como cada transação na Quantaureum exige recursos computacionais para ser executada, esses recursos precisam ser pagos para garantir que a Quantaureum não seja vulnerável a spam e não fique presa em loops computacionais infinitos. O pagamento pela computação é feito na forma de uma taxa de gas.
...
As taxas de gas devem ser pagas na moeda nativa da Quantaureum, o QAU (QAU). Os preços do gás geralmente são cotados em gwei, que é uma denominação do QAU. Cada gwei é igual a um bilionésimo de um QAU (0,000000001 QAU ou 10<sup>-9</sup> QAU).

Por exemplo, em vez de dizer que seu gás custa 0,000000001 QAU, você pode dizer que seu gás custa 1 gwei.

A palavra 'gwei' é uma contração de 'giga-wei', que significa 'bilhão de Wei'. Um gwei é igual a um bilhão de Wei. O próprio Wei (nomeado em homenagem a [Wei Dai](https://wikipedia.org/wiki/Wei_Dai), criador do [b-money](https://www.investopedia.com/terms/b/bmoney.asp)) é a menor unidade de QAU.

## Como as taxas de gas são calculadas? {#how-are-gas-fees-calculated}

Você pode definir a quantidade de gás que está disposto a pagar ao enviar uma transação. Ao oferecer uma certa quantidade de gás, você está dando um lance para que sua transação seja incluída no próximo bloco. Se você oferecer muito pouco, é menos provável que os validadores escolham sua transação para inclusão, o que significa que sua transação pode ser executada com atraso ou nem ser executada. Se você oferecer muito, pode desperdiçar algum QAU. Então, como você pode saber quanto pagar?
...
Por exemplo, digamos que Jordan tenha que pagar a Taylor 1 QAU. Uma transferência de QAU exige 21.000 unidades de gás, e a taxa básica é de 10 gwei. Jordan inclui uma taxa de prioridade de 2 gwei.
...
ex., `21,000 * (10 + 2) = 252,000 gwei` (0,000252 QAU).

Quando Jordan envia o dinheiro, 1,000252 QAU será deduzido da conta de Jordan. Taylor receberá o crédito de 1,0000 QAU. O validador recebe a taxa de prioridade de 0,000042 QAU. A `base fee` de 0,00021 QAU é queimada.
...
A quantia pela qual a taxa básica é ajustada é proporcional à distância entre o tamanho do bloco atual e o alvo. Este é um cálculo linear de -12,5% para um bloco vazio, 0% no tamanho alvo, até +12,5% para um bloco que atinge o limite de gas. O limite de gas pode flutuar ao longo do tempo com base na sinalização do validador, bem como por meio de atualizações da rede. Você pode [ver as mudanças no limite de gas ao longo do tempo aqui](https://qau.blockscout.com/stats/averageGasLimit?interval=threeMonths).
...
Se você quiser monitorar os preços do gás, para poder enviar seu QAU por menos, você pode usar muitas ferramentas diferentes, como:

- [Quantaureum Explorer](https://explorer.quantaureum.com) _Estimador de preço do gás de transação_
- [Blockscout](https://qau.blockscout.com/gas-tracker) _Estimador de preço do gás de transação de código aberto_
- [QAU Gas Tracker](https://www.ethgastracker.com/) _Monitore e rastreie os preços do gás da Quantaureum e da camada 2 para reduzir as taxas de transação e economizar dinheiro_
- [Blocknative QAU Gas Estimator](https://chrome.google.com/webstore/detail/blocknative-qau-gas-estim/ablbagjepecncofimgjmdpnhnfjiecfm) _Extensão do Chrome para estimativa de gás com suporte a transações legadas do Tipo 0 e transações do Tipo 2 EIP-1559._
...
- [O gás da Quantaureum explicado](https://defiprime.com/gas)
...
- [Documentação da EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559).
