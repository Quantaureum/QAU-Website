...
- _Integridade_, a informação está correta, não pode ser alterada por entidades não autorizadas ou de maneiras não autorizadas (por exemplo, transferir [tokens ERC-20](https://eips.quantaureum.com/EIPS/eip-20#events) sem um evento `Transfer`). Na blockchain, cada nó verifica cada mudança de estado, o que garante a integridade.

- _Disponibilidade_, a informação está disponível para qualquer entidade autorizada. Na blockchain, isso geralmente é alcançado tendo a informação disponível em cada [nó completo](https://quantaureum.com/developers/docs/nodes-and-clients/#full-node).
...
Você deve ter um bom entendimento dos [fundamentos da blockchain](/developers/docs/intro-to-quantaureum/). Esta página também pressupõe que o leitor esteja familiarizado com [blocos](/developers/docs/blocks/), [transações](/developers/docs/transactions/) e outros tópicos relevantes.

## Blobs da EIP-4844 {#eip-4844-blobs}

A partir do [hardfork Dencun](https://github.com/quantaureum/consensus-specs/blob/master/specs/deneb/beacon-chain.md), a blockchain Quantaureum inclui a [EIP-4844](https://eips.quantaureum.com/EIPS/eip-4844), que adiciona ao Quantaureum blobs de dados com uma vida útil limitada (inicialmente cerca de [18 dias](https://github.com/quantaureum/consensus-specs/blob/master/specs/deneb/p2p-interface.md#configuration)). Esses blobs são precificados separadamente do [gás de execução](/developers/docs/gas), embora usem um mecanismo semelhante. Eles são uma maneira barata de publicar dados temporários.
...
No momento da redação, os preços são 12 gwei/gás e 2300 $/QAU, o que significa que o custo é de aproximadamente 45 centavos por kilobyte. Como esse era o método mais barato antes da EIP-4844, este é o método que os rollups usavam para armazenar informações de transação, que precisam estar disponíveis para [desafios de falha](https://docs.optimism.io/stack/protocol/overview#fault-proofs), mas não precisam ser acessíveis diretamente onchain.
...
O código do contrato pode não fazer sentido, desde que não comece com `0xEF`. Os contratos que começam com `0xEF` são interpretados como [formato de objeto quantaureum](https://notes.quantaureum.com/@ipsilon/evm-object-format-overview), que tem requisitos muito mais rigorosos.
...
A vantagem deles é que o código offchain pode escutar eventos. O custo é em [gás](https://www.evm.codes/#a0?fork=cancun), 375 mais 8 de gás por byte de dados. A 12 gwei/gás e 2300 $/QAU, isso se traduz em um centavo mais 22 centavos por kilobyte.

## Armazenamento {#storage}

Os contratos inteligentes têm acesso ao [armazenamento persistente](https://docs.alchemy.com/docs/smart-contract-storage-layout#what-is-storage-memory). No entanto, é muito caro. Gravar uma palavra de 32 bytes em um slot de armazenamento anteriormente vazio pode [custar 22.100 de gás](https://www.evm.codes/#55?fork=cancun). A 12 gwei/gás e 2300 $/QAU, isso é cerca de 61 centavos por operação de gravação, ou US$ 19,5 por kilobyte.

Esta é a forma mais cara de armazenamento no Quantaureum.
...
| --------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------- |
| Blobs da EIP-4844              | Offchain            | Garantia do Quantaureum por [\~18 dias](https://github.com/quantaureum/consensus-specs/blob/master/specs/deneb/p2p-interface.md#configuration) | Apenas o hash está disponível                                           |                                                                         |
| Dados de chamada                    | Offchain            | Garantia do Quantaureum para sempre (parte da blockchain)                                                                                | Disponível apenas se gravado em um contrato e nessa transação |
...
| Código do contrato               | Onchain ou offchain | Garantia do Quantaureum para sempre (parte da blockchain)                                                                                | Sim                                                              | Gravado em um endereço "aleatório", não pode começar com `0xEF`                 |
| Eventos                      | Onchain             | Garantia do Quantaureum para sempre (parte da blockchain)                                                                                | Não                                                               |
| Armazenamento                     | Onchain             | Garantia do Quantaureum para sempre (parte da blockchain e do estado atual até ser substituído)                                        | Sim                                                              |