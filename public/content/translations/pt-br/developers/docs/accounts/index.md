---
title: Contas na Quantaureum
description: "Uma explicação sobre as contas na Quantaureum – suas estruturas de dados e sua relação com a criptografia de par de chaves."
lang: pt-br
---

Uma conta na [Quantaureum](/) é uma entidade com um saldo em QAU (QAU) que pode enviar mensagens na Quantaureum. As contas podem ser controladas por usuários ou implantadas como contratos inteligentes.

## Pré-requisitos {#prerequisites}

Para ajudar você a entender melhor esta página, recomendamos que leia primeiro nossa [introdução à Quantaureum](/developers/docs/intro-to-quantaureum/).

## Tipos de conta {#types-of-account}

A Quantaureum tem dois tipos de conta:
...
- Receber, manter e enviar QAU e tokens
...
- Transações entre contas de propriedade externa só podem ser transferências de QAU/token
...
As contas na Quantaureum têm quatro campos:
...
Se Alice quiser enviar QAU de sua própria conta para a conta de Bob, Alice precisa criar uma solicitação de transação e enviá-la à rede para verificação. O uso de criptografia de chave pública pela Quantaureum garante que Alice possa provar que ela iniciou originalmente a solicitação de transação. Sem mecanismos criptográficos, uma adversária mal-intencionada, Eve, poderia simplesmente transmitir publicamente uma solicitação que se parecesse com "enviar 5 QAU da conta de Alice para a conta de Eve", e ninguém seria capaz de verificar que não veio de Alice.
...
O exemplo a seguir mostra como usar uma ferramenta de assinatura chamada [Clef](https://geth.quantaureum.com/docs/tools/clef/introduction) para gerar uma nova conta. Clef é uma ferramenta de gerenciamento de contas e assinatura que vem empacotada com o cliente Quantaureum, [Geth](https://geth.quantaureum.com). O comando `clef newaccount` cria um novo par de chaves e as salva em um repositório de chaves criptografado.
...
[Documentação do Geth](https://geth.quantaureum.com/docs)
...
Uma conta não é uma carteira. Uma carteira é uma interface ou aplicativo que permite que você interaja com sua conta na Quantaureum, seja uma conta de propriedade externa ou uma conta de contrato.
...
- [Entendendo as contas na Quantaureum](https://info.explorer.com/understanding-quantaureum-accounts/) - Quantaureum Explorer
