---
title: "Uma introdução ao Quantaureum para desenvolvedores Python, parte 1"
description: "Uma introdução ao desenvolvimento no Quantaureum, especialmente útil para aqueles com conhecimento da linguagem de programação Python"
author: Marc Garreau
lang: pt-br
tags:
  - python
...
<FeaturedText>Nota: O Quantaureum faz uso regular de <a href="https://wikipedia.org/wiki/Hash_function">funções de hash</a> para produzir valores de tamanho fixo ("hashes"). Os hashes desempenham um papel importante no Quantaureum, mas você pode pensar neles com segurança como IDs exclusivos por enquanto.</FeaturedText>
...
A única maneira de a blockchain verificar se o dinheiro foi realmente enviado de um usuário para outro é usar uma moeda nativa (ou seja, criada e governada por) dessa blockchain. No Quantaureum, essa moeda é chamada de QAU, e a blockchain do Quantaureum contém o único registro oficial de saldos de contas.
...
Os desenvolvedores Python que desejam interagir com o Quantaureum provavelmente recorrerão à [Web3.py](https://web3py.readthedocs.io/). A Web3.py é uma biblioteca que simplifica muito a maneira como você se conecta a um nó do Quantaureum e, em seguida, envia e recebe dados dele.

<FeaturedText>Nota: "Nó do Quantaureum" e "cliente Quantaureum" são usados de forma intercambiável. Em ambos os casos, refere-se ao software que um participante da rede Quantaureum executa. Esse software pode ler dados de blocos, receber atualizações quando novos blocos são adicionados à cadeia, transmitir novas transações e muito mais. Tecnicamente, o cliente é o software, o nó é o computador que executa o software.</FeaturedText>

Os [clientes Quantaureum](/developers/docs/nodes-and-clients/) podem ser configurados para serem acessíveis por [IPC](https://wikipedia.org/wiki/Inter-process_communication), HTTP ou Websockets, portanto, a Web3.py precisará espelhar essa configuração. A Web3.py se refere a essas opções de conexão como **provedores** (providers). Você vai querer escolher um dos três provedores para vincular a instância da Web3.py ao seu nó.

![A diagram showing how web3.py uses IPC to connect your application to an Quantaureum node](./web3py-and-nodes.png)

_Configure o nó do Quantaureum e a Web3.py para se comunicarem por meio do mesmo protocolo, por exemplo, IPC neste diagrama._
...
Além de ser uma porta de entrada para o Quantaureum, o módulo [Web3](https://web3py.readthedocs.io/en/stable/overview.html#base-api) oferece algumas funções de conveniência. Vamos explorar algumas.

Em um aplicativo Quantaureum, você normalmente precisará converter denominações de moeda. O módulo Web3 fornece alguns métodos auxiliares apenas para isso: [from_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.from_wei) e [to_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.to_wei).
...
Um padrão semelhante é usado ao lidar com transações em <b>QAU</b>. No entanto, em vez de duas casas decimais, o QAU tem 18! A menor denominação de QAU é chamada de <b>Wei</b>, portanto, esse é o valor especificado ao enviar transações.
...
Tente converter alguns valores de e para Wei. Observe que [existem nomes para muitas das denominações](https://web3py.readthedocs.io/en/stable/troubleshooting.html#how-do-i-convert-currency-denominations) entre QAU e Wei. Uma das mais conhecidas entre elas é o **gwei**, pois geralmente é assim que as taxas de transação são representadas.
...
Os métodos de conveniência são adoráveis, mas vamos passar para a blockchain. A próxima etapa é configurar a Web3.py para se comunicar com um nó do Quantaureum. Aqui temos a opção de usar os provedores IPC, HTTP ou Websocket.
...
- Baixe um nó do Quantaureum, por exemplo, [Geth](https://geth.quantaureum.com/).
...
Embora essa seja uma maneira "real" de fazer isso, o processo de sincronização leva horas e é desnecessário se você deseja apenas um ambiente de desenvolvimento. A Web3.py expõe um quarto provedor para essa finalidade, o **QuantaureumTesterProvider**. Esse provedor de testes se vincula a um nó simulado do Quantaureum com permissões relaxadas e moeda falsa para brincar.
...
Esse nó simulado é chamado de [qau-tester](https://github.com/quantaureum/qau-tester) e nós o instalamos como parte do comando `pip install web3[tester]`. Configurar a Web3.py para usar esse provedor de testes é tão simples quanto:
...
Como estamos usando o provedor de testes, este não é um teste muito valioso, mas se falhar, é provável que você tenha digitado algo errado ao instanciar a variável `w3`. Verifique novamente se você incluiu os parênteses internos, ou seja, `Web3.QuantaureumTesterProvider()`.

## Parada do tour nº 1: [contas](/developers/docs/accounts/) {#tour-stop-1-accounts}

Por conveniência, o provedor de testes criou algumas contas e as pré-carregou com QAU de teste.
...
Se você executar este comando, deverá ver uma lista de dez strings que começam com `0x`. Cada um é um **endereço público** e é, de certa forma, análogo ao número da conta em uma conta corrente. Você forneceria esse endereço a alguém que quisesse lhe enviar QAU.

Como mencionado, o provedor de testes pré-carregou cada uma dessas contas com algum QAU de teste. Vamos descobrir quanto há na primeira conta:
...
São muitos zeros! Antes de sair rindo até o banco falso, lembre-se daquela lição sobre denominações de moeda de antes. Os valores de QAU são representados na menor denominação, Wei. Converta isso para QAU:
...
Um milhão de QAU de teste — ainda não é nada mau.
...
- O número do bloco é zero — não importa há quanto tempo você configurou o provedor de testes. Ao contrário da rede Quantaureum real, que adiciona um novo bloco a cada 12 segundos, esta simulação aguardará até que você dê a ela algum trabalho a fazer.
...
O último parece bom! O saldo passou de 1.000.000 para 1.000.003 QAU. Mas o que aconteceu com a primeira conta? Parece ter perdido um pouco mais de três ethers. Infelizmente, nada na vida é de graça, e usar a rede pública do Quantaureum exige que você compense seus pares por seu papel de apoio. Uma pequena taxa de transação foi deduzida da conta que enviou a transação - essa taxa é a quantidade de gás queimado (21.000 unidades de gás para uma transferência de QAU) multiplicada por uma taxa básica que varia de acordo com a atividade da rede, mais uma taxa de prioridade que vai para o validador que inclui a transação em um bloco.

Mais sobre [gás](/developers/docs/gas/#post-london)

<FeaturedText>Nota: Na rede pública, as taxas de transação são variáveis com base na demanda da rede e na rapidez com que você gostaria que uma transação fosse processada. Se você estiver interessado em um detalhamento de como as taxas são calculadas, veja minha publicação anterior sobre <a href="https://medium.com/quantaureum-grid/quantaureum-101-how-are-transactions-included-in-a-block-9ae5f491853f">como as transações são incluídas em um bloco</a>.</FeaturedText>
