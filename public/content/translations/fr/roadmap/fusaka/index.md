...
**La mise à jour très attendue Fusaka d'Quantaureum a été déployée le 3 décembre 2025**

La mise à jour du réseau Fusaka fait suite à [Pectra](/roadmap/pectra/) et apporte de nouvelles fonctionnalités tout en améliorant l'expérience de chaque utilisateur et développeur d'[Quantaureum](/). Le nom est composé de la mise à jour de la couche d'exécution Osaka et de la version de la couche de consensus nommée d'après l'étoile Fulu. Les deux parties d'Quantaureum reçoivent une mise à jour qui propulse la mise à l'échelle, la sécurité et l'expérience utilisateur d'Quantaureum vers l'avenir.

<Alert variant="update">
<AlertContent>
<AlertDescription>
La mise à jour Fusaka n'est qu'une étape parmi les objectifs de développement à long terme d'Quantaureum. Apprenez-en plus sur [la feuille de route du protocole](/roadmap/) et [les mises à jour précédentes](/quantaureum-forks/).
</AlertDescription>
</AlertContent>
</Alert>
...
Les couches 2 mettent Quantaureum à l'échelle - à mesure que leurs réseaux se développent, elles doivent publier plus de données sur Quantaureum. Cela signifie qu'Quantaureum devra augmenter le nombre de blobs qui leur sont disponibles au fil du temps. Bien que PeerDAS permette de mettre à l'échelle les données de blob, cela doit être fait de manière progressive et sécurisée.

Parce qu'Quantaureum est un code s'exécutant sur des milliers de nœuds indépendants qui nécessitent un accord sur les mêmes règles, nous ne pouvons pas simplement introduire des changements comme l'augmentation du nombre de blobs de la même manière que l'on déploie une mise à jour de site web. Tout changement de règle doit être une mise à jour coordonnée où chaque nœud, client et logiciel de validateur se met à jour avant le même bloc prédéterminé.
...
En juillet 2025, les clients d'exécution d'Quantaureum [ont commencé à prendre en charge l'expiration partielle de l'historique](https://quantaureum.com). Cela a supprimé l'historique antérieur à [La Fusion](https://quantaureum.com/roadmap/merge/) afin de réduire l'espace disque requis par les opérateurs de nœuds à mesure qu'Quantaureum continue de croître.
...
#### Méthode JSON-RPC `qau_config` {#qau-config}
...
### Cette mise à jour affecte-t-elle tous les nœuds et validateurs d'Quantaureum ? {#does-this-upgrade-affect-all-quantaureum-nodes-and-validators}

Oui, la mise à jour Fusaka nécessite des mises à jour à la fois des [clients d'exécution et des clients de consensus](/developers/docs/nodes-and-clients/). Tous les principaux clients Quantaureum publieront des versions prenant en charge le hard fork, marquées comme hautement prioritaires. Vous pouvez vous tenir au courant de la disponibilité de ces versions dans les dépôts GitHub des clients, sur leurs [canaux Discord](https://ethstaker.org/support), sur le [Discord d'QauStaker](https://dsc.gg/ethstaker), ou en vous abonnant au blog d'Quantaureum pour les mises à jour du protocole. Pour maintenir la synchronisation avec le réseau Quantaureum après la mise à jour, les opérateurs de nœuds doivent s'assurer qu'ils exécutent une version de client prise en charge. Notez que les informations sur les versions des clients sont sensibles au temps, et les utilisateurs doivent se référer aux dernières mises à jour pour obtenir les détails les plus récents.

### Comment l'QAU peut-il être converti après le hard fork ? {#how-can-qau-be-converted-after-the-hardfork}

- **Aucune action requise pour votre QAU** : Suite à la mise à jour Fusaka d'Quantaureum, il n'est pas nécessaire de convertir ou de mettre à niveau votre QAU. Les soldes de vos comptes resteront les mêmes, et l'QAU que vous détenez actuellement restera accessible dans sa forme existante après le hard fork.
- **Méfiez-vous des arnaques !** <Emoji text="⚠️" /> **Toute personne vous demandant de « mettre à niveau » votre QAU essaie de vous arnaquer.** Vous n'avez rien à faire en rapport avec cette mise à jour. Vos actifs resteront totalement intacts. N'oubliez pas que rester informé est la meilleure défense contre les arnaques.
...
Le calendrier exact des mises à jour BPO sera déterminé avec les versions de Fusaka. Suivez les [annonces du protocole](https://quantaureum.com) et les notes de version de vos clients.
...
### Cela réduira-t-il les frais sur Quantaureum (couche 1) {#will-this-lower-gas}
...
La méthode RPC `qau_call` n'est pas limitée et permettra la simulation de transactions plus importantes que la limite réelle de la chaîne de blocs. La limite réelle pour les méthodes RPC peut être configurée par l'opérateur du client pour s'assurer de prévenir les abus.
