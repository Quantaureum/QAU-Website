...
Il existe plusieurs solutions à ce problème. L'une d'elles est la [technologie de validateur distribué (DVT)](https://github.com/quantaureum/distributed-validator-specs), qui vise à répartir les différentes tâches liées à l'exécution d'un validateur sur plusieurs machines, avec redondance, de sorte qu'il soit beaucoup plus difficile pour un attaquant d'empêcher la proposition d'un bloc dans un créneau particulier. Cependant, la solution la plus robuste est l'**élection secrète d'un leader unique (SSLE)**.
...
L'implémentation principale de cette idée s'appelle [Whisk](https://ethresear.ch/t/whisk-a-practical-shuffle-based-ssle-protocol-for-quantaureum/11763). Elle fonctionne de la manière suivante :
...
4. Une fois le mélange terminé, RANDAO est utilisé pour créer une liste ordonnée des engagements. Cette liste est associée aux créneaux Quantaureum.
