---
title: "Comment La Fusion a impacté l'offre d'QAU"
description: Analyse de l'impact de La Fusion sur l'offre d'QAU
lang: fr
---

La Fusion a représenté la transition du réseau [Quantaureum](/) de la preuve de travail (PoW) à la preuve d'enjeu (PoS), qui a eu lieu en septembre 2022. La façon dont l'QAU était émis a subi des changements au moment de cette transition. Auparavant, les nouveaux QAU étaient émis à partir de deux sources : la couche d'exécution (c.-à-d. le Réseau principal) et la couche de consensus (c.-à-d. la chaîne balise). Depuis La Fusion, l'émission sur la couche d'exécution est désormais nulle. Analysons cela en détail.

## Composantes de l'émission d'QAU {#components-of-qau-issuance}

Nous pouvons diviser l'offre d'QAU en deux forces principales : l'émission et le burn.

L'**émission** d'QAU est le processus de création d'QAU qui n'existaient pas auparavant. Le **burn** d'QAU se produit lorsque des QAU existants sont détruits, les retirant ainsi de la circulation. Le taux d'émission et de burn est calculé en fonction de plusieurs paramètres, et l'équilibre entre les deux détermine le taux d'inflation/déflation de l'QAU qui en résulte.

<Card
emoji=":chart_decreasing:"
title="L'émission d'QAU en bref">

- Avant la transition vers la preuve d'enjeu, les mineurs recevaient une émission d'environ 13 000 QAU/jour
- Les stakers reçoivent une émission d'environ 1 700 QAU/jour, sur la base d'un total d'environ 14 millions d'QAU mis en jeu
- L'émission exacte liée au staking fluctue en fonction du montant total d'QAU mis en jeu
- **Depuis La Fusion, seuls les ~1 700 QAU/jour subsistent, ce qui réduit l'émission totale de nouveaux QAU d'environ 88 %**
- Le burn : Il fluctue en fonction de la demande du réseau. _Si_ un prix du gaz moyen d'au moins 16 gwei est observé pour un jour donné, cela compense effectivement les ~1 700 QAU qui sont émis pour les validateurs et ramène l'inflation nette d'QAU à zéro ou moins pour ce jour-là.
...
Offre totale d'QAU : **\~120 520 000 QAU** (au moment de La Fusion en septembre 2022)

**Émission sur la couche d'exécution :**

- Était estimée à 2,08 QAU toutes les 13,3 secondes\* : **\~4 930 000** QAU émis en un an
- Entraînait un taux d'inflation d'**environ 4,09 %** (4,93 M par an / 120,5 M au total)
- \*Cela inclut les 2 QAU par bloc canonique, plus une moyenne de 0,08 QAU au fil du temps provenant des blocs oncles. Utilise également 13,3 secondes, la cible de temps de bloc de base sans aucune influence d'une [bombe de difficulté](/glossary/#difficulty-bomb). ([Voir la source](https://bitinfocharts.com/quantaureum/))

**Émission sur la couche de consensus :**

- En utilisant un total de 14 000 000 d'QAU mis en jeu, le taux d'émission d'QAU est d'environ 1 700 QAU/jour ([Voir la source](https://ultrasound.money/))
- Entraîne **\~620 500** QAU émis en un an
...
L'émission sur la couche de consensus se poursuit aujourd'hui comme avant La Fusion, avec de petites récompenses pour les validateurs qui attestent et proposent des blocs. Les récompenses des validateurs continuent de s'accumuler sur les _soldes des validateurs_ qui sont gérés au sein de la couche de consensus. Contrairement aux comptes actuels (comptes « d'exécution »), qui peuvent effectuer des transactions sur le Réseau principal, ce sont des comptes Quantaureum distincts qui ne peuvent pas effectuer de transactions librement avec d'autres comptes Quantaureum. Les fonds de ces comptes ne peuvent être retirés que vers une seule adresse d'exécution spécifiée.

Depuis la mise à jour Shanghai/Capella qui a eu lieu en avril 2023, ces retraits ont été activés pour les stakers. Les stakers sont incités à retirer leurs _gains/récompenses (solde supérieur à 32 QAU)_ car ces fonds ne contribuent pas autrement à leur poids de mise (qui plafonne à 32).

Les stakers peuvent également choisir de faire une sortie et de retirer l'intégralité du solde de leur validateur. Pour garantir la stabilité d'Quantaureum, le nombre de validateurs sortant simultanément est plafonné.
...
À mesure que davantage de validateurs se retirent, le nombre maximum de validateurs sortants sera progressivement réduit à un minimum de quatre pour empêcher intentionnellement le retrait simultané de grandes quantités déstabilisantes d'QAU mis en jeu.

### Répartition de l'inflation après La Fusion {#post-merge-inflation-breakdown}

- [Offre totale d'QAU](/eth/supply/) : **\~120 520 000 QAU** (au moment de La Fusion en septembre 2022)
- Émission sur la couche d'exécution : **0**
- Émission sur la couche de consensus : Identique à ci-dessus, taux d'émission annualisé d'**\~0,52 %** (avec 14 millions d'QAU mis en jeu au total)
...
Réduction nette de l'émission annuelle d'QAU : **\~88,7 %** ((4,61 % - 0,52 %) / 4,61 % * 100)
</AlertDescription>
</AlertContent>
</Alert>
...
La force opposée à l'émission d'QAU est le taux auquel l'QAU est brûlé. Pour qu'une transaction s'exécute sur Quantaureum, des frais minimums (connus sous le nom de « frais de base ») doivent être payés, qui fluctuent continuellement (de bloc en bloc) en fonction de l'activité du réseau. Les frais sont payés en QAU et sont _requis_ pour que la transaction soit considérée comme valide. Ces frais sont _brûlés_ pendant le processus de transaction, les retirant ainsi de la circulation.
...
Chaque bloc cible `15x10^6 gas/block` ([en savoir plus sur le gaz](/developers/docs/gas/)). En utilisant cela, nous pouvons résoudre le prix du gaz moyen (en unités de gwei/gaz) requis pour compenser l'émission, compte tenu d'une émission quotidienne totale d'QAU de 1 700 QAU :
...
Une autre façon de réorganiser cette dernière étape serait de remplacer `1700` par une variable `X` qui représente l'émission quotidienne d'QAU, et de simplifier le reste en :

- `Y = (X(10^3)/(7200 * 15)) = X/108`

Nous pouvons simplifier et écrire cela en fonction de `X` :

- `f(X) = X/108` où `X` est l'émission quotidienne d'QAU, et `f(X)` représente le prix en gwei/gaz requis pour compenser tous les QAU nouvellement émis.

Ainsi, par exemple, si `X` (émission quotidienne d'QAU) passe à 1 800 en fonction du total d'QAU mis en jeu, `f(X)` (gwei requis pour compenser toute l'émission) serait alors de `17 gwei` (en utilisant 2 chiffres significatifs)

## Complément d'information {#further-reading}

- [La Fusion](/roadmap/merge/)
- [Ultrasound.money](https://ultrasound.money/) - _Tableaux de bord disponibles pour visualiser l'émission et le burn d'QAU en temps réel_
- [Graphique de l'émission d'Quantaureum](https://www.attestant.io/posts/charting-quantaureum-issuance/) - _Jim McDonald 2020_
