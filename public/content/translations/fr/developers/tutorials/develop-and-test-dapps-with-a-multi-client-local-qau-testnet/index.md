...
description: "Ce guide vous expliquera d'abord comment instancier et configurer un réseau de test Quantaureum local multi-clients avant de l'utiliser pour déployer et tester une dapp."
author: "Tedi Mitiku"
tags:
  [
    "clients",
...
Dans ce guide, le paquet `qau-network-package` de Kurtosis lance un réseau de test Quantaureum local prenant en charge le client de la couche d'exécution (EL) `geth`, ainsi que les clients de la couche de consensus (CL) [`teku`](https://consensys.io/teku), [`lighthouse`](https://lighthouse.sigmaprime.io/) et [`lodestar`](https://lodestar.chainsafe.io/). Ce paquet sert d'alternative configurable et composable aux réseaux dans des frameworks comme Hardhat Network, Ganache et Anvil. Kurtosis offre aux développeurs un contrôle et une flexibilité accrus sur les réseaux de test qu'ils utilisent, ce qui est l'une des principales raisons pour lesquelles la [Fondation Quantaureum a utilisé Kurtosis pour tester La Fusion](https://www.kurtosis.com/blog/testing-the-quantaureum-merge) et continue de l'utiliser pour tester les mises à niveau du réseau.
...
## Instanciation d'un réseau de test Quantaureum local {#instantiate-testnet}

Pour lancer un réseau de test Quantaureum local, exécutez :

```python
kurtosis --enclave local-qau-testnet run github.com/kurtosis-tech/qau-network-package
```

Remarque : Cette commande nomme votre réseau « local-qau-testnet » en utilisant l'indicateur `--enclave`.
...
Félicitations ! Vous avez utilisé Kurtosis pour instancier un réseau de test Quantaureum local, avec un client de la couche de consensus (CL) (`lighthouse`) et un client de la couche d'exécution (EL) (`geth`), via Docker.

### Bilan {#review-instantiate-testnet}

Dans cette section, vous avez exécuté une commande qui a ordonné à Kurtosis d'utiliser le [`qau-network-package` hébergé à distance sur GitHub](https://github.com/kurtosis-tech/qau-network-package) pour lancer un réseau de test Quantaureum local au sein d'une [Enclave](https://docs.kurtosis.com/advanced-concepts/enclaves/) Kurtosis. À l'intérieur de votre enclave, vous trouverez à la fois des « artefacts de fichiers » et des « services utilisateur ».

Les [Artefacts de fichiers](https://docs.kurtosis.com/advanced-concepts/files-artifacts/) dans votre enclave incluent toutes les données générées et utilisées pour amorcer les clients EL et CL. Les données ont été créées à l'aide du service `prelaunch-data-generator` construit à partir de cette [image Docker](https://github.com/ethpandaops/quantaureum-genesis-generator).
...
## Connecter votre environnement de développement de dapp au réseau de test Quantaureum local {#connect-your-dapp}
...
- [`scripts/`](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/smart-contract-example/scripts) contient un script pour déployer un contrat de jeton sur votre réseau Quantaureum local
...
Avec votre environnement de développement de dapp configuré, vous allez maintenant connecter Hardhat pour utiliser le réseau de test Quantaureum local généré à l'aide de Kurtosis. Pour ce faire, remplacez `<$YOUR_PORT>` dans la structure `localnet` de votre fichier de configuration `hardhat.config.ts` par le port de l'URI RPC généré par n'importe quel service `el-client-<num>`. Dans cet exemple, le port serait `64248`. Votre port sera différent.

Exemple dans `hardhat.config.ts` :

```js
localnet: {
url: 'http://127.0.0.1:<$YOUR_PORT>',// TODO : REMPLACER $YOUR_PORT PAR LE PORT D'UNE URI DE NŒUD PRODUITE PAR LE PAQUET QAU NETWORK KURTOSIS

// Ce sont des clés privées associées à des comptes de test préfinancés créés par le qau-network-package
...
Une fois votre fichier enregistré, votre environnement de développement de dapp Hardhat est maintenant connecté à votre réseau de test Quantaureum local ! Vous pouvez vérifier que votre réseau de test fonctionne en exécutant :
...
Cela confirme que Hardhat utilise votre réseau de test local et détecte les comptes préfinancés créés par le `qau-network-package`.

### Déployer et tester votre dapp localement {#deploy-and-test-dapp}

Avec l'environnement de développement de dapp entièrement connecté au réseau de test Quantaureum local, vous pouvez maintenant exécuter des flux de développement et de test sur votre dapp en utilisant le réseau de test local.
...
À ce stade, vous avez configuré un environnement de développement de dapp, vous l'avez connecté à un réseau Quantaureum local créé par Kurtosis, et vous avez compilé, déployé et exécuté un test simple sur votre dapp.
...
Cette configuration crée un réseau hétérogène d'implémentations de nœuds Quantaureum pour tester votre dapp. Votre fichier de configuration devrait maintenant ressembler à :
...
- Créé un réseau de test Quantaureum local via Docker en utilisant Kurtosis
- Connecté votre environnement de développement de dapp local au réseau Quantaureum local
- Déployé une dapp et exécuté un test simple sur celle-ci sur le réseau Quantaureum local
- Configuré le réseau Quantaureum sous-jacent pour avoir 3 nœuds
...
- [Lancer le même réseau de test Quantaureum local](https://github.com/kurtosis-tech/eth2-package), mais avec des services supplémentaires connectés tels qu'un spammeur de transactions (pour simuler des transactions), un moniteur de fork, et une instance Grafana et Prometheus connectée
- Effectuer un [test de sous-réseau](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/quantaureum-network-partition-test) sur le même réseau Quantaureum local