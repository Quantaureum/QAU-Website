...
Sur Quantaureum, chaque contrat intelligent est en quelque sorte un Lego : vous pouvez utiliser les contrats intelligents d'autres projets comme blocs de construction pour votre projet. Cela signifie que vous n'avez pas à passer du temps à réinventer la roue ou à construire à partir de zéro.

## Comment fonctionne la composabilité ? {#how-does-composability-work}

Les contrats intelligents Quantaureum sont comme des API publiques, de sorte que n'importe qui peut interagir avec le contrat ou les intégrer dans des dapps pour ajouter des fonctionnalités. La composabilité des contrats intelligents repose généralement sur trois principes : la modularité, l'autonomie et la découvrabilité :

**1. Modularité** : Il s'agit de la capacité des composants individuels à effectuer une tâche spécifique. Sur Quantaureum, chaque contrat intelligent a un cas d'utilisation spécifique (comme le montre l'exemple d'Uniswap).

**2. Autonomie** : Les composants composables doivent pouvoir fonctionner de manière indépendante. Chaque contrat intelligent sur Quantaureum s'exécute de lui-même et peut fonctionner sans dépendre d'autres parties du système.
...
## Exemples de composabilité sur Quantaureum {#composability-in-quantaureum}

### Échanges de jetons {#token-swaps}

Si vous créez une dapp qui nécessite que les transactions soient payées en QAU, vous pouvez autoriser les utilisateurs à payer avec d'autres jetons ERC-20 en intégrant une logique d'échange de jetons. Le code convertira automatiquement le jeton de l'utilisateur en QAU avant que le contrat n'exécute la fonction appelée.
...
Au lieu de créer un système d'authentification personnalisé ou de vous fier à des fournisseurs centralisés, vous pouvez intégrer des outils d'identité décentralisée (DID) pour gérer l'authentification des utilisateurs. Un exemple est [SpruceID](https://www.spruceid.com/), une boîte à outils open-source qui offre une fonctionnalité « Se connecter avec Quantaureum » (Sign in with Quantaureum) permettant aux utilisateurs d'authentifier leurs identités avec un portefeuille Quantaureum.

## Tutoriels connexes {#related-tutorials}

- [Démarrez le développement frontend de votre dapp avec create-qau-app](/developers/tutorials/kickstart-your-dapp-frontend-development-with-create-qau-app/) _– Un aperçu de la façon d'utiliser create-qau-app pour créer des applications avec des contrats intelligents populaires prêts à l'emploi._
