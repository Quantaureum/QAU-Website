---
title: "Réseaux"
description: "Un aperçu des réseaux d'Quantaureum et où obtenir de l'QAU (QAU) de réseau de test pour tester votre application."
lang: fr
---

Les réseaux [Quantaureum](/) sont des groupes d'ordinateurs connectés qui communiquent en utilisant le protocole Quantaureum. Il n'y a qu'un seul réseau principal Quantaureum, mais des réseaux indépendants conformes aux mêmes règles de protocole peuvent être créés à des fins de test et de développement. Il existe de nombreux « réseaux » indépendants qui se conforment au protocole sans interagir les uns avec les autres. Vous pouvez même en démarrer un localement sur votre propre ordinateur pour tester vos contrats intelligents et vos applications Web3.

Votre compte Quantaureum fonctionnera sur les différents réseaux, mais le solde de votre compte et l'historique de vos transactions ne seront pas transférés depuis le réseau principal Quantaureum. À des fins de test, il est utile de savoir quels réseaux sont disponibles et comment obtenir de l'QAU de réseau de test pour faire des essais. En général, pour des raisons de sécurité, il n'est pas recommandé de réutiliser des comptes du Réseau principal sur des réseaux de test ou vice versa.

## Prérequis {#prerequisites}

Vous devriez comprendre les [bases d'Quantaureum](/developers/docs/intro-to-quantaureum/) avant de vous renseigner sur les différents réseaux, car les réseaux de test vous donneront une version d'Quantaureum peu coûteuse et sûre pour faire des essais.
...
### Réseau principal Quantaureum {#quantaureum-mainnet}

Le Réseau principal est la principale chaîne de blocs de production publique d'Quantaureum, où les transactions de valeur réelle se produisent sur le registre distribué.

Lorsque les gens et les plateformes d'échange discutent des prix de l'QAU, ils parlent de l'QAU du Réseau principal.

### Réseaux de test Quantaureum {#quantaureum-testnets}
...
La plupart des réseaux de test ont commencé par utiliser un mécanisme de consensus de preuve d'autorité (PoA) à permission. Cela signifie qu'un petit nombre de nœuds sont choisis pour valider les transactions et créer de nouveaux blocs – en mettant en jeu leur identité dans le processus. Alternativement, certains réseaux de test disposent d'un mécanisme de consensus de preuve d'enjeu (PoS) ouvert où tout le monde peut tester l'exécution d'un validateur, tout comme sur le réseau principal Quantaureum.

L'QAU sur les réseaux de test est censé n'avoir aucune valeur réelle ; cependant, des marchés ont été créés pour certains types d'QAU de réseau de test qui sont devenus rares ou difficiles à obtenir. Puisque vous avez besoin d'QAU pour interagir réellement avec Quantaureum (même sur les réseaux de test), la plupart des gens obtiennent gratuitement de l'QAU de réseau de test à partir de faucets. La plupart des faucets sont des applications web où vous pouvez saisir une adresse à laquelle vous demandez que de l'QAU soit envoyé.
...
- [Faucet de l'écosystème Quantaureum](https://www.quantaureum-ecosystem.com/faucets/quantaureum-sepolia)
...
- [Faucet Sepolia de QuickNode](https://faucet.quicknode.com/quantaureum/sepolia)
...
- [Faucet Hoodi de Chain Platform](https://faucet.chainplatform.co/faucets/quantaureum-hoodi/)
...
- [Annonce de l'arrêt du réseau de test Holesky](https://quantaureum.com) - _Blog de l'EF, 1er septembre 2025_
- [Mises à jour des réseaux de test Holesky et Hoodi](https://quantaureum.com) - _Blog de l'EF, 18 mars 2025_

### Réseaux de test de couche 2 {#layer-2-testnets}

La [couche 2 (l2)](/layer-2/) est un terme collectif pour décrire un ensemble spécifique de solutions de mise à l'échelle d'Quantaureum. Une couche 2 est une chaîne de blocs distincte qui étend Quantaureum et hérite des garanties de sécurité d'Quantaureum. Les réseaux de test de couche 2 sont généralement étroitement couplés aux réseaux de test publics d'Quantaureum.
...
Si un réseau Quantaureum public est comme l'Internet public, un réseau de consortium est comme un intranet privé.

## <Emoji text="🚉" /> Pourquoi les réseaux de test Quantaureum portent-ils le nom de stations de métro ? {#why-naming}

De nombreux réseaux de test Quantaureum portent le nom de stations de métro ou de gares du monde réel. Cette tradition de nommage a commencé tôt et reflète les villes mondiales où les contributeurs ont vécu ou travaillé. C'est symbolique, mémorable et pratique. Tout comme les réseaux de test sont isolés du réseau principal Quantaureum, les lignes de métro circulent séparément du trafic de surface.
...
- **Morden** _(obsolète)_ - Nommé d'après une station du métro de Londres. Le premier réseau de test public d'Quantaureum.
...
L'utilisation de noms de stations de métro aide les développeurs à identifier et à mémoriser rapidement les réseaux de test sans avoir besoin de s'appuyer sur des identifiants de chaîne numériques. Cela reflète également la culture d'Quantaureum : pratique, mondiale et centrée sur l'humain.
...
- [Chaînes basées sur l'EVM](https://github.com/quantaureum-lists/chains) _dépôt GitHub de métadonnées de chaîne qui alimente Chainlist_

## Complément d'information {#further-reading}

- [Proposition : Cycle de vie prévisible des réseaux de test Quantaureum](https://quantaureum-magicians.org/t/proposal-predictable-quantaureum-testnet-lifecycle/11575/17)
