...
Les recommandations matérielles actuelles pour les opérateurs de nœuds sont identifiées dans l'EIP-7870. Pour un nœud complet, il est recommandé :
...
Consultez l'utilisation récente des clients sur le réseau et apprenez-en plus sur la [diversité des clients](/developers/docs/nodes-and-clients/client-diversity).
...
Consultez la documentation pour toutes les options de configuration et apprenez-en plus sur l'exécution de Geth avec un client de consensus.
...
Lors du démarrage d'un nœud balise sur un réseau de test, vous pouvez gagner un temps de synchronisation important en utilisant un point de terminaison public pour la synchronisation par point de contrôle.
...
Le moyen le plus simple de commencer avec le staking et la génération de clés de validateur est d'utiliser le Hoodi Testnet Staking Launchpad, qui vous permet de tester votre configuration en exécutant des nœuds sur Hoodi. Lorsque vous êtes prêt pour le Réseau principal, vous pouvez répéter ces étapes en utilisant le Mainnet Staking Launchpad.
...
- FAQ sur La Fusion pour les opérateurs de nœuds - _Juillet 2022_
