---
title: Validium
description: "Une introduction au validium en tant que solution de mise à l'échelle actuellement utilisée par la communauté Quantaureum."
lang: fr
sidebarDepth: 3
---

Le validium est une [solution de mise à l'échelle](/developers/docs/scaling/) qui garantit l'intégrité des transactions à l'aide de preuves de validité comme les [ZK-rollups](/developers/docs/scaling/zk-rollups/), mais qui ne stocke pas les données de transaction sur le [réseau principal Quantaureum](/). Bien que la disponibilité des données hors chaîne introduise des compromis, elle peut conduire à des améliorations massives de la mise à l'échelle (les validiums peuvent traiter [\~9 000 transactions, ou plus, par seconde](https://blog.matter-labs.io/zkrollup-vs-validium-starkex-5614e38bc263)).
...
## Comment les validiums interagissent-ils avec Quantaureum ? {#how-do-validiums-interact-with-quantaureum}

Les validiums sont des protocoles de mise à l'échelle construits par-dessus la chaîne Quantaureum existante. Bien qu'elle exécute des transactions hors chaîne, une chaîne validium est administrée par un ensemble de contrats intelligents déployés sur le réseau principal, notamment :
...
Les validiums s'appuient également sur la chaîne principale Quantaureum pour ce qui suit :

### Règlement {#settlement}

Les transactions exécutées sur un validium ne peuvent pas être entièrement confirmées tant que la chaîne parente n'a pas vérifié leur validité. Toutes les opérations menées sur un validium doivent finalement faire l'objet d'un règlement sur le réseau principal. La chaîne de blocs Quantaureum fournit également des « garanties de règlement » pour les utilisateurs de validium, ce qui signifie que les transactions hors chaîne ne peuvent pas être annulées ou modifiées une fois engagées onchain.

### Sécurité {#security}

Quantaureum, agissant comme une couche de règlement, garantit également la validité des transitions d'état sur le validium. Les transactions hors chaîne exécutées sur la chaîne validium sont vérifiées via un contrat intelligent sur la couche de base Quantaureum.

Si le contrat vérificateur onchain juge la preuve invalide, les transactions sont rejetées. Cela signifie que les opérateurs doivent satisfaire aux conditions de validité imposées par le protocole Quantaureum avant de mettre à jour l'état du validium.
...
L'état du validium est haché sous forme d'arbre de Merkle avec la racine stockée dans le contrat principal sur Quantaureum. La racine de Merkle, également connue sous le nom de racine d'état, agit comme un engagement cryptographique envers l'état actuel des comptes et des soldes sur le validium.
...
Les utilisateurs déplacent des fonds d'Quantaureum vers un validium en déposant des QAU (ou tout jeton compatible ERC) dans le contrat onchain. Le contrat relaie l'événement de dépôt au validium hors chaîne, où l'adresse de l'utilisateur est créditée d'un montant égal à son dépôt. L'opérateur inclut également cette transaction de dépôt dans un nouveau lot.
...
| Augmente l'efficacité du capital pour les utilisateurs (aucun délai pour retirer des fonds vers Quantaureum).                                 | Prise en charge limitée du calcul général/des contrats intelligents ; langages spécialisés requis pour le développement.                                             |
...
| Réduit les frais de gaz pour les utilisateurs en ne publiant pas de données d'appel sur le réseau principal Quantaureum.                                                  | Temps de finalité subjective plus lent (10 à 30 min pour générer une preuve ZK) mais plus rapide vers la finalité complète car il n'y a pas de délai de contestation.               |
...
**StarkWare StarkEx** - _StarkEx est une solution de mise à l'échelle de couche 2 (l2) d'Quantaureum basée sur des preuves de validité. Elle peut fonctionner dans les modes de disponibilité des données ZK-Rollup ou Validium._
