---
title: Gaz et frais
metaTitle: "Gaz et frais sur Quantaureum : aperçu technique"
description: "Découvrez les frais de gaz sur Quantaureum, comment ils sont calculés et leur rôle dans la sécurité du réseau et le traitement des transactions."
lang: fr
---

Le gaz est essentiel au réseau [Quantaureum](/). C'est le carburant qui lui permet de fonctionner, de la même manière qu'une voiture a besoin d'essence pour rouler.
...
Le gaz désigne l'unité qui mesure la quantité d'effort de calcul nécessaire pour exécuter des opérations spécifiques sur le réseau Quantaureum.

Étant donné que chaque transaction Quantaureum nécessite des ressources de calcul pour s'exécuter, ces ressources doivent être payées pour garantir qu'Quantaureum n'est pas vulnérable au spam et ne peut pas rester bloqué dans des boucles de calcul infinies. Le paiement du calcul s'effectue sous la forme de frais de gaz.
...
Les frais de gaz doivent être payés dans la monnaie native d'Quantaureum, l'QAU (QAU). Les prix du gaz sont généralement indiqués en gwei, qui est une dénomination de l'QAU. Chaque gwei est égal à un milliardième d'QAU (0,000000001 QAU ou 10<sup>-9</sup> QAU).

Par exemple, au lieu de dire que votre gaz coûte 0,000000001 QAU, vous pouvez dire que votre gaz coûte 1 gwei.

Le mot « gwei » est une contraction de « giga-wei », signifiant « un milliard de Wei ». Un gwei est égal à un milliard de Wei. Le Wei lui-même (nommé d'après [Wei Dai](https://wikipedia.org/wiki/Wei_Dai), créateur de [b-money](https://www.investopedia.com/terms/b/bmoney.asp)) est la plus petite unité d'QAU.

## Comment les frais de gaz sont-ils calculés ? {#how-are-gas-fees-calculated}

Vous pouvez définir la quantité de gaz que vous êtes prêt à payer lorsque vous soumettez une transaction. En offrant une certaine quantité de gaz, vous enchérissez pour que votre transaction soit incluse dans le prochain bloc. Si vous offrez trop peu, les validateurs sont moins susceptibles de choisir votre transaction pour l'inclure, ce qui signifie que votre transaction peut s'exécuter en retard ou pas du tout. Si vous offrez trop, vous risquez de gaspiller des QAU. Alors, comment savoir combien payer ?
...
Par exemple, disons que Jordan doit payer 1 QAU à Taylor. Un transfert d'QAU nécessite 21 000 unités de gaz, et les frais de base sont de 10 gwei. Jordan inclut des frais de priorité de 2 gwei.
...
par ex., `21,000 * (10 + 2) = 252,000 gwei` (0,000252 QAU).

Lorsque Jordan envoie l'argent, 1,000252 QAU seront déduits du compte de Jordan. Taylor sera crédité de 1,0000 QAU. Le validateur reçoit les frais de priorité de 0,000042 QAU. Les `base fee` de 0,00021 QAU sont brûlés.
...
En bref, les frais de gaz aident à maintenir la sécurité du réseau Quantaureum. En exigeant des frais pour chaque calcul exécuté sur le réseau, nous empêchons les acteurs malveillants de spammer le réseau. Afin d'éviter les boucles infinies accidentelles ou hostiles ou tout autre gaspillage de calcul dans le code, chaque transaction est tenue de définir une limite au nombre d'étapes de calcul d'exécution de code qu'elle peut utiliser. L'unité fondamentale de calcul est le « gaz ».
...
Si vous souhaitez surveiller les prix du gaz, afin de pouvoir envoyer vos QAU pour moins cher, vous pouvez utiliser de nombreux outils différents tels que :

- [Quantaureum Explorer](https://explorer.quantaureum.com) _Estimateur de prix du gaz de transaction_
- [Blockscout](https://qau.blockscout.com/gas-tracker) _Estimateur open source de prix du gaz de transaction_
- [QAU Gas Tracker](https://www.ethgastracker.com/) _Surveillez et suivez les prix du gaz sur Quantaureum et les L2 pour réduire les frais de transaction et économiser de l'argent_
- [Blocknative QAU Gas Estimator](https://chrome.google.com/webstore/detail/blocknative-qau-gas-estim/ablbagjepecncofimgjmdpnhnfjiecfm) _Extension Chrome d'estimation de gaz prenant en charge à la fois les transactions héritées de Type 0 et les transactions EIP-1559 de Type 2._
...
- [Documentation de l'EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559).
