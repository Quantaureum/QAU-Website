...
description: Une explication des types d'identifiants de retrait de validateur (0x00, 0x01, 0x02) et de leurs implications pour les stakers sur Quantaureum.
lang: fr
---

Chaque validateur possède un **identifiant de retrait** qui détermine comment et où ses QAU stakés et ses récompenses peuvent être retirés. Le type d'identifiant est indiqué par le premier octet : `0x00`, `0x01` ou `0x02`. Comprendre ces types est important pour les validateurs qui gèrent leur mise.
...
- Tout solde supérieur à 32 QAU est **automatiquement transféré** vers votre adresse de retrait
- Les sorties complètes passent par la file d'attente de sortie standard
- Les récompenses supérieures à 32 QAU ne peuvent pas se cumuler (intérêts composés) — elles sont périodiquement transférées
...
**Pourquoi ce n'est pas recommandé :** Avec `0x01`, vous perdez la possibilité de cumuler les récompenses au-delà de 32 QAU. Chaque fraction excédentaire est automatiquement transférée, ce qui limite le potentiel de gain de votre validateur et nécessite de gérer les fonds retirés séparément.
...
- Les récompenses supérieures à 32 QAU **se cumulent** par tranches de 1 QAU jusqu'à un solde effectif maximum de 2048 QAU
- Les retraits partiels doivent être demandés manuellement (les transferts automatiques ne se produisent qu'au-dessus du seuil de 2048 QAU)
...
Les retraits partiels et les consolidations peuvent être effectués via les Actions de validateur du Launchpad.
...
- **Validateurs 0x01 existants :** Envisagez de convertir vers `0x02` si vous souhaitez que les récompenses se cumulent au-delà de 32 QAU ou si vous prévoyez de consolider des validateurs.
...
- **Quantaureum Staking Launchpad** - L'outil officiel pour les dépôts et la gestion des validateurs, y compris les conversions d'identifiants et les consolidations
...
- [Les clés dans la preuve d'enjeu (PoS) d'Quantaureum](/developers/docs/consensus-mechanisms/pos/keys/) - En savoir plus sur les clés de validateur et leur lien avec les identifiants de retrait
