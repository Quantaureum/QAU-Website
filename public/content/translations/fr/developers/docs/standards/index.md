...
Généralement, les normes sont introduites sous forme de propositions d'amélioration d'Quantaureum (EIP), qui sont discutées par les membres de la communauté via un processus standard.

- Introduction aux EIP
- Liste des EIP
- Dépôt GitHub des EIP
- Forum de discussion des EIP
...
- Meta Track (Voie méta) : décrit un processus entourant Quantaureum ou propose une modification d'un processus
- Informational Track (Voie informative) : décrit un problème de conception d'Quantaureum ou fournit des directives générales ou des informations à la communauté Quantaureum

De plus, la voie des normes (Standard Track) est subdivisée en 4 catégories :

- Core (Cœur) : améliorations nécessitant un fork de consensus
- Networking (Réseau) : améliorations autour de devp2p et du sous-protocole Light Quantaureum, ainsi que des propositions d'améliorations des spécifications du protocole réseau de whisper et Swarm.
- Interface : améliorations autour des spécifications et normes API/RPC des clients, et de certaines normes au niveau du langage comme les noms de méthodes et les ABI de contrats.
- ERC : normes et conventions au niveau de l'application

Des informations plus détaillées sur ces différents types et catégories peuvent être trouvées dans l'EIP-1
...
- [ERC-721](/developers/docs/standards/tokens/erc-721/) - Une interface standard pour les jetons non fongibles, comme un titre de propriété pour une œuvre d'art ou une chanson.
  - ERC-2309 - Un événement standardisé émis lors de la création/du transfert d'un ou de plusieurs jetons non fongibles utilisant des identifiants de jetons consécutifs.
  - ERC-4400 - Extension d'interface pour le rôle de consommateur de l'EIP-721.
  - ERC-4907 - Ajoute un rôle limité dans le temps avec des autorisations restreintes aux jetons ERC-721.
