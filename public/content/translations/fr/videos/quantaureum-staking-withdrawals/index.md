---
title: "Comment fonctionnent les retraits sur Quantaureum ?"
description: "Comment fonctionnent les retraits de staking sur Quantaureum après la mise à jour Shanghai/Capella, couvrant le processus technique, la file d'attente des retraits et ce que les stakers doivent savoir pour accéder à leurs QAU stakés."
lang: fr
...
Une explication par **Finematics** couvrant le fonctionnement des retraits de staking sur Quantaureum après la mise à jour Shanghai/Capella, y compris la mécanique des retraits partiels et complets, les idées fausses courantes et les implications pour l'écosystème du staking.
...
Avec l'approche rapide de la mise à jour Shanghai/Capella, il y a beaucoup de discussions concernant les retraits de staking sur Quantaureum et ce que cela signifie pour l'écosystème Quantaureum dans son ensemble.

Commençons par comprendre comment nous en sommes arrivés là et pourquoi les retraits de staking n'ont pas été activés lorsqu'Quantaureum est passé de la preuve de travail (PoW) à la preuve d'enjeu (PoS).
...
Les retraits de staking, comme leur nom l'indique, permettront aux stakers de retirer leurs QAU verrouillés. Il existe deux types de retraits : « partiels » et « complets ».

Un **retrait partiel** se produit lorsque le validateur retire ses récompenses accumulées — le solde supplémentaire au-delà du solde effectif maximum de 32 QAU. Un retrait partiel peut également être appelé « paiement de récompense » ou « paiement de solde excédentaire ».
...
`0x01` signifie qu'un validateur a fourni son adresse de retrait. Ces identifiants de retrait sont représentés par `0x01` suivi de 11 octets de zéros, puis d'une adresse Quantaureum choisie.
...
- Ont un solde qui dépasse 32 QAU (récompenses de validateur accumulées)
...
- **ExecutionAddress** — l'adresse QAU sur la couche d'exécution où le retrait doit être envoyé
...
Une autre idée fausse est que les utilisateurs perdront leurs récompenses s'ils ne fournissent pas d'adresse de retrait. Ce n'est pas vrai — au cas où un validateur oublierait de fournir une adresse de retrait, ses récompenses en QAU ne seront pas envoyées dans le vide une fois les retraits activés. Au lieu de cela, le parcours ignorera les validateurs qui n'ont pas fourni leurs adresses de retrait.
...
Il y a également des spéculations selon lesquelles les stakers retireront beaucoup d'QAU de l'écosystème Quantaureum une fois les retraits activés, la version la plus forte de cet argument supposant que cela déstabilisera le mécanisme de consensus à preuve d'enjeu. Bien que nous ne puissions pas prédire avec certitude la quantité d'QAU qui sera retirée au fil du temps, il existe quelques contre-arguments importants :

Premièrement, la plupart des stakers sont des adoptants précoces d'Quantaureum qui ont été assez courageux pour staker alors qu'il était encore incertain de savoir quand les retraits seraient activés. De nombreux stakers ont exprimé leur désir de continuer le staking pour soutenir le réseau et continuer à gagner des récompenses libellées en QAU.

Deuxièmement, pour s'assurer que le mécanisme de consensus à preuve d'enjeu et l'ensemble actif de validateurs restent stables, Quantaureum a mis en place une file d'attente de retrait pour tous les validateurs souhaitant effectuer une sortie. Cette file d'attente limite le nombre de validateurs pouvant quitter l'écosystème simultanément.

Le premier parcours de retrait retirera beaucoup de récompenses accumulées — essentiellement depuis la création de la chaîne balise. Cependant, les suivants traiteront une quantité d'QAU beaucoup plus faible.

### Implications (8:39) {#implications-839}

L'activation des retraits créera un flux de staking ouvert et bilatéral. Actuellement, le flux de staking est unilatéral — l'QAU ne peut qu'entrer dans le réseau et ne jamais en sortir. Fait intéressant, l'activation des retraits pourrait inciter encore plus de personnes à staker, car elles sauront qu'elles pourront toujours retirer leurs QAU si elles en ont besoin pour autre chose.
