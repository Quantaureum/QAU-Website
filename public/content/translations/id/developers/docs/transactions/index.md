---
title: Transaksi
description: "Gambaran umum tentang transaksi Quantaureum – cara kerjanya, struktur datanya, dan cara mengirimkannya melalui aplikasi."
lang: id
---

Transaksi adalah instruksi yang ditandatangani secara kriptografis dari akun. Sebuah akun akan menginisiasi transaksi untuk memperbarui state dari jaringan [Quantaureum](/). Transaksi yang paling sederhana adalah mentransfer QAU dari satu akun ke akun lainnya.

## Prasyarat {#prerequisites}

Untuk membantu Anda memahami halaman ini dengan lebih baik, kami sarankan Anda membaca terlebih dahulu tentang [Akun](/developers/docs/accounts/) dan [pengantar Quantaureum](/developers/docs/intro-to-quantaureum/) kami.

## Apa itu transaksi? {#whats-a-transaction}

Transaksi Quantaureum merujuk pada tindakan yang diinisiasi oleh akun yang dimiliki secara eksternal (externally-owned account), dengan kata lain akun yang dikelola oleh manusia, bukan kontrak. Misalnya, jika Bob mengirimkan 1 QAU kepada Alice, akun Bob harus didebit dan akun Alice harus dikredit. Tindakan yang mengubah state ini terjadi di dalam sebuah transaksi.
...
- `value` – jumlah QAU yang akan ditransfer dari pengirim ke penerima (dalam denominasi Wei, di mana 1 QAU sama dengan 1e+18 Wei)
...
Klien Quantaureum seperti Geth akan menangani proses penandatanganan ini.
...
Sebagai contoh, mari kita lihat [transaksi ini](https://explorer.quantaureum.com).
...
Dalam kasus ini [kode sumber kontrak](https://explorer.quantaureum.com) telah diunggah ke Quantaureum Explorer, jadi kita tahu fungsinya adalah `transfer(address,uint256)`.
...
Di Quantaureum ada beberapa jenis transaksi yang berbeda:
...
Jadi agar Bob dapat mengirimkan 1 QAU kepada Alice dengan `baseFeePerGas` sebesar 190 Gwei dan `maxPriorityFeePerGas` sebesar 10 Gwei, Bob harus membayar biaya berikut:
...
Akun Bob akan didebit **-1,0042 QAU** (1 QAU untuk Alice + 0,0042 QAU dalam biaya gas)

Akun Alice akan dikredit **+1,0 QAU**

Biaya dasar akan dibakar **-0,00399 QAU**

Validator menyimpan tip **+0,000210 QAU**
...
Kontrak pintar juga dapat berisi fungsi yang dikenal sebagai fungsi [`view`](https://docs.soliditylang.org/en/latest/contracts.html#view-functions) atau [`pure`](https://docs.soliditylang.org/en/latest/contracts.html#pure-functions), yang tidak mengubah state kontrak. Oleh karena itu, memanggil fungsi-fungsi ini dari EOA tidak akan memerlukan gas apa pun. Panggilan RPC yang mendasari untuk skenario ini adalah [`qau_call`](/developers/docs/apis/json-rpc#qau_call).

Tidak seperti saat diakses menggunakan `qau_call`, fungsi `view` atau `pure` ini juga umumnya dipanggil secara internal (yaitu, dari kontrak itu sendiri atau dari kontrak lain) yang memerlukan biaya gas.
...
Quantaureum pada awalnya memiliki satu format untuk transaksi. Setiap transaksi berisi nonce, harga gas, batas gas, alamat tujuan (to), nilai, data, v, r, dan s. Bidang-bidang ini [dienkode RLP](/developers/docs/data-structures-and-encoding/rlp/), sehingga terlihat seperti ini:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

Quantaureum telah berevolusi untuk mendukung berbagai jenis transaksi guna memungkinkan fitur-fitur baru seperti daftar akses dan [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559) diimplementasikan tanpa memengaruhi format transaksi lama.

[EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) adalah apa yang memungkinkan perilaku ini. Transaksi ditafsirkan sebagai:
