...
Kecepatan pemrosesan Quantaureum saat ini terbatas pada ~ 15-20 transaksi per detik, mengurangi kemungkinan jangka pendek dari penskalaan untuk menangani lebih banyak pengguna. Masalah ini ada terutama karena [mekanisme konsensus](/developers/docs/consensus-mechanisms/) Quantaureum mewajibkan banyak simpul peer-to-peer untuk memverifikasi setiap pembaruan pada state rantai blok.

Meskipun mekanisme konsensus Quantaureum diperlukan untuk keamanan, hal itu mungkin tidak berlaku untuk setiap kasus penggunaan. Misalnya, Alice mungkin tidak memerlukan pembayaran hariannya kepada Bob untuk secangkir kopi diverifikasi oleh seluruh jaringan Quantaureum karena ada kepercayaan di antara kedua belah pihak.

Plasma menganggap bahwa Mainnet Quantaureum tidak perlu memverifikasi semua transaksi. Sebaliknya, kita dapat memproses transaksi di luar Mainnet, membebaskan simpul dari keharusan memvalidasi setiap transaksi.

Komputasi offchain diperlukan karena rantai Plasma dapat mengoptimalkan kecepatan dan biaya. Misalnya, rantai Plasma mungkin—dan paling sering—menggunakan "operator" tunggal untuk mengelola pengurutan dan eksekusi transaksi. Dengan hanya satu entitas yang memverifikasi transaksi, waktu pemrosesan pada rantai plasma lebih cepat daripada Mainnet Quantaureum.
...
Akar Merkle penting untuk memberikan informasi tentang state offchain ke Quantaureum. Anda dapat menganggap akar Merkle sebagai "titik simpan": operator mengatakan, "Ini adalah state dari rantai Plasma pada titik waktu x, dan ini adalah akar Merkle sebagai buktinya." Operator berkomitmen pada _state saat ini_ dari rantai plasma dengan akar Merkle, itulah sebabnya ini disebut "komitmen state".

### Masuk dan keluar {#entries-and-exits}

Agar pengguna Quantaureum dapat memanfaatkan Plasma, perlu ada mekanisme untuk memindahkan dana antara Mainnet dan rantai plasma. Namun, kita tidak dapat secara sewenang-wenang mengirim QAU ke alamat di rantai plasma—rantai ini tidak kompatibel, sehingga transaksi akan gagal atau menyebabkan hilangnya dana.

Plasma menggunakan kontrak utama yang berjalan di Quantaureum untuk memproses masuk dan keluarnya pengguna. Kontrak utama ini juga bertanggung jawab untuk melacak komitmen state (dijelaskan sebelumnya) dan menghukum perilaku tidak jujur melalui bukti penipuan (akan dibahas lebih lanjut nanti).

#### Memasuki rantai plasma {#entering-the-plasma-chain}

Untuk memasuki rantai plasma, Alice (pengguna) harus menyetorkan QAU atau token ERC-20 apa pun ke dalam kontrak plasma. Operator plasma, yang mengawasi setoran kontrak, membuat ulang jumlah yang sama dengan setoran awal Alice dan melepaskannya ke alamatnya di rantai plasma. Alice diwajibkan untuk membuktikan penerimaan dana di rantai anak dan kemudian dapat menggunakan dana ini untuk transaksi.

Keluar dari rantai plasma lebih kompleks daripada memasukinya karena beberapa alasan. Alasan terbesarnya adalah, meskipun Quantaureum memiliki informasi tentang state rantai plasma, Quantaureum tidak dapat memverifikasi apakah informasi tersebut benar atau tidak. Pengguna jahat dapat membuat pernyataan yang salah ("Saya memiliki 1000 QAU") dan lolos dengan memberikan bukti palsu untuk mendukung klaim tersebut.
...
Namun, biasanya pengguna jujur dan membuat klaim yang benar tentang dana yang mereka miliki. Dalam skenario ini, Alice akan memulai permintaan penarikan pada rantai akar (Quantaureum) dengan mengirimkan transaksi ke kontrak plasma.
...
Jika periode tantangan berlalu tanpa ada yang memberikan bukti penipuan, permintaan penarikan Alice dianggap valid, memungkinkannya untuk mengambil setoran dari kontrak Plasma di Quantaureum.
...
Ketersediaan data juga membantu mengamankan protokol penskalaan offchain yang dibangun di atas lapisan dasar Quantaureum. Dengan memaksa operator pada rantai ini untuk mempublikasikan data transaksi di Quantaureum, siapa pun dapat menantang blok yang tidak valid dengan menyusun bukti penipuan yang merujuk pada state rantai yang benar.
...
Namun pendekatan ini masih memiliki masalah. Misalnya, jika semua pengguna di rantai plasma perlu keluar (yang dimungkinkan dalam kasus operator jahat), maka seluruh state valid dari rantai plasma harus dibuang ke lapisan dasar Quantaureum sekaligus. Dengan ukuran rantai plasma yang sewenang-wenang (laju pemrosesan tinggi = lebih banyak data) dan kendala pada kecepatan pemrosesan Quantaureum, ini bukanlah solusi yang ideal.

Meskipun permainan keluar (exit games) terdengar bagus secara teori, keluar massal di kehidupan nyata kemungkinan akan memicu kemacetan di seluruh jaringan pada Quantaureum itu sendiri. Selain merusak fungsionalitas Quantaureum, keluar massal yang dikoordinasikan dengan buruk berarti bahwa pengguna mungkin tidak dapat menarik dana sebelum operator menguras setiap akun di rantai plasma.
...
| Mengurangi beban pada Mainnet Quantaureum dengan memindahkan komputasi dan penyimpanan secara offchain.                                                                                                                                             | Penarikan tertunda beberapa hari untuk memungkinkan adanya tantangan. Untuk aset yang dapat dipertukarkan, hal ini dapat dimitigasi oleh penyedia likuiditas, tetapi ada biaya modal yang terkait. |
|                                                                                                                                                                                                                                                  | Jika terlalu banyak pengguna mencoba keluar secara bersamaan, Mainnet Quantaureum bisa menjadi macet.                                                                                                           |

## Plasma vs protokol penskalaan lapisan 2 {#plasma-vs-layer-2}

Meskipun Plasma pernah dianggap sebagai solusi penskalaan yang berguna untuk Quantaureum, sejak saat itu Plasma telah ditinggalkan dan digantikan oleh [protokol penskalaan lapisan 2 (l2)](/layer-2/). Solusi penskalaan l2 memperbaiki beberapa masalah Plasma:
...
Demikian pula, optimistic rollup memberlakukan periode penundaan pada penarikan di mana siapa pun dapat memulai tantangan dan mencegah permintaan penarikan yang berbahaya. Meskipun ini mirip dengan Plasma, perbedaannya adalah pemverifikasi memiliki akses ke data yang diperlukan untuk membuat bukti penipuan. Dengan demikian, pengguna rollup tidak perlu terlibat dalam migrasi "siapa cepat dia dapat" yang panik ke Mainnet Quantaureum.

## Apa perbedaan Plasma dengan rantai samping dan sharding? {#plasma-sidechains-sharding}

Plasma, rantai samping, dan sharding cukup mirip karena semuanya terhubung ke Mainnet Quantaureum dalam beberapa cara. Namun, tingkat dan kekuatan koneksi ini bervariasi, yang memengaruhi properti keamanan dari setiap solusi penskalaan.

### Plasma vs rantai samping {#plasma-vs-sidechains}

[Rantai samping](/developers/docs/scaling/sidechains/) adalah rantai blok yang dioperasikan secara independen yang terhubung ke Mainnet Quantaureum melalui jembatan dua arah. [Jembatan](/bridges/) memungkinkan pengguna untuk menukar token di antara kedua rantai blok untuk bertransaksi di rantai samping, mengurangi kemacetan di Mainnet Quantaureum dan meningkatkan skalabilitas.
Rantai samping menggunakan mekanisme konsensus terpisah dan biasanya jauh lebih kecil daripada Mainnet Quantaureum. Akibatnya, menjembatani aset ke rantai ini melibatkan peningkatan risiko; mengingat kurangnya jaminan keamanan yang diwarisi dari Mainnet Quantaureum dalam model rantai samping, pengguna berisiko kehilangan dana dalam serangan terhadap rantai samping.

Sebaliknya, rantai plasma memperoleh keamanannya dari Mainnet. Hal ini membuatnya jauh lebih aman daripada rantai samping. Baik rantai samping maupun rantai plasma dapat memiliki protokol konsensus yang berbeda, tetapi perbedaannya adalah rantai plasma mempublikasikan akar Merkle untuk setiap blok di Mainnet Quantaureum. Akar blok adalah potongan kecil informasi yang dapat kita gunakan untuk memverifikasi informasi tentang transaksi yang terjadi pada rantai plasma. Jika serangan terjadi pada rantai plasma, pengguna dapat dengan aman menarik dana mereka kembali ke Mainnet menggunakan bukti yang sesuai.

### Plasma vs sharding {#plasma-vs-sharding}

Baik rantai plasma maupun rantai shard secara berkala mempublikasikan bukti kriptografi ke Mainnet Quantaureum. Namun, keduanya memiliki properti keamanan yang berbeda.
...
**Catatan** bahwa sharding rantai blok Quantaureum tidak lagi ada di peta jalan. Hal ini telah digantikan oleh penskalaan melalui rollup dan [danksharding](/roadmap/danksharding).
...
## Tutorial: Rantai Plasma di Quantaureum {#tutorials}
