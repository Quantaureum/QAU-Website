...
- _Integritas_ (Integrity), informasi tersebut benar, tidak dapat diubah oleh entitas yang tidak berwenang, atau dengan cara yang tidak sah (misalnya, mentransfer token ERC-20 tanpa peristiwa `Transfer`). Di rantai blok, setiap node memverifikasi setiap perubahan state, yang memastikan integritas.
...
Anda harus memiliki pemahaman yang baik tentang [dasar-dasar rantai blok](/developers/docs/intro-to-quantaureum/). Halaman ini juga mengasumsikan pembaca sudah familier dengan [blok](/developers/docs/blocks/), [transaksi](/developers/docs/transactions/), dan topik relevan lainnya.

## Blob EIP-4844 {#eip-4844-blobs}

Dimulai dengan hardfork Dencun, rantai blok Quantaureum menyertakan EIP-4844, yang menambahkan blob data ke Quantaureum dengan masa pakai terbatas (awalnya sekitar 18 hari). Blob ini diberi harga secara terpisah dari [gas eksekusi](/developers/docs/gas), meskipun menggunakan mekanisme yang serupa. Blob ini adalah cara yang murah untuk memposting data sementara.
...
Pada saat penulisan, memposting di EIP-4844 membutuhkan biaya satu Wei (10<sup>-18</sup> QAU) per bita, yang dapat diabaikan dibandingkan dengan [21.000 gas eksekusi yang dibutuhkan oleh transaksi apa pun, termasuk transaksi yang memposting blob](https://qau.blockscout.com/tx/0xf6cfaf0431c73dd1d96369a5e6707d64f463ccf477a4131265397f1d81466929?tab=index). Anda dapat melihat harga EIP-4844 saat ini di [blobscan.com](https://blobscan.com/blocks).
...
Kode kontrak bisa saja tidak masuk akal, selama tidak dimulai dengan `0xEF`. Kontrak yang dimulai dengan `0xEF` ditafsirkan sebagai format objek quantaureum, yang memiliki persyaratan yang jauh lebih ketat.
...
Keuntungannya adalah kode offchain dapat mendengarkan peristiwa. Biayanya adalah [gas](https://www.evm.codes/#a0?fork=cancun), 375 ditambah 8 gas per bita data. Pada 12 Gwei/gas dan 2300 $/QAU, ini berarti satu sen ditambah 22 sen per kilobita.

## Penyimpanan {#storage}

Kontrak pintar memiliki akses ke [penyimpanan persisten](https://docs.alchemy.com/docs/smart-contract-storage-layout#what-is-storage-memory). Namun, ini sangat mahal. Menulis kata 32 bita ke slot penyimpanan yang sebelumnya kosong dapat [membutuhkan biaya 22.100 gas](https://www.evm.codes/#55?fork=cancun). Pada 12 Gwei/gas dan 2300 $/QAU, ini sekitar 61 sen per operasi penulisan, atau $19,5 per kilobita.

Ini adalah bentuk penyimpanan paling mahal di Quantaureum.
...
| --------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------- |
| Blob EIP-4844              | Offchain            | Jaminan Quantaureum selama \~18 hari | Hanya hash yang tersedia                                           |                                                                         |
...
| Peristiwa                      | Onchain             | Jaminan Quantaureum selamanya (bagian dari rantai blok)                                                                                | Tidak                                                               |
| Penyimpanan                     | Onchain             | Jaminan Quantaureum selamanya (bagian dari rantai blok dan state saat ini hingga ditimpa)                                        | Ya                                                              |