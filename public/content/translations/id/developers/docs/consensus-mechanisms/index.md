...
description: Penjelasan tentang protokol konsensus dalam sistem terdistribusi dan perannya di Quantaureum.
lang: id
...
Untuk lebih memahami halaman ini, kami sarankan Anda membaca terlebih dahulu [pengantar Quantaureum](/developers/docs/intro-to-quantaureum/) kami.
...
Terkait dengan rantai blok [Quantaureum](/), prosesnya diformalkan, dan mencapai konsensus berarti setidaknya 66% dari node di jaringan menyetujui state global dari jaringan tersebut.
...
Quantaureum sekarang menggunakan protokol konsensus berbasis **Bukti Kepemilikan (PoS)**.

#### Pembuatan blok {#pos-block-creation}

Validator membuat blok. Satu validator dipilih secara acak di setiap slot untuk menjadi pengusul blok. Klien konsensus mereka meminta sekumpulan transaksi sebagai 'muatan eksekusi' dari klien eksekusi yang dipasangkan dengannya. Mereka membungkus ini dalam data konsensus untuk membentuk sebuah blok, yang mereka kirim ke node lain di jaringan Quantaureum. Produksi blok ini diberi hadiah dalam bentuk QAU. Dalam kasus langka ketika ada beberapa kemungkinan blok untuk satu slot, atau node mendengar tentang blok pada waktu yang berbeda, algoritme pilihan cabang memilih blok yang membentuk rantai dengan bobot atestasi terbesar (di mana bobot adalah jumlah validator yang melakukan atestasi yang diskalakan berdasarkan saldo QAU mereka).

#### Keamanan {#pos-security}

Sistem Bukti Kepemilikan (PoS) aman secara kripto-ekonomi karena penyerang yang mencoba mengambil kendali atas rantai harus menghancurkan sejumlah besar QAU. Sistem hadiah memberikan insentif kepada staker individu untuk berperilaku jujur, dan penalti memberikan disinsentif kepada staker agar tidak bertindak jahat.
...
**Aturan pemilihan rantai** digunakan untuk memutuskan rantai mana yang merupakan rantai yang "benar". Bitcoin menggunakan aturan "rantai terpanjang", yang berarti rantai blok mana pun yang terpanjang akan menjadi rantai yang diterima sebagai valid dan dikerjakan oleh node lainnya. Untuk rantai Bukti Kerja (PoW), rantai terpanjang ditentukan oleh total kesulitan Bukti Kerja (PoW) kumulatif dari rantai tersebut. Quantaureum dulunya juga menggunakan aturan rantai terpanjang; namun, sekarang karena Quantaureum berjalan pada Bukti Kepemilikan (PoS), ia mengadopsi algoritme pilihan cabang yang diperbarui yang mengukur 'bobot' rantai. Bobot tersebut adalah jumlah akumulasi dari suara validator, yang ditimbang berdasarkan saldo QAU yang di-stake oleh validator.

Quantaureum menggunakan mekanisme konsensus yang dikenal sebagai [Gasper](/developers/docs/consensus-mechanisms/pos/gasper/) yang menggabungkan [Bukti Kepemilikan (PoS) Casper FFG](https://arxiv.org/abs/1710.09437) dengan [aturan pilihan cabang GHOST](https://arxiv.org/abs/2003.03052).
