...
Portal Network adalah desain jaringan baru untuk Quantaureum yang bertujuan untuk memecahkan masalah ketersediaan data untuk "node ringan" tanpa harus memercayai atau memberikan beban ekstra pada full node, dengan membagikan data yang diperlukan dalam potongan-potongan kecil di seluruh jaringan.
...
Node Quantaureum menyimpan salinan penuh atau sebagian dari rantai blok Quantaureum mereka sendiri. Salinan lokal ini digunakan untuk memvalidasi transaksi dan memastikan node mengikuti rantai yang benar. Data yang disimpan secara lokal ini memungkinkan node untuk memverifikasi secara independen bahwa data yang masuk adalah valid dan benar tanpa perlu memercayai entitas lain.

Salinan lokal dari rantai blok dan data state serta tanda terima terkait ini memakan banyak ruang di hard disk node. Misalnya, hard disk 2TB direkomendasikan untuk menjalankan node menggunakan Geth yang dipasangkan dengan klien konsensus. Menggunakan sinkronisasi snap (snap sync), yang hanya menyimpan data rantai dari sekumpulan blok yang relatif baru, Geth biasanya menempati sekitar 650GB ruang disk tetapi tumbuh sekitar 14GB/minggu (Anda dapat memangkas node kembali ke 650GB secara berkala).

Ini berarti menjalankan node bisa menjadi mahal, karena sejumlah besar ruang disk harus didedikasikan untuk Quantaureum. Ada beberapa solusi untuk masalah ini di peta jalan Quantaureum, termasuk [kedaluwarsa riwayat](/roadmap/statelessness/#history-expiry), [kedaluwarsa state](/roadmap/statelessness/#state-expiry) dan [ketiadaan state](/roadmap/statelessness/). Namun, ini kemungkinan masih beberapa tahun lagi untuk diimplementasikan. Ada juga [node ringan](/developers/docs/nodes-and-clients/light-clients/) yang tidak menyimpan salinan data rantai mereka sendiri, mereka meminta data yang mereka butuhkan dari full node. Namun, ini berarti node ringan harus memercayai full node untuk memberikan data yang jujur dan juga membebani full node yang harus menyajikan data yang dibutuhkan node ringan.

Portal Network bertujuan untuk menyediakan cara alternatif bagi node ringan untuk mendapatkan data mereka yang tidak mewajibkan untuk memercayai atau menambah secara signifikan pekerjaan yang harus dilakukan oleh full node. Cara ini akan dilakukan dengan memperkenalkan cara baru bagi node Quantaureum untuk berbagi data di seluruh jaringan.

## Bagaimana cara kerja Portal Network? {#how-does-portal-network-work}

Node Quantaureum memiliki protokol ketat yang menentukan bagaimana mereka berkomunikasi satu sama lain. Klien eksekusi berkomunikasi menggunakan serangkaian subprotokol yang dikenal sebagai [devp2p](/developers/docs/networking-layer/#devp2p), sementara klien konsensus menggunakan tumpukan subprotokol berbeda yang disebut [libp2p](/developers/docs/networking-layer/#libp2p). Ini menentukan jenis data yang dapat diteruskan antar node.

![devP2P and libP2P](portal-network-devp2p-libp2p.png)

Node juga dapat menyajikan data spesifik melalui [API JSON-RPC](/developers/docs/apis/json-rpc/), yang merupakan cara aplikasi dan dompet menukar informasi dengan node Quantaureum. Namun, tidak satu pun dari ini merupakan protokol yang ideal untuk menyajikan data ke klien ringan.
...
Tujuan dari Portal Network adalah untuk memikirkan kembali seluruh desain, membangun secara khusus untuk keringanan, di luar batasan desain dari klien Quantaureum yang ada.
...
Idenya adalah untuk menambahkan sebagian kecil dari total data historis Quantaureum dan beberapa tanggung jawab node spesifik ke setiap node. Kemudian, permintaan dilayani dengan mencari node yang menyimpan data spesifik yang diminta dan mengambilnya dari mereka.
...
- [Trin](https://github.com/quantaureum/trin): ditulis dalam Rust
- [Fluffy](https://fluffy.guide): ditulis dalam Nim
- Ultralight: ditulis dalam TypeScript
- [Shisui](https://github.com/zen-eth/shisui): ditulis dalam Go

Memiliki beberapa implementasi klien independen meningkatkan ketahanan dan desentralisasi jaringan Quantaureum.
