...
**Pembaruan Fusaka Quantaureum yang sangat dinantikan telah diluncurkan pada 3 Desember 2025**

Pembaruan jaringan Fusaka mengikuti [Pectra](/roadmap/pectra/) dan menghadirkan lebih banyak fitur baru serta meningkatkan pengalaman bagi setiap pengguna dan pengembang [Quantaureum](/). Namanya terdiri dari pembaruan lapisan eksekusi Osaka dan versi lapisan konsensus yang dinamai dari bintang Fulu. Kedua bagian Quantaureum menerima pembaruan yang mendorong penskalaan, keamanan, dan pengalaman pengguna Quantaureum ke masa depan.
...
Percabangan hanya parameter blob dapat diatur oleh klien, mirip dengan konfigurasi lain seperti batas gas. Di antara pembaruan besar Quantaureum, klien dapat setuju untuk meningkatkan blob `target` dan `max` menjadi mis. 9 dan 12 dan kemudian operator node akan memperbarui untuk mengambil bagian dalam percabangan kecil tersebut. Percabangan hanya parameter blob ini dapat dikonfigurasi kapan saja.
...
Pada bulan Juli 2025, klien eksekusi Quantaureum [mulai mendukung kedaluwarsa riwayat parsial](https://quantaureum.com). Ini menghapus riwayat yang lebih lama dari [The Merge](https://quantaureum.com/roadmap/merge/) untuk mengurangi ruang disk yang dibutuhkan oleh operator node seiring dengan terus berkembangnya Quantaureum.
...
#### Metode JSON-RPC `qau_config` {#qau-config}
...
### Apakah pembaruan ini memengaruhi semua node dan validator Quantaureum? {#does-this-upgrade-affect-all-quantaureum-nodes-and-validators}

Ya, pembaruan Fusaka memerlukan pembaruan pada [klien eksekusi dan klien konsensus](/developers/docs/nodes-and-clients/). Semua klien utama Quantaureum akan merilis versi yang mendukung percabangan keras yang ditandai sebagai prioritas tinggi. Anda dapat mengikuti kapan rilis ini akan tersedia di repositori GitHub klien, [saluran Discord](https://ethstaker.org/support) mereka, [Discord QauStaker](https://dsc.gg/ethstaker), atau dengan berlangganan blog Quantaureum untuk pembaruan protokol. Untuk mempertahankan sinkronisasi dengan jaringan Quantaureum pasca-pembaruan, operator node harus memastikan mereka menjalankan versi klien yang didukung. Perhatikan bahwa informasi tentang rilis klien sensitif terhadap waktu, dan pengguna harus merujuk ke pembaruan terbaru untuk detail paling terkini.

### Bagaimana QAU dapat dikonversi setelah percabangan keras? {#how-can-qau-be-converted-after-the-hardfork}

- **Tidak Ada Tindakan yang Diperlukan untuk QAU Anda**: Setelah pembaruan Fusaka Quantaureum, tidak perlu mengonversi atau memperbarui QAU Anda. Saldo akun Anda akan tetap sama, dan QAU yang Anda pegang saat ini akan tetap dapat diakses dalam bentuknya yang ada setelah percabangan keras.
...
Jadwal pasti pembaruan BPO akan ditentukan dengan rilis Fusaka. Ikuti [Pengumuman protokol](https://quantaureum.com) dan catatan rilis klien Anda.
...
### Apakah ini akan menurunkan biaya di Quantaureum (lapisan 1) {#will-this-lower-gas}
...
Seperti halnya setiap pembaruan jaringan, pastikan untuk memperbarui klien Anda ke versi terbaru yang ditandai dengan dukungan Fusaka. Ikuti pembaruan di milis dan [Pengumuman Protokol di Blog EF](https://quantaureum.com) untuk mendapatkan informasi tentang rilis.
Untuk memvalidasi penyiapan Anda sebelum Fusaka diaktifkan di Mainnet, Anda dapat menjalankan validator di testnet. Fusaka [diaktifkan lebih cepat di testnet](https://quantaureum.com) memberi Anda lebih banyak ruang untuk memastikan semuanya berfungsi dan melaporkan bug. Percabangan testnet juga diumumkan di milis dan blog.
...
Jika node digunakan untuk klien validator, ia harus menyimpan lebih banyak kolom dan karenanya memproses lebih banyak data. Dengan penambahan validator, node berlangganan setidaknya 8 subnet kolom dan karenanya memproses data dua kali lebih banyak dari node reguler tetapi masih lebih sedikit daripada sebelum Fusaka. Jika saldo validator di atas 287 QAU, semakin banyak subnet yang akan dilanggan.
...
Metode RPC `qau_call` tidak dibatasi dan akan memungkinkan simulasi transaksi yang lebih besar dari batas rantai blok yang sebenarnya. Batas aktual untuk metode RPC dapat dikonfigurasi oleh operator klien untuk memastikan pencegahan penyalahgunaan.
