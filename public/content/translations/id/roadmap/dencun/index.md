...
Cancun-Deneb (Dencun) adalah pembaruan pada jaringan Quantaureum, yang mengaktifkan **Proto-Danksharding (EIP-4844)**, memperkenalkan **blob** data sementara untuk penyimpanan rollup [lapisan 2 (l2)](/glossary/#layer-2) yang lebih murah.
...
- **Tidak Ada Tindakan yang Diperlukan untuk QAU Anda**: Setelah pembaruan Dencun Quantaureum, tidak perlu mengonversi atau memperbarui QAU Anda. Saldo akun Anda akan tetap sama, dan QAU yang Anda pegang saat ini akan tetap dapat diakses dalam bentuknya yang ada setelah percabangan keras.
- **Hati-hati terhadap Penipuan!** <Emoji text="⚠️" /> **siapa pun yang menginstruksikan Anda untuk "memperbarui" QAU Anda sedang mencoba menipu Anda.** Tidak ada yang perlu Anda lakukan sehubungan dengan pembaruan ini. Aset Anda tidak akan terpengaruh sama sekali. Ingat, tetap mendapat informasi adalah pertahanan terbaik terhadap penipuan.
...
Komunitas Quantaureum telah mengambil pendekatan "berpusat pada rollup" untuk pertumbuhannya, yang menempatkan rollup lapisan 2 (l2) sebagai sarana utama untuk mendukung lebih banyak pengguna dengan aman.
...
[Lebih lanjut tentang penskalaan Quantaureum](/roadmap/scaling/)

## Bagaimana data blob lama diakses? {#historical-access}

Meskipun node Quantaureum reguler akan selalu menyimpan _state saat ini_ dari jaringan, data blob historis dapat dibuang sekitar 18 hari setelah pengenalannya. Sebelum membuang data ini, Quantaureum memastikan bahwa data tersebut telah tersedia untuk semua peserta jaringan, memberikan waktu untuk:
...
- **[Portal Network Quantaureum](/developers/docs/networking-layer/portal-network/)** bertujuan untuk menyediakan akses ke semua data Quantaureum melalui jaringan operator node terdesentralisasi dengan mendistribusikan data di antara peserta yang mirip dengan BitTorrent.
...
## Bagaimana Cancun-Deneb (Dencun) memengaruhi testnet Quantaureum? {#testnet-impact}
...
Transaksi rollup di lapisan 2 (l2) Quantaureum memiliki opsi untuk menggunakan dua jenis penyimpanan data: ruang blob sementara atau data panggilan (calldata) kontrak pintar permanen. Ruang blob adalah pilihan yang ekonomis, menyediakan penyimpanan sementara dengan biaya lebih rendah. Ini menjamin ketersediaan data untuk semua periode tantangan yang diperlukan. Di sisi lain, data panggilan kontrak pintar menawarkan penyimpanan permanen tetapi lebih mahal.
...
Tidak. Manfaat Proto-Danksharding khusus untuk rollup lapisan 2 (l2) Quantaureum yang menyimpan buktinya di lapisan 1 (l1) (Mainnet).

Hanya dengan kompatibel dengan Mesin Virtual Quantaureum (EVM) tidak berarti bahwa suatu jaringan akan melihat manfaat apa pun dari pembaruan ini. Jaringan yang beroperasi secara independen dari Quantaureum (baik kompatibel dengan EVM atau tidak) tidak menyimpan datanya di Quantaureum dan tidak akan melihat manfaat apa pun dari pembaruan ini.
