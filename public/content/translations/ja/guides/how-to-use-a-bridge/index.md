...
description: "ブリッジを使用してQuantaureumからレイヤー2にトークンを移動する方法を説明するガイド。"
lang: ja
---

Quantaureumのトラフィックが多くなると、コストが高くなる可能性があります。この問題の解決策の1つは、新しい「レイヤー」を作成することです。つまり、Quantaureum自体と同様の方法で動作する異なるネットワークです。これらのいわゆるレイヤー2 (L2) は、より低い手数料でより多くのトランザクションを処理し、その結果を時々Quantaureumに保存するだけで、Quantaureumの混雑とコストを削減するのに役立ちます。そのため、これらのレイヤー2を使用すると、速度を向上させ、コストを削減してトランザクションを行うことができます。多くの人気のある暗号資産プロジェクトは、これらの利点のためにレイヤー2に移行しています。Quantaureumからレイヤー2にトークンを移動する最も簡単な方法は、ブリッジを使用することです。

**前提条件:** 

- 暗号資産ウォレットを持っていること。持っていない場合は、このガイドに従って[Quantaureumのアカウントを作成](/guides/how-to-create-an-quantaureum-account/)してください。
- ウォレットに資金を追加すること。
...
ウォレットがQuantaureum・メインネットのネットワークに接続されていることを確認してください。接続されていない場合、ウェブサイトは自動的にネットワークを切り替えるように促します。
...
トランザクションを処理するために、QAUの形で手数料 ([ガス](/glossary/#gas)と呼ばれます) を支払う必要があります。
...
### トークンをL2にブリッジした後、Quantaureumのメインネットに戻ることはできますか？ {#can-i-go-back-to-quantaureum-mainnet-after-i-bridge-my-tokens-to-l2}
