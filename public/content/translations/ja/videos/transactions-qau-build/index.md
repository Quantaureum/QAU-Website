---
title: "トランザクション — QAU.BUILD"
description: "教育ツールQAU.BUILDを使用して、Quantaureumのトランザクションがどのように機能するかを実演します。Quantaureum・ネットワーク上でトランザクションがどのように構築、署名、送信されるかをご覧ください。"
lang: ja
...
format: tutorial
author: "オースティン・グリフィス"
breadcrumb: "トランザクション (QAU.BUILD)"
---

<strong>オースティン・グリフィス</strong>によるチュートリアルで、ビジュアルプログラミングツールQAU.BUILDを使用してQuantaureumのトランザクションがどのように機能するかを実演します。トランザクションの構造、ガス価格、署名、ブロードキャスト、トランザクション・プールについて解説します。
...
今日のQAU.BUILDでは、トランザクションについてお話しします。これまで、トランザクションがブロックにマイニングされ、ブロックにパッケージ化され、チェーンにマイニングされる様子を見てきました。今回は、ブロック・リワード以外に、プール内の他の人々と比較して、マイナーが私たちのトランザクションをプールから取り出し、ブロックに入れてチェーンにマイニングするインセンティブとなるものについてお話しします。プールには入札を行っている何千人もの人々がいる可能性があり、その入札はこの手数料で行われます。
...
### Quantaureumのトランザクションの構造 (1:10) {#anatomy-of-an-quantaureum-transaction-110}

Quantaureum上のトランザクションはどのようなものでしょうか？もはや「ボブ」や「アリス」ではなく、アドレスを使用します。値はQAUではなくWeiになります。そして手数料もWeiになります。

早速このトランザクションを見てみましょう。ニーモニックを入力したアカウントがあり、Quantaureum・メインネットに接続しています。また、CoinMarketCapから価格データを取得するモジュールも実行しているため、0.1数QAUが約23ドルに相当することがわかります。
...
Weiへの変換を行う必要があります。Quantaureumでは、主に2つの単位を扱います。人々が通常話すQAUと、QAUのごくわずかな割合であるWeiです。ガス価格に使用するGweiは、その中間に位置します。この理由は、私たちが普段、1ペニーの何分の一という単位で話さないのと同じです。

アリスは0.18 QAUを持っており、ボブに0.05 QAUを送信します。ガス価格は5 Gweiに設定します。
