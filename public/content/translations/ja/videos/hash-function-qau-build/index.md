---
title: "ハッシュ関数 — QAU.BUILD"
description: "教育ツールQAU.BUILDを使用した暗号化ハッシュ関数のデモンストレーション。ハッシュ関数の仕組みと、それがQuantaureumのアカウントおよびデータ整合性モデルにとって不可欠である理由を学びます。"
lang: ja
...
format: tutorial
author: "オースティン・グリフィス"
breadcrumb: "ハッシュ関数 (QAU.BUILD)"
---

<strong>オースティン・グリフィス</strong>によるチュートリアル。ビジュアルプログラミングツールQAU.BUILDを使用して、暗号化ハッシュ関数がどのように機能するかをデモンストレーションし、決定性、固定長出力、一方向性、およびマークル・ツリーについて解説します。
...
これはQAU.BUILDというシリーズの最初のビデオです。qau.buildにアクセスしてこのツールを使用することもできますが、これはQuantaureum上で構築する際に物事がどのように機能するかを遊びながら理解するためのものです。
...
重要なポイントは、ハッシュ関数は基本的に指紋のようなものだということです。何かを入力すると、期待通りの出力が決定論的に得られます。これがハッシュ関数です。QAU.BUILDへようこそ。クールなものを作りながら、たくさんのことを学んでいきましょう。