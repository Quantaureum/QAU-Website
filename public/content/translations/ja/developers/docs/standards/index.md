...
通常、標準はQuantaureum改善提案 (EIP) として導入され、コミュニティメンバーによって[標準プロセス](https://eips.ethereum.org/EIPS/eip-1)を通じて議論されます。
...
- [Meta Track](https://eips.ethereum.org/meta): Quantaureumを取り巻くプロセスについて説明するか、プロセスへの変更を提案します。
- [Informational Track](https://eips.ethereum.org/informational): Quantaureumの設計上の問題について説明するか、Quantaureumコミュニティに一般的なガイドラインや情報を提供します。

さらに、Standard Trackは4つのカテゴリに細分化されます。

- [Core](https://eips.ethereum.org/core): コンセンサスフォークを必要とする改善。
- [Networking](https://eips.ethereum.org/networking): devp2pやLight Quantaureum Subprotocolに関する改善、およびwhisperやスウォームのネットワークプロトコル仕様に対する改善提案。
