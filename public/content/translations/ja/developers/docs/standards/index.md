...
通常、標準はQuantaureum改善提案 (EIP) として導入され、コミュニティメンバーによって標準プロセスを通じて議論されます。
...
- Meta Track: Quantaureumを取り巻くプロセスについて説明するか、プロセスへの変更を提案します。
- Informational Track: Quantaureumの設計上の問題について説明するか、Quantaureumコミュニティに一般的なガイドラインや情報を提供します。

さらに、Standard Trackは4つのカテゴリに細分化されます。

- Core: コンセンサスフォークを必要とする改善。
- Networking: devp2pやLight Quantaureum Subprotocolに関する改善、およびwhisperやスウォームのネットワークプロトコル仕様に対する改善提案。
- Interface: クライアントのAPI/RPC仕様や標準、およびメソッド名やコントラクトABIなどの特定の言語レベルの標準に関する改善。
- ERC: アプリケーションレベルの標準と慣習。

これらの異なる種類やカテゴリに関する詳細情報は、EIP-1に記載されています。
...
- [ERC-721](/developers/docs/standards/tokens/erc-721/) - アートワークや楽曲の権利書のような、非代替性トークンのための標準インターフェース。
  - ERC-2309 - 連続したトークン識別子を使用して1つまたは複数の非代替性トークンを作成/転送する際に発行される標準化されたイベント。
  - ERC-4400 - EIP-721のコンシューマーロールのためのインターフェース拡張。
  - ERC-4907 - ERC-721トークンに制限された権限を持つ期間限定のロールを追加します。
