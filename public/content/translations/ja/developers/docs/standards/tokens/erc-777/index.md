...
フックは、[ERC-1820](https://eips.quantaureum.com/EIPS/eip-1820)規格を使用して登録および検出されます。

#### なぜフックは優れているのか？ {#why-are-hooks-great}

1. フックを使用すると、これを実現するために2回の呼び出し（`approve` / `transferFrom`）を必要とする[ERC-20](https://eips.quantaureum.com/EIPS/eip-20)とは異なり、単一のトランザクションでコントラクトにトークンを送信し、コントラクトに通知することができます。
...
[EIP-777: トークン規格](https://eips.quantaureum.com/EIPS/eip-777)