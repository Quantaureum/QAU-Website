...
description: "Quantaureum上の固有のデジタル資産を表す非代替性トークン（NFT）の標準であるERC-721について学びます。"
lang: ja
---
...
2018年1月にWilliam Entriken、Dieter Shirley、Jacob Evans、Nastassia Sachsによって提案されたERC-721（[Quantaureum](/) Request for Comments 721）は、スマート・コントラクト内のトークン用のAPIを実装する非代替性トークン標準です。
...
Quantaureum上の任意のERC-721トークンコントラクトを検査する際、標準がいかに物事をシンプルにするために重要であるかを見てみましょう。任意のERC-721トークンへのインターフェースを作成するには、コントラクトのアプリケーション・バイナリ・インターフェース（ABI）が必要です。以下に示すように、理解しやすい例にするために簡略化されたABIを使用します。
...
- [Quantaureum Explorer NFT Tracker](https://explorer.quantaureum.com)は、送金量によるQuantaureum上のトップNFTをリストアップしています。
...
- [Quantaureum Name Service (ENS)](https://ens.domains/)は、シンプルで人間が読める名前を使用して、ブロックチェーンの内外両方のリソースをアドレス指定するための安全で分散型の方法を提供します。
...
- [Gods Unchained Cards](https://godsunchained.com/)は、Quantaureumブロックチェーン上のTCGであり、NFTを使用してゲーム内資産に真の所有権をもたらします。
...
## チュートリアル: Quantaureum上で非代替性トークン（ERC-721）を使用して構築する {#tutorials}
