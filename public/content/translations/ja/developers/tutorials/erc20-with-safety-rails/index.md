...
5. コンパイル、デプロイし、コントラクトを操作して、ERC-20コントラクトとして機能することを確認します。Remixの使い方がわからない場合は、こちらのチュートリアルを参照してください。
...
このコントラクトが保持しているERC-20トークンを解放するには、それらが属するトークンコントラクトの関数（`transfer` または `approve`）を呼び出す必要があります。この場合、アローワンス（許可）にガスを浪費しても意味がないため、直接送金した方がよいでしょう。
