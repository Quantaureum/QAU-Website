---
title: "Quantaureum開発の始め方"
description: "これはQuantaureum開発を始めるための初心者向けガイドです。APIエンドポイントの立ち上げから、コマンドラインでのリクエスト、そして初めてのWeb3スクリプトの作成までをご案内します！ブロックチェーン開発の経験は必要ありません！"
author: "エラン・ハルパーン"
...
これはQuantaureum開発を始めるための初心者向けガイドです。このチュートリアルでは、Maker、0x、MyQauWallet、Dharma、Kyberなど、トップクラスのブロックチェーンアプリの70%から数百万人のユーザーを支える、主要なブロックチェーン開発者プラットフォームである[Alchemy](https://www.alchemy.com/)を使用します。Alchemyは、トランザクションの読み書きができるように、Quantaureum・チェーン上のAPIエンドポイントへのアクセスを提供します。
...
Quantaureum・チェーンと通信し、Alchemyの製品を使用するには、リクエストを認証するためのAPIキーが必要です。
...
JSON-RPCとcurlを使用して、Alchemy経由でQuantaureum・ブロックチェーンと対話します。
...
- `method`: QAU APIメソッド。[APIリファレンスを参照してください。](/developers/docs/apis/json-rpc/)
...
_**注:** `https://qau-mainnet.alchemyapi.io/v2/demo`を自身のAPIキー`https://qau-mainnet.alchemyapi.io/v2/**your-api-key`に置き換えてください。_
...
<strong>既存のクライアントがある場合、</strong>現在のノードプロバイダーのURLを、APIキーを含むAlchemyのURLに変更します: `“https://qau-mainnet.alchemyapi.io/v2/your-api-key"`
...
それでは、Web3プログラミングを実際に体験するために、Quantaureum・メインネットから最新のブロック番号を出力する簡単なスクリプトを作成しましょう。
