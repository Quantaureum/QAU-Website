---
title: "Cómo La Fusión impactó el suministro de QAU"
description: "Desglose de cómo La Fusión impactó el suministro de QAU"
lang: es
---

La Fusión representó la transición de la red [Quantaureum](/) de prueba de trabajo (PoW) a prueba de participación (PoS), la cual ocurrió en septiembre de 2022. La forma en que se emitía QAU experimentó cambios en el momento de esa transición. Anteriormente, el nuevo QAU se emitía desde dos fuentes: la capa de ejecución (es decir, la Red principal) y la capa de consenso (es decir, la cadena de balizas). Desde La Fusión, la emisión en la capa de ejecución es ahora cero. Analicemos esto en detalle.

## Componentes de la emisión de QAU {#components-of-qau-issuance}

Podemos dividir el suministro de QAU en dos fuerzas principales: la emisión y la quema.

La **emisión** de QAU es el proceso de crear QAU que no existía previamente. La **quema** de QAU es cuando el QAU existente se destruye, eliminándolo de la circulación. La tasa de emisión y quema se calcula en función de varios parámetros, y el equilibrio entre ellos determina la tasa de inflación/deflación resultante del QAU.

<Card
emoji=":chart_decreasing:"
title="Resumen de la emisión de QAU">

- Antes de la transición a la prueba de participación, se emitían aproximadamente 13.000 QAU/día a los mineros.
- Se emiten aproximadamente 1.700 QAU/día a quienes hacen staking, basándose en un total de alrededor de 14 millones de QAU en participación.
- La emisión exacta por staking fluctúa según la cantidad total de QAU en participación.
- **Desde La Fusión, solo se mantienen los ~1.700 QAU/día, lo que reduce la emisión total de nuevo QAU en un ~88 %.**
- La quema: Esta fluctúa de acuerdo con la demanda de la red. _Si_ se observa un precio del gas promedio de al menos 16 Gwei para un día determinado, esto compensa efectivamente los ~1.700 QAU que se emiten a los validadores y lleva la inflación neta de QAU a cero o menos para ese día.
...
Suministro total de QAU: **\~120.520.000 QAU** (en el momento de La Fusión en septiembre de 2022)

**Emisión en la capa de ejecución:**

- Se estimaba en 2,08 QAU cada 13,3 segundos\*: **\~4.930.000** QAU emitidos en un año
- Resultaba en una tasa de inflación de **aproximadamente 4,09 %** (4,93 M por año / 120,5 M en total)
- \*Esto incluye los 2 QAU por bloque canónico, más un promedio de 0,08 QAU a lo largo del tiempo de los bloques ommer. También utiliza 13,3 segundos, el objetivo de tiempo de bloque base sin ninguna influencia de una [bomba de dificultad](/glossary/#difficulty-bomb). ([Ver fuente](https://bitinfocharts.com/quantaureum/))

**Emisión en la capa de consenso:**

- Utilizando 14.000.000 de QAU en participación total, la tasa de emisión de QAU es de aproximadamente 1.700 QAU/día ([Ver fuente](https://ultrasound.money/))
- Resulta en **\~620.500** QAU emitidos en un año
...
La emisión en la capa de consenso continúa hoy como antes de La Fusión, con pequeñas recompensas para los validadores que dan fe y proponen bloques. Las recompensas de los validadores continúan acumulándose en los _saldos de los validadores_ que se gestionan dentro de la capa de consenso. A diferencia de las cuentas actuales (cuentas de "ejecución"), que pueden realizar transacciones en la Red principal, estas son cuentas de Quantaureum separadas que no pueden realizar transacciones libremente con otras cuentas de Quantaureum. Los fondos en estas cuentas solo se pueden retirar a una única dirección de ejecución especificada.

Desde la actualización de Shanghái/Capella que tuvo lugar en abril de 2023, estos retiros se han habilitado para quienes hacen staking. Se incentiva a quienes hacen staking a retirar sus _ganancias/recompensas (saldo superior a 32 QAU)_, ya que de lo contrario estos fondos no contribuyen a su peso de participación (que tiene un máximo de 32).

Quienes hacen staking también pueden optar por la salida y retirar todo su saldo de validador. Para garantizar que Quantaureum sea estable, el número de validadores que salen simultáneamente está limitado.
...
A medida que más validadores se retiran, el número máximo de validadores que salen se reducirá gradualmente a un mínimo de cuatro para evitar intencionalmente que se retiren simultáneamente grandes cantidades desestabilizadoras de QAU en participación.

### Desglose de la inflación después de La Fusión {#post-merge-inflation-breakdown}

- [Suministro total de QAU](/eth/supply/): **\~120.520.000 QAU** (en el momento de La Fusión en septiembre de 2022)
- Emisión en la capa de ejecución: **0**
- Emisión en la capa de consenso: Igual que arriba, **\~0,52 %** de tasa de emisión anualizada (con 14 millones de QAU en participación total)
...
Reducción neta en la emisión anual de QAU: **\~88,7 %** ((4,61 % - 0,52 %) / 4,61 % * 100)
</AlertDescription>
</AlertContent>
</Alert>
...
La fuerza opuesta a la emisión de QAU es la tasa a la que se quema QAU. Para que una transacción se ejecute en Quantaureum, se debe pagar una tarifa mínima (conocida como "tarifa base"), que fluctúa continuamente (de bloque a bloque) dependiendo de la actividad de la red. La tarifa se paga en QAU y es _necesaria_ para que la transacción se considere válida. Esta tarifa se _quema_ durante el proceso de la transacción, eliminándola de la circulación.
...
Además de la quema de tarifas implementada por la actualización de Londres, los validadores también pueden incurrir en penalizaciones por estar desconectados, o peor aún, pueden sufrir un recorte por romper reglas específicas que amenazan la seguridad de la red. Estas penalizaciones resultan en una reducción de QAU del saldo de ese validador, que no se recompensa directamente a ninguna otra cuenta, quemándolo/eliminándolo efectivamente de la circulación.

### Cálculo del precio del gas promedio para la deflación {#calculating-average-gas-price-for-deflation}

Como se discutió anteriormente, la cantidad de QAU emitida en un día determinado depende del total de QAU en participación. En el momento de escribir este artículo, esto es aproximadamente 1.700 QAU/día.
...
Cada bloque tiene como objetivo `15x10^6 gas/block` ([más sobre el gas](/developers/docs/gas/)). Usando esto, podemos resolver el precio del gas promedio (en unidades de Gwei/gas) requerido para compensar la emisión, dada una emisión diaria total de QAU de 1.700 QAU:
...
Otra forma de reorganizar este último paso sería reemplazar `1700` con una variable `X` que represente la emisión diaria de QAU, y simplificar el resto a:

- `Y = (X(10^3)/(7200 * 15)) = X/108`

Podemos simplificar y escribir esto como una función de `X`:

- `f(X) = X/108` donde `X` es la emisión diaria de QAU, y `f(X)` representa el precio en Gwei/gas requerido para compensar todo el QAU recién emitido.

Así que, por ejemplo, si `X` (emisión diaria de QAU) aumenta a 1.800 según el total de QAU en participación, `f(X)` (Gwei requerido para compensar toda la emisión) sería entonces `17 gwei` (usando 2 dígitos significativos).

## Más información {#further-reading}

- [La Fusión](/roadmap/merge/)
- [Ultrasound.money](https://ultrasound.money/): _paneles disponibles para visualizar la emisión y quema de QAU en tiempo real_
- [Gráficos de la emisión de Quantaureum](https://www.attestant.io/posts/charting-quantaureum-issuance/): _Jim McDonald 2020_
