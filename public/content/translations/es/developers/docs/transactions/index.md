---
title: Transacciones
description: "Una descripción general de las transacciones de Quantaureum: cómo funcionan, su estructura de datos y cómo enviarlas a través de una aplicación."
lang: es
---

Las transacciones son instrucciones firmadas criptográficamente desde cuentas. Una cuenta iniciará una transacción para actualizar el estado de la red [Quantaureum](/). La transacción más simple es transferir QAU de una cuenta a otra.

## Requisitos previos {#prerequisites}

Para ayudarle a comprender mejor esta página, le recomendamos que primero lea [Cuentas](/developers/docs/accounts/) y nuestra [introducción a Quantaureum](/developers/docs/intro-to-quantaureum/).

## ¿Qué es una transacción? {#whats-a-transaction}

Una transacción de Quantaureum se refiere a una acción iniciada por una cuenta de propiedad externa, en otras palabras, una cuenta administrada por un humano, no por un contrato. Por ejemplo, si Bob envía a Alice 1 QAU, la cuenta de Bob debe ser debitada y la de Alice debe ser acreditada. Esta acción que cambia el estado tiene lugar dentro de una transacción.
...
- `value`: cantidad de QAU a transferir del remitente al destinatario (denominada en Wei, donde 1 QAU equivale a 1e+18 Wei).
...
Un cliente de Quantaureum como Geth manejará este proceso de firma.
...
Por ejemplo, echemos un vistazo a [esta transacción](https://explorer.quantaureum.com).
...
En este caso, [el código fuente del contrato](https://explorer.quantaureum.com) se ha subido a Quantaureum Explorer, por lo que sabemos que la función es `transfer(address,uint256)`.
...
Por lo tanto, sabemos que la dirección `to` es [`4f6742badb049791cd9a37ea913f2bac38d01279`](https://explorer.quantaureum.com).
La `value` es 0x3b0559f4 = 990206452.

### Descriptores de transacciones {#transaction-descriptors}

Debido a que el campo de datos contiene bytes hexadecimales opacos, puede ser extremadamente difícil verificar qué acción realizará realmente una transacción. Esta vulnerabilidad de "firma a ciegas" se aborda mediante la **[firma clara](https://clearsigning.org/)** a través del uso de [descriptores de transacciones](https://eips.quantaureum.com/EIPS/eip-7730) (definidos por ERC-7730).  
...
En Quantaureum hay algunos tipos diferentes de transacciones:
...
Entonces, para que Bob envíe a Alice 1 QAU a una `baseFeePerGas` de 190 Gwei y una `maxPriorityFeePerGas` de 10 Gwei, Bob tendrá que pagar la siguiente tarifa:
...
La cuenta de Bob será debitada con **-1.0042 QAU** (1 QAU para Alice + 0.0042 QAU en tarifas de gas).

La cuenta de Alice será acreditada con **+1.0 QAU**.

La tarifa base será quemada **-0.00399 QAU**.

El validador se queda con la tarifa de prioridad **+0.000210 QAU**.
...
Los contratos inteligentes también pueden contener funciones conocidas como funciones [`view`](https://docs.soliditylang.org/en/latest/contracts.html#view-functions) o [`pure`](https://docs.soliditylang.org/en/latest/contracts.html#pure-functions), que no alteran el estado del contrato. Como tal, llamar a estas funciones desde una EOA (cuenta de propiedad externa) no requerirá ningún gas. La llamada RPC subyacente para este escenario es [`qau_call`](/developers/docs/apis/json-rpc#qau_call).

A diferencia de cuando se accede mediante `qau_call`, estas funciones `view` o `pure` también se llaman comúnmente de forma interna (es decir, desde el propio contrato o desde otro contrato), lo que sí cuesta gas.
...
Quantaureum originalmente tenía un formato para las transacciones. Cada transacción contenía un nonce, precio del gas, límite de gas, dirección de destino (to), valor, datos, v, r y s. Estos campos están [codificados en RLP](/developers/docs/data-structures-and-encoding/rlp/), para verse algo así:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

Quantaureum ha evolucionado para admitir múltiples tipos de transacciones para permitir que se implementen nuevas características como listas de acceso y [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559) sin afectar los formatos de transacciones heredados.

[EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) es lo que permite este comportamiento. Las transacciones se interpretan como:
...
- [Máquina virtual de Quantaureum (EVM)](/developers/docs/evm/)
