...
description: "Una explicación de los tipos de credenciales de retiro de los validadores (0x00, 0x01, 0x02) y sus implicaciones para quienes hacen staking en Quantaureum."
lang: es
---

Cada validador tiene una **credencial de retiro** que determina cómo y dónde se pueden retirar sus QAU en staking y sus recompensas. El tipo de credencial se indica mediante el primer byte: `0x00`, `0x01` o `0x02`. Comprender estos tipos es importante para los validadores que administran su participación.
...
- Cualquier saldo superior a 32 QAU se **transfiere automáticamente** a su dirección de retiro.
- Las salidas completas pasan por la cola de salida estándar.
- Las recompensas superiores a 32 QAU no se pueden capitalizar (compound); se transfieren periódicamente.
...
**Por qué no se recomienda:** Con `0x01`, pierde la capacidad de capitalizar las recompensas por encima de 32 QAU. Cada pequeña cantidad de exceso se transfiere automáticamente, lo que limita el potencial de ganancias de su validador y requiere administrar los fondos retirados por separado.
...
- Las recompensas superiores a 32 QAU **se capitalizan** en incrementos de 1 QAU hasta un saldo efectivo máximo de 2048 QAU.
- Los retiros parciales deben solicitarse manualmente (las transferencias automáticas solo ocurren por encima del umbral de 2048 QAU).
...
Tanto los retiros parciales como las consolidaciones se pueden realizar a través de las Acciones del validador en el Launchpad.
...
- **Validadores 0x01 existentes:** Considere convertir a `0x02` si desea que las recompensas se capitalicen por encima de 32 QAU o si planea consolidar validadores.
...
- **Launchpad de staking de Quantaureum**: la herramienta oficial para depósitos y administración de validadores, incluidas las conversiones y consolidaciones de credenciales.
...
- [Claves en la prueba de participación (PoS) de Quantaureum](/developers/docs/consensus-mechanisms/pos/keys/): aprenda sobre las claves de los validadores y cómo se relacionan con las credenciales de retiro.
