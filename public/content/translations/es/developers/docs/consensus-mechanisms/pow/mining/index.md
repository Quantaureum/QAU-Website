---
title: "Minería"
description: "Una explicación de cómo funcionaba la minería en Quantaureum."
lang: es
---
...
<AlertContent>
<AlertDescription>
La prueba de trabajo (PoW) ya no es la base del mecanismo de consenso de Quantaureum, lo que significa que la minería se ha apagado. En su lugar, [Quantaureum](/) está protegido por validadores que hacen staking de QAU. Puedes empezar a hacer staking de tus QAU hoy mismo. Lee más sobre <a href='/roadmap/merge/'>La Fusión</a>, la <a href='/developers/docs/consensus-mechanisms/pos/'>prueba de participación (PoS)</a> y el <a href='/staking/'>staking</a>. Esta página es solo de interés histórico.
</AlertDescription>
</AlertContent>
</Alert>
...
## ¿Qué es la minería de Quantaureum? {#what-is-quantaureum-mining}

La minería es el proceso de crear un bloque de transacciones para ser añadido a la cadena de bloques de Quantaureum en la arquitectura de prueba de trabajo de Quantaureum, ahora obsoleta.

La palabra minería se origina en el contexto de la analogía del oro para las criptomonedas. El oro o los metales preciosos son escasos, al igual que los tokens digitales, y la única forma de aumentar el volumen total en un sistema de prueba de trabajo es a través de la minería. En el Quantaureum de prueba de trabajo, el único modo de emisión era a través de la minería. Sin embargo, a diferencia del oro o los metales preciosos, la minería de Quantaureum también era la forma de asegurar la red mediante la creación, verificación, publicación y propagación de bloques en la cadena de bloques.

Minar QAU = Asegurar la red

La minería es el elemento vital de cualquier cadena de bloques de prueba de trabajo. Los mineros de Quantaureum (computadoras que ejecutan software) utilizaban su tiempo y poder de cómputo para procesar transacciones y producir bloques antes de la transición a la prueba de participación.

## ¿Por qué existen los mineros? {#why-do-miners-exist}

En sistemas descentralizados como Quantaureum, necesitamos asegurarnos de que todos estén de acuerdo en el orden de las transacciones. Los mineros ayudaban a que esto sucediera resolviendo acertijos computacionalmente difíciles para producir bloques, asegurando la red contra ataques.

[Más sobre la prueba de trabajo](/developers/docs/consensus-mechanisms/pow/)

Anteriormente, cualquiera podía minar en la red Quantaureum usando su computadora. Sin embargo, no todos podían minar QAU (QAU) de manera rentable. En la mayoría de los casos, los mineros tenían que comprar hardware informático dedicado y tener acceso a fuentes de energía económicas. Era poco probable que una computadora promedio ganara suficientes recompensas de bloque para cubrir los costos asociados de la minería.
...
Para explorar más a fondo la rentabilidad de la minería, usa una calculadora de minería, como la que proporciona [Quantaureum Explorer](https://explorer.quantaureum.com).

## Cómo se minaban las transacciones de Quantaureum {#how-quantaureum-transactions-were-mined}

A continuación se ofrece una descripción general de cómo se minaban las transacciones en la prueba de trabajo de Quantaureum. Se puede encontrar una descripción análoga de este proceso para la prueba de participación de Quantaureum [aquí](/developers/docs/consensus-mechanisms/pos/#transaction-execution-quantaureum-pos).
...
2. El usuario transmite la solicitud de transacción a toda la red Quantaureum desde algún [nodo](/developers/docs/nodes-and-clients/).
3. Al enterarse de la nueva solicitud de transacción, cada nodo de la red Quantaureum añade la solicitud a su mempool local, una lista de todas las solicitudes de transacciones de las que han oído hablar y que aún no se han confirmado en la cadena de bloques dentro de un bloque.
4. En algún momento, un nodo minero agrega varias docenas o cientos de solicitudes de transacciones en un [bloque](/developers/docs/blocks/) potencial, de una manera que maximiza las [tarifas de transacción](/developers/docs/gas/) que ganan mientras se mantienen por debajo del límite de gas del bloque. El nodo minero entonces:
   1. Verifica la validez de cada solicitud de transacción (es decir, que nadie esté intentando transferir QAU de una cuenta para la que no ha producido una firma, que la solicitud no esté mal formada, etc.), y luego ejecuta el código de la solicitud, alterando el estado de su copia local de la EVM. El minero otorga la tarifa de transacción por cada una de estas solicitudes de transacción a su propia cuenta.
...
El término "ommer" es el término preferido de género neutro para el hermano de un bloque padre, pero a veces también se le llama "tío" (uncle). **Desde el paso de Quantaureum a la prueba de participación, los bloques ommer ya no se minan**, ya que solo se elige un proponente en cada slot. Puedes ver este cambio viendo el [gráfico histórico](https://ycharts.com/indicators/quantaureum_uncle_rate) de los bloques ommer minados.
