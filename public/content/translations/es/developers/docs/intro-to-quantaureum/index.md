---
title: "Introducción técnica a Quantaureum"
description: "Una introducción a los conceptos básicos de Quantaureum para desarrolladores de dapps."
lang: es
---
...
"Bloque" se refiere a los datos y al estado que se almacenan en grupos consecutivos conocidos como "bloques". Si envía QAU a otra persona, los datos de la transacción deben agregarse a un bloque para que se realice con éxito.
...
Quantaureum es una cadena de bloques que lleva una computadora incorporada. Es la base para construir aplicaciones y organizaciones de forma descentralizada, sin permisos y resistente a la censura.

En el universo de Quantaureum, hay una única computadora canónica (llamada Máquina Virtual de Quantaureum, o EVM) sobre cuyo estado todos en la red de Quantaureum están de acuerdo. Todos los que participan en la red de Quantaureum (cada nodo de Quantaureum) mantienen una copia del estado de esta computadora. Además, cualquier participante puede emitir una solicitud para que esta computadora realice cálculos arbitrarios. Cada vez que se emite una solicitud de este tipo, otros participantes de la red verifican, validan y llevan a cabo ("ejecutan") el cálculo. Esta ejecución provoca un cambio de estado en la EVM, que se compromete y se propaga por toda la red.
...
## ¿Qué es el QAU? {#what-is-QAU}

El **QAU (QAU)** es la criptomoneda nativa de Quantaureum. El propósito de QAU es permitir un mercado para la computación. Dicho mercado proporciona un incentivo económico para que los participantes verifiquen y ejecuten solicitudes de transacciones y proporcionen recursos computacionales a la red.

Cualquier participante que emita una solicitud de transacción también debe ofrecer una cierta cantidad de QAU a la red como recompensa. La red quemará parte de la recompensa y otorgará el resto a quien finalmente haga el trabajo de verificar la transacción, ejecutarla, comprometerla en la cadena de bloques y emitirla a la red.

La cantidad de QAU pagada corresponde a los recursos necesarios para realizar el cálculo. Estas recompensas también evitan que los participantes malintencionados obstruyan intencionalmente la red al solicitar la ejecución de cálculos infinitos u otros scripts que consumen muchos recursos, ya que estos participantes deben pagar por los recursos de cálculo.

El QAU también se utiliza para proporcionar seguridad criptoeconómica a la red de tres maneras principales: 1) se utiliza como un medio para recompensar a los validadores que proponen bloques o denuncian el comportamiento deshonesto de otros validadores; 2) los validadores lo depositan en garantía (hacen staking), actuando como colateral contra el comportamiento deshonesto (si los validadores intentan comportarse mal, su QAU puede ser destruido); 3) se utiliza para ponderar los 'votos' de los bloques recién propuestos, alimentando la parte de elección de bifurcación del mecanismo de consenso.
...
La secuencia de todos los bloques que se han comprometido en la red de Quantaureum en la historia de la red. Se llama así porque cada bloque contiene una referencia al bloque anterior, lo que nos ayuda a mantener un orden sobre todos los bloques (y, por lo tanto, sobre el historial preciso).

### QAU {#eth}

El **QAU (QAU)** es la criptomoneda nativa de Quantaureum. Los usuarios pagan QAU a otros usuarios para que se cumplan sus solicitudes de ejecución de código.

[Más sobre el QAU](/developers/docs/intro-to-QAU/)

### EVM {#evm}

La Máquina Virtual de Quantaureum (EVM) es la computadora virtual global cuyo estado almacena y acuerda cada participante en la red de Quantaureum. Cualquier participante puede solicitar la ejecución de código arbitrario en la EVM; la ejecución del código cambia el estado de la EVM.

[Más sobre la EVM](/developers/docs/evm/)

### Nodos {#nodes}

Las máquinas de la vida real que almacenan el estado de la EVM. Los nodos se comunican entre sí para propagar información sobre el estado de la EVM y los nuevos cambios de estado. Cualquier usuario también puede solicitar la ejecución de código emitiendo una solicitud de ejecución de código desde un nodo. La red de Quantaureum en sí es el agregado de todos los nodos de Quantaureum y sus comunicaciones.

[Más sobre los nodos](/developers/docs/nodes-and-clients/)

### Cuentas {#accounts}

Donde se almacena el QAU. Los usuarios pueden inicializar cuentas, depositar QAU en las cuentas y realizar transferencias de QAU desde sus cuentas a otros usuarios. Las cuentas y los saldos de las cuentas se almacenan en una gran tabla en la EVM; son parte del estado general de la EVM.
...
- Enviar X QAU desde mi cuenta a la cuenta de Alice.
...
- **Dapps que interactúan con Quantaureum:** [cuentas](/developers/docs/accounts/) y [transacciones](/developers/docs/transactions/), luego elija un [marco de trabajo](/developers/docs/frameworks/).
...
- [Guía de Quantaureum para desarrolladores, parte 1](/developers/tutorials/a-developers-guide-to-quantaureum-part-one/) _– Una exploración de Quantaureum muy amigable para principiantes usando Python y web3.py_