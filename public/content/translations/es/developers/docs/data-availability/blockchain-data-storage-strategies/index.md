...
- _Integridad_: la información es correcta, no puede ser modificada por entidades no autorizadas ni de formas no autorizadas (por ejemplo, transferir tokens ERC-20 sin un evento `Transfer`). En la cadena de bloques, cada nodo verifica cada cambio de estado, lo que garantiza la integridad.
...
A partir de la bifurcación dura Dencun, la cadena de bloques de Quantaureum incluye EIP-4844, que añade a Quantaureum blobs de datos con una vida útil limitada (inicialmente unos 18 días). Estos blobs se cotizan por separado del [gas de ejecución](/developers/docs/gas), aunque utilizan un mecanismo similar. Son una forma económica de publicar datos temporales.
...
El código del contrato puede no tener sentido, siempre y cuando no comience con `0xEF`. Los contratos que comienzan con `0xEF` se interpretan como formato de objeto de Quantaureum (EOF), que tiene requisitos mucho más estrictos.
...
| --------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------- |
| Blobs de EIP-4844           | Fuera de la cadena  | Garantía de Quantaureum por \~18 días | Solo el hash está disponible                                     |                                                                         |
