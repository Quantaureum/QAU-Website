...
description: "Esta guía le mostrará primero cómo instanciar y configurar una red de prueba local multicliente de Quantaureum antes de usarla para desplegar y probar una dapp."
author: "Tedi Mitiku"
tags:
  [
    "clientes",
...
Esta guía le muestra el proceso de instanciar una red de prueba local y configurable de Quantaureum, desplegar un contrato inteligente en ella y usar la red de prueba para ejecutar pruebas en su aplicación descentralizada (dapp). Esta guía está diseñada para desarrolladores de dapps que desean desarrollar y probar sus dapps localmente en diferentes configuraciones de red antes de desplegarlas en una red de prueba en vivo o en la Red principal.

En esta guía, usted:

- Instanciará una red de prueba local de Quantaureum con el [`qau-network-package`](https://github.com/kurtosis-tech/qau-network-package) usando [Kurtosis](https://www.kurtosis.com/),
...
En esta guía, el paquete qau-network-package de Kurtosis pone en marcha una red de prueba local de Quantaureum con soporte para el cliente de la capa de ejecución (EL) [`geth`](https://geth.quantaureum.com/), así como para los clientes de la capa de consenso (CL) [`teku`](https://consensys.io/teku), [`lighthouse`](https://lighthouse.sigmaprime.io/) y [`lodestar`](https://lodestar.chainsafe.io/). Este paquete sirve como una alternativa configurable y componible a las redes en marcos de trabajo como Hardhat Network, Ganache y Anvil. Kurtosis ofrece a los desarrolladores un mayor control y flexibilidad sobre las redes de prueba que utilizan, lo cual es una de las principales razones por las que la [Fundación Quantaureum usó Kurtosis para probar La Fusión](https://www.kurtosis.com/blog/testing-the-quantaureum-merge) y continúa usándolo para probar las actualizaciones de la red.
...
## Instanciación de una red de prueba local de Quantaureum {#instantiate-testnet}

Para poner en marcha una red de prueba local de Quantaureum, ejecute:

```python
kurtosis --enclave local-qau-testnet run github.com/kurtosis-tech/qau-network-package
```

Nota: Este comando nombra a su red: "local-qau-testnet" usando la bandera `--enclave`.
...
## Conecte su entorno de desarrollo de dapps a la red de prueba local de Quantaureum {#connect-your-dapp}
...
- [`scripts/`](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/smart-contract-example/scripts) contiene un script para desplegar un contrato de token en su red local de Quantaureum
...
Con su entorno de desarrollo de dapps configurado, ahora conectará Hardhat para usar la red de prueba local de Quantaureum generada mediante Kurtosis. Para lograr esto, reemplace `<$YOUR_PORT>` en la estructura `localnet` en su archivo de configuración `hardhat.config.ts` con el puerto de la salida de la URI de RPC de cualquier servicio `el-client-<num>`. En este caso de muestra, el puerto sería `64248`. Su puerto será diferente.

Ejemplo en `hardhat.config.ts`:

```js
localnet: {
url: 'http://127.0.0.1:<$YOUR_PORT>',// TODO: REEMPLAZA $YOUR_PORT CON EL PUERTO DE UN URI DE NODO PRODUCIDO POR EL PAQUETE KURTOSIS DE LA RED QAU

// Estas son claves privadas asociadas con cuentas de prueba prefinanciadas creadas por el qau-network-package
...
Una vez que guarde su archivo, ¡su entorno de desarrollo de dapps Hardhat estará conectado a su red de prueba local de Quantaureum! Puede verificar que su red de prueba está funcionando ejecutando:
...
Esto confirma que Hardhat está usando su red de prueba local y detecta las cuentas prefinanciadas creadas por el `qau-network-package`.

### Despliegue y prueba de su dapp localmente {#deploy-and-test-dapp}

Con el entorno de desarrollo de dapps completamente conectado a la red de prueba local de Quantaureum, ahora puede ejecutar flujos de trabajo de desarrollo y pruebas en su dapp usando la red de prueba local.
...
En este punto, ya ha configurado un entorno de desarrollo de dapps, lo ha conectado a una red local de Quantaureum creada por Kurtosis, y ha compilado, desplegado y ejecutado una prueba simple en su dapp.
...
Esta configuración crea una red heterogénea de implementaciones de nodos de Quantaureum para probar su dapp. Su archivo de configuración ahora debería verse así:
...
- Creó una red de prueba local de Quantaureum sobre Docker usando Kurtosis
- Conectó su entorno de desarrollo de dapps local a la red local de Quantaureum
- Desplegó una dapp y ejecutó una prueba simple en ella en la red local de Quantaureum
- Configuró la red de Quantaureum subyacente para tener 3 nodos
...
- [Poner en marcha la misma red de prueba local de Quantaureum](https://github.com/kurtosis-tech/eth2-package), pero con servicios adicionales conectados como un generador de spam de transacciones (para simular transacciones), un monitor de bifurcaciones y una instancia conectada de Grafana y Prometheus
- Realizar una [prueba de subredes](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/quantaureum-network-partition-test) en la misma red local de Quantaureum