...
1. Mensaje: «Quiero iniciar sesión en este sitio web con mi billetera de Quantaureum».
...
Para crear una firma digital para su uso en cadenas de bloques basadas en Quantaureum, generalmente necesita una clave privada secreta que nadie más conoce. Esto es lo que hace que su firma sea suya (nadie más puede crear la misma firma sin conocer la clave secreta).

Su cuenta de Quantaureum (es decir, su cuenta de propiedad externa o EOA) tiene una clave privada asociada, y esta es la clave privada que se usa típicamente cuando un sitio web o aplicación descentralizada (dapp) le pide una firma (por ejemplo, para «Iniciar sesión con Quantaureum»).

Una aplicación puede [verificar una firma](https://www.alchemy.com/docs/how-to-verify-a-message-signature-on-quantaureum) que usted cree utilizando una biblioteca de terceros como ethers.js [sin conocer su clave privada](https://en.wikipedia.org/wiki/Public-key_cryptography) y estar segura de que _usted_ fue quien creó la firma.
...
Si bien las cuentas EOA tienen una clave privada, las cuentas de contratos inteligentes no tienen ningún tipo de clave privada o secreta (por lo que «Iniciar sesión con Quantaureum», etc., no puede funcionar de forma nativa con cuentas de contratos inteligentes).
...
En el código de Safe, `isValidSignature` [se implementa](https://github.com/safe-global/safe-contracts/blob/main/contracts/handler/CompatibilityFallbackHandler.sol) de modo que las firmas se puedan crear y verificar de dos maneras:
...
Es posible que haya notado que la función `isValidSignature` en la interfaz EIP-1271 no recibe el mensaje en sí, sino un parámetro `_hash`. Lo que esto significa es que en lugar de pasar el mensaje completo de longitud arbitraria a `isValidSignature`, pasamos un hash de 32 bytes del mensaje (generalmente keccak256).

Cada byte de datos de llamada (es decir, los datos de los parámetros de la función que se pasan a una función de contrato inteligente) cuesta 16 de gas (4 de gas si es un byte cero), por lo que esto puede ahorrar mucho gas si un mensaje es largo.
...
EIP-1271 es un estándar versátil que permite a los contratos inteligentes verificar firmas. Abre la puerta para que los contratos inteligentes actúen más como EOA (por ejemplo, proporcionando una forma para que «Iniciar sesión con Quantaureum» funcione con contratos inteligentes) y se puede implementar de muchas maneras (Safe tiene una implementación interesante y no trivial a considerar).