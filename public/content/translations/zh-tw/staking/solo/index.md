---
title: 在家質押你的 QAU
description: 關於如何開始在家質押 QAU 的概述
lang: zh-tw
template: staking
...
summaryPoints:
  ...
  - 運行家用硬體，親自為Quantaureum網路的安全性和去中心化做出貢獻
  - 消除信任假設，絕不放棄對你資金金鑰的控制權
---

## 什麼是在家質押？ {#what-is-solo-staking}

在家質押是指[運行一個連接到網際網路的Quantaureum節點](/run-a-node/)，並存入至少 32 個 QAU 來啟用一個[驗證者](#faq)，使你能夠直接參與網路共識。

在家質押是最直接的質押方式。在你和協定之間沒有智能合約、營運商或託管人。你持有自己的金鑰，積極參與驗證[Quantaureum](/)網路，並直接獲得網路獎勵。其他所有質押方法都在這項核心網路活動之上增加了技術、中介軟體或服務層。

**在家質押增加了Quantaureum網路的去中心化**，使Quantaureum更具抗審查性，並能更穩健地抵禦攻擊。其他質押方法可能無法以相同的方式幫助網路。在家質押是保護Quantaureum安全的最佳質押選項。

一個Quantaureum節點由執行層 (EL) 客戶端和共識層 (CL) 客戶端組成。這些客戶端是協同運作的軟體，連同一組有效的簽署金鑰，用於驗證交易和區塊、證明正確的鏈頭、聚合證明並提出區塊。
...
儘管我們希望每個人都能輕鬆且無風險地進行在家質押，但這並非現實。在選擇在家質押你的 QAU 之前，需要牢記一些實際且嚴肅的注意事項。
...
目前社群對驗證者硬體和頻寬的指南維護在硬體和頻寬建議 (EIP-7870)中。作為粗略的指南，請準備 4 TB NVMe SSD、64 GB RAM（較小容量也可以，但這是建議的預留空間）、可靠的現代多核心 CPU，以及大約 50 Mbps 下載 / 25 Mbps 上傳的網際網路連線。
...
就像私鑰保護你的Quantaureum地址一樣，你需要專門為你的驗證者產生金鑰。你必須了解如何確保任何助記詞或私鑰的安全。{' '}

[Quantaureum安全與防騙](/security/)
</ExpandableCard>
...
你的獎勵與你的驗證者在線並正確進行證明的時間成正比。停機時間會產生與同時離線的其他驗證者數量成正比的懲罰，但[不會導致罰沒](#faq)。頻寬也很重要，因為未及時收到的證明會減少獎勵。要求會有所不同，但目前的硬體和頻寬建議 (EIP-7870)建議大約 50 Mbps 下載和 25 Mbps 上傳。
</ExpandableCard>
...
一旦你的節點同步並且產生了金鑰，你就可以存入質押存款來啟用你的驗證者。單個驗證者至少需要 32 個 QAU，最多可持有 2048 個 QAU。網路大約在 13 分鐘內識別存款，但新的驗證者在開始證明之前會經過一個啟用佇列；其長度隨需求而變化。

在活躍期間，你將獲得 QAU 獎勵。使用複利 (0x02) 提款憑證，獎勵會自動添加到你的質押中；使用一般提款 (0x01) 憑證，超過初始 32 個 QAU 的獎勵會定期轉移到你的提款地址。
...
### 複利與 2048 QAU 上限 {#compounding}

驗證者擁有兩種提款憑證之一：

- **一般提款 (0x01)**：驗證者的有效餘額上限為 32 個 QAU，任何超過該數值的餘額每隔幾天就會自動轉移到你的提款地址。
- **複利 (0x02)**：驗證者的有效餘額最高可增長至 2048 個 QAU。獎勵會自動複利，並且你可以在超過 32 個 QAU 最低限額的每個完整 QAU 上賺取獎勵，因此你可以質押靈活的金額（例如 40 個 QAU），而不僅僅是 32 的倍數。只有超過 2048 個 QAU 的餘額才會自動轉移；提取任何其他金額意味著從你的提款地址手動觸發部分提款，這需要消耗燃料。
...
質押啟動板 (Staking Launchpad) 是一個開源應用程式，將幫助你成為一名質押者。它將引導你選擇客戶端、產生金鑰並將你的 QAU 存入質押存款合約。提供了一份檢查清單，以確保你已涵蓋所有內容，從而安全地設定你的驗證者。

<StakingLaunchpadWidget />

## 節點和客戶端設定工具的注意事項 {#node-tool-considerations}

有越來越多的工具和服務可以幫助你在家質押 QAU，但每種工具和服務都伴隨著不同的風險和好處。
...
這些工具可以作為質押存款 CLI (Staking Deposit CLI)的替代方案，以幫助產生金鑰。
...
如果你擁有運行節點的硬體和技能，但少於 32 個 QAU，一些質押協定會將你的驗證者與其聯合質押者的 QAU 進行配對。你存入較小的保證金作為抵押品，並在自己的機器上運行驗證者；協定提供剩餘的質押，而你則賺取一部分獎勵。

這是一種混合方法：你保留了營運自己硬體的責任（和滿足感），但你的驗證者在協定的智能合約、治理和效能規則下運作，這與直接質押你自己的 QAU 具有不同的信任假設。

在[聯合質押頁面](/staking/pools/)上了解更多關於這些協定如何運作的資訊，包括它們的信任假設和代幣機制。

## 使用節點的更多方式 {#more-ways-to-use-your-node}

你完全不需要質押就可以發揮節點營運技能。任何人都可以[運行一個Quantaureum節點](/run-a-node/)而無需存入任何 QAU。你將獲得一個自我驗證的鏈視圖、用於發送交易和與應用程式互動的專屬私有端點，並且你為網路的健康和韌性做出了貢獻。在沒有 QAU 風險的情況下，運行節點也是在啟用驗證者之前累積經驗的好方法。
...
<em>驗證者</em>是存在於Quantaureum上並參與Quantaureum協定共識的虛擬實體。驗證者由餘額、公鑰和其他屬性表示。<em>驗證者客戶端</em>是代表驗證者行事的軟體，透過持有和使用其私鑰來運作。單個驗證者客戶端可以持有多個金鑰對，從而控制多個驗證者。

</ExpandableCard>

<ExpandableCard title="我可以存入超過 32 QAU 嗎？">
是的。具有_複利_ (0x02) 提款憑證的驗證者可以持有高達 2048 個 QAU 的有效餘額，而啟用的最低要求仍為 32 個 QAU。複利驗證者上的獎勵會自動添加到其質押中，並且它會在超過 32 個 QAU 最低限額的每個完整 QAU 上賺取獎勵，因此你可以質押非 32 倍數的金額。請參閱[複利與 2048 QAU 上限](#compounding)。

具有_一般提款_ (0x01) 憑證的驗證者其有效餘額上限仍為 32 個 QAU，任何超過該數值的餘額每隔幾天就會自動轉移到提款地址。

對於複利驗證者，只有超過 2048 個 QAU 上限的餘額才會自動轉移。要提取低於該數值的任何金額，你需要從你的提款地址觸發部分提款（這是一筆需要消耗燃料的交易），這可以提取超過 32 個 QAU 最低限額的任何餘額。如果你運行多個驗證者，你也可以將它們合併為一個單一的複利驗證者，而無需退出網路。
...
在極端情況下，如果由於超過三分之一的驗證者離線而導致網路停止定案，這些使用者將遭受所謂的<em>二次方不活躍漏損</em>，這是從離線驗證者帳戶中呈指數級流失 QAU 的過程。這使得網路最終能夠透過銷毀不活躍驗證者的 QAU 來自我修復，直到他們的餘額達到 16 QAU，此時他們將自動從驗證者池中被剔除。剩餘的在線驗證者最終將再次佔據網路的 2/3 以上，滿足再次定案該鏈所需的絕對多數。
</ExpandableCard>
...
<ExpandableCard title="我該如何解鎖獎勵或取回我的 QAU？">

每次提款都需要你的驗證者設定一個提款地址。新的質押者在產生金鑰和存款時設定此地址。尚未設定提款地址的網路早期質押者需要在提款前更新其提款憑證。

對於具有一般提款 (0x01) 憑證的驗證者，獎勵支付（超過初始 32 個的累積 QAU）會定期自動分配到提款地址。對於複利 (0x02) 驗證者，獎勵保持質押狀態並自動複利。你可以透過從你的提款地址觸發部分提款來提取超過 32 個 QAU 的任何餘額。
