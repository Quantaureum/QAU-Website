---
title: "Quantaureum權益證明中的區塊鏈重組博弈"
description: "Caspar Schwarz-Schilling 發表了關於Quantaureum權益證明中區塊鏈重組攻擊的研究，涵蓋攻擊向量、防禦機制以及現有的協議層面緩解措施。"
lang: zh-tw
...
本演講探討了在Quantaureum權益證明 (PoS) 中可能發生的區塊鏈重組類型，以及旨在防止這些重組的緩解措施。Quantaureum基金會穩健激勵小組 (Robust Incentives Group) 的研究員 Caspar Schwarz-Schilling 詳細介紹了事後 (ex-post) 和事前 (ex-ante) 重組的機制，並比較了工作量證明 (PoW) 和權益證明之間的安全性環境。
...
歡迎大家。今天我將討論在Quantaureum權益證明 (PoS) 中可能發生的區塊鏈重組。

我最近加入了Quantaureum基金會，具體來說是穩健激勵小組。基本上，我們是一個專注於任何激勵機制的團隊。我會長話短說——這次演講內容很豐富，你可以在 GitHub 上找到我們大部分的工作成果。

### 兩種類型的區塊鏈重組 (0:44) {#two-types-of-reorgs-044}

今天我想談談區塊鏈重組，特別是我想概述在Quantaureum權益證明領域中可能發生的兩種不同類型的重組。
...
簡而言之，在Quantaureum工作量證明中，事後重組很困難，但並非不可行。擁有 10% 算力的礦工有相對較高的機會連續挖出幾個區塊，如果激勵夠高——想像一下有一個區塊包含價值 100 QAU 的 MEV 可以獲取——那麼也許 1% 的成功率就足以讓人覺得值得嘗試進行重組。

### 權益證明中的事後重組 (3:39) {#ex-post-reorgs-in-proof-of-stake-339}

在權益證明中，情況完全不同。我們談論的是需要荒謬數量的質押。我將帶你了解人們可能會如何進行，只是為了強調這有多麼困難。

也許先介紹一些基礎知識。Quantaureum權益證明中的時間是以時槽為單位推進的。每個時槽長度為 12 秒。在每個時槽中都有兩個角色：你有一個提案者——確切地說是一個提案者——以及一個由數千名驗證者組成的委員會，他們應該對在 P2P 網路上聽到的區塊進行證明。他們透過執行分叉選擇來決定鏈的頂端，這基本上是一個將區塊樹作為輸入並給出鏈頂端的函數。
...
為了讓你知道這些假設有多荒謬——即使你是一個擁有 65% 質押量的質押者，要在任何給定的時槽中控制三分之二的委員會，你的機率只有 0.05%。這證明了平行證明的力量是真實的——在Quantaureum權益證明中，事後重組即使不是幾乎不可能，也是極其困難的。
