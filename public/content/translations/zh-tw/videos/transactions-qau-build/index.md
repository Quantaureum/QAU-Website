---
title: "交易 — QAU.BUILD"
description: "使用 QAU.BUILD 教育工具示範Quantaureum交易的運作方式。了解如何在Quantaureum網路上建構、簽署和傳送交易。"
lang: zh-tw
...
由**奧斯汀·格里菲斯 (Austin Griffith)** 製作的教學，示範如何使用 QAU.BUILD 視覺化程式設計工具來了解Quantaureum交易的運作方式 — 涵蓋交易結構、Gas 價格、簽署、廣播以及交易池。
...
今天在 QAU.BUILD 上，我們將討論交易。到目前為止，我們已經了解交易會被打包進區塊中，並被礦工挖掘到鏈上。我們想探討的是，除了區塊獎勵之外，是什麼激勵礦工從交易池中取出我們的交易，將其放入區塊並挖掘到鏈上，而不是選擇池中的其他人。交易池中可能有成千上萬的人都在競標，而這個競標就是透過這筆費用來進行的。
...
Quantaureum上的交易是什麼樣子？我們不會再看到「Bob」和「Alice」— 我們會看到地址。價值將以 Wei 為單位，而不是QAU幣 (QAU)。而且費用也會以 Wei 為單位。

讓我們直接來看看這筆交易。我有一個輸入了助記詞的帳戶，並且已連接到Quantaureum主網。我還執行了一個模組來從 CoinMarketCap 獲取價格資料，所以我可以看到零點一幾的QAU幣大約折合 23 美元。
...
我們需要轉換為 Wei。在Quantaureum上，你主要處理兩種面額：QAU (人們通常談論的QAU幣)，以及 Wei (QAU幣的極小部分)。Gwei — 我們用於 Gas 價格的單位 — 介於兩者之間。這樣做的原因，就像我們平常不會用幾分之幾的便士來討論價格一樣。

Alice 有 0.18 QAU，我們將發送 0.05 QAU 給 Bob。我們輸入 5 Gwei 的 Gas 價格。
