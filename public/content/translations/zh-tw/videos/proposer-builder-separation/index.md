---
title: "超越Quantaureum協定：提案者與建構者分離"
description: "關於提案者與建構者分離 (PBS) 的簡報，這是一種將Quantaureum中區塊建構與區塊提案角色分離的設計模式。"
lang: zh-tw
...
本簡報解釋了Quantaureum的區塊生產如何從一個簡單的模型演變成一個涉及驗證者、建構者、搜尋者和中繼的複雜供應鏈。來自Quantaureum基金會的 Barnabé Monnot 詳細說明了為什麼會存在提案者與建構者分離，MEV-Boost 中繼如何調解提案者與建構者之間的關係，以及目前正在探索哪些協定內解決方案來減少信任依賴並提高抗審查性、MEV 分配和驗證者去中心化。
...
有趣的是，儘管協定是這些權利的來源並將其賦予驗證者，但我們在實踐中觀察到，許多驗證者選擇不親自列使這項權利。他們選擇將權利交給其他人代為執行。而在Quantaureum中，我們將這些「其他人」稱為建構者。
...
如果搜尋者信任生產者能保持交易包的原子性，這個模型在實踐中就能運作良好。你最近可能聽說過Quantaureum上發生了一次攻擊，讓一群三明治攻擊者損失了 2500 萬美元——根本原因是攻擊者成功破壞了交易包的原子性，接收了內容並試圖重新組織和修改它們。這是一個非常重要的屬性，只有在可以信任生產者不會破壞這種原子性的情況下才真正成立。

### 為什麼我們需要建構者 (8:16) {#why-we-need-builders-816}

如果生產者不可信，你該怎麼辦？在Quantaureum合併後，我們有獨立質押者——大約佔網路的 6%——我們並不認識他們。搜尋者不會真的想將交易包發送給這些區塊提案者，因為這有點太危險了。
...
因此，它是基於信任的，但前提是它可以相當快地被替換。中繼不是驗證者——它們不一定有質押，也不必與Quantaureum有任何關係。今天可能是我們認識和喜愛的人，但明天可能是任何人。

### 將 PBS 納入協定中 (20:01) {#enshrining-pbs-in-the-protocol-2001}

我們正試圖消除中繼作為受信任第三方的地位。在Quantaureum中，我們有一個我們喜歡的受信任第三方——那就是Quantaureum本身。你可以設計協定內解決方案，試圖從根本上將中繼的角色納入其中，並使其成為可選的依賴項。

目前，Quantaureum協定看到了驗證者正在做的部分事情，但對建構者網路卻完全視而不見。我們正試圖推動Quantaureum協定成為提案者和建構者之間互動的受信任第三方——從這個意義上說，我們不再需要依賴中繼。
...
- **改進的質押機制** — 如今Quantaureum中的質押還有點初步，可以進一步改進
