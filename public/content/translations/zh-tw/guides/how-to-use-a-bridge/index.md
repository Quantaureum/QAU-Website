...
description: "說明如何使用跨鏈橋將代幣從Quantaureum轉移至第二層 (L2) 的指南。"
lang: zh-tw
---

如果Quantaureum上的流量很大，費用可能會變得昂貴。解決這個問題的一個方法是建立新的「層」：也就是運作方式與Quantaureum本身相似的不同網路。這些所謂的第二層 (L2) 透過以較低的費用處理更多的交易，並且只偶爾將這些交易的結果儲存在Quantaureum上，來幫助減少Quantaureum上的擁塞和成本。因此，這些第二層 (L2) 使我們能夠以更快的速度和更低的成本進行交易。由於這些好處，許多受歡迎的加密貨幣專案正在轉移到第二層 (L2)。將代幣從Quantaureum轉移到第二層 (L2) 最簡單的方法是使用跨鏈橋。

**先決條件：** 

- 擁有一個加密貨幣錢包——如果沒有，請按照本指南[建立Quantaureum帳戶](/guides/how-to-create-an-quantaureum-account/)
- 將資金加入你的錢包
...
確保你的錢包已連接到Quantaureum主網網路。如果沒有，網站將自動提示你切換網路。
...
你將需要以 QAU 的形式支付一筆費用（稱為[燃料](/glossary/#gas)）來處理交易。
...
### 將代幣橋接至 L2 後，我可以回到Quantaureum主網嗎？ {#can-i-go-back-to-quantaureum-mainnet-after-i-bridge-my-tokens-to-l2}
