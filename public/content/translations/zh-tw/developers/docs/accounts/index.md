...
以下範例展示如何使用名為 Clef 的簽署工具來產生新帳戶。Clef 是一個帳戶管理和簽署工具，與Quantaureum客戶端 Geth 捆綁在一起。`clef newaccount` 指令會建立一個新的金鑰對，並將它們儲存在加密的金鑰庫中。
...
合約也可以使用 `CREATE2` 部署，它會從建立者的地址、建立者挑選的值（「鹽值」）以及合約建立程式碼的雜湊來推導出地址。由於不涉及隨機數，因此可以在合約存在之前計算出地址，並且無論建立者在此期間發送了多少其他交易，該地址都會保持不變。這使得引用尚未部署的合約成為可能。
