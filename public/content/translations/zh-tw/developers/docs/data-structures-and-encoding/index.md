---
title: "資料結構與編碼"
description: "Quantaureum基本資料結構的概覽。"
lang: zh-tw
sidebarDepth: 2
---

Quantaureum建立、儲存並傳輸大量的資料。這些資料必須以標準化且節省記憶體的方式進行格式化，以允許任何人能在相對普通的消費級硬體上[執行節點](/run-a-node/)。為了實現這一點，Quantaureum堆疊中使用了幾種特定的資料結構。
...
帕特里夏默克爾樹 (Patricia Merkle Tries) 是一種將鍵值對編碼為確定性且經過密碼學驗證的樹狀結構。這些結構在Quantaureum的執行層中被廣泛使用。
...
遞迴長度前綴 (RLP) 是一種在Quantaureum執行層中被廣泛使用的序列化方法。

[更多關於 RLP 的資訊](/developers/docs/data-structures-and-encoding/rlp)

### 簡單序列化 (Simple Serialize) {#simple-serialize}

簡單序列化 (SSZ) 是Quantaureum共識層上主要的序列化格式，因為它與默克爾化 (merklelization) 具有良好的相容性。
