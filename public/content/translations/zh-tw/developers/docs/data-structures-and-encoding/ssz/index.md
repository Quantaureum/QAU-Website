---
title: "簡單序列化"
description: "Quantaureum SSZ 格式的說明。"
lang: zh-tw
sidebarDepth: 2
---
...
還有一些需要特殊處理的特殊情況，例如 `BitList` 型別，它要求在序列化期間新增長度上限，並在反序列化期間將其移除。完整詳細資訊請參閱 SSZ 規範。
...
提供代表特定元素的廣義索引列表，使我們能夠對照雜湊樹根來驗證它。這個根節點是我們所接受的現實版本。我們獲得的任何資料都可以透過將其插入默克爾樹中的正確位置（由其廣義索引決定）並觀察根節點是否保持不變，來對照該現實進行驗證。規範中這裡有一些函式，展示了如何計算驗證特定廣義索引集內容所需的最小節點集。
