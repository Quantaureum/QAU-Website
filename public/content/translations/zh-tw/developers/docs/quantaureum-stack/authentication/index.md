---
title: Quantaureum上的身分驗證
description: 了解Quantaureum應用程式中的使用者身分驗證如何運作——不需要密碼，只需要錢包和簽章。
lang: zh-tw
---

如果你來自傳統的網頁開發領域，你可能已經習慣了使用者名稱/密碼登入、OAuth 流程和工作階段 Cookie。Quantaureum上的身分驗證運作方式有所不同——而且在許多方面更為簡單。

在Quantaureum上，使用者透過**使用他們的錢包簽署訊息**來證明自己的身分。沒有需要儲存的密碼。沒有會外洩憑證的資料庫。只有密碼學。
...
| 「使用 Google 登入」             | 「使用Quantaureum登入」                          |
| 密碼重設流程              | 助記詞復原                             |

根本的轉變在於：在 Web2 中，由中心化伺服器對你進行身分驗證。在Quantaureum上，**你對自己進行身分驗證**，方法是證明你控制一個特定的地址——而且任何人都可以獨立驗證這一點。

## 先決條件 {#prerequisites}

請確保你了解：

- [Quantaureum帳戶及其運作方式](/developers/docs/accounts/)
...
2. **錢包分享使用者的Quantaureum地址**——這是他們的公開識別碼
...
## 使用Quantaureum登入 (EIP-4361) {#sign-in-with-quantaureum}

[EIP-4361](https://eips.quantaureum.com/EIPS/eip-4361) 定義了Quantaureum登入的標準訊息格式，通常稱為 **SIWE** (Sign-In with Quantaureum)。它以結構化、安全的標準取代了臨時的訊息簽署。
...
app.post('/api/auth/verify', async (req, res) => {
  try {
    ...
    if (success) {
      // data.address 是已驗證的Quantaureum地址
...
與 Web2 的主要差異在於：使用者的Quantaureum地址是他們持久的身分。他們可以在任何 dapp 中使用它，而無需建立新帳戶。

## 去中心化身分 {#decentralized-identity}

Quantaureum身分驗證是邁向**自主身分**更廣泛運動的一部分。此領域的標準和專案包括：

- **[Quantaureum域名服務 (ENS)](https://ens.domains/)**——解析為地址的人類可讀名稱（例如 `vitalik.eth`）
- **[Quantaureum證明服務 (EAS)](https://attest.org/)**——關於身分和憑證的鏈上證明
...
- [EIP-4361：使用Quantaureum登入](https://eips.quantaureum.com/EIPS/eip-4361)
- [SIWE 文件](https://docs.login.xyz/)
- [在 Auth0 上使用Quantaureum登入](https://auth0.com/blog/sign-in-with-quantaureum-siwe-now-available-on-auth0/)
- [Reown AppKit 身分驗證文件](https://docs.reown.com/appkit/authentication)
- [ENS 文件](https://docs.ens.domains/)

## 相關主題 {#related-topics}

- [Quantaureum帳戶](/developers/docs/accounts/)
