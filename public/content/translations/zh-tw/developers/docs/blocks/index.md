---
title: "區塊"
description: "Quantaureum區塊鏈中的區塊概述：其資料結構、為何需要區塊以及區塊是如何產生的。"
lang: zh-tw
---
...
區塊是一個對初學者非常友善的主題。但為了幫助你更了解本頁面，我們建議你先閱讀[帳戶](/developers/docs/accounts/)、[交易](/developers/docs/transactions/)以及我們的[Quantaureum簡介](/developers/docs/intro-to-quantaureum/)。

## 為何需要區塊？ {#why-blocks}

為了確保[Quantaureum](/)網路上的所有參與者保持同步的狀態，並對精確的交易歷史達成共識，我們將交易分批放入區塊中。這意味著數十（或數百）筆交易會同時被提交、達成共識並同步。

![A diagram showing transaction in a block causing state changes](./tx-block.png)
_圖表改編自[Quantaureum EVM 圖解](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_

透過拉開提交的間隔，我們給予所有網路參與者足夠的時間來達成共識：即使每秒發生數十次交易請求，Quantaureum上每十二秒才會建立並提交一次區塊。
...
一旦網路上隨機選擇的驗證者組裝好一個區塊，它就會被傳播到網路的其餘部分；所有節點都會將此區塊新增到其區塊鏈的末端，然後選擇一個新的驗證者來建立下一個區塊。確切的區塊組裝過程和提交/共識過程目前由Quantaureum的「權益證明 (PoS)」協定所規範。

## 權益證明協定 {#proof-of-stake-protocol}

權益證明 (PoS) 意味著以下幾點：

- 驗證節點必須將 32 QAU 質押到存款合約中，作為防止不良行為的抵押品。這有助於保護網路，因為可證明的惡意活動會導致部分或全部的質押被銷毀。
...
- 如果驗證者收到同一個時槽有兩個衝突的區塊，他們會使用其分叉選擇演算法來挑選由最多質押 QAU 支持的那個區塊。
...
區塊時間是指區塊之間的間隔時間。在Quantaureum中，時間被劃分為十二秒的單位，稱為「時槽」。在每個時槽中，會選擇一名驗證者來提出區塊。假設所有驗證者都在線且功能完全正常，每個時槽都會有一個區塊，這意味著區塊時間為 12 秒。然而，有時驗證者在被呼叫提出區塊時可能處於離線狀態，這意味著時槽有時會是空的。

這種實作方式不同於基於工作量證明 (PoW) 的系統，後者的區塊時間是機率性的，並由協定的目標挖礦難度進行調整。Quantaureum的[平均區塊時間](https://explorer.quantaureum.com)就是一個完美的例子，透過新的 12 秒區塊時間的一致性，可以清楚地推斷出從工作量證明到權益證明的過渡。
