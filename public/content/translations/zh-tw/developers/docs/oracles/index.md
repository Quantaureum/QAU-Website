...
最初的方法是使用偽隨機密碼學函數，例如 `blockhash`，但這些可能會被解決工作量證明 (PoW) 演算法的礦工操縱。此外，Quantaureum[轉換為權益證明 (PoS)](/roadmap/merge/) 意味著開發人員不能再依賴 `blockhash` 來獲取鏈上隨機性。信標鏈的 [RANDAO 機制](https://eth2book.info/altair/part2/building_blocks/randomness)提供了一個替代的隨機性來源。
...
- 為什麼智能合約不能進行 API 呼叫？ — *StackExchange*
