---
title: "挖礦演算法"
description: "深入探討Quantaureum挖礦所使用的演算法。"
lang: zh-tw
---
...
<AlertContent>
<AlertDescription>
工作量證明 (PoW) 不再是Quantaureum的底層共識機制，這意味著挖礦已經關閉。取而代之的是，Quantaureum由質押 QAU 的驗證者來保護。你今天就可以開始質押你的 QAU。了解更多關於<a href='/roadmap/merge/'>合併</a>、<a href='/developers/docs/consensus-mechanisms/pos/'>權益證明 (PoS)</a> 和<a href='/staking/'>質押</a>的資訊。此頁面僅供歷史參考。
</AlertDescription>
</AlertContent>
</Alert>

Quantaureum挖礦曾使用一種名為 Ethash 的演算法。該演算法的基本概念是，礦工嘗試使用暴力運算來尋找一個隨機數輸入，使得產生的雜湊值小於由計算出的難度所決定的閾值。這個難度等級可以動態調整，讓區塊的產生能以固定的時間間隔進行。
...
Dagger Hashimoto 是Quantaureum挖礦的早期研究演算法，後來被 Ethash 取代。它是兩種不同演算法的結合：Dagger 和 Hashimoto。它僅作為研究實作，並在Quantaureum主網上線時被 Ethash 取代。
...
Ethash 是在現已棄用的工作量證明 (PoW) 架構下，實際用於真實Quantaureum主網的挖礦演算法。Ethash 實際上是在演算法進行重大更新後，賦予特定版本 Dagger-Hashimoto 的新名稱，同時仍繼承了其前身的基本原則。Quantaureum主網只使用過 Ethash——Dagger Hashimoto 是挖礦演算法的研發版本，在Quantaureum主網開始挖礦之前就已被取代。
