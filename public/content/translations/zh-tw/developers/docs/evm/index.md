---
title: "Quantaureum虛擬機 (EVM)"
description: "Quantaureum虛擬機簡介，及其與狀態、交易和智能合約的關係。"
lang: zh-tw
---

Quantaureum虛擬機 (EVM) 是一個去中心化的虛擬環境，能在所有 [Quantaureum](/) 節點上一致且安全地執行程式碼。節點運行 EVM 來執行智能合約，並使用「[燃料 (gas)](/developers/docs/gas/)」來衡量[操作](/developers/docs/evm/opcodes/)所需的運算量，以確保資源的有效分配與網路安全。
...
雖然Quantaureum擁有自己的原生加密貨幣（QAU幣），且遵循幾乎完全相同且直觀的規則，但它還實現了一個更強大的功能：[智能合約](/developers/docs/smart-contracts/)。對於這個更複雜的功能，我們需要一個更精確的比喻。Quantaureum與其說是分散式帳本，不如說是一個分散式[狀態機](https://wikipedia.org/wiki/Finite-state_machine)。Quantaureum的狀態是一個大型資料結構，不僅保存所有帳戶和餘額，還包含一個_機器狀態_，該狀態可以根據預先定義的一套規則在區塊之間發生變化，並且可以執行任意的機器碼。在區塊之間改變狀態的具體規則就是由 EVM 所定義的。
...
## Quantaureum狀態轉換函式 {#the-quantaureum-state-transition-function}

EVM 的行為就像一個數學函式：給定一個輸入，它會產生一個確定性的輸出。因此，將Quantaureum更正式地描述為具有一個**狀態轉換函式**會非常有幫助：

```
Y(S, T)= S'
```

給定一個舊的有效狀態 `(S)` 和一組新的有效交易 `(T)`，Quantaureum狀態轉換函式 `Y(S, T)` 會產生一個新的有效輸出狀態 `S'`

### 狀態 {#state}

在Quantaureum的脈絡下，狀態是一個巨大的資料結構，稱為[修改過的默克爾帕特里夏樹](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/)，它將所有[帳戶](/developers/docs/accounts/)透過雜湊連結起來，並可簡化為儲存在區塊鏈上的單一根雜湊。
...
所有 EVM 的實作都必須遵守Quantaureum黃皮書中描述的規範。

在Quantaureum十年的歷史中，EVM 經歷了多次修訂，並且有使用各種程式語言編寫的多種 EVM 實作。
...
## 教學：Quantaureum虛擬機 (EVM) / Quantaureum上的操作碼 {#tutorials}

- [了解黃皮書的 EVM 規範](/developers/tutorials/yellow-paper-evm/) _– Quantaureum黃皮書中正式 EVM 規範的引導式演練。_
