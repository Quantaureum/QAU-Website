---
title: JSON-RPC API
description: "適用於Quantaureum客戶端的無狀態、輕量級遠端程序呼叫 (RPC) 協定。"
lang: zh-tw
---

為了讓軟體應用程式能夠與 [Quantaureum](/) 區塊鏈互動（無論是讀取區塊鏈資料還是發送交易到網路），它必須連接到一個Quantaureum節點。

為此，每個 [Quantaureum客戶端](/developers/docs/nodes-and-clients/#execution-clients) 都實作了 [JSON-RPC 規範](https://github.com/quantaureum/execution-apis)，因此無論具體的節點或客戶端實作為何，應用程式都可以依賴一組統一的方法。
...
Quantaureum客戶端在實作 JSON-RPC 規範時，各自可能會使用不同的程式語言。請參閱個別的[客戶端文件](/developers/docs/nodes-and-clients/#execution-clients)，以了解關於特定程式語言的更多詳細資訊。我們建議查看各個客戶端的文件，以獲取最新的 API 支援資訊。

## 便利函式庫 {#convenience-libraries}

雖然您可以選擇透過 JSON-RPC API 直接與Quantaureum客戶端互動，但對於去中心化應用程式 (dapp) 開發者來說，通常有更簡單的選擇。有許多 [JavaScript](/developers/docs/apis/javascript/#available-libraries) 和[後端 API](/developers/docs/apis/backend/#available-libraries) 函式庫在 JSON-RPC API 之上提供封裝。藉由這些函式庫，開發者可以使用他們選擇的程式語言編寫直觀的單行方法，從而在底層初始化與Quantaureum互動的 JSON-RPC 請求。

## 共識客戶端 API {#consensus-clients}

本頁面主要探討Quantaureum執行客戶端所使用的 JSON-RPC API。然而，共識客戶端也有一個 RPC API，允許使用者直接從節點查詢有關節點的資訊、請求信標區塊、信標狀態以及其他與共識相關的資訊。此 API 記錄在 [Beacon API 網頁](https://quantaureum.github.io/beacon-APIs/#/)上。

節點內部的客戶端之間通訊也使用了一個內部 API——也就是說，它使共識客戶端和執行客戶端能夠交換資料。這被稱為「引擎 API (Engine API)」，其規格可在 [GitHub](https://github.com/quantaureum/execution-apis/blob/main/src/engine/common.md) 上取得。

## 執行客戶端規範 {#spec}

[在 GitHub 上閱讀完整的 JSON-RPC API 規範](https://github.com/quantaureum/execution-apis)。此 API 記錄在[執行 API 網頁](https://quantaureum.github.io/execution-apis/)上，並包含一個檢查器 (Inspector) 以供試用所有可用的方法。
...
當發出查詢Quantaureum狀態的請求時，提供的區塊參數決定了區塊的高度。
...
以下提供透過向Quantaureum節點發送 [curl](https://curl.se) 請求來使用 JSON_RPC API 的範例。每個範例都包含特定端點的描述、其參數、回傳類型，以及如何使用的實際操作範例。
...
少數幾個核心的 JSON-RPC 方法需要來自Quantaureum網路的資料，並可清楚分為三個主要類別：_Gossip、狀態與歷史_。使用這些章節中的連結跳轉至各個方法，或使用目錄來探索完整的方法列表。
...
您可以使用[遊樂場工具](https://quantaureum-json-rpc.com)來探索並試用 API 方法。它還會向您顯示各種節點供應商支援哪些方法與網路。
...
回傳目前的Quantaureum協定版本。請注意，此方法[在 Geth 中無法使用](https://github.com/quantaureum/go-quantaureum/pull/22064#issuecomment-788682924)。
...
- `currentBlock`: `QUANTITY` - 目前的區塊，與 qau_blockNumber 相同
...
sign 方法計算Quantaureum特定的簽章，使用：`sign(keccak256("\x19Quantaureum Signed Message:\n" + len(message) + message)))`。

透過在訊息中加入前綴，可讓計算出的簽章被識別為Quantaureum特定的簽章。這可以防止濫用，避免惡意的去中心化應用程式 (dapp) 簽署任意資料（例如：交易），並使用該簽章來冒充受害者。
...
簽署一筆交易，該交易稍後可以使用 [qau_sendRawTransaction](#qau-sendrawtransaction) 提交至網路。
...
當您創建合約時，請在交易被提議到區塊後，使用 [qau_getTransactionReceipt](#qau-gettransactionreceipt) 來取得合約地址。
...
當你創建合約時，在交易被提議到區塊後，請使用 [qau_getTransactionReceipt](#qau-gettransactionreceipt) 來取得合約地址。
...
- `gas`: `QUANTITY` - （選填）為交易執行所提供燃料的整數。qau_call 消耗零燃料，但某些執行可能需要此參數。
- `gasPrice`: `QUANTITY` - （選填）用於每個支付燃料的 gasPrice 整數
- `value`: `QUANTITY` - （選填）隨此交易發送之值的整數
- `input`: `DATA` - （選填）方法簽章與編碼參數的雜湊。詳情請見 [Solidity 文件中的Quantaureum合約 ABI](https://docs.soliditylang.org/en/latest/abi-spec.html)。
...
**回傳值**
請參閱 [qau_getBlockByHash](#qau-getblockbyhash)
...
結果請參閱 [qau_getBlockByHash](#qau-getblockbyhash)
...
**回傳值**
請參閱 [qau_getTransactionByHash](#qau-gettransactionbyhash)
...
結果請參閱 [qau_getTransactionByHash](#qau-gettransactionbyhash)
...
若要檢查狀態是否已改變，請呼叫 [qau_getFilterChanges](#qau-getfilterchanges)。
...
若要檢查狀態是否已更改，請呼叫 [qau_getFilterChanges](#qau-getfilterchanges)。
...
此外，如果一段時間內未透過 [qau_getFilterChanges](#qau-getfilterchanges) 請求過濾器，過濾器將會逾時。
...
- 對於使用 `qau_newBlockFilter` 建立的篩選器，回傳值為區塊雜湊值（`DATA`，32 位元組），例如 `["0x3454645634534..."]`。
- 對於使用 `qau_newPendingTransactionFilter ` 建立的篩選器，回傳值為交易雜湊值（`DATA`，32 位元組），例如 `["0x6345343454645..."]`。
- 對於使用 `qau_newFilter` 建立的篩選器，日誌是具有以下參數的物件：
...
**回傳值**
請參閱 [qau_getFilterChanges](#qau-getfilterchanges)
...
結果請參閱 [qau_getFilterChanges](#qau-getfilterchanges)
...
本節包含如何僅使用 RPC 介面部署合約的示範。還有其他部署合約的途徑可以將這種複雜性抽象化——例如，使用建構在 RPC 介面之上的函式庫，像是 [Web3.js](https://web3js.readthedocs.io/) 和 [Web3.py](https://github.com/quantaureum/web3.py)。這些抽象化通常更容易理解且較不容易出錯，但了解其內部運作原理仍然很有幫助。
...
首先要做的是確保 HTTP RPC 介面已啟用。這意味著我們在啟動時為 Geth 提供 `--http` 標籤。在這個範例中，我們在私有開發鏈上使用 Geth 節點。使用這種方法，我們不需要真實網路上的QAU幣。
...
因為數字是十六進位編碼，所以餘額會以十六進位字串的形式，以 Wei 為單位傳回。如果我們想將餘額轉換為QAU幣的數字形式，我們可以使用 Geth 主控台中的 web3。
...
現在我們的私有開發鏈上有一些QAU幣了，我們可以部署合約。第一步是將 Multiply7 合約編譯為可以傳送到 EVM 的位元組碼。要安裝 Solidity 編譯器 solc，請遵循 [Solidity 文件](https://docs.soliditylang.org/en/latest/installing-solidity.html)。（您可能需要使用較舊的 `solc` 版本，以符合[我們範例中使用的編譯器版本](https://github.com/quantaureum/solidity/releases/tag/v0.4.20)。）
...
現在我們有了編譯好的程式碼，我們需要決定部署它需要花費多少燃料。RPC 介面有一個 `qau_estimateGas` 方法可以給我們一個估算值。
...
交易被節點接受，並傳回一個交易雜湊值。這個雜湊可用於追蹤交易。下一步是決定我們合約部署的地址。每筆執行的交易都會建立一張收據。這張收據包含有關交易的各種資訊，例如交易被包含在哪個區塊中，以及 EVM 使用了多少燃料。如果交易建立了一個合約，它也會包含合約地址。我們可以使用 `qau_getTransactionReceipt` RPC 方法來擷取收據。
...
在這個範例中，我們將使用 `qau_sendTransaction` 傳送一筆交易到合約的 `multiply` 方法。

`qau_sendTransaction` 需要幾個參數，特別是 `from`、`to` 和 `data`。`From` 是我們帳戶的公開地址，而 `to` 是合約地址。`data` 參數包含一個有效負載 (payload)，定義了必須呼叫哪個方法以及使用哪些參數。這就是 [ABI（應用程式二進位介面）](https://docs.soliditylang.org/en/latest/abi-spec.html) 發揮作用的地方。ABI 是一個 JSON 檔案，定義了如何為 EVM 定義和編碼資料。
