...
- **函式選擇器**：合約的函式少於 256 個，因此我們可以用單個位元組來區分它們。這些位元組通常非零，因此花費 16 燃料。
- **零**：這些位元組始終為零，因為 20 位元組的地址不需要 32 位元組的字組來容納它。值為零的位元組花費 4 燃料（請參閱黃皮書，附錄 G，第 27 頁，`G`<sub>`txdatazero`</sub> 的值）。
...
不幸的是，查看 ERC-20 規範，這只剩下一個函式：`transfer`。
