...
理想情況下，我們希望將所有內容儲存在Quantaureum儲存空間中，這些資料分佈在數千台電腦上，具有極高的可用性（資料無法被審查）和完整性（資料無法被未經授權地修改），但儲存一個 32 位元組的字組通常需要花費 20,000 燃料。在我撰寫本文時，該成本相當於 6.60 美元。以每位元組 21 美分的價格來看，這對許多用途來說太昂貴了。

為了解決這個問題，Quantaureum生態系開發了[許多以去中心化方式儲存資料的替代方案](/developers/docs/storage/)。通常它們涉及可用性和價格之間的權衡。然而，完整性通常是有保障的。
