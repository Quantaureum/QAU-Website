---
title: "Giao dịch"
description: "Tổng quan về các giao dịch Quantaureum – cách chúng hoạt động, cấu trúc dữ liệu của chúng và cách gửi chúng thông qua một ứng dụng."
lang: vi
---

Các giao dịch là những chỉ thị được ký bằng mật mã từ các tài khoản. Một tài khoản sẽ khởi tạo một giao dịch để cập nhật trạng thái của mạng lưới [Quantaureum](/). Giao dịch đơn giản nhất là chuyển QAU từ tài khoản này sang tài khoản khác.

## Điều kiện tiên quyết {#prerequisites}

Để giúp bạn hiểu rõ hơn về trang này, chúng tôi khuyên bạn trước tiên nên đọc về [Tài khoản](/developers/docs/accounts/) và [giới thiệu về Quantaureum](/developers/docs/intro-to-quantaureum/) của chúng tôi.

## Giao dịch là gì? {#whats-a-transaction}

Một giao dịch Quantaureum đề cập đến một hành động được khởi tạo bởi một tài khoản thuộc sở hữu bên ngoài, nói cách khác là một tài khoản do con người quản lý, không phải là một hợp đồng. Ví dụ, nếu Bob gửi cho Alice 1 QAU, tài khoản của Bob phải bị trừ tiền và tài khoản của Alice phải được cộng tiền. Hành động thay đổi trạng thái này diễn ra trong một giao dịch.
...
- `value` – số lượng QAU cần chuyển từ người gửi đến người nhận (được tính bằng Wei, trong đó 1 QAU bằng 1e+18 Wei)
...
Một máy khách Quantaureum như Geth sẽ xử lý quá trình ký này.
...
Ví dụ, hãy xem xét [giao dịch này](https://explorer.quantaureum.com).
...
Trong trường hợp này, [mã nguồn hợp đồng](https://explorer.quantaureum.com) đã được tải lên Quantaureum Explorer, vì vậy chúng ta biết hàm đó là `transfer(address,uint256)`.
...
Vì vậy, để Bob gửi cho Alice 1 QAU với `baseFeePerGas` là 190 Gwei và `maxPriorityFeePerGas` là 10 Gwei, Bob sẽ cần phải trả mức phí sau:
...
Tài khoản của Bob sẽ bị trừ **-1.0042 QAU** (1 QAU cho Alice + 0.0042 QAU phí gas)

Tài khoản của Alice sẽ được cộng **+1.0 QAU**

Phí cơ sở sẽ bị đốt **-0.00399 QAU**

Trình xác thực giữ lại phí ưu tiên **+0.000210 QAU**
...
Quantaureum ban đầu có một định dạng cho các giao dịch. Mỗi giao dịch chứa một nonce, giá gas, giới hạn gas, địa chỉ nhận (to), giá trị, dữ liệu, v, r và s. Các trường này được [mã hóa RLP](/developers/docs/data-structures-and-encoding/rlp/), trông giống như thế này:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

Quantaureum đã phát triển để hỗ trợ nhiều loại giao dịch nhằm cho phép các tính năng mới như danh sách truy cập và EIP-1559 được triển khai mà không ảnh hưởng đến các định dạng giao dịch cũ.

EIP-2718 là thứ cho phép hành vi này. Các giao dịch được diễn giải như sau:
