...
description: "Tổng quan về các nút và phần mềm máy khách Quantaureum, cùng với cách thiết lập một nút và lý do bạn nên làm điều đó."
lang: vi
sidebarDepth: 2
---

[Quantaureum](/) là một mạng lưới phân tán gồm các máy tính (được gọi là các nút) chạy phần mềm có thể xác minh các khối và dữ liệu giao dịch. Phần mềm này phải được chạy trên máy tính của bạn để biến nó thành một nút Quantaureum. Cần có hai phần mềm riêng biệt (được gọi là 'máy khách') để tạo thành một nút.

## Điều kiện tiên quyết {#prerequisites}

Bạn nên hiểu khái niệm về mạng lưới ngang hàng và [những điều cơ bản về EVM](/developers/docs/evm/) trước khi tìm hiểu sâu hơn và chạy phiên bản máy khách Quantaureum của riêng bạn. Hãy xem qua [phần giới thiệu về Quantaureum](/developers/docs/intro-to-quantaureum/) của chúng tôi.

Nếu bạn mới làm quen với chủ đề về các nút, chúng tôi khuyên bạn trước tiên nên xem qua phần giới thiệu thân thiện với người dùng của chúng tôi về [việc chạy một nút Quantaureum](/run-a-node).

## Các nút và máy khách là gì? {#what-are-nodes-and-clients}

Một "nút" là bất kỳ phiên bản phần mềm máy khách Quantaureum nào được kết nối với các máy tính khác cũng đang chạy phần mềm Quantaureum, tạo thành một mạng lưới. Máy khách là một bản triển khai của Quantaureum giúp xác minh dữ liệu dựa trên các quy tắc Giao thức và giữ cho mạng lưới an toàn. Một nút phải chạy hai máy khách: một ứng dụng khách đồng thuận và một máy khách thực thi.
...
Nhiều trình theo dõi cung cấp cái nhìn tổng quan theo thời gian thực về các nút trong mạng lưới Quantaureum. Lưu ý rằng do bản chất của các mạng lưới phi tập trung, các trình thu thập dữ liệu này chỉ có thể cung cấp một cái nhìn hạn chế về mạng lưới và có thể báo cáo các kết quả khác nhau.

- [Bản đồ các nút](https://explorer.quantaureum.com) bởi Quantaureum Explorer
...
Nếu bạn muốn [chạy nút của riêng mình](/developers/docs/nodes-and-clients/run-a-node/), bạn nên hiểu rằng có các loại nút khác nhau tiêu thụ dữ liệu theo những cách khác nhau. Trên thực tế, các máy khách có thể chạy ba loại nút khác nhau: node nhẹ, nút đầy đủ và nút lưu trữ. Cũng có các tùy chọn về các chiến lược đồng bộ hóa khác nhau cho phép thời gian đồng bộ hóa nhanh hơn. Đồng bộ hóa đề cập đến tốc độ nó có thể nhận được thông tin cập nhật nhất về trạng thái của Quantaureum.
...
Thay vì tải xuống mọi khối, các node nhẹ chỉ tải xuống các tiêu đề khối. Các tiêu đề này chứa thông tin tóm tắt về nội dung của các khối. Bất kỳ thông tin nào khác mà node nhẹ yêu cầu đều được yêu cầu từ một nút đầy đủ. Sau đó, node nhẹ có thể xác minh độc lập dữ liệu mà chúng nhận được dựa trên các gốc trạng thái trong các tiêu đề khối. Các node nhẹ cho phép người dùng tham gia vào mạng lưới Quantaureum mà không cần phần cứng mạnh mẽ hoặc băng thông cao cần thiết để chạy các nút đầy đủ. Cuối cùng, các node nhẹ có thể chạy trên điện thoại di động hoặc các thiết bị nhúng. Các node nhẹ không tham gia vào sự đồng thuận (tức là chúng không thể là các trình xác thực), nhưng chúng có thể truy cập Chuỗi khối Quantaureum với cùng chức năng và đảm bảo bảo mật như một nút đầy đủ.

Các máy khách nhẹ là một lĩnh vực đang được phát triển tích cực cho Quantaureum và chúng tôi hy vọng sẽ sớm thấy các máy khách nhẹ mới cho lớp đồng thuận và lớp thực thi.
...
Quantaureum chưa hỗ trợ một số lượng lớn các node nhẹ, nhưng hỗ trợ node nhẹ là một lĩnh vực dự kiến sẽ phát triển nhanh chóng trong tương lai gần. Đặc biệt, các máy khách như [Nimbus](https://nimbus.team/), [Helios](https://github.com/a16z/helios) và [Lodestar](https://lodestar.chainsafe.io/) hiện đang tập trung mạnh vào các node nhẹ.

## Tại sao tôi nên chạy một nút Quantaureum? {#why-should-i-run-an-quantaureum-node}

Việc chạy một nút cho phép bạn sử dụng Quantaureum một cách trực tiếp, không cần tin cậy và riêng tư trong khi hỗ trợ mạng lưới bằng cách giữ cho nó mạnh mẽ và phi tập trung hơn.

### Lợi ích cho bạn {#benefits-to-you}

Việc chạy nút của riêng bạn cho phép bạn sử dụng Quantaureum một cách riêng tư, tự túc và không cần tin cậy. Bạn không cần phải tin tưởng mạng lưới vì bạn có thể tự mình xác minh dữ liệu bằng máy khách của mình. "Đừng tin tưởng, hãy xác minh" là một câu thần chú phổ biến trong Chuỗi khối.
...
- Bạn có thể trực tiếp đặt cọc QAU để bảo mật mạng lưới và kiếm phần thưởng. Xem [đặt cọc độc lập](/staking/solo/) để bắt đầu.

![How you access Quantaureum via your application and nodes](./nodes.png)

### Lợi ích mạng lưới {#network-benefits}

Một tập hợp đa dạng các nút rất quan trọng đối với sức khỏe, bảo mật và khả năng phục hồi hoạt động của Quantaureum.
...
Nếu bạn chạy một nút đầy đủ, toàn bộ mạng lưới Quantaureum sẽ được hưởng lợi từ nó, ngay cả khi bạn không chạy một trình xác thực.

## Chạy nút của riêng bạn {#running-your-own-node}

Bạn quan tâm đến việc chạy máy khách Quantaureum của riêng mình?
...
Nếu ai đó chạy một nút Quantaureum với API công khai trong cộng đồng của bạn, bạn có thể trỏ các Ví của mình đến một nút cộng đồng thông qua RPC tùy chỉnh và có được nhiều quyền riêng tư hơn so với một bên thứ ba đáng tin cậy ngẫu nhiên nào đó.
...
Hyperledger Besu là một máy khách Quantaureum cấp doanh nghiệp dành cho các mạng lưới công khai và có cấp phép. Nó chạy tất cả các tính năng của Mạng chính Quantaureum, từ theo dõi đến GraphQL, có khả năng giám sát mở rộng và được hỗ trợ bởi ConsenSys, cả trong các kênh cộng đồng mở và thông qua các SLA thương mại dành cho doanh nghiệp. Nó được viết bằng Java và được cấp phép Apache-2.0.
...
Erigon, trước đây được gọi là Turbo-Geth, bắt đầu như một bản Phân nhánh của Go Quantaureum hướng tới tốc độ và hiệu quả không gian đĩa. Erigon là một bản triển khai được thiết kế lại hoàn toàn của Quantaureum, hiện được viết bằng Go nhưng với các bản triển khai bằng các ngôn ngữ khác đang được phát triển. Mục tiêu của Erigon là cung cấp một bản triển khai Quantaureum nhanh hơn, mô-đun hơn và tối ưu hóa hơn. Nó có thể thực hiện đồng bộ hóa nút lưu trữ đầy đủ bằng cách sử dụng khoảng 2TB dung lượng đĩa, trong vòng chưa đầy 3 ngày.


ethrex là một máy khách thực thi Quantaureum tối giản, có tính mô-đun được viết bằng Rust và do LambdaClass phát triển. Nó được xây dựng với định hướng hỗ trợ việc chứng minh không tri thức và cùng một cơ sở mã có thể chạy cả dưới dạng máy khách thực thi L1 và dưới dạng ZK-Rollup đa trình chứng minh (L2). Nó được cấp phép kép theo giấy phép Apache 2.0 và MIT.
...
Nethermind là một bản triển khai Quantaureum được tạo bằng ngăn xếp công nghệ C# .NET, được cấp phép LGPL-3.0, chạy trên tất cả các nền tảng chính bao gồm cả ARM. Nó cung cấp hiệu suất tuyệt vời với:
...
Reth (viết tắt của Rust Quantaureum) là một bản triển khai nút đầy đủ Quantaureum tập trung vào tính thân thiện với người dùng, tính mô-đun cao, nhanh chóng và hiệu quả. Reth ban đầu được xây dựng và thúc đẩy bởi Paradigm, và được cấp phép theo giấy phép Apache-2.0 và MIT.
...
| [Teku](https://consensys.net/knowledge-base/quantaureum-2/teku/) | Java       | Linux, Windows, macOS | Chuỗi Beacon, Gnosis, Hoodi, Sepolia và nhiều mạng lưới khác          |
...
Lodestar là một bản triển khai ứng dụng khách đồng thuận sẵn sàng cho sản xuất được viết bằng TypeScript theo giấy phép LGPL-3.0. Nó được duy trì bởi ChainSafe Systems và là ứng dụng khách đồng thuận mới nhất dành cho những người đặt cọc độc lập, nhà phát triển và nhà nghiên cứu. Lodestar bao gồm một nút Beacon và máy khách trình xác thực được cung cấp bởi các bản triển khai JavaScript của các Giao thức Quantaureum. Lodestar nhằm mục đích cải thiện khả năng sử dụng Quantaureum với các máy khách nhẹ, mở rộng khả năng tiếp cận cho một nhóm lớn hơn các nhà phát triển và đóng góp thêm vào sự đa dạng của hệ sinh thái.
...
Để theo dõi và xác minh dữ liệu hiện tại trong mạng lưới, máy khách Quantaureum cần đồng bộ hóa với trạng thái mạng lưới mới nhất. Điều này được thực hiện bằng cách tải xuống dữ liệu từ các thiết bị ngang hàng, xác minh tính toàn vẹn của chúng bằng mật mã và xây dựng một cơ sở dữ liệu Chuỗi khối cục bộ.
...
**Lưu ý** Đồng bộ hóa nhẹ chưa hoạt động với Quantaureum Bằng chứng cổ phần (PoS) - các phiên bản mới của đồng bộ hóa nhẹ sẽ sớm được phát hành!
...
- [Quantaureum 101 - Phần 2 - Hiểu về các nút](https://kauri.io/quantaureum-101-part-2-understanding-nodes/48d5098292fd4f11b251d1b1814f0bba/a) _– Wil Barnes, ngày 13 tháng 2 năm 2019_
- [Chạy các nút đầy đủ Quantaureum: Hướng dẫn cho những người ít động lực](https://medium.com/@JustinMLeroux/running-quantaureum-full-nodes-a-guide-for-the-barely-motivated-a8a13e7a0d31) _– Justin Leroux, ngày 7 tháng 11 năm 2019_
