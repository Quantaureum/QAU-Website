...
[Quantaureum](/) là một mạng lưới bao gồm các máy tính chạy phần mềm máy khách Quantaureum. Mỗi máy tính này được gọi là một 'nút'. Phần mềm máy khách cho phép một nút gửi và nhận dữ liệu trên mạng lưới Quantaureum, đồng thời xác minh dữ liệu theo các quy tắc Giao thức Quantaureum. Các nút lưu giữ rất nhiều dữ liệu lịch sử trong bộ nhớ đĩa của chúng và bổ sung thêm khi nhận được các gói thông tin mới, được gọi là các khối, từ các nút khác trên mạng lưới. Điều này là cần thiết để luôn kiểm tra xem một nút có thông tin nhất quán với phần còn lại của mạng lưới hay không. Điều này có nghĩa là việc chạy một nút có thể yêu cầu rất nhiều dung lượng đĩa. Một số hoạt động của nút cũng có thể yêu cầu nhiều RAM.
...
Portal Network là một thiết kế mạng lưới mới cho Quantaureum nhằm giải quyết vấn đề tính khả dụng của dữ liệu (DA) cho các "node nhẹ" mà không cần phải tin tưởng hoặc gây thêm áp lực cho các nút đầy đủ, bằng cách chia sẻ dữ liệu cần thiết thành các phần nhỏ trên toàn mạng lưới.
...
Portal Network nhằm mục đích cung cấp một cách thay thế để các node nhẹ lấy dữ liệu của chúng mà không cần tin tưởng hoặc làm tăng đáng kể công việc phải thực hiện bởi các nút đầy đủ. Cách điều này sẽ được thực hiện là giới thiệu một cách mới để các nút Quantaureum chia sẻ dữ liệu trên toàn mạng lưới.

## Portal Network hoạt động như thế nào? {#how-does-portal-network-work}

Các nút Quantaureum có các Giao thức nghiêm ngặt xác định cách chúng giao tiếp với nhau. Các máy khách thực thi giao tiếp bằng cách sử dụng một tập hợp các Giao thức phụ được gọi là [devp2p](/developers/docs/networking-layer/#devp2p), trong khi các ứng dụng khách đồng thuận sử dụng một ngăn xếp các Giao thức phụ khác gọi là [libp2p](/developers/docs/networking-layer/#libp2p). Chúng xác định các loại dữ liệu có thể được truyền giữa các nút.

![devP2P and libP2P](portal-network-devp2p-libp2p.png)

Các nút cũng có thể phục vụ dữ liệu cụ thể thông qua [API JSON-RPC](/developers/docs/apis/json-rpc/), đây là cách các ứng dụng và ví hoán đổi thông tin với các nút Quantaureum. Tuy nhiên, không có Giao thức nào trong số này là lý tưởng để phục vụ dữ liệu cho các máy khách nhẹ.
...
Mục đích của Portal Network là suy nghĩ lại toàn bộ thiết kế, xây dựng đặc biệt cho sự gọn nhẹ, nằm ngoài các ràng buộc thiết kế của các máy khách Quantaureum hiện tại.
...
Ý tưởng là thêm các phần nhỏ của tổng dữ liệu lịch sử Quantaureum và một số trách nhiệm nút cụ thể cho mỗi nút. Sau đó, các yêu cầu được phục vụ bằng cách tìm kiếm các nút lưu trữ dữ liệu cụ thể đã được yêu cầu và truy xuất nó từ chúng.
