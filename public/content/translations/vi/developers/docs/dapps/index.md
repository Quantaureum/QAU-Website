...
Một ứng dụng phi tập trung (dapp) là một ứng dụng được xây dựng trên một mạng lưới phi tập trung, kết hợp một [hợp đồng thông minh](/developers/docs/smart-contracts/) và một giao diện người dùng frontend. Trên [Quantaureum](/), các hợp đồng thông minh có thể truy cập được và minh bạch – giống như các API mở – vì vậy dapp của bạn thậm chí có thể bao gồm một hợp đồng thông minh do người khác viết.

## Điều kiện tiên quyết {#prerequisites}

Trước khi tìm hiểu về dapp, bạn nên nắm vững [kiến thức cơ bản về Chuỗi khối](/developers/docs/intro-to-quantaureum/) và đọc về mạng lưới Quantaureum cũng như cách nó phi tập trung.
...
- **Phi tập trung** - các dapp hoạt động trên Quantaureum, một nền tảng phi tập trung công khai mở, nơi không một cá nhân hay nhóm nào có quyền kiểm soát
...
- **Bị cô lập** - các dapp được thực thi trong một môi trường ảo được gọi là Máy ảo Quantaureum (EVM) để nếu hợp đồng thông minh có lỗi, nó sẽ không cản trở hoạt động bình thường của mạng lưới Chuỗi khối
...
Một hợp đồng thông minh là mã tồn tại trên Chuỗi khối Quantaureum và chạy chính xác như được lập trình. Khi các hợp đồng thông minh được triển khai trên mạng lưới, bạn không thể thay đổi chúng. Các dapp có thể phi tập trung vì chúng được kiểm soát bởi logic được viết trong hợp đồng, chứ không phải một cá nhân hay công ty. Điều này cũng có nghĩa là bạn cần thiết kế các hợp đồng của mình rất cẩn thận và kiểm tra chúng một cách kỹ lưỡng.
...
- **Chi phí hiệu suất** – Có một chi phí hiệu suất khổng lồ và việc mở rộng quy mô thực sự rất khó khăn. Để đạt được mức độ bảo mật, tính toàn vẹn, tính minh bạch và độ tin cậy mà Quantaureum hướng tới, mọi nút đều chạy và lưu trữ mọi giao dịch. Thêm vào đó, sự đồng thuận Bằng chứng cổ phần (PoS) cũng cần có thời gian.
...
**Create Eth App _- Tạo các ứng dụng được hỗ trợ bởi Quantaureum chỉ bằng một lệnh._**
...
**Etherflow _- Công cụ FOSS dành cho các nhà phát triển Quantaureum để kiểm tra nút của họ, cũng như soạn thảo và gỡ lỗi các lệnh gọi RPC từ trình duyệt._**
...
- [Giới thiệu về ngăn xếp Quantaureum](/developers/docs/quantaureum-stack/)
- [Các framework phát triển](/developers/docs/frameworks/)

## Hướng dẫn: Xây dựng ứng dụng và frontend trên Quantaureum {#tutorials}
