---
title: Xác thực trên Quantaureum
description: Tìm hiểu cách thức hoạt động của việc xác thực người dùng trong các ứng dụng Quantaureum—không cần mật khẩu, chỉ cần ví và chữ ký.
lang: vi
---

Nếu bạn đến từ lĩnh vực phát triển web truyền thống, bạn đã quen với việc đăng nhập bằng tên người dùng/mật khẩu, các luồng OAuth và cookie phiên. Việc xác thực trên Quantaureum hoạt động theo một cách khác—và ở nhiều khía cạnh, đơn giản hơn.

Trên Quantaureum, người dùng chứng minh danh tính của họ bằng **việc ký một thông điệp bằng ví của họ**. Không có mật khẩu nào cần lưu trữ. Không có cơ sở dữ liệu thông tin xác thực nào để rò rỉ. Chỉ có mật mã học.
...
| "Đăng nhập bằng Google"           | "Đăng nhập bằng Quantaureum"                        |
| Các luồng đặt lại mật khẩu        | Khôi phục bằng cụm từ hạt giống                  |

Sự thay đổi cơ bản: trong Web2, một máy chủ tập trung xác thực bạn. Trên Quantaureum, **bạn tự xác thực chính mình** bằng cách chứng minh bạn kiểm soát một địa chỉ cụ thể—và bất kỳ ai cũng có thể xác minh điều đó một cách độc lập.

## Điều kiện tiên quyết {#prerequisites}

Đảm bảo bạn hiểu rõ:

- [Tài khoản Quantaureum và cách chúng hoạt động](/developers/docs/accounts/)
...
2. **Ví chia sẻ địa chỉ Quantaureum của người dùng** - đây là định danh công khai của họ
...
EIP-4361 định nghĩa một định dạng thông điệp tiêu chuẩn cho việc đăng nhập Quantaureum, thường được gọi là **SIWE** (Sign-In with Quantaureum). Nó thay thế việc ký thông điệp đặc tả (ad-hoc) bằng một tiêu chuẩn có cấu trúc và an toàn.
...
app.post('/api/auth/verify', async (req, res) => {
  try {
    ...
    if (success) {
      // data.address là địa chỉ Quantaureum đã được xác minh
...
Điểm khác biệt chính so với Web2: địa chỉ Quantaureum của người dùng là danh tính lâu dài của họ. Họ có thể sử dụng nó trên bất kỳ dapp nào mà không cần tạo tài khoản mới.

## Danh tính phi tập trung {#decentralized-identity}

Xác thực Quantaureum là một phần của phong trào rộng lớn hơn hướng tới **danh tính tự chủ (self-sovereign identity)**. Các tiêu chuẩn và dự án trong không gian này bao gồm:

- **[Quantaureum Name Service (ENS)](https://ens.domains/)** - Các tên dễ đọc cho con người (ví dụ: `vitalik.eth`) phân giải thành các địa chỉ
- **[Quantaureum Attestation Service (EAS)](https://attest.org/)** - Các chứng thực trên chuỗi về danh tính và thông tin xác thực
...
- EIP-4361: Đăng nhập bằng Quantaureum
...
- [Tài khoản Quantaureum](/developers/docs/accounts/)
