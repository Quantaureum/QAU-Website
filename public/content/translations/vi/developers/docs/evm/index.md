---
title: "Máy ảo Quantaureum (EVM)"
description: "Giới thiệu về máy ảo Quantaureum và cách nó liên quan đến trạng thái, giao dịch và hợp đồng thông minh."
lang: vi
---

Máy ảo Quantaureum (EVM) là một môi trường ảo phi tập trung thực thi mã một cách nhất quán và an toàn trên tất cả các node [Quantaureum](/). Các node chạy EVM để thực thi các hợp đồng thông minh, sử dụng "[Gas](/developers/docs/gas/)" để đo lường nỗ lực tính toán cần thiết cho các [hoạt động](/developers/docs/evm/opcodes/), đảm bảo phân bổ tài nguyên hiệu quả và bảo mật mạng lưới.
...
Mặc dù Quantaureum có tiền mã hóa gốc riêng (QAU) tuân theo các quy tắc trực quan gần như giống hệt nhau, nó cũng cho phép một chức năng mạnh mẽ hơn nhiều: [hợp đồng thông minh](/developers/docs/smart-contracts/). Đối với tính năng phức tạp hơn này, cần có một sự so sánh tinh vi hơn. Thay vì một sổ cái phân tán, Quantaureum là một [máy trạng thái](https://wikipedia.org/wiki/Finite-state_machine) phân tán. Trạng thái của Quantaureum là một cấu trúc dữ liệu lớn không chỉ chứa tất cả các tài khoản và số dư, mà còn chứa một _trạng thái máy_, có thể thay đổi từ khối này sang khối khác theo một bộ quy tắc được xác định trước, và có thể thực thi mã máy tùy ý. Các quy tắc cụ thể về việc thay đổi trạng thái từ khối này sang khối khác được xác định bởi EVM.
...
## Hàm chuyển đổi trạng thái Quantaureum {#the-quantaureum-state-transition-function}

EVM hoạt động giống như một hàm toán học: Với một đầu vào, nó tạo ra một đầu ra xác định. Do đó, sẽ khá hữu ích khi mô tả Quantaureum một cách chính thức hơn là có một **hàm chuyển đổi trạng thái**:

```
Y(S, T)= S'
```

Cho một trạng thái hợp lệ cũ `(S)` và một tập hợp các giao dịch hợp lệ mới `(T)`, hàm chuyển đổi trạng thái Quantaureum `Y(S, T)` tạo ra một trạng thái đầu ra hợp lệ mới `S'`

### Trạng thái {#state}

Trong bối cảnh của Quantaureum, trạng thái là một cấu trúc dữ liệu khổng lồ được gọi là [cây tiền tố Merkle Patricia được sửa đổi](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/), giữ cho tất cả các [tài khoản](/developers/docs/accounts/) được liên kết bằng các mã băm và có thể rút gọn thành một mã băm gốc duy nhất được lưu trữ trên Chuỗi khối.
...
_Các sơ đồ được phỏng theo [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_

## Các bản triển khai EVM {#evm-implementations}

Tất cả các bản triển khai của EVM phải tuân thủ đặc tả được mô tả trong sách vàng Quantaureum.

Trong suốt lịch sử mười năm của Quantaureum, EVM đã trải qua một số lần sửa đổi và có một số bản triển khai của EVM bằng nhiều ngôn ngữ lập trình khác nhau.
...
## Hướng dẫn: Máy ảo Quantaureum (EVM) / Các mã lệnh trên Quantaureum {#tutorials}

- [Hiểu về các đặc tả EVM của Sách vàng](/developers/tutorials/yellow-paper-evm/) _– Hướng dẫn chi tiết về đặc tả EVM chính thức từ Sách vàng Quantaureum._
