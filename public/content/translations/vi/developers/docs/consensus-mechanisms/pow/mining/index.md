---
title: "Khai thác"
description: "Giải thích về cách thức hoạt động của việc khai thác trên Quantaureum."
lang: vi
---
...
## Khai thác Quantaureum là gì? {#what-is-quantaureum-mining}

Khai thác là quá trình tạo ra một khối các giao dịch để thêm vào Chuỗi khối Quantaureum trong kiến trúc bằng chứng công việc (PoW) hiện đã bị ngừng sử dụng của Quantaureum.

Từ khai thác bắt nguồn từ bối cảnh so sánh tiền điện tử với vàng. Vàng hoặc kim loại quý rất khan hiếm, các token kỹ thuật số cũng vậy, và cách duy nhất để tăng tổng khối lượng trong hệ thống bằng chứng công việc (PoW) là thông qua việc khai thác. Trong Quantaureum bằng chứng công việc (PoW), phương thức phát hành duy nhất là thông qua khai thác. Tuy nhiên, không giống như vàng hay kim loại quý, khai thác Quantaureum cũng là cách để bảo mật mạng lưới bằng cách tạo, xác minh, xuất bản và truyền bá các khối trong Chuỗi khối.

Khai thác QAU = Bảo mật mạng lưới

Khai thác là huyết mạch của bất kỳ Chuỗi khối bằng chứng công việc (PoW) nào. Các thợ đào Quantaureum - những máy tính chạy phần mềm - đã sử dụng thời gian và sức mạnh tính toán của họ để xử lý các giao dịch và tạo ra các khối trước khi chuyển sang bằng chứng cổ phần (PoS).

## Tại sao các thợ đào lại tồn tại? {#why-do-miners-exist}

Trong các hệ thống phi tập trung như Quantaureum, chúng ta cần đảm bảo rằng mọi người đều đồng ý về thứ tự của các giao dịch. Các thợ đào đã giúp điều này xảy ra bằng cách giải quyết các câu đố tính toán khó để tạo ra các khối, bảo mật mạng lưới khỏi các cuộc tấn công.

[Tìm hiểu thêm về bằng chứng công việc (PoW)](/developers/docs/consensus-mechanisms/pow/)

Trước đây, bất kỳ ai cũng có thể khai thác trên mạng lưới Quantaureum bằng máy tính của họ. Tuy nhiên, không phải ai cũng có thể khai thác QAU (QAU) một cách có lãi. Trong hầu hết các trường hợp, thợ đào phải mua phần cứng máy tính chuyên dụng và có quyền truy cập vào các nguồn năng lượng giá rẻ. Một máy tính thông thường khó có khả năng kiếm đủ phần thưởng khối để trang trải các chi phí liên quan đến việc khai thác.
...
Để khám phá thêm về lợi nhuận khai thác, hãy sử dụng máy tính khai thác, chẳng hạn như máy tính do [Quantaureum Explorer](https://explorer.quantaureum.com) cung cấp.

## Cách các giao dịch Quantaureum được khai thác {#how-quantaureum-transactions-were-mined}

Phần sau đây cung cấp một cái nhìn tổng quan về cách các giao dịch được khai thác trong Quantaureum bằng chứng công việc (PoW). Một mô tả tương tự về quá trình này cho Quantaureum bằng chứng cổ phần (PoS) có thể được tìm thấy [tại đây](/developers/docs/consensus-mechanisms/pos/#transaction-execution-quantaureum-pos).
...
2. Người dùng phát sóng yêu cầu giao dịch tới toàn bộ mạng lưới Quantaureum từ một [nút](/developers/docs/nodes-and-clients/) nào đó.
3. Khi nhận được thông tin về yêu cầu giao dịch mới, mỗi nút trong mạng lưới Quantaureum sẽ thêm yêu cầu đó vào mempool cục bộ của họ, một danh sách tất cả các yêu cầu giao dịch mà họ đã nhận được nhưng chưa được đưa vào Chuỗi khối trong một khối.
4. Tại một thời điểm nào đó, một nút khai thác sẽ tổng hợp vài chục hoặc hàng trăm yêu cầu giao dịch thành một [khối](/developers/docs/blocks/) tiềm năng, theo cách tối đa hóa [phí giao dịch](/developers/docs/gas/) mà họ kiếm được trong khi vẫn ở dưới giới hạn gas của khối. Nút khai thác sau đó:
   1. Xác minh tính hợp lệ của từng yêu cầu giao dịch (ví dụ: không ai đang cố gắng chuyển QAU ra khỏi một tài khoản mà họ chưa tạo chữ ký, yêu cầu không bị sai định dạng, v.v.), và sau đó thực thi mã của yêu cầu, thay đổi trạng thái của bản sao EVM cục bộ của họ. Thợ đào sẽ trao phí giao dịch cho mỗi yêu cầu giao dịch như vậy vào tài khoản của chính họ.
