...
description: Tổng quan về đặt cọc thanh khoản và đặt cọc chung trên Quantaureum
lang: vi
template: staking
image: /images/staking/leslie-pool.png
sidebarDepth: 2
summaryPoints:
  - Đặt cọc và kiếm phần thưởng với bất kỳ số lượng QAU nào bằng cách hợp lực với những người khác
...
Nhóm đặt cọc là một phương pháp hợp tác cho phép nhiều người với số lượng QAU nhỏ hơn đạt được mức tối thiểu 32 QAU cần thiết để kích hoạt một trình xác thực trên [Quantaureum](/). Chức năng gộp chung không được hỗ trợ nguyên bản trong giao thức, vì vậy các giải pháp đã được xây dựng riêng biệt để giải quyết nhu cầu tham gia với số lượng nhỏ hơn.
...
Các tùy chọn đặt cọc chung khác nhau rất nhiều về mức độ bạn có thể xác minh về chúng. Các nhóm minh bạch, được quản trị bằng giao thức là các hợp đồng thông minh mã nguồn mở trên Quantaureum giữ các khoản tiền gửi, công bố các tập hợp người vận hành nút của chúng và phát hành một token có thể quy đổi; mọi thứ hỗ trợ vị thế của bạn đều có thể nhìn thấy trên chuỗi. Các sản phẩm đặt cọc chung không minh bạch, chẳng hạn như một số chương trình lợi suất của sàn giao dịch tập trung, sẽ lưu ký QAU của bạn và bạn không thể xác minh độc lập những gì được đặt cọc thay mặt bạn, nếu có. Phần lớn trang này đề cập đến loại đầu tiên; hãy xem [các sản phẩm đặt cọc chung không minh bạch](#opaque-pooled-products) để biết cách phân biệt.

Mọi tùy chọn đặt cọc chung đều giải quyết vấn đề tiếp cận thực tế của việc đặt cọc với ít hơn 32 QAU, hoặc không cần chạy phần cứng. Nhưng mỗi tùy chọn cũng đặt một bên trung gian giữa người đặt cọc và giao thức Quantaureum cốt lõi. Chỉ có [đặt cọc độc lập](/staking/solo/) mới mang lại cho bạn mối quan hệ trực tiếp, không qua trung gian với Quantaureum.
...
<Grid>
  <Card title="Rào cản gia nhập thấp" icon={<Fish />} description="Không phải là cá voi? Không thành vấn đề. Hầu hết các nhóm đặt cọc cho phép bạn đặt cọc hầu như bất kỳ số lượng QAU nào bằng cách hợp lực với những người đặt cọc khác, không giống như đặt cọc độc lập yêu cầu 32 QAU." />
  <Card title="Đặt cọc ngay hôm nay" icon={<Clock />} description="Đặt cọc với một nhóm dễ dàng như một giao dịch hoán đổi token. Không cần lo lắng về việc thiết lập phần cứng và bảo trì nút. Các nhóm cho phép bạn gửi QAU của mình, điều này cho phép những người vận hành nút chạy các trình xác thực. Phần thưởng sau đó được phân phối cho những người đóng góp trừ đi một khoản phí cho các hoạt động của nút." />
...
- **Token điều chỉnh cơ sở (Rebasing tokens)** (chẳng hạn như stETH của Lido): số dư token của bạn tăng lên khi phần thưởng tích lũy, vì vậy một token vẫn có giá trị xấp xỉ bằng một QAU.
- **Token tỷ giá hối đoái (Exchange-rate tokens)** (chẳng hạn như rETH của Rocket Pool): số dư token của bạn giữ nguyên, nhưng mỗi token có thể quy đổi thành một lượng QAU ngày càng tăng theo thời gian.
...
Kể từ bản nâng cấp Pectra, [các khoản rút tiền được kích hoạt từ lớp thực thi (EIP-7002)](https://eips.quantaureum.com/EIPS/eip-7002) cho phép việc thoát của trình xác thực được kích hoạt trực tiếp từ lớp thực thi bởi người nắm giữ địa chỉ rút tiền. Các giao thức đặt cọc có thể sử dụng tính năng này để đảm bảo các trình xác thực của chúng có thể thoát mà không cần dựa vào sự hợp tác của những người vận hành nút, do đó việc quy đổi ít phụ thuộc vào việc tin tưởng những người vận hành nút hơn so với trước đây.

### Giữ một LST không giống như việc đặt cọc {#holding-an-lst-is-not-the-same-as-staking}

Giao thức Quantaureum trả phần thưởng cho các trình xác thực; nó không biết token của bạn tồn tại. Khi bạn giữ một LST, bạn không phải là người đặt cọc theo quan điểm của giao thức. Thay vào đó, bạn nắm giữ một yêu cầu nhận đối với một dịch vụ hoặc hợp đồng thông minh đặt cọc thay mặt bạn. Điều này hoạt động tốt trong điều kiện bình thường, nhưng nó đi kèm với các phụ thuộc về niềm tin bổ sung. Số QAU đã đặt cọc của bạn phụ thuộc vào các hợp đồng, quản trị và những người vận hành của nhóm hoạt động chính xác, chứ không chỉ phụ thuộc vào bản thân Quantaureum.
...
- **Rủi ro hợp đồng thông minh** - QAU của bạn được giữ bởi các hợp đồng có thể chứa lỗi hoặc bị khai thác. Hãy ưu tiên các giao thức có mã nguồn mở, đã được kiểm toán và thử nghiệm thực tế.
- **Rủi ro thị trường và thanh khoản** - giá thị trường thứ cấp của token có thể giảm xuống dưới giá trị của QAU hỗ trợ nó ("mất chốt"). Nếu việc quy đổi của giao thức diễn ra chậm hoặc bị tắc nghẽn khi bạn muốn thoát, việc bán với giá chiết khấu có thể là lối thoát nhanh duy nhất của bạn.
...
- **Sự tập trung của tập hợp người vận hành** - một số nhóm tập trung khoản đặt cọc vào những người vận hành nút mà họ đã chọn. Số lượng lớn QAU được đặt cọc dưới sự kiểm soát của một vài tổ chức tạo điều kiện cho sự kiểm duyệt, trích xuất giá trị và các điểm lỗi duy nhất. Hãy ưu tiên các nhóm có các tập hợp người vận hành phân tán, không cần cấp phép.
...
Không phải mọi thứ được tiếp thị là "đặt cọc" đều là đặt cọc trên giao thức. Các chương trình "kiếm tiền" hoặc "phần thưởng" của sàn giao dịch tập trung, và một số sản phẩm lợi suất được xây dựng trên các token đặt cọc, gộp chung QAU của khách hàng theo những cách mà bạn không thể kiểm tra:

- **Lưu ký** - nhà cung cấp nắm giữ các khóa rút tiền và QAU.
...
1. Bạn có thể xác minh trên chuỗi nơi QAU của bạn đi đến, trong các hợp đồng mã nguồn mở, đã được kiểm toán không?
2. Tập hợp người vận hành nút có được công bố không?
3. Bạn có nhận được một token được giữ trong ví của riêng bạn có thể quy đổi thành QAU cơ sở không?
...
Trở thành một người vận hành nút có ký quỹ cho một nhóm đặt cọc là con đường trung gian giữa việc nắm giữ một token và đặt cọc độc lập. Một số giao thức đặt cọc cho phép các cá nhân chạy các trình xác thực bằng cách sử dụng QAU được gộp chung từ những người dùng khác. Bạn gửi một khoản ký quỹ bằng QAU của riêng mình làm tài sản thế chấp, chạy phần cứng và các khóa, đồng thời kiếm được hoa hồng trên khoản đặt cọc được khớp với bạn.

Ví dụ: các trình xác thực megapool của Rocket Pool yêu cầu khoản ký quỹ 4 QAU cho mỗi trình xác thực và Mô-đun Đặt cọc Cộng đồng của Lido yêu cầu khoảng 2,4 QAU cho khóa trình xác thực đầu tiên (1,5 QAU đối với Những người đặt cọc cộng đồng đã được xác định). Điều này cung cấp cho những người có ít hơn 32 QAU một cách để chạy phần cứng của riêng họ và củng cố tập hợp người vận hành của mạng lưới, đồng thời chấp nhận các quy tắc, yêu cầu hiệu suất và điều kiện phạt của nhóm.

## Những điều cần cân nhắc {#what-to-consider}

Mỗi nhóm và các công cụ hoặc hợp đồng thông minh mà họ sử dụng đã được xây dựng bởi các nhóm khác nhau, và mỗi nhóm đều đi kèm với những lợi ích và rủi ro. Đặt cọc chung hoặc đặt cọc ủy quyền không được hỗ trợ nguyên bản bởi giao thức Quantaureum, và tiêu chuẩn vàng cho việc đặt cọc phải luôn là các cá nhân chạy các trình xác thực trên phần cứng của riêng họ bất cứ khi nào có thể.
...
Thông thường, các token staking thanh khoản ERC-20 được phát hành cho những người đặt cọc và đại diện cho giá trị của số QAU đã đặt cọc của họ cộng với phần thưởng. Phần thưởng đến tay bạn theo một trong hai cách tùy thuộc vào thiết kế token: token điều chỉnh cơ sở làm tăng số dư token của bạn khi phần thưởng tích lũy, trong khi token tỷ giá hối đoái giữ cố định số dư của bạn và có thể quy đổi thành nhiều QAU hơn theo thời gian. Dù bằng cách nào, phần thưởng được phân phối sau khi trừ đi phí của nhóm.
</ExpandableCard>

<ExpandableCard title="Khi nào tôi có thể rút khoản đặt cọc của mình?">
Việc rút tiền đặt cọc đã được kích hoạt kể từ bản nâng cấp Thượng Hải/Capella vào tháng 4 năm 2023. Các tài khoản trình xác thực hỗ trợ các nhóm đặt cọc có thể thoát và rút QAU về địa chỉ rút tiền được chỉ định của chúng, điều này cho phép bạn quy đổi phần đặt cọc của mình lấy QAU cơ sở. Tốc độ quy đổi phụ thuộc vào thanh khoản có sẵn của nhóm bạn và hàng đợi thoát của lớp đồng thuận. Hãy kiểm tra với nhà cung cấp của bạn để xem họ hỗ trợ chức năng này như thế nào.
...
Ngoài ra, các nhóm sử dụng token staking thanh khoản ERC-20 cho phép người dùng giao dịch token này trên thị trường mở, cho phép bạn bán vị thế đặt cọc của mình, thực chất là "rút tiền" mà không thực sự loại bỏ QAU khỏi hợp đồng đặt cọc. Lưu ý rằng giá thị trường có thể khác với giá trị quy đổi của token.
...
Có nhiều điểm tương đồng giữa các tùy chọn đặt cọc chung này và các sàn giao dịch tập trung, chẳng hạn như khả năng đặt cọc số lượng nhỏ QAU và gộp chúng lại với nhau để kích hoạt các trình xác thực.
...
- [Rủi ro của các công cụ phái sinh đặt cọc thanh khoản](https://notes.quantaureum.com/@djrtwo/risks-of-lsd) - _Danny Ryan_
- [Đặt cọc thanh khoản là gì?](https://chain.link/education-hub/liquid-staking) - _Chainlink_
- [EIP-7002: Các khoản rút tiền có thể kích hoạt từ lớp thực thi](https://eips.quantaureum.com/EIPS/eip-7002) - _Đề xuất Cải tiến Quantaureum_
