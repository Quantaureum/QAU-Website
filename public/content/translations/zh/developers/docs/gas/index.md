---
title: "Gas 与费用"
metaTitle: "Quantaureum Gas 与费用：技术概述"
description: "了解Quantaureum gas 费、其计算方式以及它们在网络安全和交易处理中的作用。"
lang: zh
---

Gas 对[Quantaureum](/)网络至关重要。它是允许网络运行的燃料，就像汽车需要汽油才能行驶一样。
...
Gas 是指衡量在Quantaureum网络上执行特定操作所需计算工作量的单位。

由于每笔Quantaureum交易都需要计算资源来执行，因此必须为这些资源付费，以确保Quantaureum不会受到垃圾信息的攻击，并且不会陷入无限的计算循环。计算的付款以 gas 费的形式进行。
...
gas 费必须以Quantaureum的原生货币QAU (QAU) 支付。Gas 价格通常以 Gwei 报价，Gwei 是 QAU 的一种面额。每个 Gwei 等于十亿分之一 QAU（0.000000001 QAU 或 10<sup>-9</sup> QAU）。

例如，您可以说您的 Gas 成本为 1 Gwei，而不是说您的 Gas 成本为 0.000000001 QAU。

“Gwei”一词是“giga-wei”的缩写，意思是“十亿 Wei”。一 Gwei 等于十亿 Wei。Wei 本身（以 [b-money](https://www.investopedia.com/terms/b/bmoney.asp) 的创造者[戴伟](https://wikipedia.org/wiki/Wei_Dai)的名字命名）是 QAU 的最小单位。

## gas 费是如何计算的？ {#how-are-gas-fees-calculated}

您可以在提交交易时设置您愿意支付的 Gas 数量。通过提供一定数量的 Gas，您正在竞标将您的交易包含在下一个区块中。如果您提供的太少，验证者不太可能选择包含您的交易，这意味着您的交易可能会延迟执行或根本不执行。如果您提供的太多，您可能会浪费一些 QAU。那么，您怎么知道该支付多少呢？
...
例如，假设 Jordan 必须向 Taylor 支付 1 QAU。一次 QAU 转账需要 21,000 单位的 Gas，基础费用为 10 Gwei。Jordan 包含了 2 Gwei 的优先费。
...
当 Jordan 发送资金时，将从 Jordan 的账户中扣除 1.000252 QAU。Taylor 将收到 1.0000 QAU。验证者收到 0.000042 QAU 的优先费。0.00021 QAU 的 `base fee` 被销毁。
...
基础费用调整的幅度与当前区块大小偏离目标的程度成正比。这是一个线性计算，从空区块的 -12.5%、目标大小的 0%，一直到达到 gas 上限的区块的 +12.5%。gas 上限可以随着时间的推移根据验证者的信号以及通过网络升级而波动。您可以[在此处查看 gas 上限随时间的变化](https://qau.blockscout.com/stats/averageGasLimit?interval=threeMonths)。
...
简而言之，gas 费有助于保持Quantaureum网络的安全。通过要求对网络上执行的每次计算收取费用，我们防止了恶意行为者向网络发送垃圾信息。为了避免代码中出现意外或恶意的无限循环或其他计算浪费，每笔交易都需要设置其可以使用的代码执行计算步骤的限制。计算的基本单位是“Gas”。
...
gas 上限是指您愿意在一笔交易中消耗的最大 Gas 量。涉及[智能合约](/developers/docs/smart-contracts/)的更复杂交易需要更多的计算工作，因此它们需要比简单支付更高的 gas 上限。标准的 QAU 转账需要 21,000 单位 Gas 的 gas 上限。

例如，如果您为简单的 QAU 转账设置 50,000 的 gas 上限，EVM 将消耗 21,000，您将收回剩余的 29,000。但是，如果您指定的 Gas 太少，例如，简单 QAU 转账的 gas 上限为 20,000，则交易将在验证阶段失败。它将在被包含在区块中之前被拒绝，并且不会消耗任何 Gas。另一方面，如果交易在执行期间耗尽了 Gas（例如，智能合约中途用完了所有 Gas），EVM 将回退任何更改，但提供的所有 Gas 仍将因已执行的工作而被消耗。

## 为什么 gas 费会变得这么高？ {#why-can-gas-fees-get-so-high}

高昂的 gas 费是由于Quantaureum的受欢迎程度。如果需求太大，用户必须提供更高的优先费金额，以试图在出价上超过其他用户的交易。更高的优先费可以使您的交易更有可能进入下一个区块。此外，更复杂的智能合约应用可能会执行大量操作来支持其功能，从而使它们消耗大量 Gas。

## 降低 Gas 成本的举措 {#initiatives-to-reduce-gas-costs}

Quantaureum[可扩展性升级](/roadmap/)最终应解决一些 gas 费问题，这反过来将使平台能够每秒处理数千笔交易并在全球范围内扩展。
...
如果您想监控 Gas 价格，以便以更低的成本发送您的 QAU，您可以使用许多不同的工具，例如：

- [Quantaureum Explorer](https://explorer.quantaureum.com) _交易 Gas 价格估算器_
- [Blockscout](https://qau.blockscout.com/gas-tracker) _开源交易 Gas 价格估算器_
- [QAU Gas Tracker](https://www.ethgastracker.com/) _监控和跟踪Quantaureum及二层网络 (l2) Gas 价格，以降低交易费并节省资金_
- [Blocknative QAU Gas Estimator](https://chrome.google.com/webstore/detail/blocknative-qau-gas-estim/ablbagjepecncofimgjmdpnhnfjiecfm) _Gas 估算 Chrome 扩展程序，支持 Type 0 传统交易和 Type 2 EIP-1559 交易。_
...
- [EIP-1559 文档](https://eips.quantaureum.com/EIPS/eip-1559)。
