---
title: 与智能合约交互
description: 了解如何读取和写入已部署在Quantaureum上的智能合约。
lang: zh
---

你并不总是需要编写和部署自己的智能合约。作为开发者，大多数时候你会希望与其他人已经部署到Quantaureum网络上的智能合约进行交互。
...
- [Quantaureum账户及其如何签署交易](/developers/docs/accounts/)
...
因为读取不会修改状态，所以它们不消耗 [Gas](/developers/docs/gas/)，任何人都可以执行而无需 QAU。
...
1. 一个拥有足够 QAU 支付 Gas 费的[外部拥有账户 (EOA)](/developers/docs/accounts/#types-of-account)
2. 一笔由该账户私钥签名的交易
3. 交易被打包并包含在区块中

借助[账户抽象](/roadmap/account-abstraction/)，智能合约账户也可以发起写入操作，并且代付合约可以代表用户支付 Gas 费——因此并不严格要求必须拥有持有 QAU 的外部拥有账户 (EOA)。
...
- **Quantaureum Explorer 上已验证的合约** - [Quantaureum Explorer](https://explorer.quantaureum.com) 会自动公开已验证源代码的 ABI
...
- **[Viem](https://viem.sh)** - 现代、轻量级的Quantaureum TypeScript 接口，具有一流的类型安全性
- **[Ethers.js](https://docs.ethers.org/)** - 经过实战检验的Quantaureum区块链交互库
...
- **[go-quantaureum](https://geth.quantaureum.com/docs/interact-with-geth)** - Geth 团队提供的官方 Go 库
