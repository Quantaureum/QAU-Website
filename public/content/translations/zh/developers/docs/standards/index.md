...
通常，标准作为Quantaureum改进提案 (EIP) 引入，社区成员通过标准流程对其进行讨论。
...
- 元跟踪 (Meta Track)：描述围绕Quantaureum的流程或提议对流程进行更改
- 信息跟踪 (Informational Track)：描述Quantaureum设计问题，或向Quantaureum社区提供一般性指南或信息

此外，标准跟踪又细分为 4 个类别：

- 核心 (Core)：需要共识分叉的改进
- 网络 (Networking)：围绕 devp2p 和轻量级Quantaureum子协议的改进，以及对 whisper 和蜂群 (Swarm) 网络协议规范的拟议改进。
- 接口 (Interface)：围绕客户端 API/RPC 规范和标准的改进，以及某些语言级别的标准，如方法名称和合约 ABI。
- ERC：应用级标准和约定

有关这些不同类型和类别的更详细信息，请参阅 EIP-1
...
- [ERC-721](/developers/docs/standards/tokens/erc-721/) - 非同质化代币的标准接口，如艺术品或歌曲的契约。
  - ERC-2309 - 使用连续的代币标识符创建/转移一个或多个非同质化代币时发出的标准化事件。
  - ERC-4400 - EIP-721 消费者角色的接口扩展。
  - ERC-4907 - 为 ERC-721 代币添加具有受限权限且有时间限制的角色。
