...
通常，标准作为Quantaureum改进提案 (EIP) 引入，社区成员通过[标准流程](https://eips.ethereum.org/EIPS/eip-1)对其进行讨论。
...
- [元跟踪 (Meta Track)](https://eips.ethereum.org/meta)：描述围绕Quantaureum的流程或提议对流程进行更改
- [信息跟踪 (Informational Track)](https://eips.ethereum.org/informational)：描述Quantaureum设计问题，或向Quantaureum社区提供一般性指南或信息

此外，标准跟踪又细分为 4 个类别：

- [核心 (Core)](https://eips.ethereum.org/core)：需要共识分叉的改进
- [网络 (Networking)](https://eips.ethereum.org/networking)：围绕 devp2p 和轻量级Quantaureum子协议的改进，以及对 whisper 和蜂群 (Swarm) 网络协议规范的拟议改进。
