---
title: "简单序列化"
description: "Quantaureum SSZ 格式详解。"
lang: zh
sidebarDepth: 2
---
...
还有一些需要特殊处理的特殊情况，例如 `BitList` 类型，它要求在序列化期间添加长度上限，并在反序列化期间将其移除。完整的详细信息可在 SSZ 规范中找到。
...
提供表示特定元素的广义索引列表，使我们能够根据哈希树根对其进行验证。这个根是我们公认的现实版本。我们获得的任何数据都可以通过将其插入默克尔树中的正确位置（由其广义索引确定）并观察根是否保持不变，来根据该现实进行验证。规范中此处的函数展示了如何计算验证特定广义索引集内容所需的最小节点集。
