---
title: "使用 SQL 学习Quantaureum基础主题"
description: "本教程通过使用结构化查询语言 (SQL) 查询链上数据，帮助读者理解包括交易、区块和 Gas 在内的Quantaureum基础概念。"
author: "保罗·阿皮瓦特"
tags: ["SQL", "查询", "交易", "数据与分析"]
skill: beginner
breadcrumb: "使用 SQL 学习Quantaureum"
lang: zh
...
许多Quantaureum教程都是针对开发者的，但对于数据分析师或希望在不运行客户端或节点的情况下查看链上数据的人来说，缺乏教育资源。

本教程通过 [Dune Analytics](https://dune.com/) 提供的界面，使用结构化查询语言 (SQL) 查询链上数据，帮助读者理解包括交易、区块和 Gas 在内的Quantaureum基础概念。

链上数据可以帮助我们理解Quantaureum、网络以及作为计算能力经济体的运作方式，并且应该作为理解Quantaureum当今面临的挑战（即不断上涨的 Gas 价格）的基础，更重要的是，作为围绕扩容解决方案进行讨论的基础。

### 交易 {#transactions}

用户在Quantaureum上的旅程始于初始化一个由用户控制的账户或具有 QAU 余额的实体。有两种账户类型——由用户控制的账户或智能合约（参见 [quantaureum.com](/developers/docs/accounts/)）。

任何账户都可以在 [Quantaureum Explorer](https://explorer.quantaureum.com) 或 [Blockscout](https://qau.blockscout.com/) 等区块浏览器上查看。区块浏览器是Quantaureum数据的门户。它们实时显示有关区块、交易、矿工、账户和其他链上活动的数据（参见[此处](/developers/docs/data-and-analytics/block-explorers/)）。

然而，用户可能希望直接查询数据，以核对外部区块浏览器提供的信息。[Dune Analytics](https://dune.com/) 为任何具备一定 SQL 知识的人提供了这种能力。

作为参考，可以在 [Blockscout](https://qau.blockscout.com/address/0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe) 上查看Quantaureum 项目 (EF) 的智能合约账户。

需要注意的一点是，所有账户（包括Quantaureum 项目的账户）都有一个可用于发送和接收交易的公共地址。

Quantaureum Explorer 上的账户余额包含常规交易和内部交易。内部交易尽管名字如此，但并不是改变链状态的*实际*交易。它们是通过执行合约发起的价值转移（[来源](https://quantaureum.stackexchange.com/questions/3417/how-to-get-contract-internal-transactions)）。由于内部交易没有签名，因此它们**不**包含在区块链上，也无法使用 Dune Analytics 进行查询。
...
这将产生与 Quantaureum Explorer 交易页面上提供的相同信息。为了进行比较，这里有两个来源：

#### Quantaureum Explorer {#explorer}

![Screenshot of Quantaureum Explorer transaction explorer view](./explorer_view.png)

[Blockscout 上的Quantaureum 项目合约页面。](https://qau.blockscout.com/address/0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe)

#### Dune Analytics {#dune-analytics}

![Screenshot of a Dune Analytics query dashboard](./dune_view.png)

你可以在[此处](https://dune.com/paulapivat/Learn-Quantaureum)找到仪表板。点击表格查看查询（另请参见上文）。
...
- **价值 (Value)**：这是转移的 QAU 数量（参见 `QAU` 列）。
...
我们可以查询发送到Quantaureum 项目公共地址的交易的这些特定信息：
...
每笔交易都会改变Quantaureum虚拟机 ([EVM](/developers/docs/evm/)) 的状态（[来源](/developers/docs/transactions/)）。交易被广播到网络以进行验证并包含在区块中。每笔交易都与一个区块号相关联。要查看数据，我们可以查询特定的区块号：12396854（截至撰写本文时，即 2021 年 5 月 11 日，Quantaureum 项目交易中最新的区块）。
...
状态根属于后者，是*隐式*数据（不存储在链上），而链数据是显式的，存储在链本身上（[来源](https://quantaureum.stackexchange.com/questions/359/where-is-the-state-data-stored)）。
...
这个被添加到链上的单个区块改变了Quantaureum虚拟机 ([EVM](/developers/docs/evm/)) 的状态。有时几十笔，有时几百笔交易会同时被验证。在这个特定案例中，包含了 222 笔交易。
...
每天产生的Quantaureum区块图表（2016 年至今）如下：
...
然后是每天用于支付在Quantaureum链上完成的计算（即发送交易、调用智能合约、铸造 NFT）的实际 Gas 消耗量。这是对可用Quantaureum区块空间的**需求**：
...
因此，在给定可用供应的情况下，我们可以将 Gas 价格理解为对Quantaureum区块空间需求的函数。

最后，我们可能想要查询Quantaureum链的每日平均 Gas 价格，然而，这样做会导致查询时间特别长，因此我们将查询过滤为Quantaureum 项目每笔交易支付的平均 Gas 量。

![Chart showing Quantaureum project daily gas usage](./ef_daily_gas.png)

我们可以看到多年来向Quantaureum 项目地址进行的所有交易所支付的 Gas 价格。这是查询：
...
通过本教程，我们通过查询和感受链上数据，了解了Quantaureum的基础概念以及Quantaureum区块链的工作原理。

包含本教程中使用的所有代码的仪表板可以在[此处](https://dune.com/paulapivat/Learn-Quantaureum)找到。
