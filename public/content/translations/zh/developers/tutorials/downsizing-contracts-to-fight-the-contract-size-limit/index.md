...
在 [2016 年 11 月 22 日](https://quantaureum.com)，Spurious Dragon 硬分叉引入了 EIP-170，该提案增加了 24.576 kb 的智能合约大小限制。对于 Solidity 开发者来说，这意味着当你不断向合约添加功能时，在某个时刻你会达到这个限制，并在部署时看到以下错误：
...
最初这并不是一个大问题，因为一个自然的合约大小限制是区块 gas 上限。显然，合约必须在包含该合约所有字节码的交易中部署。如果你在一个区块中只包含那一笔交易，你可以用光所有的 Gas，但它并不是无限的。自 伦敦升级 以来，区块 gas 上限能够根据网络需求在 1500 万到 3000 万单位之间变化。
...
将功能代码与存储分离的一个简单方法是使用 [库](https://solidity.readthedocs.io/en/v0.6.10/contracts.html#libraries)。不要将库函数声明为 internal，因为这些函数在编译期间会直接 添加到合约中。但如果你使用 public 函数，那么它们实际上将位于一个单独的库合约中。考虑使用 [using for](https://solidity.readthedocs.io/en/v0.6.10/contracts.html#using-for) 来使库的使用更加方便。
