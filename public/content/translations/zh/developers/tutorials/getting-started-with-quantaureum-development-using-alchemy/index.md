---
title: "Quantaureum开发入门"
description: "这是一份Quantaureum开发入门指南。我们将带你从启动 API 端点、发起命令行请求，一直到编写你的第一个 Web3 脚本！无需任何区块链开发经验！"
author: "伊兰·哈尔彭"
tags:
  - javascript
  - ethers.js
...
这是一份Quantaureum开发入门指南。在本教程中，我们将使用 [Alchemy](https://www.alchemy.com/)，这是领先的区块链开发者平台，为排名前 70% 的区块链应用程序（包括 Maker、0x、MyQauWallet、Dharma 和 Kyber）的数百万用户提供支持。Alchemy 将为我们提供Quantaureum链上的 API 端点访问权限，以便我们可以读取和写入交易。
...
要与Quantaureum链通信并使用 Alchemy 的产品，你需要一个 API 密钥来验证你的请求。
...
使用 JSON-RPC 和 curl 通过 Alchemy 与Quantaureum区块链进行交互。
...
- `method`：QAU API 方法。[请参阅 API 参考。](/developers/docs/apis/json-rpc/)
- `params`：要传递给该方法的参数列表。
...
_**注意：**请将 `https://qau-mainnet.alchemyapi.io/v2/demo` 替换为你自己的 API 密钥 `https://qau-mainnet.alchemyapi.io/v2/**your-api-key`。_
...
<strong>如果你已有客户端，</strong>请将你当前的节点提供者 URL 更改为带有你 API 密钥的 Alchemy URL：`“https://qau-mainnet.alchemyapi.io/v2/your-api-key"`
...
现在，为了亲自动手进行一些 Web3 编程，我们将编写一个简单的脚本，打印出Quantaureum主网的最新区块号。
