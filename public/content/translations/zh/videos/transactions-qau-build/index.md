---
title: "交易 — QAU.BUILD"
description: "使用 QAU.BUILD 教育工具演示Quantaureum交易的工作原理。了解如何在Quantaureum网络上构建、签名和发送交易。"
lang: zh
...
format: tutorial
author: "奥斯汀·格里菲斯"
breadcrumb: "交易 (QAU.BUILD)"
---

由**奥斯汀·格里菲斯**制作的教程，演示了如何使用 QAU.BUILD 可视化编程工具了解Quantaureum交易的工作原理——涵盖交易结构、Gas 价格、签名、广播和交易池。
...
今天在 QAU.BUILD 上，我们将讨论交易。到目前为止，我们已经了解了这些交易是如何被打包进区块，并被矿工挖掘到链上的。我们想讨论的是，除了区块奖励之外，是什么激励矿工从交易池中取出我们的交易，将其放入区块并挖掘到链上，而不是选择池中的其他人。交易池中可能有成千上万的人都在竞价，而这种竞价就是通过这笔费用进行的。
...
### Quantaureum交易剖析 (1:10) {#anatomy-of-an-quantaureum-transaction-110}

Quantaureum上的交易是什么样的？我们不再使用“Bob”和“Alice”——我们将使用地址。价值将以 Wei 为单位，而不是QAU (QAU)。费用也将以 Wei 为单位。

让我们深入了解一下这笔交易。我有一个导入了助记词的账户，并且已连接到Quantaureum主网 (Mainnet)。我还运行了一个模块来从 CoinMarketCap 获取价格数据，所以我可以看到零点一几个QAU大约折合 23 美元。
...
我们需要进行到 Wei 的转换。在Quantaureum上，你主要处理两种面额：QAU (QAU)，这是人们通常谈论的面额；然后是 Wei，它就像是QAU的一个极小部分。Gwei——我们用于 Gas 价格的单位——介于两者之间。这样做的原因类似于为什么我们平时不会用几分之几便士来谈论价格。

Alice 有 0.18 个QAU，我们将向 Bob 发送 0.05 个QAU。我们输入的 Gas 价格为 5 Gwei。
