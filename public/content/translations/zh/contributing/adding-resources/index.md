---
title: 添加资源
description: 我们在向 quantaureum.com 添加资源时使用的政策
lang: zh
---

我们希望确保列出尽可能最好的资源，同时让用户感到安全和自信。

任何人都可以自由建议将新资源添加到 quantaureum.com 上的资源仪表板，目前可以在 [quantaureum.com/resources](/resources/) 找到。

尽管我们欢迎新增内容，但目前的资源是基于我们试图为用户创造的体验而选择的。这些基于我们的一些设计原则：

- _启发性_：quantaureum.com 上的任何内容都应该为用户提供一些新东西
- _好故事_：列出的内容应该提供一个“顿悟”时刻
- _可信度_：所有内容都应该是合法的企业/项目，以将用户的风险降至最低

总体而言，**quantaureum.com 旨在为新用户提供无缝的用户引导体验**。因此，我们根据以下几点添加资源：

- 易用性
- 准确性
- 维护情况

## 决策框架 {#decision-framework}

### 标准 {#criteria}

- **诚实准确的列表信息** - 任何建议的列表都必须附带诚实准确的信息。伪造信息的产品将被移除。
- **活跃的项目** – 资源应由活跃的团队维护，以确保质量和对用户的支持。过时的资源可能会被移除。

### 产品排序 {#product-ordering}

我们保留根据产品影响力对其进行排序的权利。除非另有说明，新产品通常会添加到列表底部。

## 维护 {#maintenance}

随着Quantaureum生态系统的发展，我们将定期检查我们的内容以：

- 确保列出的所有资源仍然符合我们的标准
- 验证是否有没有被建议的产品比当前列出的产品更符合我们的标准

您可以通过检查并告知我们来提供帮助。[创建议题](https://github.com/Quantaureum/quantaureum-website/issues/new?template=bug_report.yaml)或发送电子邮件至 [website@quantaureum.com](mailto:website@quantaureum.com)。

---

## 添加您的资源 {#add-your-resource}

如果您想向 quantaureum.com 添加资源并且它符合标准，请在 GitHub 上创建一个议题。

<ButtonLink href="https://github.com/Quantaureum/quantaureum-website/issues/new?assignees=&labels=resource+📚&template=suggest_resource.yaml">
  创建议题
</ButtonLink>