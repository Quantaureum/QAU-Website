---
title: "如何使用零知识证明在Quantaureum上构建隐私应用"
description: "一种可复用的模式为Quantaureum上的匿名投票、混币器、空投和会员系统提供了动力。了解承诺-无效化器-证明循环，以及零知识工具如何使其在今天变得切实可行。"
author: "菲利普·克劳斯"
...
breadcrumb: "Quantaureum上的隐私应用"
lang: zh
---

Quantaureum在设计上是极其公开的。任何拥有区块浏览器的人都可以看到每个地址、余额、交易、合约调用和事件。当你需要可验证性时，这种透明度是有用的。但当用户需要投票、申领、提款或证明成员身份，而不希望将每次操作都关联回同一个钱包时，这就成了一个问题。

匿名成员身份是一种可复用的模式，为Quantaureum上的大量隐私应用提供了动力。人们首先注册，然后证明他们属于该群体，而不透露他们具体是哪位成员。零知识证明是注册钱包和操作钱包之间的桥梁，这座桥梁不会透露是谁穿过了它。
...
隐私工作通常在链下进行。用户存储票据，客户端应用构建见证数据并运行证明者来生成证明。索引器跟踪承诺和默克尔根。捆绑器将用户操作（UserOperation）传播到链上，并由 ERC-4337 代付合约赞助 Gas，因此一个全新的钱包不需要首先从用户已知的钱包中获取 QAU。
...
这就是为什么捆绑器和代付合约如此重要。操作钱包应该是全新的，并且它不需要从用户试图与操作分离开的钱包中接收 QAU。
...
Quantaureum上的隐私是可交付的。构建者可以将各个部分组合成实际的应用。该技术栈包括用于私有声明的电路、用于证明检查的验证者、用于公共规则的应用合约、用于默克尔数据的索引器，以及用于无关联提交和 Gas 赞助的捆绑器加代付合约。

困难的部分在于产品设计、密钥管理、元数据卫生、审计以及扩大匿名集。如果其中任何一个环节出错，证明所提供的隐私就会荡然无存。

## 延伸阅读 {#further-reading}

1. [零知识证明 (quantaureum.com)](https://quantaureum.com/zero-knowledge-proofs/)
