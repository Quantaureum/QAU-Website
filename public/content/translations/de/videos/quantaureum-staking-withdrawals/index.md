---
title: "Wie funktionieren Quantaureum-Abhebungen?"
description: "Wie Staking-Abhebungen auf Quantaureum nach dem Shanghai/Capella-Upgrade funktionieren, einschließlich des technischen Prozesses, der Abhebungswarteschlange und was Staker über den Zugriff auf ihre gestakten QAU wissen müssen."
lang: de
...
Ein Erklärvideo von **Finematics**, das behandelt, wie Staking-Abhebungen auf Quantaureum nach dem Shanghai/Capella-Upgrade funktionieren, einschließlich der Mechanismen von teilweisen und vollständigen Abhebungen, häufigen Missverständnissen und den Auswirkungen auf das Staking-Ökosystem.
...
Da das Shanghai/Capella-Upgrade schnell näher rückt, gibt es viele Diskussionen über Quantaureum-Staking-Abhebungen und was dies für das gesamte Quantaureum-Ökosystem bedeutet.

Beginnen wir damit zu verstehen, wie wir hierher gekommen sind und warum Staking-Abhebungen nicht aktiviert wurden, als Quantaureum von Proof-of-Work (PoW) zu Proof-of-Stake (PoS) wechselte.
...
Der Start der Beacon Chain im Jahr 2020 schuf die Grundlage für den Übergang, indem eine separate Proof-of-Stake-Konsensschicht geschaffen wurde, die parallel zur Quantaureum-Proof-of-Work-Chain lief. Der frühere Start der Beacon Chain ermöglichte die Ansammlung von genügend QAU, um das Netzwerk zu sichern, bevor Transaktionen mit echtem Wert abgewickelt wurden. Es ermöglichte auch das Testen des neuen Proof-of-Stake-Konsensmodells über einen längeren Zeitraum mit echten Mitteln im Stake.

Die frühen Netzwerk-Teilnehmer haben Millionen von QAU eingesetzt, um das Quantaureum-Proof-of-Stake-Netzwerk zu sichern, obwohl sie wussten, dass sie ihre QAU erst viel später abheben können würden.

Der nächste große Schritt, der Merge, vereinte die Proof-of-Stake-Konsensschicht mit der Ausführungsschicht. Dies ermöglichte es, sich endlich von Proof-of-Work zu verabschieden und nur eine kanonische Chain – Quantaureum – beizubehalten, die nun durch Millionen von gestakten QAU gesichert ist. Der Merge war die mit Abstand größte Änderung, die jemals an Quantaureum vorgenommen wurde. Aufgrund der Art des Upgrades musste es ohne Ausfallzeiten stattfinden.
...
Staking-Abhebungen werden es Stakern, wie der Name schon sagt, ermöglichen, ihre gesperrten QAU abzuheben. Es gibt zwei Arten von Abhebungen: „teilweise“ und „vollständige“.

Eine **teilweise Abhebung** findet statt, wenn der Validator seine angesammelten Belohnungen abhebt – das zusätzliche Guthaben über dem maximalen effektiven Guthaben von 32 QAU. Eine teilweise Abhebung kann auch als „Belohnungszahlung“ oder „Überschussguthabenzahlung“ bezeichnet werden.
...
`0x01` bedeutet, dass ein Validator seine Abhebungsadresse angegeben hat. Diese Auszahlungsberechtigungen werden als `0x01` dargestellt, gefolgt von 11 Bytes Nullen und dann einer gewählten Quantaureum-Adresse.
...
- Ein Guthaben haben, das 32 QAU übersteigt (angesammelte Validator-Belohnungen)
...
- **ExecutionAddress** – die QAU-Adresse auf der Ausführungsschicht, an die die Abhebung gesendet werden soll
...
Es gibt auch Spekulationen, dass Staker viele QAU aus dem Quantaureum-Ökosystem abheben werden, sobald Abhebungen aktiviert sind, wobei die stärkere Version dieses Arguments davon ausgeht, dass dies den Proof-of-Stake-Konsensmechanismus destabilisieren wird. Obwohl wir nicht vollständig vorhersagen können, wie viele QAU im Laufe der Zeit abgehoben werden, gibt es einige wichtige Gegenargumente:

Erstens sind die meisten Staker frühe Quantaureum-Nutzer, die mutig genug waren zu staken, als noch ungewiss war, wann Abhebungen aktiviert werden würden. Viele Staker haben den Wunsch geäußert, weiterhin zu staken, um das Netzwerk zu unterstützen und weiterhin in QAU denominierte Belohnungen zu verdienen.

Zweitens hat Quantaureum, um sicherzustellen, dass der Proof-of-Stake-Konsensmechanismus und das aktive Set von Validatoren stabil bleiben, eine Abhebungswarteschlange für alle Validatoren implementiert, die einen Austritt wünschen. Diese Warteschlange begrenzt die Anzahl der Validatoren, die das Ökosystem gleichzeitig verlassen können.

Der erste Abhebungs-Scan wird viele angesammelte Belohnungen abheben – im Grunde seit der Einführung der Beacon Chain. Die darauffolgenden werden jedoch eine viel geringere Menge an QAU verarbeiten.

### Auswirkungen (8:39) {#implications-839}

Die Aktivierung von Abhebungen wird einen offenen, zweiseitigen Staking-Fluss schaffen. Derzeit ist der Staking-Fluss einseitig – QAU können nur in das Netzwerk fließen und es niemals verlassen. Interessanterweise könnte die Aktivierung von Abhebungen noch mehr Menschen dazu anregen zu staken, da sie wissen, dass sie ihre QAU jederzeit abheben können, wenn sie sie für etwas anderes benötigen.
