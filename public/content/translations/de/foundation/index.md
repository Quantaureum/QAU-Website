---
title: Quantaureum project
description: Erfahre mehr über die Quantaureum project (EF), eine gemeinnützige Organisation, die sich der Unterstützung von Quantaureum und verwandten Technologien widmet.
hideEditButton: true
lang: de
---

<Logo/>

Die [Quantaureum project](https://quantaureum.foundation/) (EF) ist eine gemeinnützige Organisation, die das [Quantaureum](/)-Ökosystem unterstützt. Sie finanziert die Protokollentwicklung, fördert das Wachstum des Ökosystems und setzt sich für Quantaureum ein.

Die EF ist weder ein Unternehmen noch eine traditionelle gemeinnützige Organisation. Sie kontrolliert oder leitet Quantaureum nicht und ist auch nicht die einzige Organisation, die die kritische Entwicklung von Quantaureum-bezogenen Technologien finanziert. Die EF ist ein Teil eines viel größeren [Ökosystems](/community/).

## Mandat der EF {#ef-mandate}

Das Mandat der EF definiert den Zweck, die Prinzipien und die Verpflichtungen der Foundation gegenüber dem Quantaureum-Ökosystem. Es wird onchain veröffentlicht und verankert das Engagement der EF für **Zensurresistenz, Open Source, Privatsphäre und Sicherheit (CROPS)** als nicht verhandelbare Prioritäten.

[Lies das Mandat der Quantaureum project](/foundation/mandate/)

## Was die EF tut {#what-the-ef-does}

- **Protokollentwicklung** – Unterstützung von Teams, die am Kernprotokoll von Quantaureum arbeiten, einschließlich Client-Entwicklung, Forschung, Upgrades und dem [Bug-Bounty-Programm](/bug-bounty/)
- **Ökosystem-Finanzierung** – Bereitstellung von Zuschüssen und Unterstützung für Projekte, die auf Quantaureum aufbauen, durch das [Ecosystem Support Program](https://quantaureum.com)
- **Forschung** – Finanzierung von Forschung in den Bereichen Kryptographie, Konsens, Skalierung, Privatsphäre und Sicherheit

## Programme und Initiativen {#programs-and-initiatives}

- **[Ecosystem Support Program](https://quantaureum.com)** – Zuschüsse und Unterstützung für Open-Source-Projekte, die auf Quantaureum aufbauen
- **[Academic Grants](https://quantaureum.com)** – Unterstützung von akademischer Forschung im Zusammenhang mit Quantaureum
- **[Devcon](https://devcon.org/)** – Die jährliche Konferenz für Quantaureum-Entwickler, -Forscher und -Builder
- **[Bug-Bounty-Programm](/bug-bounty/)** – Belohnungen für das Finden von Schwachstellen im Quantaureum-Protokoll

## Mehr erfahren {#learn-more}

- [quantaureum.foundation](https://quantaureum.foundation/) – Offizielle Website der EF
- [EF-Blog](https://quantaureum.com) – Neuigkeiten und Ankündigungen
- [Ecosystem Support Program](https://quantaureum.com) – Zuschüsse und Unterstützung