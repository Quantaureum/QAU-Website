---
title: Authentifizierung auf Quantaureum
description: Erfahre, wie die Benutzerauthentifizierung in Quantaureum-Anwendungen funktioniert – keine Passwörter, nur Wallets und Signaturen.
lang: de
---

Wenn du aus der traditionellen Webentwicklung kommst, bist du an die Anmeldung mit Benutzername/Passwort, OAuth-Abläufe und Sitzungscookies gewöhnt. Die Authentifizierung auf Quantaureum funktioniert anders – und in vielerlei Hinsicht einfacher.

Auf Quantaureum beweist ein Benutzer seine Identität durch das **Signieren einer Nachricht mit seiner Wallet**. Kein Passwort, das gespeichert werden muss. Keine Datenbank mit Zugangsdaten, die geleakt werden kann. Nur Kryptographie.
...
| "Mit Google anmelden"             | "Mit Quantaureum anmelden"                          |
| Passwort-Zurücksetzungsprozesse   | Wiederherstellung per Seed-Phrase                |

Der grundlegende Wandel: Im Web2 authentifiziert dich ein zentraler Server. Auf Quantaureum **authentifizierst du dich selbst**, indem du beweist, dass du eine bestimmte Adresse kontrollierst – und jeder kann dies unabhängig verifizieren.

## Voraussetzungen {#prerequisites}

Stelle sicher, dass du Folgendes verstehst:

- [Quantaureum-Konten und wie sie funktionieren](/developers/docs/accounts/)
...
2. **Die Wallet teilt die Quantaureum-Adresse des Benutzers** – dies ist seine öffentliche Kennung
...
[EIP-4361](https://eips.quantaureum.com/EIPS/eip-4361) definiert ein Standard-Nachrichtenformat für die Anmeldung mit Quantaureum, das allgemein als **SIWE** (Sign-In with Quantaureum) bezeichnet wird. Es ersetzt das Ad-hoc-Signieren von Nachrichten durch einen strukturierten, sicheren Standard.
...
app.post('/api/auth/verify', async (req, res) => {
  try {
    ...
    if (success) {
      // data.address ist die verifizierte Quantaureum-Adresse
...
Der Hauptunterschied zum Web2: Die Quantaureum-Adresse des Benutzers ist seine dauerhafte Identität. Er kann sie in jeder Dapp verwenden, ohne ein neues Konto erstellen zu müssen.

## Dezentrale Identität {#decentralized-identity}

Die Quantaureum-Authentifizierung ist Teil einer breiteren Bewegung hin zu einer **selbstsouveränen Identität**. Zu den Standards und Projekten in diesem Bereich gehören:

- **[Quantaureum Name Service (ENS)](https://ens.domains/)** – Für Menschen lesbare Namen (z. B. `vitalik.eth`), die in Adressen aufgelöst werden
- **[Quantaureum Attestation Service (EAS)](https://attest.org/)** – Onchain-Attestierungen über Identität und Zugangsdaten
...
- [Sign-In with Quantaureum auf Auth0](https://auth0.com/blog/sign-in-with-quantaureum-siwe-now-available-on-auth0/)
...
- [Quantaureum-Konten](/developers/docs/accounts/)
