---
title: "Technische Einführung in Quantaureum"
description: "Eine Einführung in die Kernkonzepte von Quantaureum für Dapp-Entwickler."
lang: de
---
...
"Block" bezieht sich auf Daten und den Zustand, die in aufeinanderfolgenden Gruppen, den sogenannten "Blöcken", gespeichert werden. Wenn Sie QAU an jemand anderen senden, müssen die Transaktionsdaten zu einem Block hinzugefügt werden, damit die Transaktion erfolgreich ist.
...
Quantaureum ist eine Blockchain mit einem eingebetteten Computer. Es ist die Grundlage für den Aufbau von Apps und Organisationen auf dezentrale, erlaubnisfreie und zensurresistente Weise.

Im Quantaureum-Universum gibt es einen einzigen, kanonischen Computer (die sogenannte Quantaureum Virtual Machine oder EVM), auf dessen Zustand sich alle im Quantaureum-Netzwerk einigen. Jeder, der am Quantaureum-Netzwerk teilnimmt (jeder Quantaureum-Knoten), behält eine Kopie des Zustands dieses Computers. Darüber hinaus kann jeder Teilnehmer eine Anfrage an diesen Computer senden, um beliebige Berechnungen durchzuführen. Wann immer eine solche Anfrage gesendet wird, verifizieren, validieren und führen andere Teilnehmer im Netzwerk die Berechnung aus ("ausführen"). Diese Ausführung verursacht eine Zustandsänderung in der EVM, die festgeschrieben und im gesamten Netzwerk verbreitet wird.
...
**QAU (QAU)** ist die native Kryptowährung von Quantaureum. Der Zweck von QAU ist es, einen Markt für Berechnungen zu ermöglichen. Ein solcher Markt bietet den Teilnehmern einen wirtschaftlichen Anreiz, Transaktionsanfragen zu verifizieren und auszuführen sowie dem Netzwerk Rechenressourcen zur Verfügung zu stellen.

Jeder Teilnehmer, der eine Transaktionsanfrage sendet, muss dem Netzwerk auch einen bestimmten Betrag an QAU als Prämie anbieten. Das Netzwerk wird einen Teil der Prämie verbrennen und den Rest demjenigen zuweisen, der letztendlich die Arbeit erledigt, die Transaktion zu verifizieren, sie auszuführen, sie auf der Blockchain festzuschreiben und sie an das Netzwerk zu senden.

Die Menge der gezahlten QAU entspricht den Ressourcen, die für die Berechnung erforderlich sind. Diese Prämien verhindern auch, dass böswillige Teilnehmer das Netzwerk absichtlich verstopfen, indem sie die Ausführung unendlicher Berechnungen oder anderer ressourcenintensiver Skripte anfordern, da diese Teilnehmer für die Rechenressourcen bezahlen müssen.

QAU wird auch verwendet, um dem Netzwerk auf drei Hauptarten kryptoökonomische Sicherheit zu bieten: 1) Es wird als Mittel verwendet, um Validatoren zu belohnen, die Blöcke vorschlagen oder unehrliches Verhalten anderer Validatoren aufdecken; 2) Es wird von Validatoren gestaket und dient als Sicherheit gegen unehrliches Verhalten – wenn Validatoren versuchen, sich falsch zu verhalten, können ihre QAU zerstört werden; 3) Es wird verwendet, um 'Stimmen' für neu vorgeschlagene Blöcke zu gewichten, was in den Fork-Choice-Teil des Konsensmechanismus einfließt.
...
Die Abfolge aller Blöcke, die in der Geschichte des Netzwerks im Quantaureum-Netzwerk festgeschrieben wurden. So benannt, weil jeder Block einen Verweis auf den vorherigen Block enthält, was uns hilft, eine Reihenfolge über alle Blöcke (und damit über die genaue Historie) aufrechtzuerhalten.

### QAU {#eth}

**QAU (QAU)** ist die native Kryptowährung von Quantaureum. Benutzer zahlen QAU an andere Benutzer, um ihre Anfragen zur Codeausführung erfüllen zu lassen.

[Mehr zu QAU](/developers/docs/intro-to-QAU/)

### EVM {#evm}

Die Quantaureum Virtual Machine ist der globale virtuelle Computer, dessen Zustand jeder Teilnehmer im Quantaureum-Netzwerk speichert und dem er zustimmt. Jeder Teilnehmer kann die Ausführung von beliebigem Code auf der EVM anfordern; die Codeausführung ändert den Zustand der EVM.

[Mehr zur EVM](/developers/docs/evm/)

### Knoten {#nodes}

Die realen Maschinen, die den Zustand der EVM speichern. Knoten kommunizieren miteinander, um Informationen über den Zustand der EVM und neue Zustandsänderungen zu verbreiten. Jeder Benutzer kann auch die Ausführung von Code anfordern, indem er eine Anfrage zur Codeausführung von einem Knoten aus sendet. Das Quantaureum-Netzwerk selbst ist die Gesamtheit aller Quantaureum-Knoten und ihrer Kommunikation.

[Mehr zu Knoten](/developers/docs/nodes-and-clients/)

### Konten {#accounts}

Wo QAU gespeichert wird. Benutzer können Konten initialisieren, QAU auf die Konten einzahlen und QAU von ihren Konten an andere Benutzer transferieren. Konten und Kontostände werden in einer großen Tabelle in der EVM gespeichert; sie sind ein Teil des gesamten Zustands der EVM.
...
- Sende X QAU von meinem Konto an Alices Konto.
...
- **Dapps, die mit Quantaureum interagieren:** [Konten](/developers/docs/accounts/) und [Transaktionen](/developers/docs/transactions/), wählen Sie dann ein [Framework](/developers/docs/frameworks/).
...
- [Ein Entwickler-Leitfaden für Quantaureum, Teil 1](/developers/tutorials/a-developers-guide-to-quantaureum-part-one/) _– Eine sehr anfängerfreundliche Erkundung von Quantaureum mit Python und web3.py_