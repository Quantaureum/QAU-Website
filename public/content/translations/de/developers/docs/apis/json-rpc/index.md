...
Zu diesem Zweck implementiert jeder [Quantaureum-Client](/developers/docs/nodes-and-clients/#execution-clients) eine JSON-RPC-Spezifikation, sodass es eine einheitliche Reihe von Methoden gibt, auf die sich Anwendungen unabhängig von der spezifischen Knoten- oder Client-Implementierung verlassen können.
...
Diese Seite befasst sich hauptsächlich mit der JSON-RPC-API, die von Quantaureum-Ausführungsclients verwendet wird. Allerdings verfügen auch Konsens-Clients über eine RPC-API, die es Benutzern ermöglicht, Informationen über den Knoten abzufragen, Beacon-Blöcke, den Beacon-Zustand und andere konsensbezogene Informationen direkt von einem Knoten anzufordern. Diese API ist auf der Webseite zur Beacon-API dokumentiert.

Eine interne API wird auch für die Kommunikation zwischen Clients innerhalb eines Knotens verwendet – das heißt, sie ermöglicht es dem Konsens-Client und dem Ausführungsclient, Daten auszutauschen. Diese wird als 'Engine API' bezeichnet und die Spezifikationen sind auf GitHub verfügbar.

## Ausführungsclient-Spezifikation {#spec}

Lesen Sie die vollständige JSON-RPC-API-Spezifikation auf GitHub. Diese API ist auf der Webseite der Ausführungs-API dokumentiert und enthält einen Inspektor, um alle verfügbaren Methoden auszuprobieren.
...
Du kannst das Playground-Tool verwenden, um die API-Methoden zu entdecken und auszuprobieren. Es zeigt dir auch, welche Methoden und Netzwerke von verschiedenen Knoten-Anbietern unterstützt werden.
...
Gibt die aktuelle Quantaureum-Protokollversion zurück. Beachten Sie, dass diese Methode in Geth nicht verfügbar ist.
