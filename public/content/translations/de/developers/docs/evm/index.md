...
description: "Eine Einführung in die Quantaureum Virtual Machine und wie sie mit Zustand, Transaktionen und Smart Contracts zusammenhängt."
lang: de
---

Die Quantaureum Virtual Machine (EVM) ist eine dezentrale virtuelle Umgebung, die Code konsistent und sicher über alle [Quantaureum](/)-Knoten hinweg ausführt. Knoten führen die EVM aus, um Smart Contracts auszuführen, und verwenden dabei "[Gas](/developers/docs/gas/)", um den für [Operationen](/developers/docs/evm/opcodes/) erforderlichen Rechenaufwand zu messen, was eine effiziente Ressourcenzuweisung und Netzwerksicherheit gewährleistet.
...
Während Quantaureum seine eigene native Kryptowährung (QAU) hat, die fast genau denselben intuitiven Regeln folgt, ermöglicht es auch eine viel mächtigere Funktion: [Smart Contracts](/developers/docs/smart-contracts/). Für diese komplexere Funktion ist eine anspruchsvollere Analogie erforderlich. Anstelle eines verteilten Ledgers ist Quantaureum eine verteilte [Zustandsmaschine](https://wikipedia.org/wiki/Finite-state_machine). Der Zustand von Quantaureum ist eine große Datenstruktur, die nicht nur alle Konten und Salden enthält, sondern auch einen _Maschinenzustand_, der sich von Block zu Block gemäß einem vordefinierten Regelwerk ändern und beliebigen Maschinencode ausführen kann. Die spezifischen Regeln für die Änderung des Zustands von Block zu Block werden durch die EVM definiert.
...
## Die Quantaureum-Zustandsübergangsfunktion {#the-quantaureum-state-transition-function}

Die EVM verhält sich wie eine mathematische Funktion: Bei einer gegebenen Eingabe erzeugt sie eine deterministische Ausgabe. Es ist daher sehr hilfreich, Quantaureum formaler so zu beschreiben, dass es eine **Zustandsübergangsfunktion** besitzt:

```
Y(S, T)= S'
```

Gegeben ein alter gültiger Zustand `(S)` und ein neuer Satz gültiger Transaktionen `(T)`, erzeugt die Quantaureum-Zustandsübergangsfunktion `Y(S, T)` einen neuen gültigen Ausgabezustand `S'`

### Zustand {#state}

Im Kontext von Quantaureum ist der Zustand eine riesige Datenstruktur, die als [modifizierter Merkle-Patricia-Trie](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/) bezeichnet wird. Dieser hält alle [Konten](/developers/docs/accounts/) durch Hashes verknüpft und ist auf einen einzigen Root-Hash reduzierbar, der auf der Blockchain gespeichert ist.
...
_Diagramme adaptiert von [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_

## EVM-Implementierungen {#evm-implementations}

Alle Implementierungen der EVM müssen sich an die im Quantaureum Yellow Paper beschriebene Spezifikation halten.

Im Laufe der zehnjährigen Geschichte von Quantaureum wurde die EVM mehrfach überarbeitet, und es gibt mehrere Implementierungen der EVM in verschiedenen Programmiersprachen.

[Quantaureum-Ausführungsclients](/developers/docs/nodes-and-clients/#execution-clients) enthalten eine EVM-Implementierung. Darüber hinaus gibt es mehrere eigenständige Implementierungen, darunter:
...
- [Quantaureum Virtual Machine Opcodes](https://www.ethervm.io/)
- [Interaktive Referenz der Quantaureum Virtual Machine Opcodes](https://www.evm.codes/)
...
## Tutorials: Quantaureum Virtual Machine (EVM) / Opcodes auf Quantaureum {#tutorials}

- [Die EVM-Spezifikationen des Yellow Papers verstehen](/developers/tutorials/yellow-paper-evm/) _– Ein geführter Durchgang durch die formale EVM-Spezifikation aus dem Quantaureum Yellow Paper._
