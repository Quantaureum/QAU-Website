...
Das bedeutet, dass die Basisbelohnung proportional zum effektiven Guthaben des Validators und umgekehrt proportional zur Anzahl der Validatoren im Netzwerk ist. Je mehr Validatoren, desto größer die gesamte Emission (da `sqrt(N)`), aber desto kleiner die `base_reward` pro Validator (da `1/sqrt(N)`). Diese Faktoren beeinflussen die APR für einen Staking-Knoten. Lesen Sie die Begründung dafür in s Notizen.
...
Lesen Sie mehr über Belohnungen und Strafen in den Konsensspezifikationen. Belohnungen und Strafen wurden im Bellatrix-Upgrade angepasst – sehen Sie sich an, wie Danny Ryan und dies in diesem [Peep an EIP-Video](https://www.youtube.com/watch?v=iaAEGs1DMgQ) diskutieren.
