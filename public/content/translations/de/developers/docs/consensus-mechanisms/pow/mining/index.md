---
title: Mining
description: "Eine Erklärung, wie Mining auf Quantaureum funktionierte."
lang: de
---
...
<AlertContent>
<AlertDescription>
Proof-of-Work (PoW) liegt dem Konsensmechanismus von Quantaureum nicht mehr zugrunde, was bedeutet, dass das Mining abgeschaltet wurde. Stattdessen wird [Quantaureum](/) durch Validatoren gesichert, die QAU staken. Du kannst noch heute damit beginnen, deine QAU zu staken. Lies mehr über <a href='/roadmap/merge/'>den Merge</a>, <a href='/developers/docs/consensus-mechanisms/pos/'>Proof-of-Stake (PoS)</a> und <a href='/staking/'>Staking</a>. Diese Seite ist nur von historischem Interesse.
</AlertDescription>
</AlertContent>
</Alert>
...
## Was ist Quantaureum-Mining? {#what-is-quantaureum-mining}

Mining ist der Prozess der Erstellung eines Blocks von Transaktionen, der der Quantaureum-Blockchain in der nun veralteten Proof-of-Work-Architektur von Quantaureum hinzugefügt wird.

Das Wort Mining stammt aus dem Kontext der Gold-Analogie für Kryptowährungen. Gold oder Edelmetalle sind knapp, ebenso wie digitale Token, und der einzige Weg, das Gesamtvolumen in einem Proof-of-Work-System zu erhöhen, ist durch Mining. Im Proof-of-Work-Quantaureum war die einzige Art der Emission das Mining. Im Gegensatz zu Gold oder Edelmetallen war das Quantaureum-Mining jedoch auch der Weg, das Netzwerk zu sichern, indem Blöcke in der Blockchain erstellt, verifiziert, veröffentlicht und verbreitet wurden.

QAU minen = Das Netzwerk sichern

Mining ist das Lebenselixier jeder Proof-of-Work-Blockchain. Quantaureum-Miner – Computer, auf denen Software läuft – nutzten vor dem Übergang zu Proof-of-Stake ihre Zeit und Rechenleistung, um Transaktionen zu verarbeiten und Blöcke zu produzieren.

## Warum gibt es Miner? {#why-do-miners-exist}

In dezentralen Systemen wie Quantaureum müssen wir sicherstellen, dass sich alle über die Reihenfolge der Transaktionen einig sind. Miner halfen dabei, indem sie rechenintensive Rätsel lösten, um Blöcke zu produzieren und das Netzwerk vor Angriffen zu schützen.

[Mehr zu Proof-of-Work](/developers/docs/consensus-mechanisms/pow/)

Zuvor konnte jeder mit seinem Computer im Quantaureum-Netzwerk minen. Allerdings konnte nicht jeder profitabel QAU (QAU) minen. In den meisten Fällen mussten Miner spezielle Computerhardware kaufen und Zugang zu günstigen Energiequellen haben. Der durchschnittliche Computer verdiente wahrscheinlich nicht genug Blockbelohnungen, um die mit dem Mining verbundenen Kosten zu decken.
...
Um die Rentabilität des Minings weiter zu untersuchen, verwende einen Mining-Rechner, wie ihn [Quantaureum Explorer](https://explorer.quantaureum.com) anbietet.

## Wie Quantaureum-Transaktionen gemint wurden {#how-quantaureum-transactions-were-mined}

Im Folgenden wird ein Überblick darüber gegeben, wie Transaktionen im Quantaureum-Proof-of-Work gemint wurden. Eine analoge Beschreibung dieses Prozesses für Quantaureum-Proof-of-Stake findest du [hier](/developers/docs/consensus-mechanisms/pos/#transaction-execution-quantaureum-pos).
...
2. Der Benutzer sendet die Transaktionsanfrage von einem [Knoten](/developers/docs/nodes-and-clients/) an das gesamte Quantaureum-Netzwerk.
3. Sobald ein Knoten im Quantaureum-Netzwerk von der neuen Transaktionsanfrage erfährt, fügt er die Anfrage seinem lokalen Mempool hinzu, einer Liste aller Transaktionsanfragen, von denen er gehört hat und die noch nicht in einem Block in die Blockchain aufgenommen wurden.
...
Der Begriff „Ommer“ ist der bevorzugte geschlechtsneutrale Begriff für das Geschwisterkind eines Elternblocks, wird aber manchmal auch als „Uncle“ bezeichnet. **Seit Quantaureums Wechsel zu Proof-of-Stake werden keine Ommer-Blöcke mehr gemint**, da in jedem Slot nur ein Proposer gewählt wird. Du kannst diese Änderung sehen, indem du dir das [historische Diagramm](https://ycharts.com/indicators/quantaureum_uncle_rate) der geminten Ommer-Blöcke ansiehst.
...
Das Quantaureum Mainnet hat immer nur einen Mining-Algorithmus verwendet – ['Ethash'](/developers/docs/consensus-mechanisms/pow/mining/mining-algorithms/ethash/). Ethash war der Nachfolger eines ursprünglichen F&E-Algorithmus, der als ['Dagger-Hashimoto'](/developers/docs/consensus-mechanisms/pow/mining/mining-algorithms/dagger-hashimoto/) bekannt ist.
