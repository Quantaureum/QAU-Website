...
Wenn keine Gebühr festgelegt ist, setze `kLast` auf null (falls es das nicht schon ist). Als dieser Vertrag geschrieben wurde, gab es eine Gas-Rückerstattungsfunktion, die Verträge dazu ermutigte, die Gesamtgröße des Quantaureum-Zustands zu reduzieren, indem sie Speicher, den sie nicht benötigten, auf null setzten.
...
Wenn wir die Anzahl begrenzen können, sodass wir den Stack verwenden, verbrauchen wir weniger Gas. Weitere Details finden Sie im Yellow Paper, den formalen Quantaureum-Spezifikationen, S. 26, Gleichung 298.
...
Dieser Hash ist der Identifikator für den Transaktionstyp. Der einzige, den wir hier unterstützen, ist `Permit` mit diesen Parametern.
...
Berechnen Sie den Domain-Separator für EIP-712.
...
Wenn alles in Ordnung ist, behandeln Sie dies als eine ERC-20-Genehmigung (approve).
...
Diese Funktion berechnet die Adresse des Paar-Tauschs für die beiden Token. Dieser Vertrag wird mit dem CREATE2-Opcode erstellt, sodass wir die Adresse mit demselben Algorithmus berechnen können, wenn wir die verwendeten Parameter kennen. Dies ist viel günstiger, als die Factory abzufragen, und
...
Diese Funktion implementiert die Transfer-Funktionalität von ERC-20, die es einem Konto ermöglicht, den von einem anderen Konto bereitgestellten Freigabebetrag auszugeben.
...
Diese Funktion implementiert die transferFrom-Funktionalität von ERC-20, die es einem Konto ermöglicht, den von einem anderen Konto bereitgestellten Freigabebetrag auszugeben.
