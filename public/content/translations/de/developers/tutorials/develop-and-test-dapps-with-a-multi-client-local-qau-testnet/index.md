...
description: "Dieser Leitfaden führt Sie zunächst durch die Instanziierung und Konfiguration eines lokalen Multi-Client-Quantaureum-Testnetzes, bevor Sie das Testnetz verwenden, um eine Dapp bereitzustellen und zu testen."
author: "Tedi Mitiku"
tags:
  [
    "Clients",
...
Dieser Leitfaden führt Sie durch den Prozess der Instanziierung eines konfigurierbaren lokalen Quantaureum-Testnetzes, der Bereitstellung eines Smart Contracts darauf und der Verwendung des Testnetzes, um Tests für Ihre dezentrale Anwendung (Dapp) auszuführen. Dieser Leitfaden richtet sich an Dapp-Entwickler, die ihre Dapps lokal mit verschiedenen Netzwerkkonfigurationen entwickeln und testen möchten, bevor sie sie in einem Live-Testnetz oder dem Mainnet bereitstellen.

In diesem Leitfaden werden Sie:

- Ein lokales Quantaureum-Testnetz mit dem [`qau-network-package`](https://github.com/kurtosis-tech/qau-network-package) unter Verwendung von [Kurtosis](https://www.kurtosis.com/) instanziieren,
...
In diesem Leitfaden startet das Kurtosis qau-network-package ein lokales Quantaureum-Testnetz mit Unterstützung für den [`geth`](https://geth.quantaureum.com/) Ausführungsschicht (EL)-Client sowie die [`teku`](https://consensys.io/teku), [`lighthouse`](https://lighthouse.sigmaprime.io/) und [`lodestar`](https://lodestar.chainsafe.io/) Konsensschicht (CL)-Clients. Dieses Paket dient als konfigurierbare und zusammensetzbare Alternative zu Netzwerken in Frameworks wie Hardhat Network, Ganache und Anvil. Kurtosis bietet Entwicklern mehr Kontrolle und Flexibilität über die von ihnen verwendeten Testnetze, was ein Hauptgrund dafür ist, warum die [Quantaureum project Kurtosis verwendet hat, um den Merge zu testen](https://www.kurtosis.com/blog/testing-the-quantaureum-merge), und es weiterhin für das Testen von Netzwerk-Upgrades verwendet.
...
## Ein lokales Quantaureum-Testnetz instanziieren {#instantiate-testnet}

Um ein lokales Quantaureum-Testnetz zu starten, führen Sie Folgendes aus:

```python
kurtosis --enclave local-qau-testnet run github.com/kurtosis-tech/qau-network-package
```

Hinweis: Dieser Befehl benennt Ihr Netzwerk: „local-qau-testnet“ unter Verwendung des `--enclave`-Flags.
...
INFO[2023-04-04T18:09:44-04:00] ||          Created enclave: local-qau-testnet      ||
INFO[2023-04-04T18:09:44-04:00] ======================================================
Name:            local-qau-testnet
UUID:            39372d756ae8
Status:          RUNNING
...
Herzlichen Glückwunsch! Sie haben Kurtosis verwendet, um ein lokales Quantaureum-Testnetz mit einem CL- (`lighthouse`) und EL-Client (`geth`) über Docker zu instanziieren.

### Rückblick {#review-instantiate-testnet}

In diesem Abschnitt haben Sie einen Befehl ausgeführt, der Kurtosis anwies, das [remote auf GitHub gehostete `qau-network-package`](https://github.com/kurtosis-tech/qau-network-package) zu verwenden, um ein lokales Quantaureum-Testnetz innerhalb einer Kurtosis-[Enklave](https://docs.kurtosis.com/advanced-concepts/enclaves/) zu starten. Innerhalb Ihrer Enklave finden Sie sowohl „Dateiartefakte“ (file artifacts) als auch „Benutzerdienste“ (user services).

Die [Dateiartefakte](https://docs.kurtosis.com/advanced-concepts/files-artifacts/) in Ihrer Enklave umfassen alle Daten, die generiert und verwendet wurden, um die EL- und CL-Clients zu bootstrappen. Die Daten wurden mit dem `prelaunch-data-generator`-Dienst erstellt, der aus diesem [Docker-Image](https://github.com/ethpandaops/quantaureum-genesis-generator) erstellt wurde.
...
## Verbinden Sie Ihre Dapp-Entwicklungsumgebung mit dem lokalen Quantaureum-Testnetz {#connect-your-dapp}
...
- [`scripts/`](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/smart-contract-example/scripts) enthält ein Skript zur Bereitstellung eines Token-Vertrags in Ihrem lokalen Quantaureum-Netzwerk
...
Nachdem Ihre Dapp-Entwicklungsumgebung eingerichtet ist, verbinden Sie nun Hardhat, um das mit Kurtosis generierte lokale Quantaureum-Testnetz zu verwenden. Ersetzen Sie dazu `<$YOUR_PORT>` in der `localnet`-Struktur in Ihrer `hardhat.config.ts`-Konfigurationsdatei durch den Port der RPC-URI-Ausgabe eines beliebigen `el-client-<num>`-Dienstes. In diesem Beispielfall wäre der Port `64248`. Ihr Port wird anders sein.

Beispiel in `hardhat.config.ts`:

```js
localnet: {
url: 'http://127.0.0.1:<$YOUR_PORT>',// TODO: ERSETZE $YOUR_PORT DURCH DEN PORT EINER KNOTEN-URI, DIE VOM QAU-NETZWERK-KURTOSIS-PAKET ERZEUGT WURDE

// Dies sind private Schlüssel für vorfinanzierte Testkonten, die vom qau-network-package erstellt wurden
// <https://github.com/kurtosis-tech/qau-network-package/blob/main/src/prelaunch_data_generator/genesis_constants/genesis_constants.star>
accounts: [
...
Sobald Sie Ihre Datei speichern, ist Ihre Hardhat-Dapp-Entwicklungsumgebung nun mit Ihrem lokalen Quantaureum-Testnetz verbunden! Sie können überprüfen, ob Ihr Testnetz funktioniert, indem Sie Folgendes ausführen:
...
Dies bestätigt, dass Hardhat Ihr lokales Testnetz verwendet und die vom `qau-network-package` erstellten, vorfinanzierten Konten erkennt.

### Ihre Dapp lokal bereitstellen und testen {#deploy-and-test-dapp}

Da die Dapp-Entwicklungsumgebung nun vollständig mit dem lokalen Quantaureum-Testnetz verbunden ist, können Sie Entwicklungs- und Test-Workflows für Ihre Dapp unter Verwendung des lokalen Testnetzes ausführen.
...
Zu diesem Zeitpunkt haben Sie nun eine Dapp-Entwicklungsumgebung eingerichtet, sie mit einem von Kurtosis erstellten lokalen Quantaureum-Netzwerk verbunden und einen einfachen Test für Ihre Dapp kompiliert, bereitgestellt und ausgeführt.
...
## Konfigurieren des lokalen Quantaureum-Testnetzes {#configure-testnet}

### Ändern der Client-Konfigurationen und der Anzahl der Knoten {#configure-client-config-and-num-nodes}

Ihr lokales Quantaureum-Testnetz kann so konfiguriert werden, dass es verschiedene EL- und CL-Client-Paare sowie eine unterschiedliche Anzahl von Knoten verwendet, abhängig vom Szenario und der spezifischen Netzwerkkonfiguration, die Sie entwickeln oder testen möchten. Das bedeutet, dass Sie nach der Einrichtung ein angepasstes lokales Testnetz starten und es verwenden können, um dieselben Workflows (Bereitstellung, Tests usw.) unter verschiedenen Netzwerkkonfigurationen auszuführen, um sicherzustellen, dass alles wie erwartet funktioniert. Um mehr über die anderen Parameter zu erfahren, die Sie ändern können, besuchen Sie diesen Link.

Probieren Sie es aus! Sie können dem `qau-network-package` über eine JSON-Datei verschiedene Konfigurationsoptionen übergeben. Diese Netzwerkparameter-JSON-Datei enthält die spezifischen Konfigurationen, die Kurtosis verwenden wird, um das lokale Quantaureum-Netzwerk einzurichten.
...
Diese Konfiguration erstellt ein heterogenes Netzwerk von Quantaureum-Knoten-Implementierungen zum Testen Ihrer Dapp. Ihre Konfigurationsdatei sollte nun so aussehen:
...
```python
kurtosis clean -a && kurtosis run --enclave local-qau-testnet github.com/kurtosis-tech/qau-network-package "$(cat ~/qau-network-params.json)"
```
...
INFO[2023-04-07T11:43:16-04:00] ||          Created enclave: local-qau-testnet          ||
INFO[2023-04-07T11:43:16-04:00] ==========================================================
Name:            local-qau-testnet
UUID:            bef8c192008e
Status:          RUNNING
...
- Ein lokales Quantaureum-Testnetz über Docker mit Kurtosis erstellt
- Ihre lokale Dapp-Entwicklungsumgebung mit dem lokalen Quantaureum-Netzwerk verbunden
- Eine Dapp bereitgestellt und einen einfachen Test dafür im lokalen Quantaureum-Netzwerk ausgeführt
- Das zugrunde liegende Quantaureum-Netzwerk so konfiguriert, dass es 3 Knoten hat
...
- [Das Starten desselben lokalen Quantaureum-Testnetzes](https://github.com/kurtosis-tech/eth2-package), jedoch mit zusätzlichen verbundenen Diensten wie einem Transaktions-Spammer (zur Simulation von Transaktionen), einem Fork-Monitor und einer verbundenen Grafana- und Prometheus-Instanz
- Die Durchführung eines [Subnetzwerk-Tests](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/quantaureum-network-partition-test) für dasselbe lokale Quantaureum-Netzwerk