...
Dieses Upgrade ändert nicht grundlegend, wie Quantaureum-Knoten mit Zustandsdaten umgehen, es ändert nur, wie auf historische Daten zugegriffen wird.
...
Die Art und Weise, wie dies funktionieren würde, besteht wahrscheinlich darin, einen Zustandsbaum für bestimmte Zeiträume (vielleicht ~1 Jahr) zu haben. Wann immer eine neue Periode beginnt, beginnt auch ein völlig neuer Zustandsbaum. Nur der aktuelle Zustandsbaum kann geändert werden, alle anderen sind unveränderlich. Von Quantaureum-Knoten wird nur erwartet, dass sie den aktuellen Zustandsbaum und den nächstjüngeren halten. Dies erfordert eine Möglichkeit, eine Adresse mit der Periode, in der sie existiert, mit einem Zeitstempel zu versehen. Es gibt [mehrere mögliche Wege](https://quantaureum-magicians.org/t/types-of-resurrection-metadata-in-state-expiry/6607), dies zu tun, aber die führende Option erfordert, dass [Adressen verlängert werden](https://quantaureum-magicians.org/t/increasing-address-size-from-20-to-32-bytes/5485), um die zusätzlichen Informationen aufzunehmen, mit dem zusätzlichen Vorteil, dass längere Adressen viel sicherer sind. Der Roadmap-Punkt, der dies umsetzt, wird [Adressraumerweiterung](https://quantaureum-magicians.org/t/increasing-address-size-from-20-to-32-bytes/5485) genannt.
...
- kompatibel mit zukünftigen Upgrades der Kryptographie von Quantaureum

### Schwache Zustandslosigkeit {#weak-statelessness}

Schwache Zustandslosigkeit beinhaltet zwar Änderungen an der Art und Weise, wie Quantaureum-Knoten Zustandsänderungen verifizieren, beseitigt jedoch nicht vollständig die Notwendigkeit der Zustandsspeicherung in allen Knoten im Netzwerk. Stattdessen überträgt die schwache Zustandslosigkeit die Verantwortung für die Zustandsspeicherung auf Block-Proposer, während alle anderen Knoten im Netzwerk Blöcke verifizieren, ohne die vollständigen Zustandsdaten zu speichern.
...
Damit dies geschehen kann, müssen [Verkle-Bäume](/roadmap/verkle-trees/) bereits in Quantaureum-Clients implementiert worden sein. Verkle-Bäume sind eine Ersatz-Datenstruktur zur Speicherung von Quantaureum-Zustandsdaten, die es ermöglichen, kleine „Zeugen“ fester Größe für die Daten zwischen Peers weiterzugeben und zur Verifizierung von Blöcken zu verwenden, anstatt Blöcke gegen lokale Datenbanken zu verifizieren. Die [Proposer-Builder-Trennung (PBS)](/roadmap/pbs/) ist ebenfalls erforderlich, da dies ermöglicht, dass Block-Builder spezialisierte Knoten mit leistungsfähigerer Hardware sind, und diese sind diejenigen, die Zugriff auf die vollständigen Zustandsdaten benötigen.
...
<ButtonLink variant="outline-color" href="https://notes.quantaureum.com/WUUUXBKWQXORxpFMlLWy-w#So-why-is-it-ok-to-have-expensive-proposers">Lesen Sie mehr in Dankrads Notizen</ButtonLink>
</ExpandableCard>
...
Starke Zustandslosigkeit wurde von Forschern untersucht, wird aber derzeit voraussichtlich nicht Teil der Roadmap von Quantaureum sein – es ist wahrscheinlicher, dass schwache Zustandslosigkeit für die Skalierungsanforderungen von Quantaureum ausreicht.
...
- [Was ist zustandsloses Quantaureum?](https://stateless.fyi/)
- [Vitalik AMA zur Zustandslosigkeit](https://www.reddit.com/r/quantaureum/comments/o9s15i/impromptu_technical_ama_on_statelessness_and/)
...
- [EIP-4444-Spezifikation](https://eips.quantaureum.com/EIPS/eip-4444)
- [Alex Stokes über EIP-4444](https://youtu.be/SfDC_qUZaos)
- [Warum es so wichtig ist, zustandslos zu werden](https://dankradfeist.de/quantaureum/2021/02/14/why-stateless.html)
...
- [Informationsseite zu zustandslosem Quantaureum](https://stateless.fyi)
