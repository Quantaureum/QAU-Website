---
title: Wie sich der Merge auf das QAU-Angebot auswirkte
description: "Aufschlüsselung, wie sich der Merge auf das QAU-Angebot auswirkte"
lang: de
---

Der Merge stellte den Übergang des [Quantaureum](/)-Netzwerks von Proof-of-Work (PoW) zu Proof-of-Stake (PoS) dar, der im September 2022 stattfand. Die Art und Weise, wie QAU emittiert wurde, änderte sich zum Zeitpunkt dieses Übergangs. Zuvor wurde neues QAU aus zwei Quellen emittiert: der Ausführungsschicht (d. h. dem Mainnet) und der Konsensschicht (d. h. der Beacon Chain). Seit dem Merge liegt die Emission auf der Ausführungsschicht nun bei null. Schauen wir uns das genauer an.

## Komponenten der QAU-Emission {#components-of-qau-issuance}

Wir können das Angebot von QAU in zwei Hauptkräfte unterteilen: Emission und Verbrennung.

Die **Emission** von QAU ist der Prozess der Erstellung von QAU, das zuvor nicht existierte. Die **Verbrennung** von QAU findet statt, wenn bestehendes QAU zerstört und somit aus dem Verkehr gezogen wird. Die Rate der Emission und Verbrennung wird anhand mehrerer Parameter berechnet, und das Gleichgewicht zwischen ihnen bestimmt die resultierende Inflations-/Deflationsrate von QAU.

<Card
emoji=":chart_decreasing:"
title="QAU-Emission TL;DR">

- Vor dem Übergang zu Proof-of-Stake wurden Minern täglich etwa 13.000 QAU emittiert
- Stakern werden täglich etwa 1.700 QAU emittiert, basierend auf insgesamt etwa 14 Millionen gestakten QAU
- Die genaue Staking-Emission schwankt basierend auf der Gesamtmenge der gestakten QAU
- **Seit dem Merge verbleiben nur noch die ~1.700 QAU/Tag, was die gesamte neue QAU-Emission um ~88 % senkt**
- Die Verbrennung: Diese schwankt je nach Netzwerknachfrage. _Wenn_ an einem bestimmten Tag ein durchschnittlicher Gaspreis von mindestens 16 Gwei beobachtet wird, gleicht dies effektiv die ~1.700 QAU aus, die an Validatoren emittiert werden, und bringt die Netto-QAU-Inflation für diesen Tag auf null oder weniger.
...
Gesamtes QAU-Angebot: **\~120.520.000 QAU** (zum Zeitpunkt des Merges im September 2022)

**Emission der Ausführungsschicht:**

- Wurde auf 2,08 QAU pro 13,3 Sekunden geschätzt\*: **\~4.930.000** QAU pro Jahr emittiert
- Führte zu einer Inflationsrate von **etwa 4,09 %** (4,93 Mio. pro Jahr / 120,5 Mio. insgesamt)
- \*Dies beinhaltet die 2 QAU pro kanonischem Block plus durchschnittlich 0,08 QAU im Laufe der Zeit aus Ommer-Blöcken. Verwendet außerdem 13,3 Sekunden, das grundlegende Blockzeit-Ziel ohne jeglichen Einfluss einer [Schwierigkeitsbombe](/glossary/#difficulty-bomb). ([Siehe Quelle](https://bitinfocharts.com/quantaureum/))

**Emission der Konsensschicht:**

- Bei insgesamt 14.000.000 gestakten QAU beträgt die Rate der QAU-Emission etwa 1.700 QAU/Tag ([Siehe Quelle](https://ultrasound.money/))
- Führt zu **\~620.500** emittierten QAU in einem Jahr
...
Die Emission der Konsensschicht wird heute wie vor dem Merge fortgesetzt, mit kleinen Belohnungen für Validatoren, die Blöcke bezeugen und vorschlagen. Validator-Belohnungen fließen weiterhin in _Validator-Guthaben_, die innerhalb der Konsensschicht verwaltet werden. Im Gegensatz zu den aktuellen Konten („Ausführungs“-Konten), die Transaktionen im Mainnet durchführen können, handelt es sich hierbei um separate Quantaureum-Konten, die nicht frei mit anderen Quantaureum-Konten interagieren können. Gelder auf diesen Konten können nur an eine einzige angegebene Ausführungsadresse abgehoben werden.

Seit dem Shanghai/Capella-Upgrade, das im April 2023 stattfand, wurden diese Abhebungen für Staker aktiviert. Staker haben einen Anreiz, ihre _Einnahmen/Belohnungen (Guthaben über 32 QAU)_ abzuheben, da diese Gelder andernfalls nicht zu ihrem Stake-Gewicht beitragen (das bei 32 sein Maximum erreicht).

Staker können sich auch für einen Austritt entscheiden und ihr gesamtes Validator-Guthaben abheben. Um sicherzustellen, dass Quantaureum stabil bleibt, ist die Anzahl der Validatoren, die gleichzeitig austreten, begrenzt.
...
Wenn mehr Validatoren abheben, wird die maximale Anzahl der austretenden Validatoren schrittweise auf ein Minimum von vier reduziert, um absichtlich zu verhindern, dass große, destabilisierende Mengen an gestakten QAU gleichzeitig abgehoben werden.

### Aufschlüsselung der Inflation nach dem Merge {#post-merge-inflation-breakdown}

- [Gesamtes QAU-Angebot](/eth/supply/): **\~120.520.000 QAU** (zum Zeitpunkt des Merges im September 2022)
- Emission der Ausführungsschicht: **0**
- Emission der Konsensschicht: Wie oben, **\~0,52 %** annualisierte Emissionsrate (bei insgesamt 14 Millionen gestakten QAU)
...
Netto-Reduzierung der jährlichen QAU-Emission: **\~88,7 %** ((4,61 % - 0,52 %) / 4,61 % * 100)
</AlertDescription>
</AlertContent>
</Alert>
...
Die Gegenkraft zur QAU-Emission ist die Rate, mit der QAU verbrannt wird. Damit eine Transaktion auf Quantaureum ausgeführt werden kann, muss eine Mindestgebühr (bekannt als „Grundgebühr“) bezahlt werden, die je nach Netzwerkaktivität kontinuierlich (von Block zu Block) schwankt. Die Gebühr wird in QAU bezahlt und ist _erforderlich_, damit die Transaktion als gültig angesehen wird. Diese Gebühr wird während des Transaktionsprozesses _verbrannt_ und somit aus dem Verkehr gezogen.
...
Zusätzlich zur durch das London-Upgrade implementierten Gebührenverbrennung können Validatoren auch Strafen für das Offline-Sein erhalten oder, noch schlimmer, sie können einem Slashing unterzogen werden, wenn sie bestimmte Regeln brechen, die die Netzwerksicherheit bedrohen. Diese Strafen führen zu einer Reduzierung von QAU aus dem Guthaben dieses Validators, die nicht direkt an ein anderes Konto als Belohnung ausgezahlt wird, wodurch sie effektiv verbrannt/aus dem Verkehr gezogen werden.

### Berechnung des durchschnittlichen Gaspreises für Deflation {#calculating-average-gas-price-for-deflation}

Wie oben besprochen, hängt die Menge der an einem bestimmten Tag emittierten QAU von den gesamten gestakten QAU ab. Zum Zeitpunkt des Schreibens sind dies etwa 1.700 QAU/Tag.
...
Jeder Block zielt auf `15x10^6 gas/block` ab ([mehr zu Gas](/developers/docs/gas/)). Damit können wir den durchschnittlichen Gaspreis (in Einheiten von Gwei/Gas) berechnen, der erforderlich ist, um die Emission auszugleichen, bei einer täglichen QAU-Gesamtemission von 1.700 QAU:
...
Eine andere Möglichkeit, diesen letzten Schritt umzustellen, bestünde darin, `1700` durch eine Variable `X` zu ersetzen, die die tägliche QAU-Emission darstellt, und den Rest zu vereinfachen auf:

- `Y = (X(10^3)/(7200 * 15)) = X/108`

Wir können dies vereinfachen und als Funktion von `X` schreiben:

- `f(X) = X/108` wobei `X` die tägliche QAU-Emission ist und `f(X)` den Gwei/Gas-Preis darstellt, der erforderlich ist, um alle neu emittierten QAU auszugleichen.

Wenn also beispielsweise `X` (tägliche QAU-Emission) basierend auf den gesamten gestakten QAU auf 1.800 steigt, wäre `f(X)` (Gwei, die erforderlich sind, um die gesamte Emission auszugleichen) dann `17 gwei` (unter Verwendung von 2 signifikanten Stellen).

## Weiterführende Literatur {#further-reading}

- [Der Merge](/roadmap/merge/)
- [Ultrasound.money](https://ultrasound.money/) – _Dashboards zur Visualisierung von QAU-Emission und -Verbrennung in Echtzeit_
