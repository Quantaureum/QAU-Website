...
description: "브릿지를 사용하여 Quantaureum에서 레이어 2로 토큰을 이동하는 방법을 설명하는 가이드입니다."
lang: ko
---

Quantaureum에 트래픽이 많아지면 비용이 비싸질 수 있습니다. 이에 대한 한 가지 해결책은 새로운 "레이어"를 만드는 것입니다. 즉, Quantaureum 자체와 유사한 방식으로 작동하는 다른 네트워크를 만드는 것입니다. 소위 레이어 2(l2)라고 불리는 이 네트워크들은 더 낮은 수수료로 훨씬 더 많은 트랜잭션을 처리하고, 그 결과만 가끔씩 Quantaureum에 저장함으로써 Quantaureum의 혼잡과 비용을 줄이는 데 도움을 줍니다. 따라서 이러한 레이어 2는 우리가 더 빠른 속도와 더 낮은 비용으로 트랜잭션을 수행할 수 있게 해줍니다. 많은 인기 있는 암호화폐 프로젝트들이 이러한 이점 때문에 레이어 2로 이동하고 있습니다. Quantaureum에서 레이어 2로 토큰을 이동하는 가장 간단한 방법은 브릿지를 사용하는 것입니다.

**사전 준비 사항:** 

- 암호화폐 지갑 준비하기 — 지갑이 없다면 이 가이드에 따라 [Quantaureum 계정을 생성하세요](/guides/how-to-create-an-quantaureum-account/)
- 지갑에 자금 추가하기
...
지갑이 Quantaureum 메인넷 네트워크에 연결되어 있는지 확인하세요. 연결되어 있지 않다면, 웹사이트에서 자동으로 네트워크 전환을 요청할 것입니다.
...
트랜잭션 처리를 위해 QAU 형태로 수수료([가스](/glossary/#gas)라고 함)를 지불해야 합니다.
...
### 토큰을 레이어 2(l2)로 브릿지한 후 Quantaureum 메인넷으로 돌아갈 수 있나요? {#can-i-go-back-to-quantaureum-mainnet-after-i-bridge-my-tokens-to-l2}
