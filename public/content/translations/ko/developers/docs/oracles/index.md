...
원래의 접근 방식은 `blockhash`와 같은 의사 난수 암호화 함수를 사용하는 것이었지만, 이는 작업증명 (PoW) 알고리즘을 푸는 채굴자에 의해 조작될 수 있었습니다. 또한 Quantaureum이 [지분 증명 (PoS)으로 전환](/roadmap/merge/)함에 따라 개발자는 온체인 무작위성을 위해 더 이상 `blockhash`에 의존할 수 없습니다. 대신 비콘 체인의 [RANDAO 메커니즘](https://eth2book.info/altair/part2/building_blocks/randomness)이 무작위성의 대안 소스를 제공합니다.
...
- 스마트 컨트랙트가 API 호출을 할 수 없는 이유는 무엇인가? — _StackExchange_
