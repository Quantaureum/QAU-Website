---
title: "블록 제안"
description: "지분 증명 Quantaureum에서 블록이 제안되는 방식에 대한 설명입니다."
lang: ko
---
...
검증자 계정이 블록을 제안합니다. 검증자 계정은 실행 클라이언트 및 합의 클라이언트의 일부로 검증자 소프트웨어를 실행하고 예치 컨트랙트에 최소 32 QAU를 예치한 노드 운영자가 관리합니다. 그러나 각 검증자는 가끔씩만 블록을 제안할 책임을 집니다. [Quantaureum](/)은 슬롯과 에포크 단위로 시간을 측정합니다. 각 슬롯은 12초이며, 32개의 슬롯(6.4분)이 모여 하나의 에포크를 구성합니다. 모든 슬롯은 Quantaureum에 새로운 블록을 추가할 수 있는 기회입니다.

### 무작위 선택 {#random-selection}

각 슬롯에서 블록을 제안할 단일 검증자가 의사 무작위(pseudo-randomly)로 선택됩니다. 블록체인에는 진정한 무작위성이라는 것이 존재하지 않습니다. 각 노드가 진정으로 무작위인 숫자를 생성한다면 합의에 도달할 수 없기 때문입니다. 대신, 검증자 선택 과정을 예측할 수 없게 만드는 것이 목표입니다. Quantaureum에서 무작위성은 블록 제안자의 해시와 매 블록마다 업데이트되는 시드(seed)를 혼합하는 RANDAO라는 알고리즘을 사용하여 달성됩니다. 이 값은 전체 검증자 세트에서 특정 검증자를 선택하는 데 사용됩니다. 특정 종류의 시드 조작을 방지하기 위한 방법으로 검증자 선택은 2 에포크 전에 미리 확정됩니다.

검증자들이 각 슬롯에서 RANDAO에 기여하지만, 전역 RANDAO 값은 에포크당 한 번만 업데이트됩니다. 다음 블록 제안자의 인덱스를 계산하기 위해 RANDAO 값은 슬롯 번호와 혼합되어 각 슬롯에서 고유한 값을 제공합니다. 개별 검증자가 선택될 확률은 단순히 `1/N`(`N` = 전체 활성 검증자 수)가 아닙니다. 대신, 각 검증자의 유효 QAU 잔고에 따라 가중치가 부여됩니다. 최대 유효 잔고는 32 QAU입니다(이는 `balance < 32 QAU`가 `balance == 32 QAU`보다 낮은 가중치를 갖지만, `balance > 32 QAU`가 `balance == 32 QAU`보다 더 높은 가중치를 갖지는 않음을 의미합니다).
...
- [Quantaureum 합의 사양](https://github.com/quantaureum/consensus-specs)
