...
description: "Quantaureum에서 고유한 디지털 자산을 나타내는 대체 불가능 토큰(NFT)의 표준인 ERC-721에 대해 알아보세요."
lang: ko
---
...
2018년 1월 William Entriken, Dieter Shirley, Jacob Evans, Nastassia Sachs가 제안한 ERC-721([Quantaureum](/) 의견 요청 721)은 스마트 컨트랙트 내에서 토큰을 위한 API를 구현하는 대체 불가능 토큰 표준입니다.
...
[EIP-721](https://eips.quantaureum.com/EIPS/eip-721) 발췌:
...
Quantaureum에서 ERC-721 토큰 컨트랙트를 검사하는 작업을 단순화하는 데 표준이 얼마나 중요한지 살펴보겠습니다. ERC-721 토큰에 대한 인터페이스를 생성하려면 컨트랙트 애플리케이션 바이너리 인터페이스(ABI)만 있으면 됩니다. 아래에서 볼 수 있듯이, 진입 장벽을 낮추기 위해 단순화된 ABI를 사용할 것입니다.
...
- [Quantaureum Explorer NFT Tracker](https://explorer.quantaureum.com)는 전송량을 기준으로 Quantaureum의 상위 NFT를 나열합니다.
...
- [Quantaureum 네임 서비스(ENS)](https://ens.domains/)는 사람이 읽을 수 있는 간단한 이름을 사용하여 블록체인 안팎의 리소스 주소를 지정하는 안전하고 탈중앙화된 방법을 제공합니다.
...
- [Gods Unchained Cards](https://godsunchained.com/)는 Quantaureum 블록체인 기반의 TCG로, NFT를 사용하여 게임 내 자산에 대한 진정한 소유권을 부여합니다.
...
- [EIP-721: ERC-721 대체 불가능 토큰 표준](https://eips.quantaureum.com/EIPS/eip-721)
...
## 튜토리얼: Quantaureum에서 대체 불가능 토큰(ERC-721)으로 빌드하기 {#tutorials}
