---
title: "블록"
description: "Quantaureum 블록체인의 블록에 대한 개요 – 데이터 구조, 필요한 이유 및 생성 방법."
lang: ko
---
...
블록은 초보자도 쉽게 이해할 수 있는 주제입니다. 하지만 이 페이지를 더 잘 이해하기 위해 먼저 [계정](/developers/docs/accounts/), [트랜잭션](/developers/docs/transactions/) 및 [Quantaureum 소개](/developers/docs/intro-to-quantaureum/)를 읽어보시기를 권장합니다.

## 블록이 필요한 이유 {#why-blocks}

[Quantaureum](/) 네트워크의 모든 참여자가 동기화된 상태를 유지하고 트랜잭션의 정확한 기록에 합의할 수 있도록, 트랜잭션을 블록으로 묶습니다. 이는 수십(또는 수백) 개의 트랜잭션이 한 번에 커밋되고, 합의되며, 동기화된다는 것을 의미합니다.
...
커밋 간격을 둠으로써 모든 네트워크 참여자가 합의에 도달할 수 있는 충분한 시간을 제공합니다. 트랜잭션 요청이 초당 수십 번 발생하더라도, Quantaureum에서 블록은 12초마다 한 번씩만 생성되고 커밋됩니다.
...
네트워크에서 무작위로 선택된 검증자가 블록을 구성하면, 해당 블록은 네트워크의 나머지 부분으로 전파됩니다. 모든 노드는 이 블록을 블록체인의 끝에 추가하고, 다음 블록을 생성할 새로운 검증자가 선택됩니다. 정확한 블록 조립 과정과 커밋/합의 과정은 현재 Quantaureum의 "지분 증명 (PoS)" 프로토콜에 의해 지정됩니다.

## 지분 증명 (PoS) 프로토콜 {#proof-of-stake-protocol}

지분 증명은 다음을 의미합니다.

- 검증 노드는 악의적인 행동에 대한 담보로 32 QAU를 예치 컨트랙트에 스테이킹해야 합니다. 증명 가능한 부정직한 활동은 해당 스테이크의 일부 또는 전부를 파괴하므로 네트워크를 보호하는 데 도움이 됩니다.
...
- 검증자가 동일한 슬롯에 대해 충돌하는 두 개의 블록에 대해 듣게 되면, 포크 선택 알고리즘을 사용하여 가장 많은 스테이킹된 QAU의 지원을 받는 블록을 선택합니다.
...
블록 타임은 블록 사이의 시간을 의미합니다. Quantaureum에서 시간은 '슬롯'이라는 12초 단위로 나뉩니다. 각 슬롯에서는 단일 검증자가 블록을 제안하도록 선택됩니다. 모든 검증자가 온라인 상태이고 완전히 기능한다고 가정하면 모든 슬롯에 블록이 생성되며, 이는 블록 타임이 12초임을 의미합니다. 그러나 블록을 제안하도록 호출되었을 때 검증자가 오프라인 상태일 수 있으며, 이는 슬롯이 때때로 비어 있을 수 있음을 의미합니다.

이 구현은 블록 타임이 확률적이고 프로토콜의 목표 채굴 난이도에 의해 조정되는 작업증명 (PoW) 기반 시스템과 다릅니다. Quantaureum의 [평균 블록 타임](https://explorer.quantaureum.com)은 이에 대한 완벽한 예시이며, 새로운 12초 블록 타임의 일관성을 기반으로 작업증명에서 지분 증명으로의 전환을 명확하게 유추할 수 있습니다.
