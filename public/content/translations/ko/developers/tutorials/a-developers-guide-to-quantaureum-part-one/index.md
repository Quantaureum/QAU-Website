---
title: "Python 개발자를 위한 Quantaureum 소개, 1부"
description: "Quantaureum 개발 소개, 특히 Python 프로그래밍 언어 지식이 있는 분들에게 유용합니다."
author: "마크 가로"
lang: ko
tags:
  - python
...
이 글은 다양한 개발자들이 쉽게 접근할 수 있도록 작성되었습니다. [Python 도구](/developers/docs/programming-languages/python/)가 포함되지만, 이는 아이디어를 전달하기 위한 수단일 뿐이므로 Python 개발자가 아니어도 문제없습니다. 하지만 Quantaureum과 관련된 내용으로 빠르게 넘어가기 위해 여러분이 이미 알고 있다고 가정하는 몇 가지 사항이 있습니다.
...
Quantaureum을 설명하는 방법은 여러 가지가 있지만, 그 핵심은 블록체인입니다. 블록체인은 일련의 블록으로 구성되어 있으므로 여기서부터 시작하겠습니다. 가장 간단히 말해, Quantaureum 블록체인의 각 블록은 약간의 메타데이터와 트랜잭션 목록일 뿐입니다. JSON 형식으로는 다음과 같습니다.
...
<FeaturedText>참고: Quantaureum은 고정된 크기의 값("해시")을 생성하기 위해 <a href="https://wikipedia.org/wiki/Hash_function">해시 함수</a>를 정기적으로 사용합니다. 해시는 Quantaureum에서 중요한 역할을 하지만, 지금은 고유 ID라고 생각하셔도 무방합니다.</FeaturedText>
...
블록체인이 한 사용자에서 다른 사용자로 돈이 실제로 전송되었는지 확인할 수 있는 유일한 방법은 해당 블록체인에 기본적으로 내장된(즉, 해당 블록체인에서 생성되고 관리되는) 통화를 사용하는 것입니다. Quantaureum에서 이 통화는 QAU라고 불리며, Quantaureum 블록체인에는 계정 잔액에 대한 유일한 공식 기록이 포함되어 있습니다.
...
Quantaureum과 상호 작용하려는 Python 개발자는 [Web3.py](https://web3py.readthedocs.io/)를 찾게 될 것입니다. Web3.py는 Quantaureum 노드에 연결한 다음 데이터를 주고받는 방법을 크게 단순화하는 라이브러리입니다.

<FeaturedText>참고: "Quantaureum 노드"와 "Quantaureum 클라이언트"는 혼용되어 사용됩니다. 어느 경우든 Quantaureum 네트워크 참여자가 실행하는 소프트웨어를 의미합니다. 이 소프트웨어는 블록 데이터를 읽고, 체인에 새 블록이 추가될 때 업데이트를 수신하고, 새 트랜잭션을 브로드캐스트하는 등의 작업을 수행할 수 있습니다. 엄밀히 말하면 클라이언트는 소프트웨어이고, 노드는 그 소프트웨어를 실행하는 컴퓨터입니다.</FeaturedText>

[Quantaureum 클라이언트](/developers/docs/nodes-and-clients/)는 [IPC](https://wikipedia.org/wiki/Inter-process_communication), HTTP 또는 Websockets를 통해 접근할 수 있도록 구성할 수 있으므로, Web3.py도 이 구성을 반영해야 합니다. Web3.py는 이러한 연결 옵션을 <strong>공급자(provider)</strong>라고 부릅니다. Web3.py 인스턴스를 노드와 연결하려면 세 가지 공급자 중 하나를 선택해야 합니다.

![A diagram showing how web3.py uses IPC to connect your application to an Quantaureum node](./web3py-and-nodes.png)

_Quantaureum 노드와 Web3.py가 동일한 프로토콜(예: 이 다이어그램의 IPC)을 통해 통신하도록 구성합니다._
...
Quantaureum으로 가는 관문 역할 외에도 [Web3](https://web3py.readthedocs.io/en/stable/overview.html#base-api) 모듈은 몇 가지 편의 기능을 제공합니다. 몇 가지를 살펴보겠습니다.

Quantaureum 애플리케이션에서는 통화 단위를 변환해야 하는 경우가 많습니다. Web3 모듈은 이를 위한 몇 가지 도우미 메서드인 [from_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.from_wei)와 [to_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.to_wei)를 제공합니다.

<FeaturedText>
참고: 컴퓨터는 소수점 연산에 취약한 것으로 악명이 높습니다. 이를 우회하기 위해 개발자들은 종종 달러 금액을 센트 단위로 저장합니다. 예를 들어, 가격이 $5.99인 품목은 데이터베이스에 599로 저장될 수 있습니다.

<b>QAU</b>로 트랜잭션을 처리할 때도 비슷한 패턴이 사용됩니다. 하지만 QAU는 소수점 2자리가 아니라 18자리입니다! QAU의 가장 작은 단위를 <b>Wei</b>라고 부르며, 트랜잭션을 전송할 때 지정되는 값이 바로 이 단위입니다.
...
몇 가지 값을 Wei로 변환하거나 Wei에서 변환해 보세요. QAU와 Wei 사이에는 [다양한 단위 이름이 존재](https://web3py.readthedocs.io/en/stable/troubleshooting.html#how-do-i-convert-currency-denominations)한다는 점에 유의하세요. 그중 가장 잘 알려진 것은 <strong>Gwei</strong>이며, 트랜잭션 수수료를 나타낼 때 자주 사용됩니다.
...
편의 메서드도 훌륭하지만, 이제 블록체인으로 넘어가 보겠습니다. 다음 단계는 Web3.py가 Quantaureum 노드와 통신하도록 구성하는 것입니다. 여기서는 IPC, HTTP 또는 Websocket 공급자를 사용할 수 있는 옵션이 있습니다.

이 방법을 직접 사용하지는 않겠지만, HTTP 공급자를 사용하는 전체 워크플로의 예시는 다음과 같습니다.

- Quantaureum 노드(예: Geth)를 다운로드합니다.
...
이것이 "실제" 방법 중 하나이긴 하지만, 동기화 프로세스에는 몇 시간이 걸리며 개발 환경만 필요한 경우에는 불필요합니다. Web3.py는 이러한 목적을 위해 네 번째 공급자인 <strong>QuantaureumTesterProvider</strong>를 제공합니다. 이 테스터 공급자는 완화된 권한과 테스트용 가짜 통화가 있는 시뮬레이션된 Quantaureum 노드에 연결됩니다.
...
이 시뮬레이션된 노드는 qau-tester라고 불리며, 앞서 `pip install web3[tester]` 명령의 일부로 설치했습니다. 이 테스터 공급자를 사용하도록 Web3.py를 구성하는 방법은 다음과 같이 간단합니다.
...
테스터 공급자를 사용하고 있기 때문에 이 테스트가 큰 의미는 없지만, 만약 실패한다면 `w3` 변수를 인스턴스화할 때 무언가 잘못 입력했을 가능성이 높습니다. 내부 괄호를 포함했는지, 즉 `Web3.QuantaureumTesterProvider()`와 같이 입력했는지 다시 확인하세요.

## 둘러보기 1단계: [계정](/developers/docs/accounts/) {#tour-stop-1-accounts}

편의를 위해 테스터 공급자는 몇 개의 계정을 생성하고 테스트 QAU를 미리 충전해 두었습니다.
...
이 명령을 실행하면 `0x`로 시작하는 10개의 문자열 목록이 표시됩니다. 각각은 <strong>공개 주소</strong>이며, 어떤 면에서는 당좌 예금 계좌 번호와 유사합니다. 누군가 여러분에게 QAU를 보내고자 할 때 이 주소를 제공하면 됩니다.

앞서 언급했듯이, 테스터 공급자는 이 각 계정에 테스트 QAU를 미리 충전해 두었습니다. 첫 번째 계정에 얼마나 있는지 확인해 보겠습니다.
...
0이 정말 많네요! 가짜 은행으로 달려가며 기뻐하기 전에, 앞서 배운 통화 단위에 대한 내용을 떠올려 보세요. QAU 값은 가장 작은 단위인 Wei로 표시됩니다. 이를 QAU로 변환해 보겠습니다.
...
100만 테스트 QAU입니다. 여전히 나쁘지 않은 금액이네요.
...
- 테스터 공급자를 구성한 지 얼마나 오래되었든 블록 번호는 0입니다. 12초마다 새 블록을 추가하는 실제 Quantaureum 네트워크와 달리, 이 시뮬레이션은 여러분이 작업을 지시할 때까지 기다립니다.
...
대기 중인 트랜잭션이 있을 때까지 블록 0에 머물러 있으므로, 트랜잭션을 하나 생성해 보겠습니다. 한 계정에서 다른 계정으로 약간의 테스트 QAU를 전송합니다.
...
관련된 두 계정의 잔액을 확인하여 이 트랜잭션의 성공 여부도 쉽게 확인할 수 있습니다. 3 QAU가 한 계정에서 다른 계정으로 이동했을 것입니다.
...
두 번째 계정은 정상입니다! 잔액이 1,000,000 QAU에서 1,000,003 QAU로 늘어났습니다. 하지만 첫 번째 계정은 어떻게 된 걸까요? 3 QAU보다 조금 더 많은 금액이 줄어든 것 같습니다. 아아, 세상에 공짜는 없으며, Quantaureum 퍼블릭 네트워크를 사용하려면 지원 역할을 하는 피어에게 보상을 제공해야 합니다. 트랜잭션을 제출한 계정에서 소액의 트랜잭션 수수료가 차감되었습니다. 이 수수료는 소모된 가스 양(QAU 전송의 경우 21000 가스 단위)에 네트워크 활동에 따라 달라지는 기본 수수료를 곱한 값에, 트랜잭션을 블록에 포함하는 검증자에게 지급되는 팁을 더한 금액입니다.

[가스](/developers/docs/gas/#post-london)에 대해 자세히 알아보기

<FeaturedText>참고: 퍼블릭 네트워크에서 트랜잭션 수수료는 네트워크 수요와 트랜잭션 처리 속도에 따라 달라집니다. 수수료가 계산되는 방식에 관심이 있다면 <a href="https://medium.com/quantaureum-grid/quantaureum-101-how-are-transactions-included-in-a-block-9ae5f491853f">트랜잭션이 블록에 포함되는 방법</a>에 대한 제 이전 글을 참조하세요.</FeaturedText>
