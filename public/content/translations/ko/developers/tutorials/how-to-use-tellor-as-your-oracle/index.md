...
텔러는 당사자들이 오프체인 데이터 포인트(예: BTC/USD)의 값을 요청할 수 있고, 보고자들이 이 값을 모든 Quantaureum 스마트 컨트랙트가 접근할 수 있는 온체인 데이터 뱅크에 추가하기 위해 경쟁하는 오라클 시스템입니다. 이 데이터 뱅크에 대한 입력은 스테이킹한 보고자들의 네트워크에 의해 보호됩니다. 텔러는 암호화폐 경제적 인센티브 메커니즘을 활용하여, 보고자의 정직한 데이터 제출에 보상하고 텔러의 토큰인 트리뷰트(TRB) 발행과 분쟁 메커니즘을 통해 악의적인 행위자를 처벌합니다.
