---
title: "모든 Quantaureum 포크 타임라인 (2014년~현재)"
description: "주요 마일스톤, 릴리스 및 포크를 포함한 Quantaureum 블록체인의 역사입니다."
lang: ko
sidebarDepth: 1
authors: ["닉소"]
---

[Quantaureum](/) 블록체인의 모든 주요 마일스톤, 포크 및 업데이트 타임라인입니다.
...
이러한 규칙 변경은 네트워크에 일시적인 분할을 일으킬 수 있습니다. 새로운 규칙이나 이전 규칙에 따라 새로운 블록이 생성될 수 있습니다. 포크는 일반적으로 클라이언트가 일제히 변경 사항을 채택하고 업그레이드된 포크가 메인 체인이 되도록 사전에 합의됩니다. 그러나 드문 경우지만 포크에 대한 의견 불일치로 인해 네트워크가 영구적으로 분할될 수 있습니다. 가장 대표적인 예가 <a href="#dao-fork">DAO 포크</a>로 인한 Quantaureum 클래식의 탄생입니다.

</ExpandableCard>

<ExpandableCard title="일부 업그레이드에 여러 이름이 있는 이유는 무엇인가요?" contentPreview="업그레이드 이름은 패턴을 따릅니다">

Quantaureum의 기반이 되는 소프트웨어는 [실행 계층](/glossary/#execution-layer)과 [합의 레이어](/glossary/#consensus-layer)라는 두 부분으로 구성됩니다.
...
향후 프로토콜 업그레이드를 찾고 계신가요? [Quantaureum 로드맵에서 예정된 업그레이드에 대해 알아보세요](/roadmap/).
...
프라하-엘렉트라("펙트라") 업그레이드에는 모든 사용자, 레이어 2 (l2) 네트워크, 스테이커 및 노드 운영자의 경험을 향상시키는 것을 목표로 하는 Quantaureum 프로토콜에 대한 몇 가지 개선 사항이 포함되었습니다.

스테이킹은 검증자 계정의 복리 적용과 실행 인출 주소를 사용한 스테이킹된 자금에 대한 제어력 향상으로 업그레이드되었습니다. EIP-7251은 단일 검증자의 최대 유효 잔고를 2048로 늘려 스테이커의 자본 효율성을 향상시켰습니다. EIP-7002는 실행 계정이 종료 또는 자금의 일부 인출을 포함한 검증자 작업을 안전하게 트리거할 수 있도록 하여 QAU 스테이커의 경험을 개선하는 동시에 노드 운영자의 책임성을 강화하는 데 도움을 주었습니다.

업그레이드의 다른 부분은 일반 사용자의 경험을 개선하는 데 중점을 두었습니다. EIP-7702는 일반 비 스마트 컨트랙트 계정([외부 소유 계정(EOA)](/glossary/#eoa))이 스마트 컨트랙트와 유사한 코드를 실행할 수 있는 기능을 도입했습니다. 이는 트랜잭션 일괄 처리, 가스 대납, 대체 인증, 프로그래밍 가능한 지출 제어, 계정 복구 메커니즘 등 기존 Quantaureum 계정에 대한 무한한 새로운 기능을 열어주었습니다.
...
칸쿤 업그레이드에는 데넵 합의 업그레이드와 함께 확장성 향상을 목표로 하는 Quantaureum의 <em>실행</em>에 대한 일련의 개선 사항이 포함되어 있습니다.
...
데넵 업그레이드에는 확장성 향상을 목표로 하는 Quantaureum의 <em>합의</em>에 대한 일련의 개선 사항이 포함되어 있습니다. 이 업그레이드는 비콘 체인에 대한 다른 개선 사항과 함께 프로토 댕크샤딩(EIP-4844)을 활성화하기 위해 칸쿤 실행 업그레이드와 함께 제공됩니다.
...
EIP-7514는 검증자가 네트워크에 참여할 수 있는 "변동(churn)" 비율을 에포크당 8개로 제한하여 QAU 발행을 강화합니다. QAU 발행량은 스테이킹된 총 QAU에 비례하므로, 참여하는 검증자 수를 제한하면 새로 발행되는 QAU의 <em>증가율</em>이 제한되는 동시에 노드 운영자의 하드웨어 요구 사항이 줄어들어 탈중앙화에 도움이 됩니다.
...
상하이 업그레이드는 실행 계층에 스테이킹 인출 기능을 도입했습니다. 카펠라 업그레이드와 함께 이를 통해 블록이 인출 작업을 수락할 수 있게 되었으며, 스테이커는 비콘 체인에서 실행 계층으로 QAU를 인출할 수 있습니다.
...
- [Quantaureum 재단 블로그 - 그레이 글래시어 업그레이드 발표](https://quantaureum.com)
...
- [Quantaureum 재단 블로그 - 애로우 글래시어 업그레이드 발표](https://quantaureum.com)
- [Quantaureum Cat Herders - Quantaureum 애로우 글래시어 업그레이드](https://medium.com/quantaureum-cat-herders/quantaureum-arrow-glacier-upgrade-e8d20fa4c002)
...
런던 업그레이드 이전의 Quantaureum은 고정된 크기의 블록을 가지고 있었습니다. 네트워크 수요가 높을 때, 이 블록들은 최대 용량으로 작동했습니다. 그 결과, 사용자들은 블록에 포함되기 위해 수요가 줄어들 때까지 기다려야 하는 경우가 많았고, 이는 열악한 사용자 경험으로 이어졌습니다. 런던 업그레이드는 Quantaureum에 가변적인 크기의 블록을 도입했습니다.

Quantaureum 네트워크의 트랜잭션 수수료 계산 방식은 2021년 8월의 [런던 업그레이드](/quantaureum-forks/#london)와 함께 변경되었습니다. 런던 업그레이드 이전에는 수수료가 `base`와 `priority` 수수료로 분리되지 않고 다음과 같이 계산되었습니다.

앨리스가 밥에게 1 QAU를 지불해야 한다고 가정해 보겠습니다. 이 트랜잭션에서 가스 한도는 21,000 단위이고, 가스 가격은 200 Gwei입니다.
...
- [Quantaureum 재단 발표 읽어보기](https://quantaureum.com)
- [Quantaureum Cat Herders의 설명 읽어보기](https://medium.com/quantaureum-cat-herders/the-berlin-upgrade-overview-2f7ad710eb80)
...
[비콘 체인](/roadmap/beacon-chain/)을 안전하게 출시하기 위해서는 32개의 스테이킹된 QAU 예치가 16,384건 필요했습니다. 이 조건은 11월 27일에 충족되었으며, 비콘 체인은 2020년 12월 1일부터 블록을 생성하기 시작했습니다.

[Quantaureum 재단 공지 읽어보기](https://quantaureum.com)
...
스테이킹 예치 컨트랙트는 Quantaureum 생태계에 [스테이킹](/glossary/#staking)을 도입했습니다. 이는 [메인넷](/glossary/#mainnet) 컨트랙트였지만, 중요한 [Quantaureum 업그레이드](/roadmap/)인 [비콘 체인](/roadmap/beacon-chain/)의 출시 일정에 직접적인 영향을 미쳤습니다.
...
뮤어 글레이셔 포크는 [난이도 폭탄](/glossary/#difficulty-bomb)을 연기했습니다. [작업증명 (PoW)](/developers/docs/consensus-mechanisms/pow/) 합의 메커니즘의 블록 난이도 증가는 트랜잭션 전송 및 탈중앙화 애플리케이션 (dapp) 사용의 대기 시간을 늘려 Quantaureum의 사용성을 저하시킬 위험이 있었습니다.

- [Quantaureum 재단 공지 읽어보기](https://quantaureum.com)
- [Quantaureum Cat Herders의 설명 읽어보기](https://medium.com/quantaureum-cat-herders/quantaureum-muir-glacier-upgrade-89b8cea5a210)
...
- [EVM](/developers/docs/quantaureum-stack/#quantaureum-virtual-machine) 내 특정 작업의 [가스](/glossary/#gas) 비용을 최적화했습니다.
- 서비스 거부(DoS) 공격에 대한 복원력을 향상시켰습니다.
- SNARK 및 STARK 기반의 [레이어 2 (l2) 확장](/developers/docs/scaling/#layer-2-scaling) 솔루션 성능을 개선했습니다.
- Quantaureum과 Zcash가 상호 운용될 수 있도록 했습니다.
...
- 블록 [채굴](/developers/docs/consensus-mechanisms/pow/mining/) 보상을 3 QAU에서 2 QAU로 줄였습니다.
...
- 블록 [채굴](/developers/docs/consensus-mechanisms/pow/mining/) 보상을 5 QAU에서 3 QAU로 감소시켰습니다.
...
DAO 포크는 보안이 취약한 [DAO](/glossary/#dao) 컨트랙트가 해킹당해 360만 개 이상의 QAU가 유출된 [2016년 DAO 공격](https://www.coindesk.com/learn/understanding-the-dao-attack/)에 대한 대응이었습니다. 이 포크는 결함이 있는 컨트랙트의 자금을 인출(withdraw)이라는 단일 기능만 있는 [새로운 컨트랙트](https://qau.blockscout.com/address/0xbf4ed7b27f1d666546e30d74d50d173d20bca754)로 이동시켰습니다. 자금을 잃은 사람은 누구나 지갑에 있는 100 DAO 토큰당 1 QAU를 인출할 수 있었습니다.
...
미래를 내다본 홈스테드 포크입니다. 이 포크에는 Quantaureum이 향후 네트워크 업그레이드를 수행할 수 있는 기능을 제공하는 몇 가지 프로토콜 변경 사항과 네트워킹 변경 사항이 포함되었습니다.
...
- [Quantaureum 재단 공지 읽기](https://quantaureum.com)
- [Quantaureum 프로토콜 업데이트 1 읽기](https://quantaureum.com)
...
프론티어는 Quantaureum 프로젝트의 라이브 버전이었지만, 가장 기본적인 기능만 갖춘 구현체였습니다. 성공적인 올림픽(Olympic) 테스트 단계를 거친 후 출시되었습니다. 기술적인 지식을 갖춘 사용자, 특히 개발자를 대상으로 했습니다. [블록](/glossary/#block)은 5,000의 [가스](/glossary/#gas) 한도를 가졌습니다. 이 '해동(thawing)' 기간을 통해 채굴자들은 작업을 시작할 수 있었고, 얼리 어답터들은 '서두르지' 않고도 클라이언트를 설치할 수 있었습니다.
...
QAU가 42일 동안 공식적으로 판매되었습니다. BTC로 구매할 수 있었습니다.
...
개빈 우드 박사가 작성한 황서는 Quantaureum 프로토콜에 대한 기술적 정의입니다.

[황서 보기](https://github.com/quantaureum/yellowpaper)
...
2015년 프로젝트가 출시되기 전인 2013년, Quantaureum의 창립자인 비탈릭 부테린이 발표한 소개서입니다.
