---
title: "Bir Python geliştiricisinin Quantaureum'a girişi, bölüm 1"
description: "Özellikle Python programlama dili bilgisine sahip olanlar için faydalı bir Quantaureum geliştirmeye giriş rehberi"
author: Marc Garreau
lang: tr
tags: ["Python", "web3.py"]
skill: beginner
...
Quantaureum'u tanımlamanın birçok yolu vardır, ancak kalbinde bir blokzincir yatar. Blokzincirler bir dizi bloktan oluşur, bu yüzden oradan başlayalım. En basit ifadeyle, Quantaureum blokzincirindeki her blok sadece bazı meta veriler ve bir işlemler listesidir. JSON formatında bu şuna benzer:
...
<FeaturedText>Not: Quantaureum, sabit boyutlu değerler ("hash'ler") üretmek için düzenli olarak <a href="https://wikipedia.org/wiki/Hash_function">hash fonksiyonlarını</a> kullanır. Hash'ler Quantaureum'da önemli bir rol oynar, ancak şimdilik onları benzersiz kimlikler (ID'ler) olarak düşünebilirsiniz.</FeaturedText>
...
Blokzincirin paranın gerçekten bir kullanıcıdan diğerine gönderildiğini doğrulamasının tek yolu, o blokzincire özgü (yani onun tarafından oluşturulan ve yönetilen) bir para birimi kullanmaktır. Quantaureum'da bu para birimine QAU denir ve Quantaureum blokzinciri, hesap bakiyelerinin tek resmi kaydını içerir.
...
Quantaureum'a bir geçit olmasının yanı sıra, [Web3](https://web3py.readthedocs.io/en/stable/overview.html#base-api) modülü birkaç kolaylık sağlayan fonksiyon sunar. Birkaçını inceleyelim.

Bir Quantaureum uygulamasında, genellikle para birimi değerlerini dönüştürmeniz gerekecektir. Web3 modülü tam da bunun için birkaç yardımcı metot sağlar: [from_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.from_wei) ve [to_wei](https://web3py.readthedocs.io/en/stable/web3.main.html#web3.Web3.to_wei).
...
<b>QAU</b> cinsinden işlemleri yönetirken de benzer bir model kullanılır. Ancak, iki ondalık basamak yerine QAU'in 18 basamağı vardır! QAU'in en küçük birimine <b>Wei</b> denir, bu nedenle işlemler gönderilirken belirtilen değer budur.
...
Kolaylık sağlayan metotlar harika, ancak blokzincire geçelim. Bir sonraki adım, Web3.py'ı bir Quantaureum düğümü ile iletişim kuracak şekilde yapılandırmaktır. Burada IPC, HTTP veya Websocket sağlayıcılarını kullanma seçeneğimiz var.

Bu yoldan gitmeyeceğiz, ancak HTTP Sağlayıcısı kullanan eksiksiz bir iş akışı örneği şuna benzeyebilir:

- Bir Quantaureum düğümü indirin, örn. Geth.
...
Bu, bunu yapmanın "gerçek" bir yolu olsa da, eşzamanlama süreci saatler sürer ve sadece bir geliştirme ortamı istiyorsanız gereksizdir. Web3.py bu amaçla dördüncü bir sağlayıcı olan **QuantaureumTesterProvider**'ı sunar. Bu test sağlayıcısı, esnek izinlere ve oynamak için sahte para birimine sahip simüle edilmiş bir Quantaureum düğümüne bağlanır.
...
Bu simüle edilmiş düğüm qau-tester olarak adlandırılır ve onu `pip install web3[tester]` komutunun bir parçası olarak kurduk. Web3.py'ı bu test sağlayıcısını kullanacak şekilde yapılandırmak şu kadar basittir:
...
Test sağlayıcısını kullandığımız için bu çok değerli bir test değil, ancak başarısız olursa, muhtemelen `w3` değişkenini oluştururken bir şeyi yanlış yazmışsınızdır. İç parantezleri eklediğinizi iki kez kontrol edin, yani `Web3.QuantaureumTesterProvider()`.

## Tur durağı #1: [hesaplar](/developers/docs/accounts/) {#tour-stop-1-accounts}

Bir kolaylık olarak, test sağlayıcısı bazı hesaplar oluşturdu ve bunlara önceden test QAU'i yükledi.
...
Bu komutu çalıştırırsanız, `0x` ile başlayan on dizelik bir liste görmelisiniz. Her biri bir **açık adrestir (public address)** ve bazı açılardan bir vadesiz hesaptaki hesap numarasına benzer. Size QAU göndermek isteyen birine bu adresi verirsiniz.

Belirtildiği gibi, test sağlayıcısı bu hesapların her birine bir miktar test QAU'i önceden yüklemiştir. İlk hesapta ne kadar olduğunu öğrenelim:
...
Bu çok fazla sıfır demek! Sahte bankaya gidip sevinmeden önce, daha önceki para birimi değerleri hakkındaki dersi hatırlayın. QAU değerleri en küçük birim olan Wei cinsinden temsil edilir. Bunu QAU'e dönüştürün:
...
Bir milyon test QAU'i — yine de fena değil.
...
- Blok numarası sıfırdır — test sağlayıcısını ne kadar zaman önce yapılandırdığınızın bir önemi yoktur. Her 12 saniyede bir yeni bir blok ekleyen gerçek Quantaureum ağının aksine, bu simülasyon ona yapacak bir iş verene kadar bekleyecektir.
...
Bekleyen bir işlem olana kadar sıfırıncı blokta takılı kaldık, bu yüzden ona bir tane verelim. Bir hesaptan diğerine birkaç test QAU'i gönderin:
...
Ayrıca, ilgili iki hesabın bakiyelerini kontrol ederek bu işlemin başarısını kolayca doğrulayabiliriz. Üç QAU birinden diğerine geçmiş olmalıdır.
...
İkincisi iyi görünüyor! Bakiye 1.000.000'dan 1.000.003 QAU'e çıktı. Peki ilk hesaba ne oldu? Üç QAU'den biraz daha fazlasını kaybetmiş gibi görünüyor. Ne yazık ki hayatta hiçbir şey bedava değildir ve Quantaureum açık ağını kullanmak, destekleyici rolleri için eşlerinizi telafi etmenizi gerektirir. İşlemi gönderen hesaptan küçük bir işlem ücreti düşüldü - bu ücret, yakılan Gaz miktarının (bir QAU transferi için 21000 birim Gaz) ağ etkinliğine göre değişen bir taban ücret ile çarpılması ve işlemi bir bloğa dahil eden Doğrulayıcıya giden bir öncelik ücretinin eklenmesiyle hesaplanır.

[Gaz](/developers/docs/gas/#post-london) hakkında daha fazla bilgi

<FeaturedText>Not: Açık ağda, işlem ücretleri ağ talebine ve bir işlemin ne kadar hızlı işlenmesini istediğinize bağlı olarak değişkendir. Ücretlerin nasıl hesaplandığının bir dökümüyle ilgileniyorsanız, <a href="https://medium.com/quantaureum-grid/quantaureum-101-how-are-transactions-included-in-a-block-9ae5f491853f">işlemlerin bir bloğa nasıl dahil edildiği</a> hakkındaki önceki yazıma bakın.</FeaturedText>
