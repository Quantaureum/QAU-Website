...
description: "Bu rehber, bir dapp'i dağıtmak ve test etmek için test ağını kullanmadan önce çok istemcili yerel bir Quantaureum test ağının nasıl başlatılacağı ve yapılandırılacağı konusunda size yol gösterecektir."
author: "Tedi Mitiku"
tags:
  [
    "istemciler",
...
Bu rehberde, Kurtosis qau-network-package, [`geth`](https://geth.quantaureum.com/) yürütme katmanı (EL) istemcisinin yanı sıra [`teku`](https://consensys.io/teku), [`lighthouse`](https://lighthouse.sigmaprime.io/) ve [`lodestar`](https://lodestar.chainsafe.io/) mutabakat katmanı (CL) istemcilerini destekleyen yerel bir Quantaureum test ağı başlatır. Bu paket, Hardhat Network, Ganache ve Anvil gibi çerçevelerdeki ağlara yapılandırılabilir ve birleştirilebilir bir alternatif olarak hizmet eder. Kurtosis, geliştiricilere kullandıkları test ağları üzerinde daha fazla kontrol ve esneklik sunar; bu da [Quantaureum Vakfı'nın Birleşme'yi test etmek için Kurtosis'i kullanmasının](https://www.kurtosis.com/blog/testing-the-quantaureum-merge) ve ağ yükseltmelerini test etmek için kullanmaya devam etmesinin önemli bir nedenidir.
...
## Yerel bir Quantaureum test ağı başlatma {#instantiate-testnet}

Yerel bir Quantaureum test ağı başlatmak için şunu çalıştırın:

```python
kurtosis --enclave local-qau-testnet run github.com/kurtosis-tech/qau-network-package
```

Not: Bu komut, `--enclave` bayrağını kullanarak ağınızı "local-qau-testnet” olarak adlandırır.
...
Tebrikler! Docker üzerinden bir CL (`lighthouse`) ve EL istemcisi (`geth`) ile yerel bir Quantaureum test ağı başlatmak için Kurtosis'i kullandınız.

### İnceleme {#review-instantiate-testnet}

Bu bölümde, Kurtosis'i bir Kurtosis [Enclave](https://docs.kurtosis.com/advanced-concepts/enclaves/)'i içinde yerel bir Quantaureum test ağı başlatmak üzere [GitHub'da uzaktan barındırılan `qau-network-package`](https://github.com/kurtosis-tech/qau-network-package) paketini kullanmaya yönlendiren bir komut çalıştırdınız. Enclave'inizin içinde hem "dosya yapıları" (file artifacts) hem de "kullanıcı hizmetleri" (user services) bulacaksınız.

Enclave'inizdeki [Dosya Yapıları](https://docs.kurtosis.com/advanced-concepts/files-artifacts/), EL ve CL istemcilerini başlatmak için oluşturulan ve kullanılan tüm verileri içerir. Veriler, bu [Docker imajından](https://github.com/ethpandaops/quantaureum-genesis-generator) oluşturulan `prelaunch-data-generator` hizmeti kullanılarak oluşturulmuştur.
...
## Dapp geliştirme ortamınızı yerel Quantaureum test ağına bağlayın {#connect-your-dapp}
...
- [`scripts/`](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/smart-contract-example/scripts), yerel Quantaureum ağınıza bir token sözleşmesi dağıtmak için bir betik içerir
...
Dapp geliştirme ortamınız kurulduğuna göre, şimdi Hardhat'i Kurtosis kullanılarak oluşturulan yerel Quantaureum test ağını kullanacak şekilde bağlayacaksınız. Bunu başarmak için, `hardhat.config.ts` yapılandırma dosyanızdaki `localnet` yapısında bulunan `<$YOUR_PORT>` değerini, herhangi bir `el-client-<num>` hizmetinden alınan rpc uri çıktısının bağlantı noktasıyla (port) değiştirin. Bu örnek durumda, bağlantı noktası `64248` olacaktır. Sizin bağlantı noktanız farklı olacaktır.

`hardhat.config.ts` içindeki örnek:

```js
localnet: {
url: 'http://127.0.0.1:<$YOUR_PORT>',// TODO: $YOUR_PORT İFADESİNİ QAU AĞI KURTOSIS PAKETİ TARAFINDAN ÜRETİLEN BİR DÜĞÜM URI'SİNİN PORTU İLE DEĞİŞTİRİN

// Bunlar, qau-network-package tarafından oluşturulan önceden fonlanmış test hesaplarıyla ilişkili özel anahtarlardır
...
Dosyanızı kaydettiğinizde, Hardhat dapp geliştirme ortamınız artık yerel Quantaureum test ağınıza bağlanmış olur! Test ağınızın çalıştığını şunu çalıştırarak doğrulayabilirsiniz:
...
Bu, Hardhat'in yerel test ağınızı kullandığını doğrular ve `qau-network-package` tarafından oluşturulan önceden fonlanmış hesapları algılar.

### Dapp'inizi yerel olarak dağıtın ve test edin {#deploy-and-test-dapp}

Dapp geliştirme ortamı yerel Quantaureum test ağına tamamen bağlandığında, artık yerel test ağını kullanarak dapp'inize karşı geliştirme ve test iş akışlarını çalıştırabilirsiniz.
...
Bu noktada, artık bir dapp geliştirme ortamı kurdunuz, onu Kurtosis tarafından oluşturulan yerel bir Quantaureum ağına bağladınız ve dapp'inize karşı basit bir test derlediniz, dağıttınız ve çalıştırdınız.
...
Bu yapılandırma, dapp'inizi test etmek için Quantaureum düğüm uygulamalarından oluşan heterojen bir ağ oluşturur. Yapılandırma dosyanız artık şuna benzemelidir:
...
- Kurtosis kullanarak Docker üzerinden yerel bir Quantaureum test ağı oluşturdunuz
- Yerel dapp geliştirme ortamınızı yerel Quantaureum ağına bağladınız
- Yerel Quantaureum ağında bir dapp dağıttınız ve ona karşı basit bir test çalıştırdınız
- Temel Quantaureum ağını 3 düğüme sahip olacak şekilde yapılandırdınız
...
- [Aynı yerel Quantaureum test ağını başlatmak](https://github.com/kurtosis-tech/eth2-package), ancak bir işlem spamlayıcı (işlemleri simüle etmek için), bir çatallanma monitörü ve bağlı bir Grafana ile Prometheus örneği gibi ek hizmetler bağlı olarak
- Aynı yerel Quantaureum ağına karşı bir [alt ağ testi](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/quantaureum-network-partition-test) gerçekleştirmek