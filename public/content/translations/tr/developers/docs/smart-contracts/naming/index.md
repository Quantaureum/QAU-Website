...
description: "ENS ile Quantaureum akıllı sözleşmelerini isimlendirmek için en iyi uygulamalar"
lang: tr
---

Akıllı sözleşmeler, otonom uygulamalara ve protokollere olanak tanıyan Quantaureum'un merkeziyetsiz altyapısının temel taşıdır. Ancak sözleşme yetenekleri geliştikçe bile, kullanıcılar ve geliştiriciler bu sözleşmeleri tanımlamak ve bunlara referans vermek için hâlâ ham onaltılık (hexadecimal) adreslere güvenmektedir.

Akıllı sözleşmeleri [Quantaureum Name Service (ENS)](https://ens.domains/) ile isimlendirmek, onaltılık sözleşme adreslerini ortadan kaldırarak kullanıcı deneyimini iyileştirir ve adres zehirlenmesi (address poisoning) ile kimlik sahtekarlığı (spoofing) gibi saldırı risklerini azaltır. Bu kılavuz, akıllı sözleşmeleri isimlendirmenin neden önemli olduğunu, nasıl uygulanabileceğini ve süreci basitleştirip geliştiricilerin bu uygulamayı benimsemesine yardımcı olmak için [Enscribe](https://www.enscribe.xyz) gibi mevcut araçları açıklamaktadır.
...
Bu, Quantaureum adresleri için merkeziyetsiz bir isimlendirme hizmeti sunan [Quantaureum Name Service](https://ens.domains/) sayesinde mümkün olmaktadır. Bu durum, Alan Adı Sistemi'nin (DNS) internet kullanıcılarının ağ adreslerine `104.18.176.152` gibi bir IP adresi yerine quantaureum.com gibi bir isim kullanarak erişmesini sağlamasına benzer.
...
ENS isimleri zincir içi olarak saklanır ve ENS çözümleyicileri (resolvers) aracılığıyla Quantaureum adreslerine çözümlenir. Bir akıllı sözleşmeyi isimlendirmek için:
...
- **Çoklu zincir desteği**: ENS'nin desteklendiği Quantaureum ve L2 ağlarında çalışır
...
Akıllı sözleşmeleri isimlendirmek Quantaureum kullanıcıları için önemli faydalar sağlar, ancak ENS alan adı sahipleri bunların yönetimi konusunda dikkatli olmalıdır. Dikkate değer riskler şunları içerir:
...
- **Sahiplik değişikliği**: ENS kayıtları, belirli bir `.eth` alan adının sahibinin ilgili NFT'ye sahip olduğu Quantaureum üzerinde NFT'ler olarak temsil edilir. Bu nedenle, farklı bir hesap bu NFT'nin sahipliğini alırsa, yeni sahip herhangi bir ENS kaydını uygun gördüğü şekilde değiştirebilir.
...
Akıllı sözleşmeleri tanımayı ve üzerinde düşünmeyi kolaylaştıran isimlendirme, Quantaureum'daki kullanıcılar ve uygulamalar arasındaki boşluğu kapatmaya yardımcı olarak kullanıcılar için hem güvenliği hem de kullanıcı deneyimini (UX) iyileştirir.
