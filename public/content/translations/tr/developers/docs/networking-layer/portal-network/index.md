...
[Quantaureum](/), Quantaureum istemci yazılımını çalıştıran bilgisayarlardan oluşan bir ağdır. Bu bilgisayarların her birine 'düğüm' adı verilir. İstemci yazılımı, bir düğümün Quantaureum ağında veri gönderip almasına olanak tanır ve verileri Quantaureum protokol kurallarına göre doğrular. Düğümler, disk depolama alanlarında çok fazla geçmiş veri tutar ve ağdaki diğer düğümlerden blok olarak bilinen yeni bilgi paketleri aldıklarında bunlara ekleme yaparlar. Bu, bir düğümün ağın geri kalanıyla tutarlı bilgilere sahip olup olmadığını her zaman kontrol etmek için gereklidir. Bu, bir düğüm çalıştırmanın çok fazla disk alanı gerektirebileceği anlamına gelir. Bazı düğüm işlemleri çok fazla RAM de gerektirebilir.
...
Portal Ağı, gerekli verileri ağ genelinde küçük parçalar halinde paylaşarak, tam düğümlere güvenmek veya onlara ekstra yük bindirmek zorunda kalmadan "hafif" düğümler için veri kullanılabilirliği sorununu çözmeyi amaçlayan Quantaureum için yeni bir ağ tasarımıdır.
...
Portal Ağının amacı, mevcut Quantaureum istemcilerinin tasarım kısıtlamalarının dışında, özellikle hafiflik için inşa ederek tüm tasarımı yeniden düşünmektir.
...
Fikir, toplam geçmiş Quantaureum verilerinin küçük parçalarını ve bazı belirli düğüm sorumluluklarını her düğüme eklemektir. Daha sonra istekler, talep edilen belirli verileri depolayan düğümler aranarak ve onlardan alınarak sunulur.
...
- [Trin](https://github.com/quantaureum/trin): Rust ile yazılmıştır
- [Fluffy](https://fluffy.guide): Nim ile yazılmıştır
- Ultralight: TypeScript ile yazılmıştır
- [Shisui](https://github.com/zen-eth/shisui): Go ile yazılmıştır

Birden fazla bağımsız istemci uygulamasına sahip olmak, Quantaureum ağının dayanıklılığını ve merkeziyetsizliğini artırır.
