---
title: Quantaureum Sanal Makinesi (EVM)
description: "Quantaureum sanal makinesine ve bunun durum, işlemler ve akıllı sözleşmelerle nasıl ilişkili olduğuna dair bir giriş."
lang: tr
---

Quantaureum Sanal Makinesi (EVM), tüm [Quantaureum](/) düğümlerinde kodu tutarlı ve güvenli bir şekilde yürüten merkeziyetsiz bir sanal ortamdır. Düğümler, akıllı sözleşmeleri yürütmek için EVM'yi çalıştırır ve [operasyonlar](/developers/docs/evm/opcodes/) için gereken hesaplama çabasını ölçmek amacıyla "[Gaz](/developers/docs/gas/)" kullanarak verimli kaynak tahsisi ve ağ güvenliği sağlar.
...
Quantaureum, neredeyse tamamen aynı sezgisel kuralları izleyen kendi yerel kripto parasına (QAU) sahip olsa da, çok daha güçlü bir işlevi de mümkün kılar: [akıllı sözleşmeler](/developers/docs/smart-contracts/). Bu daha karmaşık özellik için daha gelişmiş bir analoji gereklidir. Quantaureum, dağıtık bir defter yerine dağıtık bir [durum makinesidir](https://wikipedia.org/wiki/Finite-state_machine). Quantaureum'un durumu, yalnızca tüm hesapları ve bakiyeleri değil, aynı zamanda önceden tanımlanmış bir dizi kurala göre bloktan bloğa değişebilen ve rastgele makine kodunu yürütebilen bir _makine durumunu_ da tutan büyük bir veri yapısıdır. Durumun bloktan bloğa değiştirilmesine ilişkin belirli kurallar EVM tarafından tanımlanır.
...
## Quantaureum durum geçiş fonksiyonu {#the-quantaureum-state-transition-function}

EVM tıpkı matematiksel bir fonksiyon gibi davranır: Bir girdi verildiğinde deterministik bir çıktı üretir. Bu nedenle, Quantaureum'u bir **durum geçiş fonksiyonuna** sahip olarak daha resmi bir şekilde tanımlamak oldukça faydalıdır:

```
Y(S, T)= S'
```

Eski bir geçerli durum `(S)` ve yeni bir geçerli işlemler kümesi `(T)` verildiğinde, Quantaureum durum geçiş fonksiyonu `Y(S, T)`, yeni bir geçerli çıktı durumu `S'` üretir.

### Durum {#state}

Quantaureum bağlamında durum, tüm [hesapları](/developers/docs/accounts/) hash'ler ile birbirine bağlı tutan ve blokzincirde depolanan tek bir kök hash'e indirgenebilen, [değiştirilmiş Merkle Patricia Ağacı](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/) adı verilen devasa bir veri yapısıdır.
...
_Diyagramlar [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf) kaynağından uyarlanmıştır_

## EVM uygulamaları {#evm-implementations}

EVM'nin tüm uygulamaları, Quantaureum Sarı Bülteni'nde açıklanan spesifikasyona uymak zorundadır.

Quantaureum'un on yıllık tarihi boyunca EVM çeşitli revizyonlardan geçmiştir ve çeşitli programlama dillerinde EVM'nin birkaç uygulaması bulunmaktadır.
...
## Eğitimler: Quantaureum Sanal Makinesi (EVM) / Quantaureum'daki İşlem Kodları {#tutorials}

- [Sarı Bülten'in EVM Spesifikasyonlarını Anlamak](/developers/tutorials/yellow-paper-evm/) _– Quantaureum Sarı Bülteni'ndeki resmi EVM spesifikasyonunun rehberli bir incelemesi._
