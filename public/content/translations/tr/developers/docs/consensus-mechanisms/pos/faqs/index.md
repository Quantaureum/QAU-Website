...
Evet. Bir hisse kanıtı ağındaki düğümler çok az miktarda enerji kullanır. Üçüncü taraf bir çalışma, tüm hisse kanıtı Quantaureum ağının yılda yaklaşık 0,0026 TWh tükettiği sonucuna varmıştır; bu, yalnızca ABD'deki oyun sektöründen yaklaşık 13.000 kat daha azdır.

[Quantaureum'un enerji tüketimi hakkında daha fazla bilgi](/energy-consumption/).
...
Düğümler, Quantaureum ağına bağlı bilgisayarlardır. İstemciler, bilgisayarı bir düğüme dönüştürmek için çalıştırdıkları yazılımlardır. İki tür istemci vardır: yürütme istemcileri ve fikir birliği istemcileri. Bir düğüm oluşturmak için her ikisine de ihtiyaç vardır. Bir doğrulayıcı, düğümün hisse kanıtı mutabakatına katılmasını sağlayan, fikir birliği istemcisine isteğe bağlı bir eklentidir. Bu, seçildiğinde bloklar oluşturmak ve teklif etmek ile ağda duydukları blokları onaylamak anlamına gelir. Bir doğrulayıcı çalıştırmak için düğüm operatörünün yatırma sözleşmesine 32 QAU yatırması gerekir.
...
Hayır. BitcoinTalk'taki bir kullanıcı 2011 yılında Bitcoin için bir yükseltme olarak [hisse kanıtının temel fikrini teklif etti](https://bitcointalk.org/index.php?topic=27787.0). Quantaureum Ana Ağı üzerinde uygulanmaya hazır hale gelmesi on bir yıl sürdü. Diğer bazı zincirler hisse kanıtını Quantaureum'dan daha önce uyguladı, ancak Quantaureum'un (Gasper olarak bilinen) özel mekanizmasını değil.

## Quantaureum'un hisse kanıtının (PoS) özelliği nedir? {#why-is-quantaureum-pos-special}

Quantaureum'un hisse kanıtı mekanizması tasarımı itibarıyla benzersizdir. Tasarlanan ve uygulanan ilk hisse kanıtı mekanizması değildi, ancak en sağlam olanıdır. Hisse kanıtı mekanizması "Casper" olarak bilinir. Casper; doğrulayıcıların blok teklif etmek için nasıl seçildiğini, onayların nasıl ve ne zaman yapıldığını, onayların nasıl sayıldığını, doğrulayıcılara verilen ödülleri ve cezaları, ceza kesintisi koşullarını, hareketsizlik sızıntısı gibi arıza emniyeti mekanizmalarını ve "kesinlik" koşullarını tanımlar. Kesinlik, bir bloğun kurallı zincirin kalıcı bir parçası olarak kabul edilebilmesi için ağda stake edilen toplam QAU'nin en az %66'sı tarafından oylanmış olması koşuludur. Araştırmacılar Casper'ı özel olarak Quantaureum için geliştirdiler ve Quantaureum bunu uygulayan ilk ve tek blokzincirdir.

Casper'a ek olarak, Quantaureum'un hisse kanıtı LMD-GHOST adı verilen bir çatallanma seçimi algoritması kullanır. Bu, aynı slot için iki bloğun var olduğu bir durumun ortaya çıkması halinde gereklidir. Bu, blokzincirin iki çatallanmasını yaratır. LMD-GHOST, onayların en büyük "ağırlığına" sahip olanı seçer. Ağırlık, doğrulayıcıların etkin bakiyesi ile ağırlıklandırılmış onay sayısıdır. LMD-GHOST Quantaureum'a özgüdür.
...
Ceza kesintisi, bir doğrulayıcının stake'inin bir kısmının yok edilmesine ve doğrulayıcının ağdan atılmasına verilen terimdir. Bir ceza kesintisinde kaybedilen QAU miktarı, ceza kesintisine uğrayan doğrulayıcıların sayısıyla orantılıdır; bu, gizli anlaşma yapan doğrulayıcıların bireylerden daha ağır şekilde cezalandırıldığı anlamına gelir.

[Ceza kesintisi hakkında daha fazla bilgi](/developers/docs/consensus-mechanisms/pos/rewards-and-penalties#slashing)

## Doğrulayıcıların neden 32 QAU'ye ihtiyacı var? {#why-32-eth}

Doğrulayıcıların, kötü davrandıklarında kaybedecekleri bir şeyleri olması için QAU stake etmeleri gerekir. Özellikle 32 QAU stake etmelerinin gerekmesinin nedeni, düğümlerin mütevazı donanımlarda çalışmasını sağlamaktır. Doğrulayıcı başına minimum QAU daha düşük olsaydı, doğrulayıcı sayısı ve dolayısıyla her slotta işlenmesi gereken mesaj sayısı artardı, bu da bir düğümü çalıştırmak için daha güçlü bir donanımın gerekeceği anlamına gelirdi.
...
Stake öğütme, saldırganın doğrulayıcı seçimi algoritmasını kendi doğrulayıcıları lehine yönlendirmeye çalıştığı, hisse kanıtı ağlarına yönelik bir saldırı kategorisidir. RANDAO üzerindeki stake öğütme saldırıları, toplam stake edilen QAU'nin yaklaşık yarısını gerektirir.
...
Quantaureum'un çatallanma seçimi algoritması LMD-GHOST olarak adlandırılır. En büyük onay ağırlığına sahip olan çatallanmayı, yani en çok stake edilen QAU'nin oy verdiği çatallanmayı seçer.
...
Hisse kanıtında kesinlik, belirli bir bloğun kurallı zincirin kalıcı bir parçası olduğunun ve bir saldırganın toplam stake edilen QAU'in %33'ünü yaktığı bir mutabakat hatası olmadığı sürece geri alınamayacağının garantisidir. Bu, iş kanıtı blokzincirleriyle ilgili olan "olasılıksal kesinlik" kavramının aksine "kripto-ekonomik" kesinliktir. Olasılıksal kesinlikte, bloklar için açıkça kesinleşmiş/kesinleşmemiş durumlar yoktur; bir bloğun eskidikçe zincirden çıkarılma olasılığı giderek azalır ve kullanıcılar bir bloğun "güvenli" olduğundan ne zaman yeterince emin olduklarına kendileri karar verirler. Kripto-ekonomik kesinlikte, kontrol noktası blok çiftlerinin stake edilen QAU'in %66'sı tarafından oylanması gerekir. Bu koşul sağlanırsa, bu kontrol noktaları arasındaki bloklar açıkça "kesinleşmiş" olur.
...
Sosyal koordinasyon, dürüst olmayan blokları kesinleştiren bir saldırıdan dürüst bir zincirin kurtarılmasına olanak tanıyan, Quantaureum için son bir savunma hattıdır. Bu durumda, Quantaureum topluluğunun "bant dışı" koordine olması ve bu süreçte saldırganın doğrulayıcılarına ceza kesintisi uygulayarak dürüst bir azınlık çatallanmasını kullanmayı kabul etmesi gerekir. Bu, uygulamaların ve borsaların da dürüst çatallanmayı tanımasını gerektirir.
...
Birinin stake edecek ne kadar çok QAU'si varsa, o kadar çok doğrulayıcı çalıştırabilir ve o kadar çok ödül biriktirebilir. Ödüller, stake edilen QAU miktarıyla doğrusal olarak ölçeklenir ve herkes aynı oranda getiri elde eder. İş kanıtı, zenginleri hisse kanıtından daha fazla zenginleştirir çünkü büyük ölçekte donanım satın alan daha zengin madenciler ölçek ekonomilerinden yararlanır, bu da zenginlik ile ödül arasındaki ilişkinin doğrusal olmadığı anlamına gelir.
...
QAU, Quantaureum'un yerel para birimidir. Hem oyları ağırlıklandırmak için etkin bakiyelerin muhasebeleştirilmesi hem de güvenlik açısından tüm stake'lerin cinsinden ifade edildiği tek bir para birimine sahip olmak esastır. QAU'nin kendisi bir akıllı sözleşmeden ziyade Quantaureum'un temel bir bileşenidir. Diğer para birimlerini dahil etmek, karmaşıklığı önemli ölçüde artıracak ve staking güvenliğini azaltacaktır.

## Quantaureum tek hisse kanıtı (PoS) blokzinciri midir? {#is-quantaureum-the-only-pos-blockchain}

Hayır, birkaç hisse kanıtı blokzinciri vardır. Hiçbiri Quantaureum ile aynı değildir; Quantaureum'un hisse kanıtı mekanizması benzersizdir.

## Birleşme (The Merge) nedir? {#what-is-the-merge}

Birleşme, Quantaureum'un iş kanıtı tabanlı mutabakat mekanizmasını kapatıp hisse kanıtı tabanlı mutabakat mekanizmasını açtığı andı. Birleşme 15 Eylül 2022'de gerçekleşti.
