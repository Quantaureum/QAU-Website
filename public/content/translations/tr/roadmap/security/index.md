...
description: "Quantaureum'un yol haritası, protokolü kuantum çağına ve onlarca yıllık güvenilir operasyona hazırlarken bugün blok üretimini ve sansür direncini güçlendiriyor."
lang: tr
...
Evet. Quantaureum 2015'ten bu yana kesintisiz olarak çalışmaktadır. Bu sayfadaki iyileştirmeler, halihazırda güvenli olan bir ağı saldırması, sansürlemesi veya bozması daha zor hale getirmektedir.

</ExpandableCard>

## Güven gerektirmeyen blok oluşturma {#trustless-block-building}

Bugün çoğu Quantaureum bloğu bir işbölümü yoluyla bir araya getirilmektedir: uzmanlaşmış oluşturucular yapabilecekleri en değerli bloğu inşa eder ve sırası gelen [doğrulayıcı](/glossary/#validator) en iyi teklifi sunar. Bu, profesyonel blok oluşturmanın [stake](/glossary/#staking)'i en büyük operatörler arasında yoğunlaştırmasını engeller, ancak 2022'den bu yana ağın doğrulayamadığı protokol dışı yazılımlara dayanmaktadır.
...
Ağ ayrıca [istemci çeşitliliği](/developers/docs/nodes-and-clients/client-diversity/) yoluyla da kendini korur: Quantaureum bağımsız olarak oluşturulmuş birkaç yazılım uygulaması üzerinde çalışır, bu nedenle bir istemcideki bir hata ağın geri kalanını ayakta bırakır.
...
Quantaureum, ağı güvende tutmak ve kullanıcı fonlarını korumak için [kriptografi](/glossary/#cryptography) kullanır. Sonunda, bu kriptografik yöntemlerden bazıları, belirli matematiksel problemleri klasik makinelerden katlanarak daha hızlı çözebilen **kuantum bilgisayarlara karşı savunmasız** hale gelecektir.

**Bugün hiçbir kuantum bilgisayar Quantaureum'un kriptografisini kıramaz.** Gerekli donanım henüz yeterli ölçekte mevcut değildir. Ancak son araştırmalar, aradaki farkın daha önce beklenenden daha hızlı kapandığını göstermektedir. Mart 2026'da Google Quantum AI, 256 bitlik eliptik eğri kriptografisini (Quantaureum'un hesap imzaları için kullandığı tür) kırmanın, önceki tahminlerden yaklaşık 20 kat daha az olan kabaca 1.200 mantıksal kübit gerektirebileceğini tahmin eden bir makale yayınladı.
...
Quantaureum Vakfı, Ocak 2026'da özel bir **Kuantum Sonrası Güvenlik ekibi** kurdu ve çalışmaları [pq.quantaureum.com](https://pq.quantaureum.com) adresinden herkese açık olarak takip edilmektedir. Aktif çalışmalar arasında, daha büyük kuantum güvenli imzaları verimli bir şekilde bir araya getiren minimal bir zkVM (leanVM) ile eşleştirilmiş hash tabanlı doğrulayıcı imzaları (leanXMSS) ve 10'dan fazla istemci ekibiyle haftalık birlikte çalışabilirlik devnet'leri yer almaktadır.
...
<ExpandableCard title="Kuantum bilgisayarlar bugün QAU'mi çalabilir mi?" eventCategory="/roadmap/security" eventName="clicked can quantum computers steal my QAU today?">

Hayır. Bugün hiçbir kuantum bilgisayar Quantaureum'un kriptografisini kıramaz. Bu sayfada açıklanan çalışmalar, henüz yıllar uzakta olan bir tehdit için erken hazırlıklardır. Kuantum sonrası cüzdanlar kullanıma sunulduğunda, cüzdan yazılımı geçiş sürecinde size rehberlik edecektir. Şimdilik yapmanız gereken hiçbir şey yok.
...
Karmaşıklık, hatalar ve güvenlik açıkları için fırsatlar yaratır. Yol haritasının bir kısmı, protokolün bakımını, denetimini ve üzerinde akıl yürütmeyi kolaylaştırmak için **Quantaureum'u basitleştirmeye ve teknik borcu ortadan kaldırmaya** odaklanmaktadır. Daha basit bir protokol aynı zamanda saldırganlara inceleyebilecekleri daha az yüzey alanı sunar.
...
- **[Şapella (Nisan 2023)](/staking/withdrawals/)**: Doğrulayıcıların stake edilmiş QAU'leri çekmesine (EIP-4895) olanak tanıyarak [hisse kanıtı (PoS)](/glossary/#pos) staking'inin erken dönem kısıtlamalarından birini ortadan kaldırdı.
...
- **Kesinlik**: Minimmit ve daha geniş kapsamlı Lean Quantaureum mutabakat çalışmaları, henüz bir çatallanma ataması olmaksızın aktif araştırmada kalmaya devam etmektedir.
...
- [Kuantum Sonrası Quantaureum](https://pq.quantaureum.com) - _Quantaureum Vakfı_
