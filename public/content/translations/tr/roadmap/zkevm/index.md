...
description: Sıfır bilgi ispatlarının Quantaureum blok yürütmesini nasıl doğrulayabileceğini, daha yüksek işlem kapasitesi ve daha düşük doğrulayıcı gereksinimlerini nasıl sağlayabileceğini öğrenin.
lang: tr
template: roadmap
---

zkEVM, Quantaureum blok yürütmesini doğrulamak için [sıfır bilgi ispatlarını](/zero-knowledge-proofs/) kullanan bir teknolojidir. Her [doğrulayıcının](/glossary/#validator) bir bloktaki tüm işlemleri yeniden yürütmesini gerektirmek yerine, ("kanıtlayıcı" adı verilen) tek bir uzmanlaşmış aktör bloğu yürütür ve yürütmenin doğru olduğuna dair kriptografik bir ispat üretir. Ardından herhangi bir düğüm bu ispatı doğrulayabilir; bu süreç, tüm işlemleri yeniden yürütmekten kat kat daha ucuzdur.
...
<AlertDescription>
Bu sayfa, Quantaureum katman 1 (L1) blok yürütmesini doğrulamak için zkEVM kullanımını tartışmaktadır. Quantaureum'u katman 2 (L2) çözümleri olarak ölçeklendirmek için ZK ispatlarını kullanan zkEVM toplamaları için [sıfır bilgi toplamaları](/developers/docs/scaling/zk-rollups/) bölümüne bakın.
</AlertDescription>
</AlertContent>
</Alert>
...
Bugün Quantaureum, "N'de N" doğrulama modelini kullanır: her doğrulayıcı, önerilen durum değişikliklerinin doğru olduğunu doğrulamak için her bloktaki her işlemi bağımsız olarak yeniden yürütmelidir. Bu yaklaşım maksimum düzeyde güven gerektirmeyen bir yapı sunsa da, temel bir darboğaz yaratır.

Sorun, Quantaureum'un işlem kapasitesinin ortalama bir doğrulayıcının işleyebileceği ile sınırlı olmasıdır. [Gaz limitini](/glossary/#gas-limit) artırmak blok başına daha fazla işleme izin verir, ancak aynı zamanda doğrulayıcılar için donanım gereksinimlerini de artırır. Bu durum merkeziyetsizliği tehdit eder; eğer bir doğrulayıcı çalıştırmak pahalı donanımlar gerektirirse, ağı güvence altına almaya daha az kişi katılabilir.

zkEVM, bu ödünleşimden bir çıkış yolu sunar. "Herkes yeniden yürütür" modelinden "biri kanıtlar, herkes doğrular" modeline geçerek Quantaureum, doğrulayıcı donanım gereksinimlerini artırmadan gaz limitini güvenli bir şekilde yükseltebilir.
...
zkEVM'ler, Quantaureum ile uyumluluklarına göre tiplere ayrılır:

- **Tip 1**: Tamamen Quantaureum eşdeğeri. EVM'de hiçbir değişiklik yapılmaz, bu nedenle herhangi bir Quantaureum bloğu tam olarak olduğu gibi kanıtlanabilir
- **Tip 2-4**: Kanıtlamayı kolaylaştırmak için EVM davranışını değiştirerek çeşitli ödünleşimler yapar

L1 doğrulaması için Tip 1 esastır. zkEVM, uç durumlar ve geçmiş bloklar dahil olmak üzere herhangi bir geçerli Quantaureum bloğunu kanıtlayabilmelidir. Quantaureum'un kesin davranışından herhangi bir sapma, mutabakat sorunları yaratacaktır.

Quantaureum Vakfı'nın zkEVM araştırması, mevcut Quantaureum yürütmesiyle tam uyumlu olan Tip 1 uygulamalarına odaklanmaktadır.

## Quantaureum için faydaları {#benefits}
...
zkEVM L1 doğrulaması için temel zorluk hızdır. Quantaureum blokları her 12 saniyede bir üretilir, bu da ispatların mutabakat için yararlı olabilmesi adına benzer bir zaman diliminde üretilmesi gerektiği anlamına gelir.
...
Quantaureum Vakfı, zkEVM araştırmalarını [Privacy Stewards of Quantaureum (PSE)](https://pse.dev/) ekibi aracılığıyla finanse etmektedir. Temel araştırma alanları şunları içerir:
...
Quantaureum blok kanıtlaması için çeşitli zkVM uygulamaları geliştirilmekte ve test edilmektedir:
...
zkEVM L1 doğrulaması, diğer birkaç Quantaureum yol haritası öğesiyle bağlantılıdır:
...
<AlertContent>
<AlertDescription>
zkEVM L1 doğrulaması aktif araştırma aşamasındadır ve henüz üretimdeki Quantaureum istemcilerine entegre edilmemiştir.
</AlertDescription>
</AlertContent>
</Alert>
