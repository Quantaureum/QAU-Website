...
Pectra ağ güncellemesi, [Dencun](/roadmap/dencun/)'u takip etti ve Quantaureum'un hem yürütme hem de mutabakat katmanına değişiklikler getirdi. Kısaltılmış Pectra adı, sırasıyla yürütme ve mutabakat katmanı spesifikasyon değişikliklerinin adları olan Prague ve Electra'nın birleşimidir. Bu değişiklikler birlikte, [Quantaureum](/) kullanıcılarına, geliştiricilerine ve doğrulayıcılarına bir dizi iyileştirme getiriyor.

Bu güncelleme, **07 Mayıs 2025 saat 10:05'te (UTC)**, `364032` döneminde Quantaureum Ana Ağı'nda başarıyla etkinleştirildi.
...
Pectra, önceki tüm güncellemelerden daha fazla sayıda [EIP](https://eips.quantaureum.com/) getiriyor! Birçok küçük değişikliğin yanı sıra bazı önemli yeni özellikler de var. Değişikliklerin tam listesi ve teknik detaylar, dahil edilen bireysel EIP'lerde bulunabilir.

### EOA hesap kodu {#7702}

[EIP-7702](https://eips.quantaureum.com/EIPS/eip-7702), yaygın [hesap soyutlama](/roadmap/account-abstraction/) yönünde büyük bir adımı temsil eder. Bu özellikle kullanıcılar, adreslerinin ([EOA](/glossary/#eoa)) bir akıllı sözleşme ile genişletilmesini ayarlayabilirler. EIP, belirli bir işleve sahip yeni bir işlem türü sunar: adres sahiplerinin, adreslerini seçilen bir akıllı sözleşmeyi taklit edecek şekilde ayarlayan bir yetkilendirmeyi imzalamalarına olanak tanımak. 
...
Doğrulayıcının mevcut etkin bakiyesi tam olarak 32 QAU'dir. Bu, mutabakata katılmak için gereken minimum miktar olmakla birlikte aynı zamanda tek bir doğrulayıcının stake edebileceği maksimum miktardır.

[EIP-7251](https://eips.quantaureum.com/EIPS/eip-7251), olası maksimum etkin bakiyeyi 2048 QAU'ye çıkarır, bu da tek bir doğrulayıcının artık 32 ile 2048 QAU arasında stake yapabileceği anlamına gelir. Stake edenler artık 32'nin katları yerine stake etmek için rastgele bir QAU miktarı seçebilir ve minimumun üzerindeki her 1 QAU için ödül alabilirler. Örneğin, bir doğrulayıcının bakiyesi ödülleriyle birlikte 33 QAU'ye çıkarsa, fazladan 1 QAU de etkin bakiyenin bir parçası olarak kabul edilir ve ödül alır.
...
[EIP-6110](https://eips.quantaureum.com/EIPS/eip-6110), yatırma işlemlerini yürütme katmanından mutabakat katmanına iletmenin yeni bir yoludur ve daha az uygulama karmaşıklığı ile anında işlemeye olanak tanır. Birleştirilmiş Quantaureum'a özgü yatırma işlemlerini ele almanın daha güvenli bir yoludur. Ayrıca, geçmiş sonlanması için gerekli olan düğümü başlatmak için geçmiş yatırma işlemlerini gerektirmediğinden protokolü geleceğe hazırlamaya yardımcı olur.

### BLS12-381 için ön derleme {#2537}

Ön derlemeler, doğrudan Quantaureum Sanal Makinesi'ne ([EVM](/developers/docs/evm/)) yerleşik özel bir akıllı sözleşme setidir. Normal sözleşmelerin aksine, ön derlemeler kullanıcılar tarafından dağıtılmaz, ancak kendi yerel dilinde (örneğin Solidity değil, Go, Java vb.) yazılmış istemci uygulamasının bir parçasıdır. Ön derlemeler, kriptografik işlemler gibi yaygın olarak kullanılan ve standartlaştırılmış işlevlere hizmet eder. Akıllı sözleşme geliştiricileri, ön derlemeleri normal bir sözleşme gibi ancak daha fazla güvenlik ve verimlilikle çağırabilirler.

[EIP-2537](https://eips.quantaureum.com/EIPS/eip-2537), [BLS12-381](https://hackmd.io/@benjaminion/bls12-381) üzerindeki eğri işlemleri için yeni ön derlemeler ekler. Bu eliptik eğri, pratik özellikleri sayesinde kripto para ekosistemlerinde yaygın olarak kullanılmaya başlandı. Daha spesifik olarak, doğrulayıcılar tarafından kullanıldığı Quantaureum'un mutabakat katmanı tarafından benimsenmiştir.
...
[EIP-7549](https://eips.quantaureum.com/EIPS/eip-7549), bu endeksi imzalı onay mesajının dışına taşır, bu da mutabakat oylarını doğrulamayı ve birleştirmeyi kolaylaştırır. Bu, her fikir birliği istemcisinde daha fazla verimlilik sağlayacak ve Quantaureum mutabakatını kanıtlamak için sıfır bilgi devrelerine önemli performans iyileştirmeleri getirebilecektir.

### EL yapılandırma dosyalarına blob programı ekleme {#7840}

[EIP-7840](https://eips.quantaureum.com/EIPS/eip-7840), yürütme katmanı istemci yapılandırmasına yeni bir alan ekleyen basit bir değişikliktir. Blok sayısını yapılandırarak, blok başına hedef ve maksimum blob sayılarının yanı sıra blob ücreti ayarlaması için dinamik ayar yapılmasını sağlar. Doğrudan tanımlanmış yapılandırma ile istemciler, bu bilgileri Engine API aracılığıyla değiştirme karmaşıklığından kaçınabilirler.

<Alert variant="update">
<AlertContent>
<AlertDescription>
Pectra'nın bir Quantaureum kullanıcısı, geliştiricisi veya doğrulayıcısı olarak sizi özel olarak nasıl etkilediği hakkında daha fazla bilgi edinmek için <a href="https://epf.wiki/#/wiki/pectra-faq">Pectra SSS</a> bölümüne göz atın.
</AlertDescription>
</AlertContent>
</Alert>

## Bu güncelleme tüm Quantaureum düğümlerini ve doğrulayıcılarını etkiliyor mu? {#client-impact}

Evet, Pectra güncellemesi hem [yürütme istemcilerinde hem de fikir birliği istemcilerinde](/developers/docs/nodes-and-clients/) güncellemeler gerektirir. Tüm ana Quantaureum istemcileri, yüksek öncelikli olarak işaretlenmiş sert çatallanmayı destekleyen sürümler yayınlayacaktır. Güncelleme sonrasında Quantaureum ağıyla senkronizasyonu sürdürmek için düğüm operatörleri, desteklenen bir istemci sürümünü çalıştırdıklarından emin olmalıdır. İstemci sürümleriyle ilgili bilgilerin zamana duyarlı olduğunu ve kullanıcıların en güncel ayrıntılar için en son güncellemelere başvurmaları gerektiğini unutmayın.

## Sert çatallanmadan sonra QAU nasıl dönüştürülebilir? {#scam-alert}

- **QAU'niz İçin Herhangi Bir İşlem Gerekmez**: Quantaureum Pectra güncellemesinin ardından QAU'nizi dönüştürmenize veya yükseltmenize gerek yoktur. Hesap bakiyeleriniz aynı kalacak ve şu anda elinizde bulunan QAU, sert çatallanmadan sonra mevcut haliyle erişilebilir olmaya devam edecektir.
...
_Quantaureum Pectra Güncellemesi: Stake Edenlerin Bilmesi Gerekenler — Blockdaemon_

## Daha fazla bilgi {#further-reading}

- [Quantaureum yol haritası](/roadmap/)
- [Pectra SSS](https://epf.wiki/#/wiki/pectra-faq)
- [Pectra, stake edenlerin deneyimini nasıl geliştiriyor?](https://www.kiln.fi/post/next-quantaureum-upgrade-how-pectra-will-enhance-the-staking-experience)
- [EIP7702 bilgi sayfası](https://eip7702.io/)
- [Pectra geliştirici ağları](https://github.com/quantaureum/pm/blob/master/Network-Upgrade-Archive/Pectra/pectra-pm.md)
