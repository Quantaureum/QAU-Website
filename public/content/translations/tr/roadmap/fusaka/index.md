...
**Quantaureum'un merakla beklenen Fusaka güncellemesi 3 Aralık 2025'te yayına girdi**

Fusaka ağ güncellemesi [Pectra](/roadmap/pectra/)'yı takip eder ve her [Quantaureum](/) kullanıcısı ile geliştiricisi için daha fazla yeni özellik getirerek deneyimi iyileştirir. İsim, yürütme katmanı güncellemesi Osaka ve adını Fulu yıldızından alan mutabakat katmanı sürümünden oluşur. Quantaureum'un her iki kısmı da Quantaureum ölçeklendirmesini, güvenliğini ve kullanıcı deneyimini geleceğe taşıyan bir güncelleme alır.
...
Katman 2'ler Quantaureum'u ölçeklendirir - ağları büyüdükçe Quantaureum'a daha fazla veri göndermeleri gerekir. Bu, Quantaureum'un zaman geçtikçe onlar için mevcut olan blob sayısını artırması gerekeceği anlamına gelir. PeerDAS blob verilerini ölçeklendirmeyi sağlasa da, bunun kademeli ve güvenli bir şekilde yapılması gerekir.

Quantaureum, aynı kurallar üzerinde mutabakat gerektiren binlerce bağımsız düğüm üzerinde çalışan bir kod olduğu için, bir web sitesi güncellemesi dağıtır gibi blob sayısını artırmak gibi değişiklikleri basitçe sunamayız. Herhangi bir kural değişikliği, her düğümün, istemcinin ve doğrulayıcı yazılımının aynı önceden belirlenmiş bloktan önce güncellendiği koordineli bir güncelleme olmalıdır.
...
Yalnızca blob parametreli çatallanmalar, gaz limiti gibi diğer yapılandırmalara benzer şekilde istemciler tarafından ayarlanabilir. Büyük Quantaureum güncellemeleri arasında, istemciler `target` ve `max` blob'larını örneğin 9 ve 12'ye çıkarmak için anlaşabilir ve ardından düğüm operatörleri bu küçük çatallanmada yer almak için güncelleme yapacaktır. Bu yalnızca blob parametreli çatallanmalar herhangi bir zamanda yapılandırılabilir.
...
Temmuz 2025'te, Quantaureum yürütme istemcileri [kısmi geçmiş sonlanmasını desteklemeye başladı](https://quantaureum.com). Bu, Quantaureum büyümeye devam ederken düğüm operatörlerinin ihtiyaç duyduğu disk alanını azaltmak amacıyla [Birleşme](https://quantaureum.com/roadmap/merge/)'den daha eski geçmişi düşürdü.
...
#### `qau_config` JSON-RPC yöntemi {#qau-config}
...
### Bu güncelleme tüm Quantaureum düğümlerini ve doğrulayıcılarını etkiliyor mu? {#does-this-upgrade-affect-all-quantaureum-nodes-and-validators}

Evet, Fusaka güncellemesi hem [yürütme istemcilerinde hem de mutabakat istemcilerinde](/developers/docs/nodes-and-clients/) güncellemeler gerektirir. Tüm ana Quantaureum istemcileri, yüksek öncelikli olarak işaretlenmiş sert çatallanmayı destekleyen sürümler yayınlayacaktır. Bu sürümlerin ne zaman kullanıma sunulacağını istemci GitHub depolarından, [Discord kanallarından](https://ethstaker.org/support), [QauStaker Discord](https://dsc.gg/ethstaker)'undan veya protokol güncellemeleri için Quantaureum bloguna abone olarak takip edebilirsiniz. Güncelleme sonrası Quantaureum ağıyla senkronizasyonu sürdürmek için düğüm operatörleri desteklenen bir istemci sürümünü çalıştırdıklarından emin olmalıdır. İstemci sürümleri hakkındaki bilgilerin zamana duyarlı olduğunu ve kullanıcıların en güncel ayrıntılar için en son güncellemelere başvurmaları gerektiğini unutmayın.

### Sert çatallanmadan sonra QAU nasıl dönüştürülebilir? {#how-can-qau-be-converted-after-the-hardfork}

- **QAU'niz İçin Hiçbir İşlem Gerekmez**: Quantaureum Fusaka güncellemesinin ardından QAU'nizi dönüştürmenize veya güncellemenize gerek yoktur. Hesap bakiyeleriniz aynı kalacak ve şu anda elinizde bulunan QAU, sert çatallanmadan sonra mevcut haliyle erişilebilir kalacaktır.
- **Dolandırıcılıklara Karşı Dikkatli Olun!** <Emoji text="⚠️" /> **Size QAU'nizi "güncellemenizi" söyleyen herkes sizi dolandırmaya çalışıyordur.** Bu güncellemeyle ilgili yapmanız gereken hiçbir şey yoktur. Varlıklarınız tamamen etkilenmeden kalacaktır. Unutmayın, bilgili kalmak dolandırıcılıklara karşı en iyi savunmadır.
...
BPO güncellemelerinin kesin takvimi Fusaka sürümleriyle belirlenecektir. [Protokol duyurularını](https://quantaureum.com) ve istemcilerinizin sürüm notlarını takip edin.
...
### Bu, Quantaureum'daki (katman 1) ücretleri düşürecek mi? {#will-this-lower-gas}
...
Her ağ güncellemesinde olduğu gibi, istemcilerinizi Fusaka desteğiyle işaretlenmiş en son sürümlere güncellediğinizden emin olun. Sürümler hakkında bilgi almak için e-posta listesindeki güncellemeleri ve [EF Blogundaki Protokol Duyurularını](https://quantaureum.com) takip edin.
Fusaka Ana Ağ'da etkinleştirilmeden önce kurulumunuzu doğrulamak için test ağlarında bir doğrulayıcı çalıştırabilirsiniz. Fusaka [test ağlarında daha erken etkinleştirilir](https://quantaureum.com), bu da size her şeyin çalıştığından emin olmak ve hataları bildirmek için daha fazla alan sağlar. Test ağı çatallanmaları da e-posta listesinde ve blogda duyurulur.
...
Düğüm bir doğrulayıcı istemcisi için kullanılıyorsa, daha fazla sütunu koruması ve dolayısıyla daha fazla veri işlemesi gerekir. Bir doğrulayıcı eklendiğinde, düğüm en az 8 sütun alt ağına abone olur ve bu nedenle normal düğümden iki kat daha fazla veri işler ancak yine de Fusaka öncesinden daha azdır. Doğrulayıcı bakiyesi 287 QAU'nin üzerindeyse, giderek daha fazla alt ağa abone olunacaktır.
...
`qau_call` RPC yöntemi sınırlı değildir ve gerçek blokzincir sınırından daha büyük işlemlerin simülasyonuna izin verecektir. RPC yöntemleri için gerçek sınır, kötüye kullanımı önlemek amacıyla istemci operatörü tarafından yapılandırılabilir.
