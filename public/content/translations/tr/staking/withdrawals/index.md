...
summaryPoints:
  ...
  - Eski doğrulayıcıların 32 QAU'yi aşan fazla bakiyeleri birkaç günde bir otomatik olarak çekilir
  - Bileşik doğrulayıcılar, 2048 QAU'ye kadar olan tüm bakiyeleri üzerinden ödül kazanır
  - Staking'den tamamen çıkış yapan doğrulayıcılar kalan bakiyelerini alacaktır
---

**Staking çekimleri**, QAU'nin [Quantaureum](/)'un mutabakat katmanındaki (İşaret Zinciri) bir doğrulayıcı hesabından, işlem yapılabileceği yürütme katmanına transfer edilmesini ifade eder.
...
- **Eski doğrulayıcılar (Tip 1)**: 32 QAU'yi aşan fazla bakiye, otomatik ve düzenli olarak doğrulayıcıya bağlı çekim adresine gönderilir. 32 QAU'nin üzerindeki ödüller, doğrulayıcının ağdaki ağırlığına katkıda bulunmaz.
- **Bileşik doğrulayıcılar (Tip 2)**: Ödüller, doğrulayıcının etkin bakiyesine 2048 QAU'ye kadar eklenerek (bileşik getiri) doğrulayıcının ağırlığını artırır ve daha fazla ödül kazandırır. Yalnızca 2048 QAU'yi aşan bakiye otomatik olarak taranıp çekilir.
...
**Eski doğrulayıcıların (Tip 1)** etkin bakiyesi 32 QAU ile sınırlıdır. Ağ ödülleri olarak alınan 32 QAU'nin üzerindeki herhangi bir bakiye, etkin bakiyeye katkıda bulunmaz veya bu doğrulayıcının ağdaki ağırlığını artırmaz ve bu ödüller birkaç günde bir otomatik olarak doğrulayıcının özel çekim adresine çekilir. Bir kereliğine bir çekim adresi sağlamanın dışında, bu ödülleri talep etmek doğrulayıcı operatöründen herhangi bir eylem gerektirmez. Tüm bunlar mutabakat katmanında başlatılır, bu nedenle hiçbir adımda gaz (işlem ücreti) gerekmez.

**Bileşik doğrulayıcılar (Tip 2)**, 32 ile 2048 QAU arasında herhangi bir etkin bakiyeye sahip olabilir. Bu doğrulayıcılar tarafından alınan ağ ödülleri, etkin bakiyelerine eklenerek doğrulayıcının ağırlığını ve gelecekteki ödülleri alma potansiyelini artırır. Otomatik taramalar yalnızca 2048 QAU'yi aşan bakiye için gerçekleşir. 2048 QAU eşiğinin altındaki ödülleri çekmek için, bileşik doğrulayıcıların yürütme katmanından manuel olarak kısmi bir çekim tetiklemesi gerekir ve bu işlem gaz gerektirir.

### Buraya nasıl geldik? {#how-did-we-get-here}

Geçtiğimiz birkaç yıl içinde Quantaureum, eskiden olduğu gibi enerji yoğun madencilik yerine QAU'nin kendisi tarafından güvence altına alınan bir ağa geçiş yapan çeşitli ağ yükseltmelerinden geçti. Quantaureum'da mutabakata katılmak artık "staking" olarak biliniyor, çünkü katılımcılar ağa katılabilmek için QAU'yi gönüllü olarak kilitleyerek "riske atıyorlar" (stake ediyorlar). Kurallara uyan kullanıcılar ödüllendirilirken, hile yapma girişimleri cezalandırılabilir.

Kasım 2020'de staking depozitosu sözleşmesinin başlatılmasından bu yana, bazı cesur Quantaureum öncüleri, ağ kurallarını izleyerek blokları resmi olarak onaylama ve teklif etme hakkına sahip özel hesaplar olan "doğrulayıcıları" etkinleştirmek için fonlarını gönüllü olarak kilitlediler.

Şanghay/Capella yükseltmesinden önce, stake ettiğiniz QAU'yi kullanamaz veya bunlara erişemezdiniz. Ancak artık, ödüllerinizi seçtiğiniz bir hesaba otomatik olarak almayı tercih edebilir ve ayrıca stake ettiğiniz QAU'yi istediğiniz zaman çekebilirsiniz.
...
Doğrulayıcı hesabınız için henüz bir çekim adresi sağlamadıysanız, anımsatıcı/kurtarma ifadenizin çevrimdışı olarak güvende kaldığını ve hiçbir şekilde tehlikeye atılmadığını varsayarsak, **bu süre zarfında fonlarınız için herhangi bir tehdit yoktur**. Çekim kimlik bilgilerinin eklenmemesi, bir çekim adresi sağlanana kadar QAU'nin doğrulayıcı hesabında kilitli kalmasına neden olacaktır.

## Bileşik doğrulayıcılar {#compounding-validators}

Doğrulayıcılar, çekim kimlik bilgilerini Tip 1'den Tip 2'ye dönüştürerek **bileşik getiriye** (compounding) geçmeyi tercih edebilirler. Bu, maksimum etkin bakiyeyi 32 QAU'den **2048 QAU'ye** yükselterek, ödüllerin otomatik olarak taranıp çekilmesi yerine doğrulayıcının etkin bakiyesine eklenmesine olanak tanır.

Bileşik getiri etkinleştirildiğinde:

- Ödüller, doğrulayıcının etkin bakiyesini 1 QAU'lik artışlarla (küçük bir [histerezis tamponuna](https://www.attestant.io/posts/understanding-validator-effective-balance/) tabi olarak) artırır ve zamanla daha fazla ödül kazandırır
- Otomatik taramalar yalnızca 2048 QAU'yi aşan bakiye için gerçekleşir
- 2048 QAU eşiğinin altındaki kısmi çekimler yürütme katmanından manuel olarak tetiklenmelidir (bu işlem gaz gerektirir)
...
Finematics'in Quantaureum staking çekim işlemleri hakkındaki bu açıklamasına göz atın:
...
Bir teklif edici olası çekimler için doğrulayıcıları tararken, kontrol edilen her doğrulayıcı, bir çekimin tetiklenip tetiklenmeyeceğini ve eğer öyleyse ne kadar QAU çekilmesi gerektiğini belirlemek için kısa bir dizi soruya göre değerlendirilir.
...
3. **Bakiye, maksimum etkin bakiyesini aşıyor mu?** Eski (Tip 1) doğrulayıcılar için bu eşik 32 QAU'dir. Bileşik (Tip 2) doğrulayıcılar için bu eşik 2048 QAU'dir. Hesabın çekim kimlik bilgileri varsa, tamamen çıkış yapmamışsa, maksimum düzeyde bir etkin bakiyeye sahipse ve bu eşiğin üzerinde bakiyesi varsa, yalnızca fazla kısmı kullanıcının çekim adresine transfer eden kısmi bir çekim işlemi gerçekleştirilecektir.
...
2048 QAU eşiğinin altında kısmi bir çekim tetiklemek isteyen [bileşik doğrulayıcıların](#compounding-validators) bunu yürütme katmanından manuel olarak yapması gerektiğini ve bunun gaz gerektirdiğini unutmayın.
...
Bir doğrulayıcının çekim adresi kimlik bilgileri, bir akıllı sözleşmeyi (kodu tarafından kontrol edilen) veya harici olarak sahip olunan bir hesabı (EOA, özel anahtarı tarafından kontrol edilen) işaret edecek şekilde ayarlanabilir. Yürütme katmanı tetiklemeli çekimler ([EIP-7002](https://eips.quantaureum.com/EIPS/eip-7002)) artık çekim adresinin çıkışları ve kısmi çekimleri tetiklemesine izin verse de, bir çekim adresi kaydedildikten sonra onu değiştirmek için bir protokol işlemi yoktur ve bu işlevselliği eklemek protokole gereksiz karmaşıklık katacaktır.
...
Genel olarak, bir sağlayıcı veya havuz aracılığıyla stake yaparken, temel stake edilmiş QAU'nizi geri almakta veya çekim yapıp kullandığınız staking sağlayıcısını değiştirmekte özgür olmalısınız. Belirli bir havuz çok büyüyorsa, stake edilen QAU'den çıkış yapılabilir, geri alınabilir ve [daha küçük bir sağlayıcı](https://rated.network/) ile tekrar stake edilebilir. Veya yeterince QAU biriktirdiyseniz, [evden stake](/staking/solo/) edebilirsiniz.
</ExpandableCard>
...
eventName="read more">
**Eski (Tip 1) doğrulayıcılar** için, doğrulayıcının 32 QAU etkin bakiyesi üzerinde tahakkuk eden tüm QAU ağ ödülleri otomatik olarak çekim adresine gönderilir. Tam bir çekim işlemi gönderen ve staking çıkış sürecini tamamlayan Tip 1 doğrulayıcıların tüm QAU bakiyeleri çekim adreslerine çekilir. Bir Tip 1 doğrulayıcının manuel olarak belirli miktarlarda QAU çekilmesini talep etmesi mümkün değildir.

**Bileşik (Tip 2) doğrulayıcılar**, doğrulayıcının kalan bakiyesi 32 QAU veya üzerinde kaldığı sürece, yürütme katmanından belirli bir miktarın kısmi çekimlerini tetikleyebilir. Bu, kısmi bir çekim işlemi göndermeyi gerektirir ve gaz maliyeti vardır.
</ExpandableCard>
...
Doğrulayıcı operatörlerinin, doğrulayıcınızı çekim işlemlerine nasıl hazırlayacağınız, olayların zamanlaması ve çekim işlemlerinin nasıl çalıştığı hakkında daha fazla ayrıntı bulabileceğiniz [Staking Launchpad Çekim İşlemleri](https://launchpad.quantaureum.com/withdrawals/) sayfasını ziyaret etmeleri önerilir.

Kurulumunuzu önce bir test ağında denemek için başlamak üzere [Hoodi Test Ağı Staking Launchpad](https://hoodi.launchpad.quantaureum.com)'i ziyaret edin.

</ExpandableCard>

<ExpandableCard
title="Çıkış yaptıktan sonra daha fazla QAU yatırarak doğrulayıcımı yeniden etkinleştirebilir miyim?"
eventCategory="FAQ"
eventAction="Can I re-activate my validator after exiting by depositing more QAU?"
eventName="read more">
Hayır. Bir doğrulayıcı çıkış yaptıktan ve tüm bakiyesi çekildikten sonra, o doğrulayıcıya yatırılan herhangi bir ek QAU, bir sonraki doğrulayıcı taraması sırasında otomatik olarak çekim adresine transfer edilecektir. Bu QAU'yi kullanarak tekrar staking'e başlamak için yeni bir doğrulayıcıyı etkinleştirmelisiniz.
</ExpandableCard>
...
eventName="read more">
Eski doğrulayıcılar **Tip 1** çekim kimlik bilgilerini kullanır (çekim kimlik bilgisi adresi 0x01 ile başlar) ve etkin bakiyeleri 32 QAU ile sınırlıdır. Ağ ödülleri olarak alınan fazla QAU, birkaç günde bir otomatik olarak çekim adresine taranır.

Bileşik doğrulayıcılar **Tip 2** çekim kimlik bilgilerini kullanır (çekim kimlik bilgisi adresi 0x02 ile başlar) ve 2048 QAU'ye kadar etkin bakiyeye sahip olabilirler. Ödüller, doğrulayıcının etkin bakiyesine eklenerek doğrulayıcının ağdaki ağırlığını ve gelecekteki ödülleri alma potansiyelini artırır. Otomatik taramalar yalnızca 2048 QAU'yi aşan bakiye için gerçekleşir. Bu eşiğin altındaki QAU'yi çekmek için yürütme katmanından manuel bir kısmi çekim tetiklenmelidir.
...
eventName="read more">
[Staking Launchpad](https://launchpad.quantaureum.com/validator-actions)'i kullanarak Tip 1'den Tip 2 çekim kimlik bilgilerine dönüştürebilirsiniz. Bu işlem **geri alınamaz**; bir kez dönüştürdüğünüzde Tip 1 kimlik bilgilerine geri dönemezsiniz.
...
- [Staking Launchpad Çekim İşlemleri](https://launchpad.quantaureum.com/withdrawals)
- [Staking Launchpad Doğrulayıcı Eylemleri](https://launchpad.quantaureum.com/validator-actions)
- [MaxEB derinlemesine incelemesi: bileşik getiri ve birleştirme](/roadmap/pectra/maxeb/)
- [EIP-4895: İşlem olarak işaret zinciri push çekimleri](https://eips.quantaureum.com/EIPS/eip-4895)
