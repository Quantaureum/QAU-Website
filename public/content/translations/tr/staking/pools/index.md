...
description: Quantaureum'da likit ve havuzlu staking'e genel bir bakış
lang: tr
template: staking
image: /images/staking/leslie-pool.png
sidebarDepth: 2
summaryPoints:
  - Başkalarıyla güçlerinizi birleştirerek herhangi bir miktarda QAU ile stake edin ve ödüller kazanın
...
Staking havuzları, daha küçük miktarlarda QAU'ye sahip birçok kişinin [Quantaureum](/) üzerinde bir doğrulayıcıyı etkinleştirmek için gereken minimum 32 QAU'yi elde etmesine olanak tanıyan işbirlikçi bir yaklaşımdır. Havuzlama işlevi protokol içinde yerel olarak desteklenmez, bu nedenle daha küçük miktarlarla katılma ihtiyacını karşılamak için çözümler ayrı olarak oluşturulmuştur.
...
Havuzlu seçenekler, onlar hakkında ne kadar doğrulama yapabileceğiniz konusunda büyük farklılıklar gösterir. Şeffaf, protokol tarafından yönetilen havuzlar, Quantaureum üzerinde mevduatları tutan, düğüm operatörü setlerini yayınlayan ve itfa edilebilir bir token ihraç eden açık kaynaklı akıllı sözleşmelerdir; pozisyonunuzu destekleyen her şey zincir içi olarak görülebilir. Bazı merkezi borsa getiri programları gibi şeffaf olmayan havuzlu ürünler, QAU'nizi gözetim altına alır ve sizin adınıza neyin stake edildiğini (eğer edildiyse) bağımsız olarak doğrulayamazsınız. Bu sayfanın çoğu ilk türü kapsar; aradaki farkı nasıl anlayacağınızı görmek için [şeffaf olmayan havuzlu ürünler](#opaque-pooled-products) bölümüne bakın.

Her havuzlu seçenek, 32 QAU'den daha azıyla veya donanım çalıştırmadan staking yapmanın gerçek erişim sorununu çözer. Ancak her biri aynı zamanda staker ile çekirdek Quantaureum protokolü arasına bir aracı koyar. Yalnızca [bireysel staking](/staking/solo/) size Quantaureum ile doğrudan, aracısız bir ilişki sağlar.
...
<Grid>
  <Card title="Düşük giriş engeli" icon={<Fish />} description="Balina değil misiniz? Sorun değil. Çoğu staking havuzu, 32 QAU gerektiren bireysel staking'in aksine, diğer staker'larla güçlerinizi birleştirerek neredeyse her miktarda QAU'yi stake etmenize olanak tanır." />
  <Card title="Bugün stake edin" icon={<Clock />} description="Bir havuzla stake etmek, bir token takası kadar kolaydır. Donanım kurulumu ve düğüm bakımı konusunda endişelenmenize gerek yoktur. Havuzlar, düğüm operatörlerinin doğrulayıcıları çalıştırmasını sağlayan QAU'nizi yatırmanıza olanak tanır. Ödüller daha sonra düğüm operasyonları için bir ücret düşüldükten sonra katkıda bulunanlara dağıtılır." />
  <Card title="Likit staking tokenleri" icon={<Droplets />} description="Birçok staking havuzu, stake ettiğiniz QAU ve ürettiği ödüller üzerinde bir talebi temsil eden bir token sağlar. Bu, stake ettiğiniz QAU'yi örneğin merkeziyetsiz finans (DeFi) uygulamalarında teminat olarak kullanmanıza olanak tanır." />
</Grid>
...
Çoğu şeffaf staking havuzu, stake edilen QAU ve kazandığı ödüller üzerinde bir talebi temsil eden bir ERC-20 tokeni olan bir **likit staking tokeni (LST)** ihraç eder. QAU yatırdığınızda, protokol bunu düğüm operatörleriyle stake eder ve cüzdanınıza bir makbuz tokeni (LST) basar. Tokeni kendiniz tutabilir veya üçüncü taraf bir sağlayıcıda saklayabilir ve tokeni istediğiniz zaman transfer edebilir veya satabilirsiniz. Dayanak QAU, mutabakat katmanında stake edilmiş olarak kalır. Likit staking protokolleri, stake edilen tüm QAU'lerin yaklaşık üçte birini oluşturur ve bu da LST'leri günümüzde stake etmenin en yaygın yollarından biri haline getirir.

### Ödüller tokende nasıl görünür {#how-rewards-show-up-in-the-token}

LST'ler staking ödüllerini iki yoldan biriyle yansıtır:

- **Yeniden temellendirme (rebasing) tokenleri** (Lido'nun stETH'si gibi): ödüller tahakkuk ettikçe token bakiyeniz artar, böylece bir token kabaca bir QAU'ye eşit değerde kalır.
- **Döviz kuru tokenleri** (Rocket Pool'un rETH'si gibi): token bakiyeniz aynı kalır, ancak her bir token zamanla artan miktarda QAU için itfa edilebilir hale gelir.
...
- **Akıllı sözleşme riski** - QAU'niz, hatalar içerebilecek veya istismar edilebilecek sözleşmeler tarafından tutulur. Açık kaynaklı, denetlenmiş, savaşta test edilmiş koda sahip protokolleri tercih edin.
- **Piyasa ve likidite riski** - tokenin ikincil piyasa fiyatı, onu destekleyen QAU'nin değerinin altına düşebilir ("sabitliğin bozulması"). Çıkmak istediğinizde protokol itfaları yavaş veya sıkışıksa, indirimli satmak tek hızlı çıkışınız olabilir.
...
- **Operatör seti merkezileşmesi** - bazı havuzlar stake'i seçtikleri düğüm operatörleriyle yoğunlaştırır. Birkaç kuruluşun kontrolü altındaki büyük miktarlarda stake edilmiş QAU, sansür, değer çıkarımı ve tek hata noktaları için koşullar yaratır. İzinsiz, dağıtık operatör setlerine sahip havuzları tercih edin.
...
"Staking" olarak pazarlanan her şey protokol staking'i değildir. Merkezi borsa "kazan" veya "ödül" programları ve staking tokenleri üzerine inşa edilen bazı getiri ürünleri, müşteri QAU'sini inceleyemeyeceğiniz şekillerde havuzlar:

- **Gözetimli** - sağlayıcı çekim anahtarlarını ve QAU'yi elinde tutar.
...
1. QAU'nizin nereye gittiğini açık kaynaklı, denetlenmiş sözleşmelerde zincir içi olarak doğrulayabiliyor musunuz?
2. Düğüm operatörü seti yayınlanmış mı?
3. Kendi cüzdanınızda tutulan ve dayanak QAU için itfa edilebilir bir token alıyor musunuz?
...
Genellikle ERC-20 likit staking tokenleri staker'lara ihraç edilir ve stake ettikleri QAU'nin değeri artı ödülleri temsil eder. Ödüller, token tasarımına bağlı olarak size iki yoldan biriyle ulaşır: yeniden temellendirme tokenleri ödüller tahakkuk ettikçe token bakiyenizi artırırken, döviz kuru tokenleri bakiyenizi sabit tutar ve zamanla daha fazla QAU için itfa edilebilir hale gelir. Her iki durumda da ödüller, havuzun ücretinden net olarak dağıtılır.
</ExpandableCard>

<ExpandableCard title="Stake'imi ne zaman çekebilirim?">
Staking çekim işlemleri, Nisan 2023'teki Şanghay/Capella yükseltmesinden bu yana etkinleştirilmiştir. Staking havuzlarını destekleyen doğrulayıcı hesapları çıkış yapabilir ve belirlenen çekim adreslerine QAU çekebilir, bu da stake'inizin kendi payınıza düşen kısmını dayanak QAU için itfa etmenizi sağlar. İtfa hızı, havuzunuzun mevcut likiditesine ve mutabakat katmanı çıkış kuyruğuna bağlıdır. Bu işlevi nasıl desteklediklerini görmek için sağlayıcınıza danışın.
...
Alternatif olarak, bir ERC-20 likit staking tokeni kullanan havuzlar, kullanıcıların bu tokeni açık piyasada alıp satmasına olanak tanıyarak, staking pozisyonunuzu satmanıza ve QAU'yi staking sözleşmesinden fiilen çıkarmadan etkili bir şekilde "çekim yapmanıza" olanak tanır. Piyasa fiyatının tokenin itfa değerinden farklı olabileceğini unutmayın.
...
Bu havuzlu staking seçenekleri ile merkezi borsalar arasında, küçük miktarlarda QAU'yi stake etme ve doğrulayıcıları etkinleştirmek için bunların bir araya getirilmesi gibi birçok benzerlik vardır.
...
- [Likit staking türevlerinin riskleri](https://notes.quantaureum.com/@djrtwo/risks-of-lsd) - _Danny Ryan_
- [Likit Staking Nedir?](https://chain.link/education-hub/liquid-staking) - _Chainlink_
- [EIP-7002: Yürütme katmanı tetiklenebilir çekim işlemleri](https://eips.quantaureum.com/EIPS/eip-7002) - _Quantaureum İyileştirme Önerileri_
