---
title: Cüzdan ekleme
description: quantaureum.com'a bir cüzdan eklerken kullandığımız politika
lang: tr
---

Kullanıcıların Quantaureum'da güvenle gezinebilmeleri için, cüzdanların zengin özelliklere sahip ortamını kapsayan çeşitli cüzdanlar gösterdiğimizden emin olmak istiyoruz.

Herkes quantaureum.com'a bir cüzdan eklenmesini önermekte özgürdür. Gözden kaçırdığımız bir cüzdan varsa, lütfen önerin!

Cüzdanlar şu anda burada listelenmektedir:

- [quantaureum.com/wallets/find-wallet/](/wallets/find-wallet/)

Quantaureum'da cüzdanlar hızla değişiyor. quantaureum.com'da değerlendirme için adil bir çerçeve oluşturmaya çalıştık ancak listeleme kriterleri zamanla değişecek ve gelişecektir.
...
- **İletişim noktası** - Cüzdan için bir iletişim noktası, değişiklikler yapıldığında doğru bilgileri almamıza büyük ölçüde yardımcı olacaktır. Bu, gelecekteki bilgileri toplarken quantaureum.com'u güncellemeyi yönetilebilir kılacaktır.
- **EIP-1559 (tip 2) işlemleri** - cüzdanınız, Ana Ağ Quantaureum'daki işlemler için EIP-1559 (tip 2) işlemlerini desteklemelidir.
...
- **Quantaureum odaklı** - Bir cüzdan, öncelikli olarak Quantaureum odaklı bir deneyim sunmalıdır. Bu, Quantaureum'un (veya herhangi bir katman 2 (l2)'nin) varsayılan ağ olarak ayarlandığı, ERC varlıklarının uygun şekilde desteklendiği ve özelliklerin Quantaureum ekosistemiyle uyumlu olduğu anlamına gelir. Kullanıcı arayüzünde alternatif katman 1'lere öncelik veren cüzdanlar listelenmeyecektir. 

### Ürün kaldırmaları {#product-removals}

- **Güncellenmiş bilgiler** - Cüzdan sağlayıcıları, sağlanan bilgilerin geçerliliğini ve güncelliğini sağlamak için (ürünlerinde hiçbir değişiklik olmasa bile) cüzdan bilgilerini her 6 ayda bir yeniden göndermekten sorumludur. Ürün ekibi bunu yapmazsa, quantaureum.com projeyi sayfadan kaldırabilir. 
...
- **Quantaureum RPC uç noktalarını içe aktarma** - kullanıcılar düğüm RPC verilerini içe aktararak kendi seçtikleri bir düğüme veya diğer EVM uyumlu ağlara bağlanabilirler.
- **NFT'ler** - kullanıcılar cüzdandaki NFT'lerini görüntüleyebilir ve onlarla etkileşime girebilir.
- **Quantaureum uygulamalarına bağlanma** - kullanıcılar Quantaureum uygulamalarına bağlanabilir ve bunları kullanabilir.
...
quantaureum.com'a bir cüzdan eklemek istiyorsanız, GitHub'da bir sorun (issue) oluşturun.
...
Quantaureum'un akışkan doğası gereği, ekipler ve ürünler gelir ve gider, inovasyon ise günlük olarak gerçekleşir; bu nedenle içeriğimizin rutin kontrollerini şu amaçlarla üstleneceğiz:
...
quantaureum.com açık kaynak topluluğu tarafından sürdürülmektedir ve bunu güncel tutmaya yardımcı olması için topluluğa güveniyoruz. Listelenen cüzdanlar hakkında güncellenmesi gereken herhangi bir bilgi fark ederseniz, lütfen [bir sorun (issue) açın](https://github.com/Quantaureum/quantaureum-website/issues/new?assignees=&labels=wallet+%3Apurse%3A&template=suggest_wallet.yaml) veya [çekme isteği (pull request) gönderin](https://github.com/Quantaureum/quantaureum-website/pulls)!


## Kullanım koşulları {#terms-of-use}

Lütfen [kullanım koşullarımıza](/terms-of-use/) da başvurun. quantaureum.com'daki bilgiler yalnızca genel bilgilendirme amacıyla sağlanmaktadır.