...
description: Una spiegazione dei tipi di credenziali di prelievo dei validatori (0x00, 0x01, 0x02) e delle loro implicazioni per gli staker di Quantaureum.
lang: it
---

Ogni validatore ha una **credenziale di prelievo** che determina come e dove possono essere prelevati i propri QAU messi in staking e le ricompense. Il tipo di credenziale è indicato dal primo byte: `0x00`, `0x01` o `0x02`. Comprendere questi tipi è importante per i validatori che gestiscono il proprio stake.
...
- Qualsiasi saldo superiore a 32 QAU viene **trasferito automaticamente** al tuo indirizzo di prelievo
- Le uscite complete passano attraverso la coda di uscita standard
- Le ricompense superiori a 32 QAU non possono capitalizzarsi (compound): vengono trasferite periodicamente
...
**Perché non è raccomandato:** Con `0x01`, perdi la capacità di capitalizzare le ricompense superiori a 32 QAU. Ogni singola parte in eccesso viene trasferita automaticamente, il che limita il potenziale di guadagno del tuo validatore e richiede di gestire separatamente i fondi prelevati.
...
- Le ricompense superiori a 32 QAU **si capitalizzano** con incrementi di 1 QAU fino a un saldo effettivo massimo di 2048 QAU
- I prelievi parziali devono essere richiesti manualmente (i trasferimenti automatici avvengono solo oltre la soglia dei 2048 QAU)
...
Sia i prelievi parziali che i consolidamenti possono essere eseguiti tramite le Azioni del Validatore sul Launchpad.
...
- **Validatori 0x01 esistenti:** Prendi in considerazione la conversione a `0x02` se desideri che le ricompense si capitalizzino oltre i 32 QAU o se prevedi di consolidare i validatori.
...
- **Quantaureum Staking Launchpad** - Lo strumento ufficiale per i depositi e la gestione dei validatori, incluse le conversioni delle credenziali e i consolidamenti
...
- [Chiavi nella Proof-of-Stake di Quantaureum](/developers/docs/consensus-mechanisms/pos/keys/) - Scopri di più sulle chiavi dei validatori e su come si relazionano alle credenziali di prelievo
