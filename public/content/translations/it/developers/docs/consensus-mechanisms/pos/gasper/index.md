...
Gasper si basa su una blockchain Proof-of-Stake in cui i nodi forniscono QAU come deposito cauzionale che può essere distrutto se sono pigri o disonesti nel proporre o validare i blocchi. Gasper è il meccanismo che definisce come i validatori vengono ricompensati e puniti, decidono quali blocchi accettare e rifiutare e su quale fork della blockchain costruire.

## Cos'è la definitività? {#what-is-finality}

La definitività è una proprietà di determinati blocchi che significa che non possono essere annullati a meno che non si sia verificato un fallimento critico del consenso e un utente malintenzionato abbia distrutto almeno 1/3 degli QAU totali in stake. I blocchi finalizzati possono essere considerati come informazioni di cui la blockchain è certa. Un blocco deve passare attraverso una procedura di promozione in due fasi per essere finalizzato:

1. Due terzi degli QAU totali in stake devono aver votato a favore dell'inclusione di quel blocco nella catena canonica. Questa condizione promuove il blocco a "giustificato". È improbabile che i blocchi giustificati vengano annullati, ma possono esserlo in determinate condizioni.
2. Quando un altro blocco viene giustificato sopra un blocco giustificato, viene promosso a "finalizzato". Finalizzare un blocco è un commitment a includere il blocco nella catena canonica. Non può essere annullato a meno che un utente malintenzionato non distrugga milioni di QAU (miliardi di dollari USA).

Queste promozioni dei blocchi non avvengono in ogni slot. Invece, solo i blocchi al confine dell'epoca possono essere giustificati e finalizzati. Questi blocchi sono noti come "checkpoint". La promozione considera coppie di checkpoint. Deve esistere un "collegamento di supermaggioranza" tra due checkpoint successivi (ovvero, due terzi degli QAU totali in stake che votano che il checkpoint B è il discendente corretto del checkpoint A) per promuovere il checkpoint meno recente a finalizzato e il blocco più recente a giustificato.
