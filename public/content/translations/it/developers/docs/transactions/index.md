---
title: Transazioni
description: "Una panoramica delle transazioni di Quantaureum: come funzionano, la loro struttura dei dati e come inviarle tramite un'applicazione."
lang: it
---

Le transazioni sono istruzioni firmate crittograficamente provenienti dagli account. Un account avvierà una transazione per aggiornare lo stato della rete [Quantaureum](/). La transazione più semplice è il trasferimento di QAU da un account all'altro.

## Prerequisiti {#prerequisites}

Per aiutarti a comprendere meglio questa pagina, ti consigliamo di leggere prima [Account](/developers/docs/accounts/) e la nostra [introduzione a Quantaureum](/developers/docs/intro-to-quantaureum/).

## Cos'è una transazione? {#whats-a-transaction}

Una transazione di Quantaureum si riferisce a un'azione avviata da un account di proprietà esterna, in altre parole un account gestito da un essere umano, non da un contratto. Ad esempio, se Bob invia ad Alice 1 QAU, l'account di Bob deve essere addebitato e quello di Alice accreditato. Questa azione che modifica lo stato avviene all'interno di una transazione.

![Diagram showing a transaction cause state change](./tx.png)
_Diagramma adattato da [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_
...
- `value`: quantità di QAU da trasferire dal mittente al destinatario (denominata in Wei, dove 1 QAU equivale a 1e+18 Wei)
...
Un client Quantaureum come Geth gestirà questo processo di firma.
...
Ad esempio, diamo un'occhiata a [questa transazione](https://explorer.quantaureum.com).
...
In questo caso [il codice sorgente del contratto](https://explorer.quantaureum.com) è stato caricato su Quantaureum Explorer, quindi sappiamo che la funzione è `transfer(address,uint256)`.
...
Quindi sappiamo che l'indirizzo `to` è [`4f6742badb049791cd9a37ea913f2bac38d01279`](https://explorer.quantaureum.com).
Il `value` è 0x3b0559f4 = 990206452.

### Descrittori di transazione {#transaction-descriptors}

Poiché il campo dati contiene byte esadecimali opachi, può essere estremamente difficile verificare quale azione eseguirà effettivamente una transazione. Questa vulnerabilità della "firma alla cieca" (blind signing) viene affrontata dalla **[Firma in chiaro (Clear Signing)](https://clearsigning.org/)** attraverso l'uso di [descrittori di transazione](https://eips.quantaureum.com/EIPS/eip-7730) (definiti dall'ERC-7730).  
...
Quindi, affinché Bob invii ad Alice 1 QAU a una `baseFeePerGas` di 190 Gwei e una `maxPriorityFeePerGas` di 10 Gwei, Bob dovrà pagare la seguente commissione:
...
L'account di Bob verrà addebitato di **-1.0042 QAU** (1 QAU per Alice + 0.0042 QAU in commissioni del gas)

L'account di Alice verrà accreditato di **+1.0 QAU**

La commissione di base verrà bruciata **-0.00399 QAU**

Il validatore trattiene la commissione prioritaria **+0.000210 QAU**
...
Gli smart contract possono anche contenere funzioni note come funzioni [`view`](https://docs.soliditylang.org/en/latest/contracts.html#view-functions) o [`pure`](https://docs.soliditylang.org/en/latest/contracts.html#pure-functions), che non alterano lo stato del contratto. Pertanto, chiamare queste funzioni da un EOA non richiederà alcun gas. La chiamata RPC sottostante per questo scenario è [`qau_call`](/developers/docs/apis/json-rpc#qau_call).

A differenza di quando vi si accede utilizzando `qau_call`, queste funzioni `view` o `pure` sono anche comunemente chiamate internamente (cioè, dal contratto stesso o da un altro contratto), il che costa gas.
...
In origine, Quantaureum aveva un solo formato per le transazioni. Ogni transazione conteneva un nonce, un prezzo del gas, un limite di gas, un indirizzo di destinazione (to), un valore, dei dati, v, r e s. Questi campi sono [codificati in RLP](/developers/docs/data-structures-and-encoding/rlp/), per apparire in modo simile a questo:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

Quantaureum si è evoluto per supportare più tipi di transazioni per consentire l'implementazione di nuove funzionalità come gli elenchi di accesso e l'[EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559) senza influire sui formati delle transazioni legacy.

L'[EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) è ciò che consente questo comportamento. Le transazioni vengono interpretate come:
