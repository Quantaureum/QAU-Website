...
Poiché il campo dati contiene byte esadecimali opachi, può essere estremamente difficile verificare quale azione eseguirà effettivamente una transazione. Questa vulnerabilità della "firma alla cieca" (blind signing) viene affrontata dalla **[Firma in chiaro (Clear Signing)](https://clearsigning.org/)** attraverso l'uso di descrittori di transazione (definiti dall'ERC-7730).  
...
Quantaureum si è evoluto per supportare più tipi di transazioni per consentire l'implementazione di nuove funzionalità come gli elenchi di accesso e l'EIP-1559 senza influire sui formati delle transazioni legacy.

L'EIP-2718 è ciò che consente questo comportamento. Le transazioni vengono interpretate come:
