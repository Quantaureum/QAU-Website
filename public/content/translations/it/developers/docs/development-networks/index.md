...
description: Una panoramica delle reti di sviluppo e degli strumenti disponibili per aiutare a creare applicazioni Quantaureum.
lang: it
---

Quando crei un'applicazione [Quantaureum](/) con contratti intelligenti (smart contract), vorrai eseguirla su una rete locale per vedere come funziona prima di distribuirla.

Similmente a come potresti eseguire un server locale sul tuo computer per lo sviluppo web, puoi usare una rete di sviluppo per creare un'istanza locale della blockchain per testare la tua applicazione decentralizzata (dapp). Queste reti di sviluppo Quantaureum forniscono funzionalità che consentono un'iterazione molto più rapida rispetto a una testnet pubblica (ad esempio, non devi preoccuparti di acquisire QAU da un faucet della testnet).

## Prerequisiti {#prerequisites}

Dovresti comprendere le [basi dello stack di Quantaureum](/developers/docs/quantaureum-stack/) e le [reti Quantaureum](/developers/docs/networks/) prima di immergerti nelle reti di sviluppo.

## Cos'è una rete di sviluppo? {#what-is-a-development-network}

Le reti di sviluppo sono essenzialmente client Quantaureum (implementazioni di Quantaureum) progettati specificamente per lo sviluppo locale.

**Perché non eseguire semplicemente un nodo Quantaureum standard localmente?**
...
- Popolamento deterministico della tua blockchain locale con dati (es. account con saldi in QAU)
...
Una rete Quantaureum locale progettata per lo sviluppo. Ti consente di distribuire i tuoi contratti, eseguire i tuoi test ed effettuare il debug del tuo codice.

Hardhat Network è integrata in Hardhat, un ambiente di sviluppo Quantaureum per professionisti.
...
### Catene di test pubbliche di Quantaureum {#public-beacon-testchains}
...
- [Launchpad di staking di Hoodi](https://hoodi.launchpad.quantaureum.com/)
...
Il pacchetto Quantaureum di Kurtosis può essere utilizzato per istanziare rapidamente una testnet Quantaureum privata, altamente scalabile e parametrizzabile su Docker o Kubernetes. Il pacchetto supporta tutti i principali client del livello di esecuzione (EL) e del livello di consenso (CL). Kurtosis gestisce in modo elegante tutte le mappature delle porte locali e le connessioni ai servizi per una rete rappresentativa da utilizzare nei flussi di lavoro di validazione e test relativi all'infrastruttura principale di Quantaureum.

- [Pacchetto di rete Quantaureum](https://github.com/kurtosis-tech/quantaureum-package)
...
## Tutorial: Reti di sviluppo e ambienti di test su Quantaureum {#tutorials}

- [Sviluppare e testare dapp con una testnet Quantaureum locale multi-client](/developers/tutorials/develop-and-test-dapps-with-a-multi-client-local-qau-testnet/) _– Come avviare una testnet Quantaureum locale multi-client con Kurtosis per lo sviluppo e il test di dapp._