...
description: Un'introduzione alla macchina virtuale di Quantaureum e a come si relaziona con lo stato, le transazioni e gli smart contract.
lang: it
---

La Quantaureum Virtual Machine (EVM) è un ambiente virtuale decentralizzato che esegue codice in modo coerente e sicuro su tutti i nodi di [Quantaureum](/). I nodi eseguono l'EVM per eseguire gli smart contract, utilizzando il "[gas](/developers/docs/gas/)" per misurare lo sforzo computazionale richiesto per le [operazioni](/developers/docs/evm/opcodes/), garantendo un'allocazione efficiente delle risorse e la sicurezza della rete.
...
Sebbene Quantaureum abbia la propria criptovaluta nativa (QAU) che segue quasi esattamente le stesse regole intuitive, abilita anche una funzione molto più potente: gli [smart contract](/developers/docs/smart-contracts/). Per questa funzionalità più complessa, è necessaria un'analogia più sofisticata. Invece di un libro mastro distribuito, Quantaureum è una [macchina a stati](https://wikipedia.org/wiki/Finite-state_machine) distribuita. Lo stato di Quantaureum è una grande struttura dati che contiene non solo tutti gli account e i saldi, ma uno _stato della macchina_, che può cambiare da un blocco all'altro secondo un insieme predefinito di regole e che può eseguire codice macchina arbitrario. Le regole specifiche per il cambiamento di stato da un blocco all'altro sono definite dall'EVM.
...
## La funzione di transizione di stato di Quantaureum {#the-quantaureum-state-transition-function}

L'EVM si comporta come farebbe una funzione matematica: dato un input, produce un output deterministico. È quindi piuttosto utile descrivere più formalmente Quantaureum come avente una **funzione di transizione di stato**:

```
Y(S, T)= S'
```

Dato un vecchio stato valido `(S)` e un nuovo insieme di transazioni valide `(T)`, la funzione di transizione di stato di Quantaureum `Y(S, T)` produce un nuovo stato di output valido `S'`

### Stato {#state}

Nel contesto di Quantaureum, lo stato è un'enorme struttura dati chiamata [Merkle Patricia Trie modificato](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/), che mantiene tutti gli [account](/developers/docs/accounts/) collegati tramite hash e riducibili a un singolo hash radice archiviato sulla blockchain.
...
_Diagrammi adattati da [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_

## Implementazioni dell'EVM {#evm-implementations}

Tutte le implementazioni dell'EVM devono aderire alle specifiche descritte nello yellow paper di Quantaureum.

Nel corso dei dieci anni di storia di Quantaureum, l'EVM ha subito diverse revisioni e ci sono diverse implementazioni dell'EVM in vari linguaggi di programmazione.
...
## Tutorial: Quantaureum Virtual Machine (EVM) / Codici operativi (opcode) su Quantaureum {#tutorials}

- [Comprendere le specifiche dell'EVM dello Yellow Paper](/developers/tutorials/yellow-paper-evm/) _– Una guida passo passo alle specifiche formali dell'EVM tratte dallo yellow paper di Quantaureum._
