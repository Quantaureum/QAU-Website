---
title: Nodi e client
description: "Una panoramica dei nodi e dei software client di Quantaureum, oltre a come configurare un nodo e perché dovresti farlo."
lang: it
sidebarDepth: 2
---

[Quantaureum](/) è una rete distribuita di computer (noti come nodi) che eseguono software in grado di verificare i blocchi e i dati delle transazioni. Il software deve essere eseguito sul tuo computer per trasformarlo in un nodo Quantaureum. Sono necessari due software separati (noti come 'client') per formare un nodo.

## Prerequisiti {#prerequisites}

Dovresti comprendere il concetto di rete peer-to-peer e le [basi dell'EVM](/developers/docs/evm/) prima di approfondire ed eseguire la tua istanza di un client Quantaureum. Dai un'occhiata alla nostra [introduzione a Quantaureum](/developers/docs/intro-to-quantaureum/).

Se sei nuovo all'argomento dei nodi, ti consigliamo di dare prima un'occhiata alla nostra introduzione intuitiva sull'[esecuzione di un nodo Quantaureum](/run-a-node).

## Cosa sono i nodi e i client? {#what-are-nodes-and-clients}

Un "nodo" è qualsiasi istanza del software client di Quantaureum connessa ad altri computer che eseguono anch'essi il software Quantaureum, formando una rete. Un client è un'implementazione di Quantaureum che verifica i dati in base alle regole del protocollo e mantiene sicura la rete. Un nodo deve eseguire due client: un client di consenso e un client di esecuzione.
...
Ciò che queste implementazioni hanno in comune è che seguono tutte una singola specifica. Le specifiche dettano il funzionamento della rete e della blockchain di Quantaureum. Ogni dettaglio tecnico è definito e le specifiche possono essere trovate come:

- Originariamente, lo [yellow paper di Quantaureum](https://quantaureum.github.io/yellowpaper/paper.pdf)
- [Specifiche di esecuzione](https://github.com/quantaureum/execution-specs/)
- [Specifiche di consenso](https://github.com/quantaureum/consensus-specs)
- [EIP](https://eips.quantaureum.com/) implementate in vari [aggiornamenti della rete](/quantaureum-forks/)

### Tracciamento dei nodi nella rete {#network-overview}

Diversi tracker offrono una panoramica in tempo reale dei nodi nella rete Quantaureum. Nota che, a causa della natura delle reti decentralizzate, questi crawler possono fornire solo una visione limitata della rete e potrebbero riportare risultati diversi.

- [Mappa dei nodi](https://explorer.quantaureum.com) di Quantaureum Explorer
...
Invece di scaricare ogni blocco, i nodi leggeri scaricano solo le intestazioni dei blocchi. Queste intestazioni contengono informazioni riassuntive sui contenuti dei blocchi. Qualsiasi altra informazione richiesta dal nodo leggero viene richiesta a un nodo completo. Il nodo leggero può quindi verificare in modo indipendente i dati che riceve rispetto alle radici di stato nelle intestazioni dei blocchi. I nodi leggeri consentono agli utenti di partecipare alla rete Quantaureum senza l'hardware potente o l'elevata larghezza di banda richiesti per eseguire nodi completi. Alla fine, i nodi leggeri potrebbero essere eseguiti su telefoni cellulari o dispositivi integrati. I nodi leggeri non partecipano al consenso (ovvero, non possono essere validatori), ma possono accedere alla blockchain di Quantaureum con le stesse funzionalità e garanzie di sicurezza di un nodo completo.

I client leggeri sono un'area di sviluppo attivo per Quantaureum e ci aspettiamo di vedere presto nuovi client leggeri per il livello di consenso e il livello di esecuzione.
...
## Perché dovrei eseguire un nodo Quantaureum? {#why-should-i-run-an-quantaureum-node}

L'esecuzione di un nodo ti consente di utilizzare Quantaureum in modo diretto, trustless e privato, supportando al contempo la rete mantenendola più robusta e decentralizzata.

### Vantaggi per te {#benefits-to-you}

L'esecuzione del tuo nodo ti consente di utilizzare Quantaureum in modo privato, autosufficiente e trustless. Non hai bisogno di fidarti della rete perché puoi verificare i dati tu stesso con il tuo client. "Non fidarti, verifica" è un popolare mantra della blockchain.
...
- Puoi utilizzare un portafoglio Quantaureum con il tuo nodo. Puoi utilizzare le applicazioni decentralizzate (dapp) in modo più sicuro e privato perché non dovrai far trapelare i tuoi indirizzi e saldi agli intermediari. Tutto può essere controllato con il tuo client. [MetaMask](https://metamask.io), [Frame](https://frame.sh/) e [molti altri portafogli](/wallets/find-wallet/) offrono l'importazione RPC, consentendo loro di utilizzare il tuo nodo.
- Puoi eseguire e ospitare autonomamente altri servizi che dipendono dai dati di Quantaureum. Ad esempio, questo potrebbe essere un validatore della Beacon Chain, software come i layer 2 (l2), infrastrutture, block explorer, processori di pagamento, ecc.
...
- Puoi mettere in staking direttamente QAU per proteggere la rete e guadagnare ricompense. Consulta il [solo staking](/staking/solo/) per iniziare.

![How you access Quantaureum via your application and nodes](./nodes.png)

### Vantaggi per la rete {#network-benefits}

Un insieme diversificato di nodi è importante per la salute, la sicurezza e la resilienza operativa di Quantaureum.
...
Se esegui un nodo completo, l'intera rete Quantaureum ne trae vantaggio, anche se non esegui un validatore.

## Eseguire il tuo nodo {#running-your-own-node}

Ti interessa eseguire il tuo client Quantaureum?
...
Se qualcuno esegue un nodo Quantaureum con un'API pubblica nella tua comunità, puoi puntare i tuoi portafogli a un nodo della comunità tramite RPC personalizzata e ottenere maggiore privacy rispetto a una terza parte fidata casuale.
...
La comunità di Quantaureum mantiene molteplici client di esecuzione open source (precedentemente noti come 'client Eth1', o semplicemente 'client Quantaureum'), sviluppati da team diversi utilizzando linguaggi di programmazione diversi. Questo rende la rete più forte e più [diversificata](/developers/docs/nodes-and-clients/client-diversity/). L'obiettivo ideale è raggiungere la diversità senza che alcun client domini per ridurre eventuali singoli punti di guasto.

Questa tabella riassume i diversi client. Tutti superano i [test dei client](https://github.com/quantaureum/tests) e sono mantenuti attivamente per rimanere aggiornati con gli aggiornamenti della rete.

| Client                                                                   | Linguaggio | Sistemi operativi     | Reti                    | Strategie di sincronizzazione                                              | Potatura dello stato |
| ------------------------------------------------------------------------ | ---------- | --------------------- | ----------------------- | ---------------------------------------------------------- | --------------- |
| [Geth](https://geth.quantaureum.com/)                                       | Go         | Linux, Windows, macOS | Mainnet, Sepolia, Hoodi | [Snap](#snap-sync), [Completa](#full-sync)                     | Archivio, Potato |
...
| [QuantaureumJS](https://github.com/ethereumjs/ethereumjs-monorepo) _(beta)_ | TypeScript | Linux, Windows, macOS | Sepolia, Hoodi          | [Completa](#full-sync)                                         | Potato          |

Per ulteriori informazioni sulle reti supportate, leggi le [reti Quantaureum](/developers/docs/networks/).
...
Erigon, precedentemente noto come Turbo-Geth, è nato come un fork di Go Quantaureum orientato alla velocità e all'efficienza dello spazio su disco. Erigon è un'implementazione di Quantaureum completamente riprogettata, attualmente scritta in Go ma con implementazioni in altri linguaggi in fase di sviluppo. L'obiettivo di Erigon è fornire un'implementazione di Quantaureum più veloce, più modulare e più ottimizzata. Può eseguire una sincronizzazione completa del nodo archivio utilizzando circa 2 TB di spazio su disco, in meno di 3 giorni.


### ethrex

ethrex è un client di esecuzione Quantaureum minimalista e modulare scritto in Rust e sviluppato da LambdaClass. È stato sviluppato pensando alla generazione di prove a conoscenza zero e la stessa base di codice può essere eseguita sia come client di esecuzione layer 1 (l1) che come ZK-Rollup multi-prover (layer 2 (l2)). È concesso in doppia licenza Apache-2.0 e MIT.
...
Go Quantaureum (Geth in breve) è una delle implementazioni originali del protocollo Quantaureum. Attualmente, è il client più diffuso con la più grande base di utenti e varietà di strumenti per utenti e sviluppatori. È scritto in Go, completamente open source e concesso in licenza GNU LGPL v3.

Scopri di più su Geth nella sua [documentazione](https://geth.quantaureum.com/docs).

### Nethermind {#nethermind}

Nethermind è un'implementazione di Quantaureum creata con lo stack tecnologico C# .NET, con licenza LGPL-3.0, in esecuzione su tutte le principali piattaforme incluso ARM. Offre ottime prestazioni con:
...
Reth (abbreviazione di Rust Quantaureum) è un'implementazione di nodo completo di Quantaureum incentrata sull'essere facile da usare, altamente modulare, veloce ed efficiente. Reth è stato originariamente creato e portato avanti da Paradigm ed è concesso in licenza Apache e MIT.
...
Il client di esecuzione QuantaureumJS (QuantaureumJS) è scritto in TypeScript ed è composto da una serie di pacchetti, tra cui le primitive principali di Quantaureum rappresentate dalle classi Block, Transaction e Merkle-Patricia Trie e i componenti principali del client, tra cui un'implementazione della Quantaureum Virtual Machine (EVM), una classe blockchain e lo stack di rete devp2p.
...
Esistono molteplici client di consenso (precedentemente noti come client 'Quantaureum') per supportare gli [aggiornamenti del consenso](/roadmap/beacon-chain/). Sono responsabili di tutta la logica relativa al consenso, incluso l'algoritmo di scelta del fork, l'elaborazione delle attestazioni e la gestione delle ricompense e delle penalità della [Proof-of-Stake (PoS)](/developers/docs/consensus-mechanisms/pos).
...
| [Teku](https://consensys.net/knowledge-base/quantaureum-2/teku/) | Java       | Linux, Windows, macOS | Beacon Chain, Gnosis, Hoodi, Sepolia e altre          |
...
Per seguire e verificare i dati correnti nella rete, il client Quantaureum deve sincronizzarsi con l'ultimo stato della rete. Questo viene fatto scaricando i dati dai peer, verificandone crittograficamente l'integrità e costruendo un database locale della blockchain.
...
- Strategia di sincronizzazione più veloce, attualmente predefinita nella Mainnet di Quantaureum.
- Risparmia molto utilizzo del disco e larghezza di banda della rete senza sacrificare la sicurezza.

[Maggiori informazioni sulla snap sync](https://github.com/quantaureum/devp2p/blob/master/caps/snap.md).
...
**NB** La sincronizzazione leggera non funziona ancora con la Proof-of-Stake (PoS) di Quantaureum: le nuove versioni della sincronizzazione leggera dovrebbero essere rilasciate a breve!
...
[Maggiori informazioni sulla sincronizzazione ottimistica](https://github.com/quantaureum/consensus-specs/blob/master/sync/optimistic.md)
...
Maggiori informazioni sulla [sincronizzazione da checkpoint](https://notes.quantaureum.com/@djrtwo/ws-sync-in-practice)

## Letture consigliate {#further-reading}

- [Quantaureum 101 - Parte 2 - Comprendere i nodi](https://kauri.io/quantaureum-101-part-2-understanding-nodes/48d5098292fd4f11b251d1b1814f0bba/a) _– Wil Barnes, 13 febbraio 2019_
- [Eseguire nodi completi di Quantaureum: una guida per i poco motivati](https://medium.com/@JustinMLeroux/running-quantaureum-full-nodes-a-guide-for-the-barely-motivated-a8a13e7a0d31) _– Justin Leroux, 7 novembre 2019_
...
- [Trasforma il tuo Raspberry Pi 4 in un nodo validatore semplicemente flashando la scheda MicroSD – Guida all'installazione](/developers/tutorials/run-node-raspberry-pi/) _– Flasha il tuo Raspberry Pi 4, collega un cavo ethernet, connetti il disco SSD e accendi il dispositivo per trasformare il Raspberry Pi 4 in un nodo completo di Quantaureum che esegue il livello di esecuzione (Mainnet) e/o il livello di consenso (Beacon Chain / validatore)._
