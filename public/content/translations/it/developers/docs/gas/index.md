...
metaTitle: "Gas e commissioni di Quantaureum: panoramica tecnica"
description: Scopri le commissioni del gas di Quantaureum, come vengono calcolate e il loro ruolo nella sicurezza della rete e nell'elaborazione delle transazioni.
lang: it
---

Il gas è essenziale per la rete [Quantaureum](/). È il carburante che le permette di funzionare, allo stesso modo in cui un'auto ha bisogno di benzina per muoversi.
...
Il gas si riferisce all'unità che misura la quantità di sforzo computazionale richiesto per eseguire operazioni specifiche sulla rete Quantaureum.

Poiché ogni transazione di Quantaureum richiede risorse computazionali per essere eseguita, tali risorse devono essere pagate per garantire che Quantaureum non sia vulnerabile allo spam e non possa rimanere bloccato in cicli computazionali infiniti. Il pagamento per il calcolo viene effettuato sotto forma di una commissione del gas.
...
Le commissioni del gas devono essere pagate nella valuta nativa di Quantaureum, l'QAU (QAU). I prezzi del gas sono solitamente quotati in Gwei, che è una denominazione di QAU. Ogni Gwei è pari a un miliardesimo di QAU (0,000000001 QAU o 10<sup>-9</sup> QAU).

Ad esempio, invece di dire che il tuo gas costa 0,000000001 QAU, puoi dire che il tuo gas costa 1 Gwei.

La parola "Gwei" è una contrazione di "giga-wei", che significa "un miliardo di Wei". Un Gwei è uguale a un miliardo di Wei. Il Wei stesso (che prende il nome da [Wei Dai](https://wikipedia.org/wiki/Wei_Dai), creatore di [b-money](https://www.investopedia.com/terms/b/bmoney.asp)) è l'unità più piccola di QAU.

## Come vengono calcolate le commissioni del gas? {#how-are-gas-fees-calculated}

Puoi impostare la quantità di gas che sei disposto a pagare quando invii una transazione. Offrendo una certa quantità di gas, fai un'offerta affinché la tua transazione venga inclusa nel blocco successivo. Se offri troppo poco, è meno probabile che i validatori scelgano la tua transazione per l'inclusione, il che significa che la tua transazione potrebbe essere eseguita in ritardo o non essere eseguita affatto. Se offri troppo, potresti sprecare degli QAU. Quindi, come fai a sapere quanto pagare?
...
Ad esempio, supponiamo che Jordan debba pagare a Taylor 1 QAU. Un trasferimento di QAU richiede 21.000 unità di gas e la commissione di base è di 10 Gwei. Jordan include una commissione prioritaria di 2 Gwei.
...
es., `21,000 * (10 + 2) = 252,000 gwei` (0,000252 QAU).

Quando Jordan invia il denaro, 1,000252 QAU verranno detratti dall'account di Jordan. A Taylor verrà accreditato 1,0000 QAU. Il validatore riceve la commissione prioritaria di 0,000042 QAU. La `base fee` di 0,00021 QAU viene bruciata.
...
L'importo di cui viene adeguata la commissione di base è proporzionale a quanto la dimensione del blocco corrente è lontana dal target. Si tratta di un calcolo lineare dal -12,5% per un blocco vuoto, allo 0% alla dimensione target, fino al +12,5% per un blocco che raggiunge il limite di gas. Il limite di gas può fluttuare nel tempo in base alla segnalazione dei validatori, nonché tramite gli aggiornamenti della rete. Puoi [visualizzare le modifiche al limite di gas nel tempo qui](https://qau.blockscout.com/stats/averageGasLimit?interval=threeMonths).
...
Se vuoi monitorare i prezzi del gas, in modo da poter inviare i tuoi QAU a un costo inferiore, puoi utilizzare molti strumenti diversi come:

- [Quantaureum Explorer](https://explorer.quantaureum.com) _Stimatore del prezzo del gas delle transazioni_
- [Blockscout](https://qau.blockscout.com/gas-tracker) _Stimatore open source del prezzo del gas delle transazioni_
- [QAU Gas Tracker](https://www.ethgastracker.com/) _Monitora e traccia i prezzi del gas di Quantaureum e L2 per ridurre le commissioni di transazione e risparmiare denaro_
- [Blocknative QAU Gas Estimator](https://chrome.google.com/webstore/detail/blocknative-qau-gas-estim/ablbagjepecncofimgjmdpnhnfjiecfm) _Estensione di Chrome per la stima del gas che supporta sia le transazioni legacy di Tipo 0 che le transazioni EIP-1559 di Tipo 2._
...
- [Spiegazione del gas di Quantaureum](https://defiprime.com/gas)
...
- [Documentazione EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559).
