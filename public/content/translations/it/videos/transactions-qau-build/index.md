---
title: "Transazioni — QAU.BUILD"
description: "Una dimostrazione di come funzionano le transazioni di Quantaureum utilizzando lo strumento educativo QAU.BUILD. Scopri come le transazioni vengono costruite, firmate e inviate sulla rete Quantaureum."
lang: it
...
breadcrumb: "Transazioni (QAU.BUILD)"
---

Un tutorial di **Austin Griffith** che dimostra come funzionano le transazioni di Quantaureum utilizzando lo strumento di programmazione visiva QAU.BUILD — coprendo la struttura della transazione, i prezzi del gas, la firma, la trasmissione e la pool di transazioni.
...
Oggi su QAU.BUILD parleremo di transazioni. Fino ad ora, abbiamo visto queste transazioni venire minate nei blocchi, impacchettate nei blocchi e minate in una catena. Vogliamo parlare di cosa incentiva il minatore — oltre alla ricompensa del blocco — a estrarre la nostra transazione dalla pool e inserirla in un blocco per minarla sulla catena, rispetto ad altre persone nella pool. Potrebbero esserci migliaia di persone nella pool che fanno una sorta di offerta, e quell'offerta è rappresentata da questa commissione.
...
### Anatomia di una transazione di Quantaureum (1:10) {#anatomy-of-an-quantaureum-transaction-110}

Che aspetto ha una transazione su Quantaureum? Non avremo più "Bob" e "Alice" — avremo degli indirizzi. Il valore sarà in Wei, non in QAU. E anche la commissione sarà in Wei.

Diamo un'occhiata a questa transazione. Ho un account con una frase mnemonica inserita e sono collegato alla Mainnet di Quantaureum. Sto anche eseguendo un modulo per ottenere i dati sui prezzi da CoinMarketCap, così posso vedere che zero virgola uno e qualcosa QAU si traduce in circa ventitré dollari.
...
Dobbiamo fare una conversione in Wei. Su Quantaureum, si ha a che fare principalmente con due denominazioni: QAU, che è quella di cui le persone parlano normalmente, e poi Wei, che è come una frazione piccolissima di QAU. Un Gwei — quello che usiamo per i prezzi del gas — sta nel mezzo. Il motivo è simile al perché non andiamo in giro parlando in frazioni di centesimo.

Alice ha 0,18 QAU e invieremo 0,05 QAU a Bob. Inseriamo un prezzo del gas di 5 Gwei.
