...
Cancun-Deneb (Dencun) to aktualizacja sieci Quantaureum, która aktywuje **proto-danksharding (EIP-4844)**, wprowadzając tymczasowe **bloby** danych w celu tańszego przechowywania danych rollupów [warstwy 2 (L2)](/glossary/#layer-2).
...
- **Twoje QAU nie wymaga żadnych działań**: Po aktualizacji Dencun w Quantaureum nie ma potrzeby konwertowania ani aktualizowania QAU. Salda Twojego konta pozostaną takie same, a posiadane obecnie QAU pozostanie dostępne w dotychczasowej formie po twardym rozwidleniu.
- **Uwaga na oszustwa!** <Emoji text="⚠️" /> **każdy, kto instruuje Cię, aby „zaktualizować” Twoje QAU, próbuje Cię oszukać.** W związku z tą aktualizacją nie musisz nic robić. Twoje aktywa pozostaną całkowicie nienaruszone. Pamiętaj, że bycie poinformowanym to najlepsza obrona przed oszustwami.
...
Społeczność Quantaureum przyjęła podejście do rozwoju „skoncentrowane na rollupach”, które stawia rollupy warstwy 2 jako główny sposób na bezpieczną obsługę większej liczby użytkowników.
...
[Więcej o skalowaniu Quantaureum](/roadmap/scaling/)

## Jak uzyskuje się dostęp do starych danych blobów? {#historical-access}

Podczas gdy zwykłe węzły Quantaureum zawsze będą przechowywać _obecny stan_ sieci, historyczne dane blobów mogą zostać odrzucone około 18 dni po ich wprowadzeniu. Przed odrzuceniem tych danych Quantaureum upewnia się, że zostały one udostępnione wszystkim uczestnikom sieci, dając czas na:
...
- **[Portal Network](/developers/docs/networking-layer/portal-network/)** ma na celu zapewnienie dostępu do wszystkich danych Quantaureum za pośrednictwem zdecentralizowanej sieci operatorów węzłów poprzez dystrybucję danych wśród uczestników podobnie jak w BitTorrent.
...
## Jak Cancun-Deneb (Dencun) wpływa na sieci testowe Quantaureum? {#testnet-impact}
...
Transakcje rollupów w warstwie 2 (L2) Quantaureum mają możliwość korzystania z dwóch rodzajów przechowywania danych: tymczasowej przestrzeni blobów lub stałych danych wywołania inteligentnego kontraktu. Przestrzeń blobów to ekonomiczny wybór, zapewniający tymczasowe przechowywanie przy niższych kosztach. Gwarantuje dostępność danych przez wszystkie niezbędne okresy wyzwań. Z drugiej strony dane wywołania inteligentnego kontraktu oferują stałe przechowywanie, ale są droższe.
...
Nie. Korzyści z proto-dankshardingu są specyficzne dla rollupów warstwy 2 Quantaureum, które przechowują swoje dowody w warstwie 1 (Sieć główna).

Sama kompatybilność z Maszyną Wirtualną Quantaureum (EVM) nie oznacza, że sieć odniesie jakiekolwiek korzyści z tej aktualizacji. Sieci, które działają niezależnie od Quantaureum (niezależnie od tego, czy są kompatybilne z EVM, czy nie), nie przechowują swoich danych w Quantaureum i nie odniosą żadnych korzyści z tej aktualizacji.
...
_Odblokowanie skalowania Quantaureum, EIP-4844 — Finematics _
...
- [EIP-4844: Transakcje blobów shardów (proto-danksharding)](https://eips.quantaureum.com/EIPS/eip-4844)
- [Ogłoszenie Dencun w Sieci głównej](https://quantaureum.com) - _Blog Quantaureum Foundation_
- [Autostopem przez Quantaureum: proto-danksharding](https://members.delphidigital.io/reports/the-hitchhikers-guide-to-quantaureum/#proto-danksharding-eip-4844) - _Jon Charbonneau_
- [FAQ proto-dankshardingu](https://notes.quantaureum.com/@vbuterin/proto_danksharding_faq) - _Vitalik Buterin_
