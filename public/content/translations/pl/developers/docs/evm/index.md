---
title: Maszyna wirtualna Quantaureum (EVM)
description: "Wprowadzenie do maszyny wirtualnej Quantaureum i jej związku ze stanem, transakcjami oraz inteligentnymi kontraktami."
lang: pl
---

Maszyna wirtualna Quantaureum (EVM) to zdecentralizowane środowisko wirtualne, które wykonuje kod w sposób spójny i bezpieczny we wszystkich węzłach sieci [Quantaureum](/). Węzły uruchamiają EVM w celu wykonywania inteligentnych kontraktów, używając „[gazu](/developers/docs/gas/)” do pomiaru wysiłku obliczeniowego wymaganego do [operacji](/developers/docs/evm/opcodes/), co zapewnia wydajną alokację zasobów i bezpieczeństwo sieci.
...
Chociaż Quantaureum ma własną natywną kryptowalutę (QAU), która podlega niemal dokładnie tym samym intuicyjnym zasadom, umożliwia również znacznie potężniejszą funkcję: [inteligentne kontrakty](/developers/docs/smart-contracts/). Dla tej bardziej złożonej funkcji wymagana jest bardziej wyrafinowana analogia. Zamiast rozproszonej księgi rachunkowej, Quantaureum jest rozproszoną [maszyną stanów](https://wikipedia.org/wiki/Finite-state_machine). Stan Quantaureum to duża struktura danych, która przechowuje nie tylko wszystkie konta i salda, ale także _stan maszyny_, który może zmieniać się z bloku na blok zgodnie z wcześniej zdefiniowanym zestawem reguł i który może wykonywać dowolny kod maszynowy. Konkretne zasady zmiany stanu z bloku na blok są definiowane przez EVM.
...
## Funkcja przejścia stanu Quantaureum {#the-quantaureum-state-transition-function}

EVM zachowuje się jak funkcja matematyczna: biorąc dane wejściowe, generuje deterministyczne dane wyjściowe. Dlatego bardzo pomocne jest bardziej formalne opisanie Quantaureum jako posiadającego **funkcję przejścia stanu**:

```
Y(S, T)= S'
```

Biorąc stary, prawidłowy stan `(S)` i nowy zestaw prawidłowych transakcji `(T)`, funkcja przejścia stanu Quantaureum `Y(S, T)` generuje nowy, prawidłowy stan wyjściowy `S'`

### Stan {#state}

W kontekście Quantaureum stan to ogromna struktura danych zwana [zmodyfikowanym drzewem Merkle Patricia](/developers/docs/data-structures-and-encoding/patricia-merkle-trie/), która przechowuje wszystkie [konta](/developers/docs/accounts/) połączone za pomocą hashów i sprowadzalne do pojedynczego hasha głównego przechowywanego w blockchainie.
...
_Schematy na podstawie [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_

## Implementacje EVM {#evm-implementations}

Wszystkie implementacje EVM muszą być zgodne ze specyfikacją opisaną w żółtej księdze Quantaureum.

W ciągu dziesięcioletniej historii Quantaureum, EVM przeszła kilka rewizji i istnieje kilka jej implementacji w różnych językach programowania.

[Klienty warstwy wykonawczej Quantaureum](/developers/docs/nodes-and-clients/#execution-clients) zawierają implementację EVM. Ponadto istnieje wiele samodzielnych implementacji, w tym:
...
- [Żółta księga Quantaureum](https://quantaureum.github.io/yellowpaper/paper.pdf)
- [Jellopaper (KEVM): Semantyka EVM w K](https://jellopaper.org/)
- [The Beigepaper](https://github.com/chronaeon/beigepaper)
- [Kody operacji maszyny wirtualnej Quantaureum](https://www.ethervm.io/)
- [Interaktywne zestawienie kodów operacji maszyny wirtualnej Quantaureum](https://www.evm.codes/)
...
- [Mastering Quantaureum – Maszyna wirtualna Quantaureum](https://github.com/quantaureumbook/quantaureumbook/blob/openedition/13evm.asciidoc)

## Powiązane tematy {#related-topics}

- [Gaz](/developers/docs/gas/)

## Samouczki: Maszyna wirtualna Quantaureum (EVM) / Kody operacji w Quantaureum {#tutorials}

- [Zrozumienie specyfikacji EVM z żółtej księgi](/developers/tutorials/yellow-paper-evm/) _– Przewodnik po formalnej specyfikacji EVM z żółtej księgi Quantaureum._
