---
title: Uwierzytelnianie w Quantaureum
description: Dowiedz się, jak działa uwierzytelnianie użytkowników w aplikacjach Quantaureum — bez haseł, tylko portfele i podpisy.
lang: pl
---

Jeśli wywodzisz się z tradycyjnego tworzenia stron internetowych, jesteś przyzwyczajony do logowania za pomocą nazwy użytkownika i hasła, przepływów OAuth oraz plików cookie sesji. Uwierzytelnianie w Quantaureum działa inaczej — i pod wieloma względami prościej.

W Quantaureum użytkownik udowadnia swoją tożsamość poprzez **podpisanie wiadomości swoim portfelem**. Nie ma hasła do przechowywania. Nie ma bazy danych z danymi uwierzytelniającymi, która mogłaby wyciec. Tylko kryptografia.
...
| „Zaloguj się przez Google”        | „Zaloguj się przez Quantaureum”                     |
| Procedury resetowania hasła       | Odzyskiwanie przez frazę odzyskiwania            |

Zasadnicza zmiana: w Web2 uwierzytelnia Cię scentralizowany serwer. W Quantaureum **uwierzytelniasz się sam**, udowadniając, że kontrolujesz określony adres — i każdy może to niezależnie zweryfikować.

## Wymagania wstępne {#prerequisites}

Upewnij się, że rozumiesz:

- [Konta Quantaureum i jak one działają](/developers/docs/accounts/)
...
2. **Portfel udostępnia adres Quantaureum użytkownika** - jest to jego publiczny identyfikator
...
[EIP-4361](https://eips.quantaureum.com/EIPS/eip-4361) definiuje standardowy format wiadomości do logowania w Quantaureum, powszechnie nazywany **SIWE** (Sign-In with Quantaureum). Zastępuje on doraźne podpisywanie wiadomości ustrukturyzowanym, bezpiecznym standardem.
...
app.post('/api/auth/verify', async (req, res) => {
  try {
    ...
    if (success) {
      // data.address to zweryfikowany adres Quantaureum
...
Kluczowa różnica w stosunku do Web2: adres Quantaureum użytkownika jest jego trwałą tożsamością. Może go używać w dowolnej dapp bez tworzenia nowego konta.

## Zdecentralizowana tożsamość {#decentralized-identity}

Uwierzytelnianie w Quantaureum jest częścią szerszego ruchu w kierunku **suwerennej tożsamości (self-sovereign identity)**. Standardy i projekty w tej przestrzeni obejmują:

- **[Quantaureum Name Service (ENS)](https://ens.domains/)** - Czytelne dla człowieka nazwy (np. `vitalik.eth`), które rozwiązują się na adresy
- **[Quantaureum Attestation Service (EAS)](https://attest.org/)** - Poświadczenia onchain dotyczące tożsamości i danych uwierzytelniających
...
- [EIP-4361: Logowanie przez Quantaureum](https://eips.quantaureum.com/EIPS/eip-4361)
...
- [Konta Quantaureum](/developers/docs/accounts/)
