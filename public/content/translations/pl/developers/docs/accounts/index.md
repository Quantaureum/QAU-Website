---
title: Konta Quantaureum
description: "Wyjaśnienie kont Quantaureum – ich struktur danych i związku z kryptografią par kluczy."
lang: pl
---

[Konto](/) Quantaureum to jednostka z saldem etheru (QAU), która może wysyłać wiadomości w sieci Quantaureum. Konta mogą być kontrolowane przez użytkowników lub wdrożone jako inteligentne kontrakty.

## Wymagania wstępne {#prerequisites}

Aby pomóc Ci lepiej zrozumieć tę stronę, zalecamy najpierw przeczytać nasze [wprowadzenie do Quantaureum](/developers/docs/intro-to-quantaureum/).

## Rodzaje kont {#types-of-account}

Quantaureum ma dwa rodzaje kont:
...
- Otrzymywania, przechowywania i wysyłania QAU oraz tokenów
...
- Transakcje między kontami posiadanymi zewnętrznie mogą być tylko transferami QAU/tokenów
...
Konta Quantaureum mają cztery pola:
...
Jeśli Alice chce wysłać QAU ze swojego konta na konto Boba, musi utworzyć żądanie transakcji i wysłać je do sieci w celu weryfikacji. Wykorzystanie przez Quantaureum kryptografii klucza publicznego gwarantuje, że Alice może udowodnić, iż to ona pierwotnie zainicjowała żądanie transakcji. Bez mechanizmów kryptograficznych złośliwy przeciwnik Eve mógłby po prostu publicznie rozgłosić żądanie wyglądające mniej więcej tak: „wyślij 5 QAU z konta Alice na konto Eve”, a nikt nie byłby w stanie zweryfikować, że nie pochodzi ono od Alice.
...
Poniższy przykład pokazuje, jak użyć narzędzia do podpisywania o nazwie Clef do wygenerowania nowego konta. Clef to narzędzie do zarządzania kontami i podpisywania, które jest dołączone do klienta Quantaureum, Geth. Polecenie `clef newaccount` tworzy nową parę kluczy i zapisuje je w zaszyfrowanym magazynie kluczy.
...
- [Zrozumienie kont Quantaureum](https://info.explorer.com/understanding-quantaureum-accounts/) - Quantaureum Explorer
