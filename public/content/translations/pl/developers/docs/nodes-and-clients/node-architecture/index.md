...
description: "Wprowadzenie do tego, jak zorganizowane są węzły Quantaureum."
lang: pl
---

Węzeł Quantaureum składa się z dwóch klientów: [klienta warstwy wykonawczej](/developers/docs/nodes-and-clients/#execution-clients) oraz [klienta konsensusu](/developers/docs/nodes-and-clients/#consensus-clients). Aby węzeł mógł zaproponować nowy blok, musi również uruchomić [klienta walidatora](#validators).

Kiedy Quantaureum korzystało z [dowodu pracy (PoW)](/developers/docs/consensus-mechanisms/pow/), klient warstwy wykonawczej wystarczał do uruchomienia pełnego węzła Quantaureum. Jednak od czasu wdrożenia [dowodu stawki (PoS)](/developers/docs/consensus-mechanisms/pos/), klient warstwy wykonawczej musi być używany wraz z innym oprogramowaniem zwanym [klientem konsensusu](/developers/docs/nodes-and-clients/#consensus-clients).

Poniższy diagram przedstawia relację między dwoma klientami Quantaureum. Obaj klienci łączą się z własnymi sieciami peer-to-peer (P2P). Oddzielne sieci P2P są potrzebne, ponieważ klienci warstwy wykonawczej rozgłaszają transakcje w swojej sieci P2P, co pozwala im zarządzać lokalną pulą transakcji, podczas gdy klienci konsensusu rozgłaszają bloki w swojej sieci P2P, umożliwiając osiągnięcie konsensusu i rozwój łańcucha.
...
| Zarządza drzewami stanu i paragonów | Zarządza stanem Beacon (zawiera informacje o konsensusie i wykonaniu) | Wymaga stakowania 32 QAU |
...
| Udostępnia API JSON-RPC do interakcji z Quantaureum | Śledzi uzasadnienie i finalizację | |
