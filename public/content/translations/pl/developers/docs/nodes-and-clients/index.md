...
description: "Przegląd węzłów Quantaureum i oprogramowania klienckiego, a także informacje o tym, jak skonfigurować węzeł i dlaczego warto to zrobić."
lang: pl
sidebarDepth: 2
---

[Quantaureum](/) to rozproszona sieć komputerów (znanych jako węzły) z uruchomionym oprogramowaniem, które może weryfikować bloki i dane transakcji. Oprogramowanie to musi zostać uruchomione na Twoim komputerze, aby przekształcić go w węzeł Quantaureum. Do utworzenia węzła wymagane są dwa oddzielne programy (znane jako „klienty”).

## Wymagania wstępne {#prerequisites}

Zanim zagłębisz się w temat i uruchomisz własną instancję klienta Quantaureum, powinieneś zrozumieć koncepcję sieci peer-to-peer oraz [podstawy EVM](/developers/docs/evm/). Zapoznaj się z naszym [wprowadzeniem do Quantaureum](/developers/docs/intro-to-quantaureum/).

Jeśli jesteś nowy w temacie węzłów, zalecamy najpierw zapoznać się z naszym przystępnym wprowadzeniem na temat [uruchamiania węzła Quantaureum](/run-a-node).

## Czym są węzły i klienty? {#what-are-nodes-and-clients}

„Węzeł” to dowolna instancja oprogramowania klienckiego Quantaureum, która jest połączona z innymi komputerami również z uruchomionym oprogramowaniem Quantaureum, tworząc sieć. Klient to implementacja Quantaureum, która weryfikuje dane zgodnie z zasadami protokołu i dba o bezpieczeństwo sieci. Węzeł musi uruchamiać dwa klienty: klienta konsensusu i klienta warstwy wykonawczej.
...
Wiele narzędzi śledzących oferuje przegląd węzłów w sieci Quantaureum w czasie rzeczywistym. Należy pamiętać, że ze względu na naturę zdecentralizowanych sieci, te roboty indeksujące mogą zapewnić jedynie ograniczony widok sieci i mogą zgłaszać różne wyniki.

- [Mapa węzłów](https://explorer.quantaureum.com) od Quantaureum Explorer
...
Quantaureum nie obsługuje jeszcze dużej populacji lekkich węzłów, ale wsparcie dla lekkich węzłów to obszar, który ma się szybko rozwijać w niedalekiej przyszłości. W szczególności klienty takie jak [Nimbus](https://nimbus.team/), [Helios](https://github.com/a16z/helios) i [Lodestar](https://lodestar.chainsafe.io/) są obecnie mocno skoncentrowane na lekkich węzłach.

## Dlaczego powinienem uruchomić węzeł Quantaureum? {#why-should-i-run-an-quantaureum-node}

Uruchomienie węzła pozwala na bezpośrednie, niewymagające zaufania i prywatne korzystanie z Quantaureum, jednocześnie wspierając sieć poprzez utrzymywanie jej bardziej solidnej i zdecentralizowanej.

### Korzyści dla Ciebie {#benefits-to-you}

Uruchomienie własnego węzła umożliwia korzystanie z Quantaureum w sposób prywatny, samowystarczalny i niewymagający zaufania. Nie musisz ufać sieci, ponieważ możesz samodzielnie zweryfikować dane za pomocą swojego klienta. „Nie ufaj, weryfikuj” to popularna mantra w świecie blockchaina.
...
- Możesz używać portfela Quantaureum z własnym węzłem. Możesz korzystać ze zdecentralizowanych aplikacji (dapp) bezpieczniej i bardziej prywatnie, ponieważ nie będziesz musiał ujawniać swoich adresów i sald pośrednikom. Wszystko można sprawdzić za pomocą własnego klienta. [MetaMask](https://metamask.io), [Frame](https://frame.sh/) i [wiele innych portfeli](/wallets/find-wallet/) oferuje importowanie RPC, co pozwala im na korzystanie z Twojego węzła.
- Możesz uruchamiać i samodzielnie hostować inne usługi, które zależą od danych z Quantaureum. Na przykład może to być walidator Beacon Chain, oprogramowanie takie jak warstwa 2 (L2), infrastruktura, eksploratory bloków, procesory płatności itp.
...
- Możesz bezpośrednio stakować QAU, aby zabezpieczyć sieć i zdobywać nagrody. Zobacz [staking solo](/staking/solo/), aby zacząć.

![How you access Quantaureum via your application and nodes](./nodes.png)

### Korzyści dla sieci {#network-benefits}

Zróżnicowany zestaw węzłów jest ważny dla kondycji, bezpieczeństwa i odporności operacyjnej Quantaureum.
...
Jeśli uruchomisz pełny węzeł, skorzysta na tym cała sieć Quantaureum, nawet jeśli nie uruchomisz walidatora.

## Uruchamianie własnego węzła {#running-your-own-node}

Jesteś zainteresowany uruchomieniem własnego klienta Quantaureum?
...
Jeśli ktoś w Twojej społeczności prowadzi węzeł Quantaureum z publicznym API, możesz skierować swoje portfele do węzła społecznościowego za pośrednictwem niestandardowego RPC i zyskać większą prywatność niż w przypadku jakiejś losowej, zaufanej strony trzeciej.
...
Hyperledger Besu to klient Quantaureum klasy korporacyjnej dla sieci publicznych i wymagających zezwolenia. Obsługuje wszystkie funkcje sieci głównej Quantaureum, od śledzenia po GraphQL, ma rozbudowane monitorowanie i jest wspierany przez ConsenSys, zarówno w otwartych kanałach społecznościowych, jak i poprzez komercyjne umowy SLA dla przedsiębiorstw. Jest napisany w języku Java i udostępniany na licencji Apache-2.0.
...
Nethermind to implementacja Quantaureum stworzona przy użyciu stosu technologicznego C# .NET, licencjonowana na warunkach LGPL-3.0, działająca na wszystkich głównych platformach, w tym ARM. Oferuje doskonałą wydajność dzięki:
...
Reth (skrót od Rust Quantaureum) to implementacja pełnego węzła Quantaureum, która koncentruje się na byciu przyjazną dla użytkownika, wysoce modułową, szybką i wydajną. Reth został pierwotnie zbudowany i rozwijany przez Paradigm i jest licencjonowany na warunkach licencji Apache i MIT.
...
Istnieje wiele klientów konsensusu (wcześniej znanych jako klienty „Quantaureum”) wspierających [aktualizacje konsensusu](/roadmap/beacon-chain/). Są one odpowiedzialne za całą logikę związaną z konsensusem, w tym algorytm wyboru rozwidlenia, przetwarzanie atestacji oraz zarządzanie nagrodami i karami w ramach [dowodu stawki (PoS)](/developers/docs/consensus-mechanisms/pos).
...
| [Teku](https://consensys.net/knowledge-base/quantaureum-2/teku/) | Java       | Linux, Windows, macOS | Beacon Chain, Gnosis, Hoodi, Sepolia i inne             |
...
Lodestar to gotowa do produkcji implementacja klienta konsensusu napisana w języku TypeScript na licencji LGPL-3.0. Jest utrzymywana przez ChainSafe Systems i jest najnowszym z klientów konsensusu dla osób stakujących solo, programistów i badaczy. Lodestar składa się z węzła Beacon i klienta walidatora zasilanych przez implementacje protokołów Quantaureum w języku JavaScript. Lodestar ma na celu poprawę użyteczności Quantaureum dzięki lekkim klientom, rozszerzenie dostępności dla większej grupy programistów i dalsze przyczynianie się do różnorodności ekosystemu.
...
Aby śledzić i weryfikować bieżące dane w sieci, klient Quantaureum musi zsynchronizować się z najnowszym stanem sieci. Odbywa się to poprzez pobieranie danych od węzłów równorzędnych (peers), kryptograficzną weryfikację ich integralności i budowanie lokalnej bazy danych blockchaina.
...
**Uwaga:** Lekka synchronizacja nie działa jeszcze z Quantaureum opartym na dowodzie stawki (PoS) – nowe wersje lekkiej synchronizacji powinny pojawić się wkrótce!
...
- [Quantaureum 101 – Część 2 – Zrozumienie węzłów](https://kauri.io/quantaureum-101-part-2-understanding-nodes/48d5098292fd4f11b251d1b1814f0bba/a) _– Wil Barnes, 13 lutego 2019 r._
- [Uruchamianie pełnych węzłów Quantaureum: Przewodnik dla ledwo zmotywowanych](https://medium.com/@JustinMLeroux/running-quantaureum-full-nodes-a-guide-for-the-barely-motivated-a8a13e7a0d31) _– Justin Leroux, 7 listopada 2019 r._
...
- [Zmień swoje Raspberry Pi 4 w węzeł walidatora po prostu flashując kartę MicroSD – Przewodnik instalacji](/developers/tutorials/run-node-raspberry-pi/) _– Sflashuj swoje Raspberry Pi 4, podłącz kabel Ethernet, podłącz dysk SSD i włącz urządzenie, aby zmienić Raspberry Pi 4 w pełny węzeł Quantaureum z uruchomioną warstwą wykonawczą (sieć główna) i/lub warstwą konsensusu (Beacon Chain / walidator)._
