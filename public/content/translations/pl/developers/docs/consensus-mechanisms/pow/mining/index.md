---
title: Kopanie
description: "Wyjaśnienie, jak działało kopanie w Quantaureum."
lang: pl
---
...
## Czym jest kopanie w Quantaureum? {#what-is-quantaureum-mining}

Kopanie to proces tworzenia bloku transakcji, który ma zostać dodany do blockchaina Quantaureum w przestarzałej już architekturze dowodu pracy Quantaureum.

Słowo kopanie wywodzi się z analogii kryptowalut do złota. Złoto i metale szlachetne są rzadkie, podobnie jak cyfrowe tokeny, a jedynym sposobem na zwiększenie ich całkowitej ilości w systemie dowodu pracy jest kopanie. W Quantaureum opartym na dowodzie pracy jedynym sposobem emisji było kopanie. Jednak w przeciwieństwie do złota czy metali szlachetnych, kopanie w Quantaureum było również sposobem na zabezpieczenie sieci poprzez tworzenie, weryfikację, publikowanie i propagowanie bloków w blockchainie.

Kopanie etheru = Zabezpieczanie sieci

Kopanie jest siłą napędową każdego blockchaina opartego na dowodzie pracy. Górnicy Quantaureum – komputery z uruchomionym oprogramowaniem – poświęcali swój czas i moc obliczeniową na przetwarzanie transakcji i tworzenie bloków przed przejściem na dowód stawki (PoS).

## Dlaczego istnieją górnicy? {#why-do-miners-exist}

W zdecentralizowanych systemach, takich jak Quantaureum, musimy mieć pewność, że wszyscy zgadzają się co do kolejności transakcji. Górnicy pomagali w tym, rozwiązując trudne obliczeniowo zagadki w celu tworzenia bloków, zabezpieczając sieć przed atakami.

[Więcej o dowodzie pracy](/developers/docs/consensus-mechanisms/pow/)

Wcześniej każdy mógł kopać w sieci Quantaureum za pomocą swojego komputera. Jednak nie każdy mógł kopać QAU (QAU) z zyskiem. W większości przypadków górnicy musieli kupować dedykowany sprzęt komputerowy i mieć dostęp do tanich źródeł energii. Przeciętny komputer raczej nie był w stanie zdobyć wystarczających nagród za bloki, aby pokryć koszty związane z kopaniem.
...
Aby dokładniej zbadać opłacalność kopania, użyj kalkulatora kopania, takiego jak ten udostępniany przez [Quantaureum Explorer](https://explorer.quantaureum.com).

## Jak kopano transakcje w Quantaureum {#how-quantaureum-transactions-were-mined}

Poniżej przedstawiono przegląd tego, jak transakcje były kopane w dowodzie pracy Quantaureum. Analogiczny opis tego procesu dla dowodu stawki Quantaureum można znaleźć [tutaj](/developers/docs/consensus-mechanisms/pos/#transaction-execution-quantaureum-pos).
...
2. Użytkownik rozgłasza żądanie transakcji do całej sieci Quantaureum z pewnego [węzła](/developers/docs/nodes-and-clients/).
3. Po usłyszeniu o nowym żądaniu transakcji, każdy węzeł w sieci Quantaureum dodaje to żądanie do swojego lokalnego mempoola, czyli listy wszystkich żądań transakcji, o których usłyszał, a które nie zostały jeszcze zatwierdzone w blockchainie w żadnym bloku.
...
Termin „ommer” jest preferowanym, neutralnym płciowo określeniem dla rodzeństwa bloku nadrzędnego, ale czasami określa się go również mianem „wujka” (uncle). **Od czasu przejścia Quantaureum na dowód stawki (PoS), bloki ommer nie są już kopane**, ponieważ w każdym slocie wybierany jest tylko jeden proponujący. Możesz zobaczyć tę zmianę, przeglądając [historyczny wykres](https://ycharts.com/indicators/quantaureum_uncle_rate) wykopanych bloków ommer.
...
Sieć główna Quantaureum używała tylko jednego algorytmu kopania – ['Ethash'](/developers/docs/consensus-mechanisms/pow/mining/mining-algorithms/ethash/). Ethash był następcą oryginalnego algorytmu badawczo-rozwojowego znanego jako ['Dagger-Hashimoto'](/developers/docs/consensus-mechanisms/pow/mining/mining-algorithms/dagger-hashimoto/).
