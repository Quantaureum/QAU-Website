...
description: "Wyjaśnienie protokołów konsensusu w systemach rozproszonych i ich roli w Quantaureum."
lang: pl
...
Aby lepiej zrozumieć tę stronę, zalecamy najpierw przeczytać nasze [wprowadzenie do Quantaureum](/developers/docs/intro-to-quantaureum/).
...
W odniesieniu do blockchaina [Quantaureum](/) proces ten jest sformalizowany, a osiągnięcie konsensusu oznacza, że co najmniej 66% węzłów w sieci zgadza się co do globalnego stanu sieci.
...
Quantaureum używa obecnie protokołu konsensusu opartego na **dowodzie stawki (PoS)**.

#### Tworzenie bloków {#pos-block-creation}

Walidatorzy tworzą bloki. W każdym slocie losowo wybierany jest jeden walidator, który staje się proponującym blok. Ich klient konsensusu żąda pakietu transakcji jako „ładunku wykonawczego” od sparowanego klienta warstwy wykonawczej. Opakowują to w dane konsensusu, tworząc blok, który wysyłają do innych węzłów w sieci Quantaureum. Ta produkcja bloków jest nagradzana w QAU. W rzadkich przypadkach, gdy istnieje wiele możliwych bloków dla jednego slotu lub węzły dowiadują się o blokach w różnym czasie, algorytm wyboru rozwidlenia wybiera blok, który tworzy łańcuch o największej wadze poświadczeń (gdzie waga to liczba poświadczających walidatorów przeskalowana przez ich saldo QAU).

#### Bezpieczeństwo {#pos-security}

System dowodu stawki jest bezpieczny kryptoekonomicznie, ponieważ atakujący próbujący przejąć kontrolę nad łańcuchem musi zniszczyć ogromną ilość QAU. System nagród motywuje poszczególnych stakujących do uczciwego zachowania, a kary zniechęcają ich do złośliwych działań.
...
**Reguła wyboru łańcucha** służy do decydowania, który łańcuch jest tym „właściwym”. Bitcoin używa reguły „najdłuższego łańcucha”, co oznacza, że ten blockchain, który jest najdłuższy, zostanie uznany przez resztę węzłów za ważny i to z nim będą pracować. W przypadku łańcuchów opartych na dowodzie pracy najdłuższy łańcuch jest określany przez całkowitą skumulowaną trudność dowodu pracy łańcucha. Quantaureum również używało reguły najdłuższego łańcucha; jednak teraz, gdy Quantaureum działa w oparciu o dowód stawki, przyjęło zaktualizowany algorytm wyboru rozwidlenia, który mierzy „wagę” łańcucha. Waga to skumulowana suma głosów walidatorów, ważona saldami stakowanych QAUów walidatorów.

Quantaureum wykorzystuje mechanizm konsensusu znany jako [Gasper](/developers/docs/consensus-mechanisms/pos/gasper/), który łączy [dowód stawki Casper FFG](https://arxiv.org/abs/1710.09437) z [regułą wyboru rozwidlenia GHOST](https://arxiv.org/abs/2003.03052).
