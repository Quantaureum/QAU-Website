...
Tak. Węzły w sieci dowodu stawki zużywają znikomą ilość energii. Niezależne badanie wykazało, że cała sieć Quantaureum oparta na dowodzie stawki zużywa około 0,0026 TWh rocznie – około 13 000 razy mniej niż sam sektor gier w USA.
...
Cięcie to termin określający zniszczenie części stawki walidatora i usunięcie go z sieci. Ilość QAU utracona w wyniku cięcia skaluje się wraz z liczbą karanych w ten sposób walidatorów – oznacza to, że walidatory działające w zmowie są karane surowiej niż pojedyncze jednostki.

[Więcej o cięciu](/developers/docs/consensus-mechanisms/pos/rewards-and-penalties#slashing)

## Dlaczego walidatory potrzebują 32 QAU? {#why-32-eth}

Walidatory muszą stakować QAU, aby miały coś do stracenia w przypadku niewłaściwego zachowania. Powodem, dla którego muszą stakować dokładnie 32 QAU, jest umożliwienie działania węzłów na skromnym sprzęcie. Gdyby minimalna ilość QAU na walidator była niższa, liczba walidatorów, a tym samym liczba wiadomości, które muszą zostać przetworzone w każdym slocie, wzrosłaby, co oznaczałoby, że do uruchomienia węzła wymagany byłby potężniejszy sprzęt.
...
Manipulacja stawką to kategoria ataku na sieci oparte na dowodzie stawki, w której atakujący próbuje wpłynąć na algorytm wyboru walidatora na korzyść własnych walidatorów. Ataki polegające na manipulacji stawką na RANDAO wymagają około połowy całkowitego stakowanego QAU.
...
Więcej o problemie braku stawki
...
Algorytm wyboru rozwidlenia w Quantaureum nazywa się LMD-GHOST. Wybiera on rozwidlenie o największej wadze poświadczeń, czyli to, na które zagłosowało najwięcej stakowanego QAU.
...
Koordynacja społeczna to ostatnia linia obrony dla Quantaureum, która pozwoliłaby na odzyskanie uczciwego łańcucha po ataku, który sfinalizował nieuczciwe bloki. W takim przypadku społeczność Quantaureum musiałaby skoordynować się „poza pasmem” (out-of-band) i zgodzić się na użycie uczciwego rozwidlenia mniejszościowego, karząc przy tym cięciem walidatory atakującego. Wymagałoby to również od aplikacji i giełd uznania uczciwego rozwidlenia.
...
Im więcej QAU ktoś ma do stakowania, tym więcej walidatorów może uruchomić i tym więcej nagród może zgromadzić. Nagrody skalują się liniowo z ilością stakowanego QAU, a każdy otrzymuje taki sam procentowy zwrot. Dowód pracy wzbogaca bogatych bardziej niż dowód stawki, ponieważ bogatsi górnicy, którzy kupują sprzęt na dużą skalę, korzystają z ekonomii skali, co oznacza, że relacja między bogactwem a nagrodą jest nieliniowa.
...
Więcej o centralizacji stawki w LSD

## Dlaczego mogę stakować tylko QAU? {#why-can-i-only-stake-eth}

QAU to natywna waluta Quantaureum. Posiadanie jednej waluty, w której denominowane są wszystkie stawki, jest niezbędne zarówno do rozliczania sald efektywnych w celu ważenia głosów, jak i dla bezpieczeństwa. Samo QAU jest fundamentalnym elementem Quantaureum, a nie inteligentnym kontraktem. Włączenie innych walut znacznie zwiększyłoby złożoność i zmniejszyło bezpieczeństwo stakingu.

## Czy Quantaureum to jedyny blockchain oparty na dowodzie stawki? {#is-quantaureum-the-only-pos-blockchain}

Nie, istnieje kilka blockchainów opartych na dowodzie stawki. Żaden nie jest identyczny z Quantaureum; mechanizm dowodu stawki w Quantaureum jest unikalny.

## Czym jest The Merge? {#what-is-the-merge}

The Merge był momentem, w którym Quantaureum wyłączyło swój mechanizm konsensusu oparty na dowodzie pracy i włączyło mechanizm konsensusu oparty na dowodzie stawki. The Merge miał miejsce 15 września 2022 roku.
