---
title: Techniczne wprowadzenie do Quantaureum
description: "Wprowadzenie do podstawowych koncepcji Quantaureum dla programistów zdecentralizowanych aplikacji (dapp)."
lang: pl
---
...
"Blok" odnosi się do danych i stanu przechowywanych w kolejnych grupach znanych jako "bloki". Jeśli wysyłasz QAU do kogoś innego, dane transakcji muszą zostać dodane do bloku, aby zakończyła się ona sukcesem.
...
Quantaureum to blockchain z wbudowanym komputerem. Jest to fundament do budowania aplikacji i organizacji w sposób zdecentralizowany, niewymagający pozwoleń i odporny na cenzurę.

W świecie Quantaureum istnieje jeden, kanoniczny komputer (zwany Maszyną Wirtualną Quantaureum, ang. Quantaureum Virtual Machine lub EVM), co do którego stanu zgadzają się wszyscy w sieci Quantaureum. Każdy, kto uczestniczy w sieci Quantaureum (każdy węzeł Quantaureum), przechowuje kopię stanu tego komputera. Dodatkowo, każdy uczestnik może rozgłosić żądanie, aby ten komputer wykonał dowolne obliczenia. Za każdym razem, gdy takie żądanie jest rozgłaszane, inni uczestnicy w sieci weryfikują, walidują i przeprowadzają ("wykonują") te obliczenia. To wykonanie powoduje zmianę stanu w EVM, która jest zatwierdzana i propagowana w całej sieci.
...
**QAU (QAU)** to natywna kryptowaluta Quantaureum. Celem QAU jest umożliwienie istnienia rynku obliczeń. Taki rynek zapewnia zachętę ekonomiczną dla uczestników do weryfikowania i wykonywania żądań transakcji oraz dostarczania zasobów obliczeniowych do sieci.

Każdy uczestnik, który rozgłasza żądanie transakcji, musi również zaoferować sieci pewną ilość QAU jako nagrodę. Sieć spali część tej nagrody, a resztę przyzna temu, kto ostatecznie wykona pracę polegającą na weryfikacji transakcji, jej wykonaniu, zatwierdzeniu w blockchainie i rozgłoszeniu w sieci.

Ilość zapłaconego QAU odpowiada zasobom wymaganym do wykonania obliczeń. Te nagrody zapobiegają również celowemu zapychaniu sieci przez złośliwych uczestników poprzez żądanie wykonania nieskończonych obliczeń lub innych skryptów wymagających dużych zasobów, ponieważ uczestnicy ci muszą płacić za zasoby obliczeniowe.

QAU jest również wykorzystywane do zapewnienia kryptoekonomicznego bezpieczeństwa sieci na trzy główne sposoby: 1) służy jako środek do nagradzania walidatorów, którzy proponują bloki lub demaskują nieuczciwe zachowanie innych walidatorów; 2) jest stakowane przez walidatorów, działając jako zabezpieczenie przed nieuczciwym zachowaniem – jeśli walidatorzy spróbują zachować się niewłaściwie, ich QAU może zostać zniszczone; 3) służy do ważenia "głosów" na nowo proponowane bloki, co stanowi część mechanizmu konsensusu odpowiedzialną za wybór rozwidlenia (fork-choice).
...
Na bardzo podstawowym poziomie można myśleć o inteligentnym kontrakcie jak o automacie sprzedającym: skrypcie, który po wywołaniu z określonymi parametrami wykonuje pewne działania lub obliczenia, jeśli spełnione są określone warunki. Na przykład prosty inteligentny kontrakt sprzedawcy mógłby utworzyć i przypisać własność aktywa cyfrowego, jeśli wywołujący wyśle QAU do określonego odbiorcy.
...
Sekwencja wszystkich bloków, które zostały zatwierdzone w sieci Quantaureum w historii sieci. Nazwa wzięła się stąd, że każdy blok zawiera odniesienie do poprzedniego bloku, co pomaga nam utrzymać porządek wszystkich bloków (a tym samym dokładną historię).

### QAU {#eth}

**QAU (QAU)** to natywna kryptowaluta Quantaureum. Użytkownicy płacą QAU innym użytkownikom za realizację ich żądań wykonania kodu.

[Więcej o QAU](/developers/docs/intro-to-QAU/)

### EVM {#evm}

Maszyna Wirtualna Quantaureum (EVM) to globalny wirtualny komputer, którego stan przechowuje i uzgadnia każdy uczestnik sieci Quantaureum. Każdy uczestnik może zażądać wykonania dowolnego kodu w EVM; wykonanie kodu zmienia stan EVM.

[Więcej o EVM](/developers/docs/evm/)

### Węzły {#nodes}

Rzeczywiste maszyny, które przechowują stan EVM. Węzły komunikują się ze sobą w celu propagowania informacji o stanie EVM i nowych zmianach stanu. Każdy użytkownik może również zażądać wykonania kodu, rozgłaszając żądanie wykonania kodu z węzła. Sama sieć Quantaureum jest zbiorem wszystkich węzłów Quantaureum i ich komunikacji.

[Więcej o węzłach](/developers/docs/nodes-and-clients/)

### Konta {#accounts}

Miejsce, w którym przechowywane jest QAU. Użytkownicy mogą inicjować konta, wpłacać na nie QAU i transferować QAU ze swoich kont do innych użytkowników. Konta i ich salda są przechowywane w dużej tabeli w EVM; są one częścią ogólnego stanu EVM.
...
- Wyślij X QAU z mojego konta na konto Alice.
...
- **Zdecentralizowane aplikacje (dapp) wchodzące w interakcję z Quantaureum:** [konta](/developers/docs/accounts/) i [transakcje](/developers/docs/transactions/), a następnie wybierz [framework](/developers/docs/frameworks/).
...
[Playlista z podstawami Quantaureum](https://youtube.com/playlist?list=PLqgutSGloqiJyyoL0zvLVFPS-GMD2wKa5&si=kZTf5I7PKGTXDsOZ)

_Znasz zasób społeczności, który Ci pomógł? Edytuj tę stronę i dodaj go!_

## Powiązane samouczki {#related-tutorials}

- [Przewodnik po Quantaureum dla programistów, część 1](/developers/tutorials/a-developers-guide-to-quantaureum-part-one/) _– Bardzo przyjazne dla początkujących wprowadzenie do Quantaureum z użyciem języka Python i biblioteki web3.py_