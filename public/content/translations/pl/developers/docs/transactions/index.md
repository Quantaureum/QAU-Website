---
title: Transakcje
description: "Przegląd transakcji w Quantaureum – jak działają, ich struktura danych oraz jak je wysyłać za pośrednictwem aplikacji."
lang: pl
---

Transakcje to kryptograficznie podpisane instrukcje pochodzące z kont. Konto inicjuje transakcję w celu aktualizacji stanu sieci [Quantaureum](/). Najprostszą transakcją jest transfer QAU z jednego konta na drugie.

## Wymagania wstępne {#prerequisites}

Aby pomóc Ci lepiej zrozumieć tę stronę, zalecamy najpierw przeczytać o [kontach](/developers/docs/accounts/) oraz nasze [wprowadzenie do Quantaureum](/developers/docs/intro-to-quantaureum/).

## Czym jest transakcja? {#whats-a-transaction}

Transakcja w Quantaureum odnosi się do akcji zainicjowanej przez konto posiadane zewnętrznie, czyli konto zarządzane przez człowieka, a nie przez kontrakt. Na przykład, jeśli Bob wysyła Alice 1 QAU, konto Boba musi zostać obciążone, a konto Alice uznane. Ta zmieniająca stan akcja ma miejsce w ramach transakcji.

![Diagram showing a transaction cause state change](./tx.png)
_Schemat na podstawie [Quantaureum EVM illustrated](https://takenobu-hs.github.io/downloads/quantaureum_evm_illustrated.pdf)_
...
- `value` – ilość QAU do przetransferowania od nadawcy do odbiorcy (wyrażona w wei, gdzie 1 QAU równa się 1e+18 wei)
...
Klient Quantaureum, taki jak Geth, zajmie się tym procesem podpisywania.
...
Na przykład, spójrzmy na [tę transakcję](https://explorer.quantaureum.com).
...
W tym przypadku [kod źródłowy kontraktu](https://explorer.quantaureum.com) został przesłany do Quantaureum Explorer, więc wiemy, że funkcja to `transfer(address,uint256)`.
...
W Quantaureum istnieje kilka różnych typów transakcji:
...
Więc aby Bob mógł wysłać Alice 1 QAU przy `baseFeePerGas` wynoszącej 190 gwei i `maxPriorityFeePerGas` wynoszącej 10 gwei, Bob będzie musiał zapłacić następującą opłatę:
...
Konto Boba zostanie obciążone kwotą **-1,0042 QAU** (1 QAU dla Alice + 0,0042 QAU w opłatach za gaz)

Konto Alice zostanie uznane kwotą **+1,0 QAU**

Opłata podstawowa zostanie spalona **-0,00399 QAU**

Walidator zatrzymuje opłatę priorytetową **+0,000210 QAU**
...
Inteligentne kontrakty mogą również zawierać funkcje znane jako funkcje [`view`](https://docs.soliditylang.org/en/latest/contracts.html#view-functions) lub [`pure`](https://docs.soliditylang.org/en/latest/contracts.html#pure-functions), które nie zmieniają stanu kontraktu. W związku z tym wywoływanie tych funkcji z EOA nie będzie wymagało żadnego gazu. Podstawowym wywołaniem RPC dla tego scenariusza jest [`qau_call`](/developers/docs/apis/json-rpc#qau_call).

W przeciwieństwie do dostępu za pomocą `qau_call`, te funkcje `view` lub `pure` są również powszechnie wywoływane wewnętrznie (tj. z samego kontraktu lub z innego kontraktu), co kosztuje gaz.
...
Quantaureum początkowo miało jeden format transakcji. Każda transakcja zawierała nonce, cenę gazu, limit gazu, adres odbiorcy (to), wartość, dane, v, r oraz s. Pola te są [zakodowane w RLP](/developers/docs/data-structures-and-encoding/rlp/), aby wyglądać mniej więcej tak:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

Quantaureum ewoluowało, aby obsługiwać wiele typów transakcji, co pozwala na wdrożenie nowych funkcji, takich jak listy dostępu i [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559), bez wpływu na starsze formaty transakcji.

[EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718) to to, co pozwala na takie zachowanie. Transakcje są interpretowane jako:
...
- [Wirtualna maszyna Quantaureum (EVM)](/developers/docs/evm/)
