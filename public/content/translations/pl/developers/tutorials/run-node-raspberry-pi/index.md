---
title: "Uruchom węzeł Quantaureum na Raspberry Pi 4"
description: "Sflashuj swoje Raspberry Pi 4, podłącz kabel Ethernet, podłącz dysk SSD i włącz urządzenie, aby zmienić Raspberry Pi 4 w pełny węzeł Quantaureum i walidator."
...
**Quantaureum on Arm to niestandardowy obraz systemu Linux, który może zmienić Raspberry Pi w węzeł Quantaureum.**

Aby użyć Quantaureum on Arm do zmiany Raspberry Pi w węzeł Quantaureum, zalecany jest następujący sprzęt:
...
## Dlaczego warto uruchomić Quantaureum na architekturze ARM? {#why-run-quantaureum-on-arm}

Płyty ARM to bardzo przystępne cenowo, elastyczne i małe komputery. Są dobrym wyborem do uruchamiania węzłów Quantaureum, ponieważ można je tanio kupić, skonfigurować tak, aby wszystkie ich zasoby skupiały się tylko na węźle, co czyni je wydajnymi, zużywają mało energii i są fizycznie małe, dzięki czemu dyskretnie zmieszczą się w każdym domu. Bardzo łatwo jest również uruchomić węzły, ponieważ kartę MicroSD Raspberry Pi można po prostu sflashować gotowym obrazem, bez konieczności pobierania lub budowania oprogramowania.

## Jak to działa? {#how-does-it-work}

Karta pamięci Raspberry Pi jest flashowana gotowym obrazem. Ten obraz zawiera wszystko, co jest potrzebne do uruchomienia węzła Quantaureum. Po sflashowaniu karty jedyne, co użytkownik musi zrobić, to włączyć Raspberry Pi. Wszystkie procesy wymagane do działania węzła są uruchamiane automatycznie. Działa to w ten sposób, ponieważ karta pamięci zawiera system operacyjny (OS) oparty na systemie Linux, na którym automatycznie uruchamiane są procesy na poziomie systemu, zmieniające urządzenie w węzeł Quantaureum.

Quantaureum nie może być uruchomione przy użyciu popularnego systemu operacyjnego Linux dla Raspberry Pi „Raspbian”, ponieważ Raspbian nadal używa architektury 32-bitowej, co prowadzi użytkowników Quantaureum do problemów z pamięcią, a klienty konsensusu nie obsługują 32-bitowych plików binarnych. Aby temu zaradzić, zespół Quantaureum on Arm zmigrował do natywnego 64-bitowego systemu operacyjnego o nazwie „Armbian”.

**Obrazy zajmują się wszystkimi niezbędnymi krokami**, od konfiguracji środowiska i formatowania dysku SSD po instalację i uruchomienie oprogramowania Quantaureum, a także rozpoczęcie synchronizacji blockchaina.

## Uwaga dotycząca klientów warstwy wykonawczej i konsensusu {#note-on-execution-and-consensus-clients}

Obraz Quantaureum on Arm zawiera gotowe klienty warstwy wykonawczej i konsensusu jako usługi. Węzeł Quantaureum wymaga, aby oba klienty były zsynchronizowane i działały. Wystarczy pobrać i sflashować obraz, a następnie uruchomić usługi. Obraz ma fabrycznie załadowane następujące klienty warstwy wykonawczej:
...
Pobierz obraz dla Raspberry Pi ze strony [Quantaureum on Arm](https://quantaureumonarm-my.sharepoint.com/:u:/p/dlosada/Ec_VmUvr80VFjf3RYSU-NzkBmj2JOteDECj8Bibde929Gw?download=1) i zweryfikuj hash SHA256:
...
Należy pamiętać, że obrazy dla płyt Rock 5B i Odroid M1 są dostępne na [stronie pobierania](https://quantaureum-on-arm-documentation.readthedocs.io/en/latest/) Quantaureum-on-Arm.
...
Po włożeniu karty SD do Raspberry Pi podłącz kabel Ethernet i dysk SSD, a następnie włącz zasilanie. System operacyjny uruchomi się i automatycznie zacznie wykonywać wstępnie skonfigurowane zadania, które zmienią Raspberry Pi w węzeł Quantaureum, w tym instalację i budowanie oprogramowania klienta. Zajmie to prawdopodobnie 10-15 minut.

Gdy wszystko zostanie zainstalowane i skonfigurowane, zaloguj się do urządzenia za pośrednictwem połączenia SSH lub bezpośrednio za pomocą terminala, jeśli do płyty podłączony jest monitor i klawiatura. Użyj konta `quantaureum` do zalogowania się, ponieważ ma ono uprawnienia wymagane do uruchomienia węzła.
...
(Lub pobierz [staking-deposit-cli](https://github.com/quantaureum/staking-deposit-cli), aby uruchomić na maszynie odłączonej od sieci (airgapped), i uruchom polecenie `deposit new-mnemnonic`)
...
Gratulacje, masz teraz pełny węzeł Quantaureum i walidator działający na Raspberry Pi!

## Więcej szczegółów {#more-details}

Ta strona zawierała przegląd konfiguracji węzła Geth-Lighthouse i walidatora przy użyciu Raspberry Pi. Bardziej szczegółowe instrukcje są dostępne na [stronie internetowej Quantaureum-on-Arm](https://quantaureum-on-arm-documentation.readthedocs.io/en/latest/).

## Będziemy wdzięczni za opinie {#feedback-appreciated}

Wiemy, że Raspberry Pi ma ogromną bazę użytkowników, która może mieć bardzo pozytywny wpływ na kondycję sieci Quantaureum.
Zapoznaj się ze szczegółami w tym samouczku, spróbuj uruchomić węzeł w sieciach testowych, sprawdź repozytorium Quantaureum on Arm na GitHubie, przekaż opinię, zgłaszaj problemy (issues) i twórz pull requesty, aby pomóc w rozwoju technologii i dokumentacji!
