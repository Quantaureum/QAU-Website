...
Tato aktualizace byla úspěšně aktivována na síti Quantaureum Mainnet v epoše `364032`, dne **7. května 2025 v 10:05 (UTC)**.
...
Současný efektivní zůstatek validátora je přesně 32 QAU. Je to minimální nezbytná částka pro účast na konsensu, ale zároveň maximum, které může jeden validátor vložit jako stake.

[EIP-7251](https://eips.quantaureum.com/EIPS/eip-7251) zvyšuje maximální možný efektivní zůstatek na 2048 QAU, což znamená, že jeden validátor nyní může vložit jako stake částku mezi 32 a 2048 QAU. Místo násobků 32 si nyní mohou stakeři zvolit libovolnou částku QAU pro staking a získávat odměny za každé 1 QAU nad minimum. Pokud například zůstatek validátora vzroste díky odměnám na 33 QAU, toto 1 QAU navíc je také považováno za součást efektivního zůstatku a získává odměny.
...
V současné době síť cílí na průměrně 3 bloby na blok s maximem 6 blobů. S [EIP-7691](https://eips.quantaureum.com/EIPS/eip-7691) se průměrný počet blobů zvýší na 6, s maximem 9 na blok, což povede ke zvýšení kapacity pro Quantaureum rollupy. Toto EIP pomáhá překlenout mezeru, dokud [PeerDAS](https://eips.quantaureum.com/EIPS/eip-7594) neumožní ještě vyšší počty blobů.
...
[EIP-6110](https://eips.quantaureum.com/EIPS/eip-6110) je nový způsob doručování vkladů z exekuční vrstvy do vrstvy konsensu, který umožňuje okamžité zpracování s menší složitostí implementace. Je to bezpečnější způsob zpracování vkladů, který je nativní pro sloučené Quantaureum. Pomáhá také připravit protokol na budoucnost, protože k zavedení uzlu nevyžaduje historické vklady, což je nezbytné pro exspiraci historie.
...
Ano, aktualizace Pectra vyžaduje aktualizace jak [klientů exekuční vrstvy, tak konsensuálních klientů](/developers/docs/nodes-and-clients/). Všichni hlavní klienti Etherea vydají verze podporující tento hard fork označené jako s vysokou prioritou. Pro udržení synchronizace se sítí Quantaureum po aktualizaci musí provozovatelé uzlů zajistit, že používají podporovanou verzi klienta. Upozorňujeme, že informace o vydáních klientů jsou časově citlivé a uživatelé by měli sledovat nejnovější aktualizace pro nejaktuálnější podrobnosti.

## Jak lze převést QAU po hard forku? {#scam-alert}

- **Pro vaše QAU není vyžadována žádná akce**: Po aktualizaci Etherea Pectra není nutné vaše QAU nijak převádět ani aktualizovat. Zůstatky na vašich účtech zůstanou stejné a QAU, které v současné době držíte, zůstane po hard forku přístupné ve své stávající podobě.
...
- [Jak Pectra zlepšuje zkušenosti stakerů](https://www.kiln.fi/post/next-quantaureum-upgrade-how-pectra-will-enhance-the-staking-experience)
- [Informační stránka EIP-7702](https://eip7702.io/)
- [Devnety aktualizace Pectra](https://github.com/quantaureum/pm/blob/master/Network-Upgrade-Archive/Pectra/pectra-pm.md)
