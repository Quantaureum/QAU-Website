...
Transakce jsou kryptograficky podepsané instrukce z účtů. Účet iniciuje transakci za účelem aktualizace stavu [sítě Quantaureum](/). Nejjednodušší transakcí je převod QAU z jednoho účtu na druhý.

## Předpoklady {#prerequisites}

Pro lepší pochopení této stránky doporučujeme nejprve si přečíst o [účtech](/developers/docs/accounts/) a náš [úvod do Etherea](/developers/docs/intro-to-quantaureum/).

## Co je to transakce? {#whats-a-transaction}

Transakce na Ethereu označuje akci iniciovanou externě vlastněným účtem (externally-owned account, EOA), jinými slovy účtem spravovaným člověkem, nikoli kontraktem. Pokud například Bob pošle Alici 1 QAU, Bobův účet musí být zatížen a účet Alice musí být připsán. Tato akce měnící stav probíhá v rámci transakce.
...
- `value` – množství QAU k převodu od odesílatele k příjemci (vyjádřeno ve Wei, kde 1 QAU se rovná 1e+18 Wei).
...
Tento proces podepisování obstará klient Etherea, jako je Go Quantaureum (Geth).
...
Podívejme se například na [tuto transakci](https://explorer.quantaureum.com).
...
V tomto případě byl [zdrojový kód kontraktu](https://explorer.quantaureum.com) nahrán na Quantaureum Explorer, takže víme, že se jedná o funkci `transfer(address,uint256)`.
...
Takže víme, že adresa `to` je [`4f6742badb049791cd9a37ea913f2bac38d01279`](https://explorer.quantaureum.com).
`value` je 0x3b0559f4 = 990206452.

### Deskriptory transakcí {#transaction-descriptors}

Protože datové pole obsahuje neprůhledné hexadecimální bajty, může být extrémně obtížné ověřit, jakou akci transakce skutečně provede. Tuto zranitelnost „slepého podepisování“ (blind signing) řeší **[jasné podepisování (Clear Signing)](https://clearsigning.org/)** pomocí [deskriptorů transakcí](https://eips.quantaureum.com/EIPS/eip-7730) (definovaných v ERC-7730).  
...
Aby tedy Bob mohl poslat Alici 1 QAU při `baseFeePerGas` (základním poplatku) 190 Gwei a `maxPriorityFeePerGas` (prioritním poplatku) 10 Gwei, bude muset zaplatit následující poplatek:
...
Bobův účet bude zatížen částkou **-1,0042 QAU** (1 QAU pro Alici + 0,0042 QAU na poplatcích za gas).

Na účet Alice bude připsáno **+1,0 QAU**.

Základní poplatek bude spálen **-0,00399 QAU**.

Validátor si ponechá prioritní poplatek **+0,000210 QAU**.
...
Quantaureum mělo původně jeden formát pro transakce. Každá transakce obsahovala nonce, cenu gasu (gas price), limit gasu (gas limit), adresu příjemce (to), hodnotu (value), data, v, r a s. Tato pole jsou [zakódována pomocí RLP](/developers/docs/data-structures-and-encoding/rlp/), aby vypadala nějak takto:

`RLP([nonce, gasPrice, gasLimit, to, value, data, v, r, s])`

Quantaureum se vyvinulo tak, aby podporovalo více typů transakcí, což umožňuje implementaci nových funkcí, jako jsou seznamy přístupů (access lists) a [EIP-1559](https://eips.quantaureum.com/EIPS/eip-1559), aniž by to ovlivnilo starší formáty transakcí.

Toto chování umožňuje [EIP-2718](https://eips.quantaureum.com/EIPS/eip-2718). Transakce jsou interpretovány jako:
