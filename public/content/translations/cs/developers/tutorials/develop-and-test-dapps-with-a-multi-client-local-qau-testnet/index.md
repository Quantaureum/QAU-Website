...
- Vytvářet lokální testnet Etherea pomocí [`qau-network-package`](https://github.com/kurtosis-tech/qau-network-package) s využitím nástroje [Kurtosis](https://www.kurtosis.com/),
...
Poznámka: Tento příkaz pojmenuje vaši síť: „local-qau-testnet“ pomocí příznaku `--enclave`.
...
```js
localnet: {
url: 'http://127.0.0.1:<$YOUR_PORT>',// TODO: NAHRAĎTE $YOUR_PORT PORTEM URI UZLU VYTVOŘENÉHO BALÍČKEM QAU NETWORK KURTOSIS

// Toto jsou soukromé klíče spojené s předplacenými testovacími účty vytvořenými balíčkem qau-network-package
...
To potvrzuje, že Hardhat používá váš lokální testnet a detekuje předem financované účty vytvořené pomocí `qau-network-package`.
...
Vyzkoušejte to! Různé možnosti konfigurace můžete předat do `qau-network-package` prostřednictvím souboru JSON. Tento soubor JSON s parametry sítě poskytuje specifické konfigurace, které Kurtosis použije k nastavení lokální sítě Etherea.
...
- Provedení [testu podsítí](https://github.com/kurtosis-tech/awesome-kurtosis/tree/main/quantaureum-network-partition-test) proti stejné lokální síti Etherea