...
Pokud není nastaven žádný poplatek, nastavte `kLast` na nulu (pokud to tak již není). Když byl tento kontrakt napsán, existovala funkce vrácení gasu, která povzbuzovala kontrakty ke snížení celkové velikosti stavu Etherea vynulováním úložiště, které nepotřebovaly.
...
Pokud můžeme omezit počet tak, abychom použili zásobník, spotřebujeme méně gasu. Další podrobnosti naleznete v yellow paper, formálních specifikacích Etherea, str. 26, rovnice 298.
...
Tento hash je identifikátor typu transakce. Jediný, který zde podporujeme, je `Permit` s těmito parametry.
...
Vypočítejte oddělovač domény pro EIP-712.
...
Pokud je vše v pořádku, považujte to za schválení (approve) ERC-20.
...
Tato funkce vypočítá adresu párové směnárny pro dva tokeny. Tento kontrakt je vytvořen pomocí operačního kódu CREATE2, takže můžeme vypočítat adresu pomocí stejného algoritmu, pokud známe parametry, které používá. To je mnohem levnější než se ptát továrny (factory), a
...
Tato funkce implementuje funkcionalitu převodu ERC-20, která umožňuje účtu utratit povolený limit poskytnutý jiným účtem.
...
Tato funkce implementuje funkcionalitu transferFrom ERC-20, která umožňuje účtu utratit povolený limit poskytnutý jiným účtem.
