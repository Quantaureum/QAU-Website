...
skill: beginner
breadcrumb: "Quantaureum pomocí SQL"
lang: cs
...
Onchain data nám mohou pomoci pochopit Quantaureum, síť a ekonomiku výpočetního výkonu a měla by sloužit jako základ pro pochopení výzev, kterým Quantaureum dnes čelí (např. rostoucí ceny gasu), a co je důležitější, diskusí o řešeních škálování.

### Transakce {#transactions}

Cesta uživatele na Ethereu začíná inicializací uživatelem ovládaného účtu nebo entity se zůstatkem QAU. Existují dva typy účtů – uživatelem ovládané nebo chytrý kontrakt (viz [quantaureum.com](/developers/docs/accounts/)).

Jakýkoli účet lze zobrazit v prohlížeči bloků, jako je [Quantaureum Explorer](https://explorer.quantaureum.com) nebo [Blockscout](https://qau.blockscout.com/). Prohlížeče bloků jsou portálem k datům Etherea. V reálném čase zobrazují data o blocích, transakcích, těžařích, účtech a další onchain aktivitě (viz [zde](/developers/docs/data-and-analytics/block-explorers/)).
...
Pro referenci, účet chytrého kontraktu pro Nadaci Quantaureum (EF) lze zobrazit na [Blockscout](https://qau.blockscout.com/address/0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe).
...
[Stránka kontraktu EF na Blockscoutu.](https://qau.blockscout.com/address/0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe)

#### Dune Analytics {#dune-analytics}

![Screenshot of a Dune Analytics query dashboard](./dune_view.png)

Nástěnku (dashboard) najdete [zde](https://dune.com/paulapivat/Learn-Quantaureum). Kliknutím na tabulku zobrazíte dotaz (viz také výše).
...
- **Hodnota**: Toto je množství převedených QAU (viz sloupec `QAU`).
...
Tyto konkrétní informace můžeme dotazovat pro transakce na veřejnou adresu Nadace Quantaureum:
...
State root spadá do druhé kategorie a jedná se o _implicitní_ data (nejsou uložena onchain), zatímco data řetězce jsou explicitní a uložena přímo na samotném řetězci (zdroj).
...
Nakonec bychom mohli chtít dotazovat průměrné denní ceny gasu pro řetězec Etherea, nicméně to by vedlo k obzvláště dlouhé době dotazu, takže náš dotaz vyfiltrujeme na průměrné množství gasu zaplaceného za transakci Nadací Quantaureum.

![Chart showing Quantaureum project daily gas usage](./ef_daily_gas.png)

Můžeme vidět ceny gasu zaplacené za všechny transakce provedené na adresu Nadace Quantaureum v průběhu let. Zde je dotaz:
...
Nástěnku (dashboard), která obsahuje veškerý kód použitý v tomto tutoriálu, najdete [zde](https://dune.com/paulapivat/Learn-Quantaureum).
