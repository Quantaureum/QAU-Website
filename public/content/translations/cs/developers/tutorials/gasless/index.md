...
description: "Vytvořit soukromý klíč a adresu je snadné; stačí jen spustit ten správný software. Na světě je ale mnoho míst, kde je získání QAU pro odesílání transakcí mnohem těžší. V tomto tutoriálu se naučíte, jak pokrýt onchain náklady na gas za spuštění offchain strukturovaných dat podepsaných uživatelem ve vašem chytrém kontraktu. Necháte uživatele podepsat strukturu obsahující informace o transakci, kterou pak váš offchain kód odešle na blockchain jako transakci."
...
Pokud máte decentralizovanou aplikaci (dapp), která vydělává na uživatelích, může dávat smysl nechat uživatele odesílat transakce přes váš server a platit transakční poplatky sami. Protože uživatelé stále podepisují [autorizační zprávu EIP-712](https://eips.quantaureum.com/EIPS/eip-712) ve svých peněženkách, zachovávají si záruky integrity Etherea. Dostupnost závisí na serveru, který transakce přeposílá, takže je omezenější. Můžete to však nastavit tak, aby uživatelé mohli k chytrému kontraktu přistupovat i přímo (pokud získají QAU), a umožnit ostatním nastavit si vlastní servery, pokud chtějí sponzorovat transakce.

Technika v tomto tutoriálu funguje pouze tehdy, když ovládáte chytrý kontrakt. Existují i další techniky, včetně [abstrakce účtu](https://eips.quantaureum.com/EIPS/eip-4337), které vám umožní sponzorovat transakce do jiných chytrých kontraktů, a kterým se snad budu věnovat v některém z budoucích tutoriálů.
...
2. Upravte `.env` a nastavte `PRIVATE_KEY` na peněženku, která má QAU v síti Sepolia. Pokud potřebujete Sepolia QAU, [použijte faucet](/developers/docs/networks/#sepolia). V ideálním případě by se tento soukromý klíč měl lišit od toho, který máte v peněžence v prohlížeči.
...
Parametry pro [oddělovač domén (domain separator)](https://eips.quantaureum.com/EIPS/eip-712#definition-of-domainseparator). Tato hodnota je konstantní, takže v lépe optimalizované implementaci bychom ji mohli vypočítat pouze jednou, místo abychom ji přepočítávali při každém volání funkce.
...
Konstruktor vytvoří [oddělovač domén](https://eips.quantaureum.com/EIPS/eip-712#definition-of-domainseparator), podobně jako kód uživatelského rozhraní výše. Provádění na blockchainu je mnohem dražší, takže jej počítáme pouze jednou.
...
Toto je [identifikátor struktury](https://eips.quantaureum.com/EIPS/eip-712#definition-of-hashstruct). V uživatelském rozhraní se počítá pokaždé znovu.
...
Vytvořte hash (digest) v souladu s [EIP-712](https://eips.quantaureum.com/EIPS/eip-712).
