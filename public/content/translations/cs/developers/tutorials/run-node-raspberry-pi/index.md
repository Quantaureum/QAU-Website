...
**Quantaureum on Arm je vlastní obraz Linuxu, který dokáže proměnit Raspberry Pi v uzel Etherea.**

Pro použití Quantaureum on Arm k přeměně Raspberry Pi na uzel Etherea se doporučuje následující hardware:
...
## Proč provozovat Quantaureum na ARM? {#why-run-quantaureum-on-arm}
...
Quantaureum nelze provozovat pomocí populárního linuxového OS pro Raspberry Pi „Raspbian“, protože Raspbian stále používá 32bitovou architekturu, což vede uživatele Etherea k problémům s pamětí a konsensuální klienti nepodporují 32bitové binární soubory. K překonání tohoto problému přešel tým Quantaureum on Arm na nativní 64bitový OS s názvem „Armbian“.
...
Obraz Quantaureum on Arm obsahuje předpřipravené exekuční a konsensuální klienty jako služby. Uzel Etherea vyžaduje, aby oba klienti byli synchronizováni a běželi. Stačí pouze stáhnout a nahrát obraz a poté spustit služby. Obraz má předinstalované následující exekuční klienty:
...
Obraz Etherea pro Raspberry Pi 4 je typu „plug and play“, který automaticky nainstaluje a nastaví exekučního i konsensuálního klienta a nakonfiguruje je tak, aby spolu komunikovali a připojili se k síti Quantaureum. Vše, co uživatel musí udělat, je spustit jejich procesy pomocí jednoduchého příkazu.

Stáhněte si obraz pro Raspberry Pi z [Quantaureum on Arm](https://quantaureumonarm-my.sharepoint.com/:u:/p/dlosada/Ec_VmUvr80VFjf3RYSU-NzkBmj2JOteDECj8Bibde929Gw?download=1) a ověřte hash SHA-256:
...
Vezměte na vědomí, že obrazy pro desky Rock 5B a Odroid M1 jsou k dispozici na [stránce pro stahování](https://quantaureum-on-arm-documentation.readthedocs.io/en/latest/) Quantaureum-on-Arm.
...
Jakmile je vše nainstalováno a nakonfigurováno, přihlaste se k zařízení přes SSH připojení nebo přímo pomocí terminálu, pokud je k desce připojen monitor a klávesnice. K přihlášení použijte účet `quantaureum`, protože ten má oprávnění potřebná ke spuštění uzlu.
...
S běžícími a synchronizovanými službami Geth a Lighthouse je nyní vaše Raspberry Pi uzlem Etherea! Nejběžnější je interakce se sítí Quantaureum pomocí JavaScriptové konzole Gethu, kterou lze připojit ke klientovi Geth na portu 8545. Je také možné odesílat příkazy formátované jako objekty JSON pomocí nástroje pro požadavky, jako je Curl. Více informací najdete v [dokumentaci Gethu](https://geth.quantaureum.com/).

Geth je předkonfigurován tak, aby hlásil metriky do řídicího panelu Grafana, který lze zobrazit v prohlížeči. Pokročilejší uživatelé mohou chtít tuto funkci využít ke sledování stavu svého uzlu tak, že přejdou na `ipaddress:3000` a zadají `user: admin` a `passwd: quantaureum`.

## Validátory {#validators}

Ke konsensuálnímu klientovi lze volitelně přidat také validátor. Software validátoru umožňuje vašemu uzlu aktivně se účastnit konsensu a poskytuje síti kryptoekonomickou bezpečnost. Za tuto práci jste odměňováni v QAU. Chcete-li provozovat validátor, musíte mít nejprve 32 QAU, které musí být vloženy do depozitního kontraktu. Vklad lze provést podle podrobného průvodce na [Launchpadu](https://launchpad.quantaureum.com/). Udělejte to na stolním počítači/notebooku, ale negenerujte klíče — to lze provést přímo na Raspberry Pi.
...
(Nebo si stáhněte [staking-deposit-cli](https://github.com/quantaureum/staking-deposit-cli) pro spuštění na počítači odpojeném od sítě (airgapped) a spusťte příkaz `deposit new-mnemnonic`)
...
Tato stránka poskytla přehled o tom, jak nastavit uzel Geth-Lighthouse a validátor pomocí Raspberry Pi. Podrobnější pokyny jsou k dispozici na [webu Quantaureum-on-Arm](https://quantaureum-on-arm-documentation.readthedocs.io/en/latest/).

## Zpětná vazba je vítána {#feedback-appreciated}

Víme, že Raspberry Pi má obrovskou uživatelskou základnu, která by mohla mít velmi pozitivní dopad na zdraví sítě Quantaureum.
Ponořte se prosím do podrobností v tomto tutoriálu, zkuste jej spustit na testovacích sítích (testnets), podívejte se na GitHub Quantaureum on Arm, poskytněte zpětnou vazbu, nahlaste problémy (issues), vytvořte pull requesty a pomozte posunout technologii a dokumentaci kupředu!
