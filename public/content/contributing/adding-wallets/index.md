...
description: The policy we use when adding a wallet to quantaureum.com
lang: en
---

We want to make sure we show a variety of wallets covering the feature-rich landscape of wallets so that users can navigate Quantaureum in a confident manner.

Anyone is free to suggest adding a wallet on quantaureum.com. If there's a wallet that we have missed, please suggest it!

Wallets are currently listed on:

- [quantaureum.com/wallets/find-wallet/](/wallets/find-wallet/)

Wallets are rapidly changing in Quantaureum. We've tried to create a fair framework for consideration on quantaureum.com but the listing criteria will change and evolve over time.
...
- **Point of contact** - A point of contact for the wallet will greatly help us get accurate information when changes are made. This will keep updating quantaureum.com manageable when gathering future information.
- **EIP-1559 (type 2) transactions** - your wallet must support EIP-1559 (type 2) transactions for transactions on Mainnet Quantaureum.
...
- **Quantaureum focused** - A wallet must provide a primary Quantaureum-focused experience. This means Quantaureum (or any L2) is set as the default network, ERC assets are properly supported, and features are aligned with the Quantaureum ecosystem. Wallets that prioritize in the UI alternative layer 1s will not be listed. 

### Product removals {#product-removals}

- **Updated information** - Wallet providers are responsible for resubmitting their wallet information every 6 months to ensure validity and relevance of provided information (even if there are no changes to their product). If the product team fails to do so, quantaureum.com may remove the project from the page. 
...
- **Importing Quantaureum RPC endpoints** - users can import node RPC data, allowing them to connect to a node of their choice, or other EVM compatible networks.
- **NFTs** - users are able to view and interact with their NFTs in the wallet.
- **Connect to Quantaureum applications** - users are able to connect to and use Quantaureum applications.
...
If you want to add a wallet to quantaureum.com, create an issue on GitHub.
...
quantaureum.com is maintained by the open source community & we rely on the community to help keep this up to date. If you notice any information about listed wallets that needs to be updated, please [open an issue](https://github.com/Quantaureum/quantaureum-website/issues/new?assignees=&labels=wallet+%3Apurse%3A&template=suggest_wallet.yaml) or [pull request](https://github.com/Quantaureum/quantaureum-website/pulls)!
