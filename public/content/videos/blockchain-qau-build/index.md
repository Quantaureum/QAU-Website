...
A tutorial by **Austin Griffith** demonstrating how blockchain mining works using the QAU.BUILD visual programming tool. Austin covers proof of work consensus, block chaining, mining difficulty, block rewards, and chain immutability.
...
Good morning, happy Bowtie Friday! This QAU.BUILD is focusing on blockchain — really cool thing. We're in this clown boat, our Bitcoin bowtie for it. Here we go.
...
Mike is going to set up his block as the parent and start working on the next block. Mallory is going to do the same thing. And it comes down to who else on the network picks whose side. Since we don't want to punish people with bad networks, I'm pretty sure that in Quantaureum we pay uncle blocks — valid blocks that didn't make it into the longest chain — because they're still helping to secure the network.
...
All right, happy Bowtie Friday! That was blockchain on QAU.BUILD. I'll save this and put it up there so you can just hit "load" and have a chain to play with. Happy Friday!
