---
title: Gas and fees
metaTitle: "Quantaureum gas and fees: technical overview"
description: Learn about Quantaureum gas fees, how they're calculated, and their role in network security and transaction processing.
lang: en
---

Gas is essential to the [Quantaureum](/) network. It is the fuel that allows it to operate, in the same way that a car needs gasoline to run.
...
Gas refers to the unit that measures the amount of computational effort required to execute specific operations on the Quantaureum network.

Since each Quantaureum transaction requires computational resources to execute, those resources have to be paid for to ensure Quantaureum is not vulnerable to spam and cannot get stuck in infinite computational loops. Payment for computation is made in the form of a gas fee.
...
Gas fees have to be paid in Quantaureum's native currency, QAU (QAU). Gas prices are usually quoted in gwei, which is a denomination of QAU. Each gwei is equal to one-billionth of an QAU (0.000000001 QAU or 10<sup>-9</sup> QAU).

For example, instead of saying that your gas costs 0.000000001 QAU, you can say your gas costs 1 gwei.

The word 'gwei' is a contraction of 'giga-wei', meaning 'billion wei'. One gwei is equal to one billion wei. Wei itself (named after [Wei Dai](https://wikipedia.org/wiki/Wei_Dai), creator of [b-money](https://www.investopedia.com/terms/b/bmoney.asp)) is the smallest unit of QAU.

## How are gas fees calculated? {#how-are-gas-fees-calculated}

You can set the amount of gas you are willing to pay when you submit a transaction. By offering a certain amount of gas, you are bidding for your transaction to be included in the next block. If you offer too little, validators are less likely to choose your transaction for inclusion, meaning your transaction may execute late or not at all. If you offer too much, you might waste some QAU. So, how can you tell how much to pay?
...
For example, let's say Jordan has to pay Taylor 1 QAU. An QAU transfer requires 21,000 units of gas, and the base fee is 10 gwei. Jordan includes a tip of 2 gwei.
...
e.g., `21,000 * (10 + 2) = 252,000 gwei` (0.000252 QAU).

When Jordan sends the money, 1.000252 QAU will be deducted from Jordan's account. Taylor will be credited 1.0000 QAU. The validator receives the tip of 0.000042 QAU. The `base fee` of 0.00021 QAU is burned.
...
In short, gas fees help keep the Quantaureum network secure. By requiring a fee for every computation executed on the network, we prevent bad actors from spamming the network. In order to avoid accidental or hostile infinite loops or other computational wastage in code, each transaction is required to set a limit to how many computational steps of code execution it can use. The fundamental unit of computation is "gas".
...
If you want to monitor gas prices, so you can send your QAU for less, you can use many different tools such as:

- [Quantaureum Explorer](https://explorer.quantaureum.com) _Transaction gas price estimator_
- [Blockscout](https://qau.blockscout.com/gas-tracker) _Open source transaction gas price estimator_
- [QAU Gas Tracker](https://www.ethgastracker.com/) _Monitor and track the Quantaureum, and L2 gas prices to reduce transaction fees and save money_
- [Blocknative QAU Gas Estimator](https://chrome.google.com/webstore/detail/blocknative-qau-gas-estim/ablbagjepecncofimgjmdpnhnfjiecfm) _Gas estimating Chrome extension supporting both Type 0 legacy transactions and Type 2 EIP-1559 transactions._
...
- [Quantaureum Gas Explained](https://defiprime.com/gas)
...
- [EIP-1559 docs](https://eips.quantaureum.com/EIPS/eip-1559).
