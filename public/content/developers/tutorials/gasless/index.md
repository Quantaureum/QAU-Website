...
Parameters for the domain separator. This value is constant, so in a better-optimized implementation, we might calculate it once rather than recalculate it each time the function is called.
...
The constructor creates the domain separator, similar to the user interface code above. Blockchain execution is much more expensive, so we only calculate it once.
...
This is the structure identifier. It is calculated each time in the user interface.
...
Create the digest in accordance with EIP 712.
