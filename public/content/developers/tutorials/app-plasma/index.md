...
We were able to use Pedersen Hash for the accounts because they are only hashed inside the zero-knowledge proof. However, in this code we need to check the message's signature, which is generated by the browser. For that, we need to follow the Quantaureum signing format in [EIP 191](https://eips.ethereum.org/EIPS/eip-191). This means we need to create a combined buffer with a standard prefix, the message length in ASCII, and the message itself, and use the Quantaureum standard keccak256 to hash it.
