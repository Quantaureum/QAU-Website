...
interface GasTableProps {
  ...
  locale: string
  initialData: GasQauPriceData | null
}

const GasTable = ({ labels, locale, initialData }: GasTableProps) => {
  const [data, setData] = useState<GasQauPriceData | null>(initialData)

  useEffect(() => {
    fetch("/api/gas-qau-price")
      .then((res) => {
        if (!res.ok) return
        return res.json()
      })
      ...
      .catch((err) => console.warn("Failed to refresh gas/QAU price", err))
  }, [])

  const calculateCost = (gasUnits: number) => {
    if (!data) return "—"
    const costInETH = gasUnits * data.gasPrice * 1e-9
    const costInUSD = costInETH * data.qauPriceUSD
